...
def atomic_write_jsonl(path, records, **dump_kwargs):
    """Line-delimited sibling of atomic_write_json() (Fix Spec batch 4,
    item #9). JSONL (one JSON object per line) isn't a single JSON
    document, so it can't just call atomic_write_json() directly — this
    builds the full newline-joined blob first, then reuses the exact
    same temp-file + fsync + .bak-copy + os.replace machinery on that
    blob as one atomic unit. Same guarantee atomic_write_json() already
    gives every other save_* function: a crash mid-write, or two scans
    running concurrently, can never truncate or interleave-corrupt the
    file — the old good file survives intact until the new one is
    fully written and fsynced.
    """
    dump_kwargs.setdefault("default", _json_default)
    blob = "".join(json.dumps(rec, **dump_kwargs) + "\n" for rec in records)
    tmp = f"{path}.{os.getpid()}.{time.time_ns()}.tmp"
    try:
        with open(tmp, "w") as f:
            f.write(blob)
...
# Group/observation surface (per chat, 2026-09-21 — group gateway work).
# Deliberately os.environ.get(), NOT os.environ[] like the two lines
# above: this must stay optional so a deploy with no group configured
# yet doesn't crash on import. GROUP_CHAT_ID == "" means "no group
# wired up" everywhere it's checked in scanner_live.py.
#
# GROUP FEATURE — WHERE EVERYTHING LIVES (read this before editing):
#   - This env var: the group chat's Telegram chat ID (a negative
#     number, e.g. -1001234567890).
#   - Code-level restriction (what the group is actually allowed to
#     do): scanner_live.py, search `GROUP_CHAT_ID and chat_id ==
#     GROUP_CHAT_ID` for the dispatch branch, and format_group_help()
#     for the canonical allowed-command list + copy shown to the group
#     (/status, /watch, /report, /help — read-only, no trading/signal
#     commands reachable there at all).
#   - Telegram's own "/" autocomplete menu is a SEPARATE, Telegram-side
#     setting (set via setMyCommands) — it does not automatically match
#     the code-level restriction above. It's scoped down for the group
#     via set_group_commands.py at the repo root; see that file's
#     docstring for why this is a second, independent step and when to
#     re-run it.
...
# SHADOW-ONLY (per chat, 2026-09-01): compute_macro_bias() currently
# accepts ANY detect_bos_impulse() leg as macro_bias_confirmed regardless
# of leg size — a 75-pip post-spike bounce gets the exact same authority
# as a 400-pip trend leg. This gates a SHADOW variant only
# (compute_macro_bias_min_leg_gate in scanner_observation.py) requiring
# the leg's origin-to-extreme range to be >= this many ATR_1H before it's
# accepted; below that, the shadow rule holds the PRIOR confirmed
# direction instead of flipping/(re)confirming on the undersized leg.
# NOT wired into compute_macro_bias() itself — see that function's own
# "ONE place direction gets decided" contract. Needs A/B log evidence
# (see shadow_minleg_ log, /biasminleg-style review) before this ever
# becomes a live gate change; that's a decision for a human, not this
# patch. Multiplier is a first guess, not tuned — revisit once the
# shadow log has real disagreement data.
MIN_LEG_SIZE_ATR_MULT = 3.0

# Second, independent qualification criterion for the SAME reversal
# gate (per chat, 2026-09-01, Vally: "not is bullish leg > X pips? but
# how significant is this leg relative to the movement/context it is
# attempting to supersede?"). ATR alone can understate a real move —
# a 14-period ATR fades the crash bar's own influence within a handful
# of bars, so a genuinely shallow bounce can still clear an ATR-only
# bar if computed in a quiet window right after. Requiring the
# candidate to also be at least this fraction of the PRIOR confirmed
# leg's own range is a regime-independent sanity check: a leg trying
# to overturn a 300-pip trend needs to look like it could plausibly BE
# a reversal of something that size, not just "big for right now."
# First guess, not tuned — same caveat as MIN_LEG_SIZE_ATR_MULT.
MIN_REVERSAL_VS_PRIOR_LEG_FRACTION = 0.5
...
# BUG FIX (2026-08-26, per chat — user caught EXPANSION->EXHAUSTION flipping
# back and forth ~5 minutes apart with nothing structurally changing).
# Root cause: compute_market_phase() compared dist_in_atr/break_count
# against a single hard threshold every scan, with no memory of the
# previous phase. dist_in_atr is computed off THIS scan's live/still-
# forming 1H candle close, which can tick a few hundredths of an ATR
# either side of PHASE_EXHAUSTION_EMA_DIST_ATR_MULT between two scans
# just from normal intra-candle price noise near the boundary — a classic
# bang-bang oscillation around a hard threshold, not a real regime change.
# Standard fix: a Schmitt-trigger-style asymmetric band. Entering
# EXHAUSTION still requires crossing the full threshold above (unchanged
# — no easier to flag aging than before). But once EXHAUSTION, REVERTING
# to EXPANSION requires dropping to this fraction of the threshold, not
# just back below it — so a leg sitting right on the boundary reports one
# stable phase instead of flapping every scan. UNVALIDATED PLACEHOLDER,
# same caveat as the thresholds above: a reasonable first guess (dropping
# by 20% before reverting), not a derived number — worth revisiting once
# MIN has logged enough phase-flip scans to see how wide the real noise
# band is.
PHASE_EXHAUSTION_EXIT_FRACTION     = 0.80
...
# DISPLAY-ONLY refinements for EXPECTED_NEXT_EVENT_MAP's own ambiguous
# entries (per chat, originally 2026-08-27, extended 2026-09-11). Lives
# here, not in brain.py where the first two entries were originally
# defined, specifically so scanner_observation.py can also apply it —
# brain.py's own classify_thesis_delta()-adjacent narrative was fixed
# first, but the raw map text was ALSO reaching the user unrefined via (a)
# MIL's generate_expectations(), which read thesis.expected_next_event
# directly, bypassing brain.py's fix entirely, and (b) the "Since last
# scan" delta bullet built in classify_thesis_delta()/format_market_thesis
# (scanner_observation.py), which brain.py has no import access to at all
# without a layer-boundary violation (scanner_observation sits BELOW
# brain, never the reverse). One shared table + one shared function,
# imported by both, is what actually stops this from recurring a third
# time whenever a new consumer of expected_next_event gets added later.
#
# Deliberately NOT editing EXPECTED_NEXT_EVENT_MAP's own values above —
# they also feed market_thesis_expected_next_event, Thesis's own
# EXP7-tagged persisted field, whose shape/content must not change (shadow
# research keys off the exact raw string for grouping). This dict refines
# only what gets DISPLAYED to a human, never what gets computed/persisted/
# compared against.
#
# The defect being fixed, in all three entries below: lumping a real
# structural signal (a CHoCH, a fresh BOS) together with a much weaker one
# (range/consolidation persisting) as if either would equally confirm the
# same thing. A CHoCH/fresh BOS is actual structural evidence; a stall on
# its own is just loss of progression, not necessarily reversal. Say that
# distinction, don't flatten it into "X or Y."
NEXT_EVENT_DISPLAY_REFINEMENTS = {
...
def refine_next_event_for_display(raw_next_event, direction_label):
    """Applies NEXT_EVENT_DISPLAY_REFINEMENTS to raw expected-next-event
    text before it's shown to a human. Anything not in the table is
    returned completely unchanged — targeted fix for known ambiguous
    entries, not a general rewrite of the map's vocabulary. `direction_label`
    should be "Bullish"/"Bearish" (title case) or None; entries with no
    {d} placeholder ignore it."""
    if not raw_next_event:
        return raw_next_event
    template = NEXT_EVENT_DISPLAY_REFINEMENTS.get(raw_next_event)
    if template is None:
        return raw_next_event
...
# ---- MARKET INTELLIGENCE NETWORK: Live Evidence Archive -------------------
# Renamed from "Intelligence Database" (per chat) — clashed with the
# new research_dataset.py/research_lab.py "Research Dataset"/"Research
# Lab", a structurally different, offline, blind-to-outcome,
# holdout-protected system. This archive is the opposite: it's the
# continuously-updated, live-scan-attached store (shadow_trade_log.jsonl
# etc.) behind the Evidence & Research Department and the Markov regime
# model, updated every scan, never blind-constructed. Comment/prose only
# — every constant/file name below stays exactly as-is.
...
# appended here forever. This IS the Live Evidence Archive: the actual
...
# Separate file, NOT appended into BIAS_AB_LOG_FILE — per this file's own
# single-owner-per-persisted-file discipline. Live (gated) vs shadow
# (min-leg-size gate) 1H bias, one entry per scan. See
# MIN_LEG_SIZE_ATR_MULT above and compute_macro_bias_min_leg_gate() in
# scanner_observation.py. Needs adding to the workflow's file commit
# list before it will actually persist across GitHub Actions runs.
BIAS_MINLEG_LOG_FILE  = os.path.join(BASE_DIR, "bias_minleg_log.json")
BIAS_MINLEG_LOG_MAX_ENTRIES = 500
...
# ---- REJECTED-LIVE EVIDENCE — price-point granularity for a persisting
# leg (per chat, 2026-08-20). drain_rejected_live_queue() previously deduped
# purely on (side, swing_high, swing_low, tier) FOREVER — a leg sitting
# rejected by the same tier for days (plausible exactly when the low-ATR
# no-signal issue is live) produced ONE shadow log for its entire lifetime,
# freezing evidence at whatever the first rejection's context happened to
# be. Fix: allow a re-log for the same (leg, tier) once price has moved
# meaningfully since the LAST logged price for that family — genuinely
# different opportunity/R-profile, not just "still sitting there." An
# unmoved-price replay (including a merge=union queue-file resurrection —
# see drain_rejected_live_queue's docstring) has delta=0, which is always
# below this floor, so the old replay-protection property is preserved,
# not weakened.
# NOTE: this multiplier is a starting judgment call, NOT evidence-backed —
# same "don't fabricate a threshold ahead of evidence" flag as everywhere
# else. Revisit once there's enough drained history to check whether it's
# over/under-splitting real setups.
REJECTED_LIVE_MIN_PRICE_DELTA_ATR_MULT = 0.5
REJECTED_LIVE_MIN_PRICE_DELTA_PIPS_FLOOR = 5  # floor when atr_pips is missing/unusable, mirrors SL_MIN_PIPS
...
# ADDED (2026-09-03, per audit — a /taken sent shortly before a signal's
# next 5-minute cron tick was losing the race to AUTO-PROMOTE and getting
# overridden with "assumed taken" despite the user actually replying in
# time). Root cause: pending_confirmation carried NO minimum elapsed-time
# check before scanner_live.py's auto-promote block would fire — the real
# reply window was "whatever's left until the next cron tick from the
# instant the signal fired," which could be single-digit seconds, not a
# true 5 minutes. This constant is a FLOOR on how long a pending
# confirmation must sit before auto-promote is allowed to claim it timed
# out — deliberately set just under two scan intervals so a signal firing
# right before a tick still gets one full real window. Does not change
# what /taken or /skip do when they DO arrive — only when silence is
# allowed to be treated as an answer.
MIN_CONFIRMATION_WINDOW_SECONDS = 540   # 9 minutes — just under 2x the 5-min cron interval
...
# ADDED (2026-08-27, per chat — Market Event lifecycle). Cap on the
# pending-events queue below — Market Events tagged against the
# currently-open leg_obs record, awaiting that leg's resolution.
# Generous relative to how many events can realistically tag one open
# leg — this is a safety cap, not an expected steady-state size.
MARKET_EVENT_PENDING_MAX = 20

# CORRECTED (2026-08-27, per audit — found while checking min-scan.yml/
# live-scan.yml directly, per the user's own explicit request to verify
# rather than assume). This was ORIGINALLY a key inside state.json
# (state["market_event_pending"]) — that was a real bug: state.json is
# exclusively LIVE-owned (live-scan.yml's "Save live-owned files" step;
# min-scan.yml's commit list does NOT include it). _close_leg_obs()
# needs to CLEAR resolved entries from within min_scanner.py's
# run_min_pass() — every such write was being silently discarded the
# moment that runner tore down. Exact same class of bug as market_
# intent_state.json/sr_levels_state.json/bank_transactions.jsonl before
# it — "FOUND MISSING" in this same pair of workflow files, more than
# once, per their own comments.
#
# Fixed the same way this codebase already fixed that exact problem for
# rejected_live_queue.jsonl: a dedicated file, one side (LIVE) appends,
# the other (MIN) drains, BOTH workflows validate and commit it, and it
# needs the same merge=union .gitattributes treatment rejected_live_
# queue.jsonl already has — see that file's entry and mirror it for
# this one. Do NOT put this back inside state.json.
MARKET_EVENT_PENDING_FILE = os.path.join(BASE_DIR, "market_event_pending.jsonl")
...
# ---- RESEARCH ARCHIVE: SCAN INDEX / STRUCTURE DIGEST LOG / ZONE LOG --------
# Added 2026-09-18, per chat — audit for the planned Research & Backtest
# Lab found three gaps blocking it: (1) no scan_id existed anywhere in the
# codebase, only timestamps (scan_log.jsonl), so nothing could answer
# "what did the bot know as of scan N"; (2) build_structure_digest()'s own
# docstring says it exists "so this scan's MarketFacts stops being thrown
# away" but it was only ever written to state["structure_digest"], a
# same-key field a later scan immediately overwrites — it WAS still being
# thrown away every scan; (3) FVG/order-block detections are recomputed
# fresh every scan from the live rolling window and were never logged
# anywhere outside leg-scoped records. All three fixed via the three files
# below, all LIVE-owned (single writer: scanner_live.py's _scan_once()),
# so — same lesson as market_intent_*/scan_log.jsonl/ohlc_*_log.jsonl
# above — added to live-scan.yml's validate + git-add lists in this SAME
# turn, not after.
SCAN_INDEX_LOG_FILE       = os.path.join(BASE_DIR, "scan_index_log.jsonl")        # permanent: scan_id -> scanned_at
STRUCTURE_DIGEST_LOG_FILE = os.path.join(BASE_DIR, "structure_digest_log.jsonl")  # permanent: one row per scan
ZONE_LOG_FILE             = os.path.join(BASE_DIR, "zone_log.jsonl")              # permanent: one row per NEW zone
# Added 2026-09-20, per chat (friend's "storing context" proposal, second
# pass). Companion to STRUCTURE_DIGEST_LOG_FILE, joined by the SAME scan_id
# — deliberately a separate file rather than a bigger structure_digest row,
# because the Market Thesis Engine's fields (break_count, transition_cause,
# trend_health, failure_risk, mtf_15m, mtf_5m, campaign) are computed LATER
# in _scan_once() than the structure digest is, and structure_digest_log.jsonl
# is written unconditionally, every scan, specifically for guaranteed
# coverage (mirrors scan_log.jsonl's own "written before any early-return
# path" discipline) — writing it late instead, so it could hold thesis
# fields in the same row, would silently lose that coverage guarantee on
# scans that never reach thesis computation. Two append-only files, one join
# key — same established pattern as scan_index_log.jsonl/zone_log.jsonl
# already joining to structure_digest_log.jsonl, not a new kind of
# fragmentation. research_dataset.py's load_full_context() (new, see that
# file) returns the two pre-joined as one table, so nothing downstream has
# to know it's two files on disk.
THESIS_CONTEXT_LOG_FILE   = os.path.join(BASE_DIR, "thesis_context_log.jsonl")   # permanent: one row per scan reaching thesis


def next_scan_id(state):
    """Mints the next monotonic scan_id and stamps it into `state` in
    place. Backfill-safe: state.get("scan_id", 0) so a pre-existing
    state.json with no such field yet starts counting from 1 rather
    than crashing (same backfill discipline as drained_leg_ids etc. —
    see min_scanner.py's load_shadow_state()). Caller still owns
    save_state(state) afterward, same as every other apply_state_
    updates() consumer — this function only mutates the in-memory dict."""
    scan_id = state.get("scan_id", 0) + 1
    state["scan_id"] = scan_id
    return scan_id


def append_permanent_jsonl(path, record):
    """Generic append-only, fsync'd, retrying JSONL writer — same
    write-hardening pattern _append_leg_obs_log()/_append_failure_case()/
    etc. already use (fsync + retry against transient disk/FS errors,
    never silently drops a record; returns True/False so a caller can
    decide whether it's safe to advance any dedup/cursor state that
    depends on the write having actually landed). Added 2026-09-18 as a
    shared helper for the three NEW research logs above — existing
    per-log appenders are left untouched (additive only, not a refactor
    ...
    print(f"[PERMANENT LOG WRITE ERROR] {path}: gave up after "
          f"{SHADOW_TRADE_LOG_WRITE_RETRIES} attempts: {last_error}")
    return False


# ---- BRAIN LOG ----------------------------------------------------------
# Added 2026-08-25, per chat: brain.py itself stays fully ephemeral/
# stateless (per ITS OWN module docstring — it receives a WorldState dict
# and returns an understanding, nothing persisted, nothing re-read next
# scan). That's the right call for Brain's own boundary, but it meant
# every /understand reply overwrote the only record of what Brain said —
# no history at all, unlike every other subsystem (shadow, bank, leg_obs,
# market_intent, ...) which all get a permanent append-only log. This is
# that log for Brain — written by scanner_live.py (the caller), never by
# brain.py itself, so Brain's own zero-import isolation is untouched.
BRAIN_LOG_FILE = os.path.join(BASE_DIR, "brain_log.jsonl")   # permanent, append-only
# Same reasoning as BRAIN_LOG_FILE above, for build_market_briefing()'s
# output specifically (brain.py Phase 4, 2026-08-24). build_market_briefing()
# bundles reasoning_snapshot/intent_snapshot INSIDE its return value
# precisely so a later "why did you say X at 9am" question is answerable —
# but only if a caller actually persists the object somewhere it survives.
# /briefing is dev/audit only (per brain.py's format_market_briefing()
# docstring: "not yet the scheduled push"), so this is a separate log from
# BRAIN_LOG_FILE rather than reusing it, since /understand and /briefing
# are auditing two different Brain outputs and mixing them would make a
# later review guess which record came from which command.
BRIEFING_LOG_FILE = os.path.join(BASE_DIR, "briefing_log.jsonl")   # permanent, append-only
...
def load_state():
    ...
    # FRESHNESS FIX (Fix Spec batch 4, item #8 — per chat, locked
    # decision). "timestamp" means "state was persisted recently" — true
    # on every save_state() call, including the scan-ID-preservation
    # save in _scan_once() that runs before any market data is even
    # fetched. It was structurally incapable of detecting "market data
    # hasn't actually refreshed in hours": a scan can keep failing
    # fetch/freshness/sanity forever and still re-stamp "timestamp" on
    # every attempt (via that early save), so this staleness check could
    # never catch the exact failure mode the audit was concerned about.
    #
    # market_data_timestamp is the dedicated field for that separate
    # claim — stamped ONLY after a scan's fetch AND freshness AND
    # sanity checks all succeed (see the stamp site in scanner_live.py,
    # immediately after the "All three data checks just passed"
    # comment). "timestamp" itself is UNCHANGED by this fix — still
    # means state persistence, still stamped by every save_state() call
    # exactly as before. Deliberately NOT moving that early save or its
    # timestamp stamp (per chat) — persistence semantics staying
    # independent of scan success is what the scan-ID-preservation save
    # was built for in the first place; mixing the two concerns back
    # together was the tempting-but-wrong fix.
    #
    # BACKWARD COMPATIBILITY: a state.json written before this field
    # existed has no market_data_timestamp key at all (not merely an
    # old one — genuinely absent). Falling back to "timestamp" ONLY in
    # that specific case avoids discarding a perfectly good pre-fix
    # state on the very first load after this deploys. Once any
    # post-fix scan successfully saves, market_data_timestamp exists
    # in the file and the strict, dedicated check takes over
    # permanently — this fallback only ever fires once per deployment.
    ts = state.get("market_data_timestamp")
    if ts is None:
...
def save_state(state):
    # "timestamp" here means "state was persisted" only — deliberately
    # UNCHANGED by Fix Spec batch 4, item #8. It does NOT stamp
    # market_data_timestamp; that field is only ever set at its one
    # dedicated stamp site in scanner_live.py, after fetch + freshness
    # + sanity checks all succeed. See load_state()'s docstring/comment
    # above for the full picture of why these are two separate fields.
    state = dict(state)
...
def load_bias_minleg_log():
    """Rolling log of live (gated) vs shadow (min-leg-size gate) 1H
    bias agreement, one entry per scan. Mirrors load_bias_ab_log()."""
    try:
        with open(BIAS_MINLEG_LOG_FILE, "r") as f:
            return json.load(f)
    except Exception:
        return []


def save_bias_minleg_log(log):
    try:
        atomic_write_json(BIAS_MINLEG_LOG_FILE, log[-BIAS_MINLEG_LOG_MAX_ENTRIES:], indent=2)
    except Exception as e:
        print("[BIAS MINLEG LOG SAVE ERROR] " + str(e))
...
def send_telegram(message, chat_id=None):
    """
    Sends `message` to TELEGRAM_CHAT_ID by default, or to `chat_id` if
    given (per chat, 2026-09-21 — group gateway work: this is what lets
    the same bot post to the group surface without a second token or a
    second polling process). Tries Markdown first (existing behaviour —
    every call site's intentional *bold*/_italic_ formatting still
    works). If Telegram rejects it — most commonly "can't parse
