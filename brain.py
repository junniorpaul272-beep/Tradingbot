...
def synthesize_market_understanding(relationships, current_condition):
    ...
    WHAT THIS ADDS: generalizes the next_watch override that used to
    exist only on the reaccelerating branch. Every branch above now gets
    a next_watch resolved HERE, once, in one place:

      - Branch already set next_watch explicitly (reaccelerating's own
        override text, or the two structural-transition branches'
        deliberate None) -> left exactly as that branch decided. Brain
        reasoned about it specifically; nothing here overrides Brain.
      - Branch didn't mention next_watch at all -> filled from
        current_condition["default_next_event"] (the WorldState atom
        build_world_state() computed off the SAME (phase, cause) pair
        Thesis's own expected_next_event used to use), refined through
        _refine_next_watch() (2026-08-27 addition, see above) to fix
        known "X or Y" ambiguity in a couple of that map's own entries
        before display. Same content/authority principle either way:
        Brain is deciding to use it, not passively inheriting Thesis's
        already-committed conclusion.
    ...
    explicit, Brain-decided answer for "what's next" — specific text,
    the shared default, or a deliberate None — never a second opinion
    borrowed from scanner_observation.py.
    """
...
def build_market_understanding(world_state):
    ...
    architecture: worldstate.py builds reality via build_world_state()
    (moved out of min_scanner.py per later physiology work — pure
    relocation, same function), this file interprets it). Takes a WorldState dict and ONLY a
    ...
    current_condition = {
        ...
        # ADDED (2026-08-27, per audit — Phase 1): raw atom, see phase's
        # own "default_next_event" comment in build_world_state()
        # (min_scanner.py) for why this exists as a WorldState fact
        # instead of brain.py importing EXPECTED_NEXT_EVENT_MAP itself.
        "default_next_event": phase.get("default_next_event"),
        # ADDED (per chat) — exact price levels for Market Event
        # headlines. Already computed and sitting in WorldState's phase
        # dict (worldstate.py's macro_leg/prior_macro_leg) — this was
        # purely a plumbing gap, not a new detection: current_condition
        # only ever surfaced direction, never the numbers behind it.
        ...
        # ADDED (per chat, "add rejections") — structured rejection
        # count against the leg's own extreme, so HFIS can say "third
        # time it's failed at X" from a real number instead of needing
        # free-text evidence to already say so. Computed in
...
def evaluate_market_event(understanding, intent_hypothesis, prev_snapshot, leg_obs_open=None,
                           mil_understanding=None):
    """
    ═══════════════════════════════════════════════════════════════════
    ...
    market-event/narration path, read this first.
        ...
        docstring for why)
            ...
            build_market_story()    -> the friend's 8-field schema, a
                                        thin pass-through of market_
                                        assessment (used by hfis.py's
                                        overview composer)
            ...
            |
            v
        hfis.py (the Narrator — the ONLY module that composes Telegram-
        facing market prose; scanner_live.py is the only caller
        permitted to import it — see check_layer_imports.py)
            narrate()             -> the live event push. "state"
                                      ...
                                      opportunity().
            narrate_overview()    -> the /overview + 9/12/3 briefing.
                                      _compose_recap() for events
                                      already pushed live (never re-
                                      uses "just discovered" framing —
                                      that was the duplicate-message
                                      bug); _compose_current_conditions()
                                      for the ongoing read, also
                                      preferring market_assessment's
                                      synthesis over raw levels.
            |
            v
        scanner_live.py
            orchestrates all of the above every scan; owns Telegram
            send and the relevant state.json keys
            (market_event_prev_snapshot, last_narrated_situation,
            overview_pending_events).

    A SEPARATE RUNTIME: min_scanner.py runs as its own GitHub Actions
    job and is architecturally forbidden from importing hfis.py or
    market_story.py (check_layer_imports.py enforces this). Its leg-
    resolution follow-up (_close_leg_obs() -> _compose_resolution_text())
    is therefore a small LOCAL composer, not routed through this
    pipeline — not an oversight, a runtime boundary.
    ═══════════════════════════════════════════════════════════════════
