...
and OWNS (sole writer of):
  ...
  - bank_transactions.jsonl (permanent Bank ledger — see
    settle_bank_transactions()'s own docstring; added 2026-08-16)
...
from scanner_common import (
    BASE_DIR, PIP_SIZE,
    REJECTED_LIVE_MIN_PRICE_DELTA_ATR_MULT, REJECTED_LIVE_MIN_PRICE_DELTA_PIPS_FLOOR,
    atomic_write_json, atomic_write_jsonl, send_telegram, atr, _json_default,
    ...
    MARKET_INTENT_STATE_FILE, MARKET_INTENT_LOG_FILE, MARKET_INTENT_CASE_LOG_FILE,
    MARKET_INTENT_METHODOLOGY_VERSION, MARKET_INTENT_CALIBRATION_MIN_N,
    MARKET_INTENT_CASE_MIN_GROUP_N,
    SR_METHODOLOGY_VERSION, SR_MAX_AGE_CANDLES, SR_MAX_FLIPS,
    BANK_STARTING_CAPITAL, BANK_RISK_PCT, BANK_COST_R, BANK_MIN_N,
    BANK_TRANSACTIONS_FILE, BANK_LEDGER_METHODOLOGY_VERSION,
    ACCOUNT_PROFILES_FILE, ACCOUNT_PROFILES_METHODOLOGY_VERSION,
    calculate_position_size, MIN_LOT_SIZE,
    ...
    # ADDED (2026-08-27, per audit — found while checking min-scan.yml/
    # live-scan.yml directly): see MARKET_EVENT_PENDING_FILE's own
    # docstring in scanner_common.py for why this is a dedicated file.
    MARKET_EVENT_PENDING_FILE,
...
# system_ledger.py is a leaf module (imports only scanner_common, same
# layer position as scanner_observation.py) — safe for min_scanner.py to
# import per the one-way dependency rule in check_layer_imports.py.
from system_ledger import run_scheduled_ledger_maintenance
from scanner_observation import (
    ...
    WatchCode,
    compute_market_phase, discover_sr_candidates, update_sr_level,
    Phase,   # ADDED (2026-09-08, bug fix): Phase(_phase_str) at line ~7104
    # was raising NameError — Phase was never imported even though
    # compute_market_phase (which produces the values) was. That
    # NameError wasn't caught by the surrounding `except ValueError`,
    # so it propagated and killed the entire MIN scanner run.
)
# brain.py imports NOTHING from this file or scanner_live.py (see its own
# module-boundary docstring) — this is a one-way import, same direction
# scanner_live.py already imports brain.py in. ADDED (2026-08-27, per
# audit — Phase 0 authority routing): format_market_thesis()/format_
# market_intent_report() now render Brain's own understanding/intent_
# hypothesis instead of independently-stitched prose, so /thesis and
# /marketintent can't disagree with /understand on the same scan.
from brain import format_understanding_narrative, format_intent_narrative
# NOTE (2026-09-14, LAYER-BOUNDARY FIX): `from hfis import narrate_deep_thesis`
# used to live here. check_layer_imports.py started failing CI on it —
# min_scanner.py (EMPIRICAL AUTHORITY) importing hfis.py (PRESENTATION,
# meant to sit above and depend on facts, not be depended on) is a
# one-way-dependency violation, not just a lint nitpick: it's a real
# authority-boundary crossing per the same rule Pass 4 of the SYSTEM
# INTEGRITY AUDIT already flags for compute_macro_bias()/MIN. Fixed by
# dependency injection instead: format_market_thesis() below now takes
# the already-rendered deep-thesis text as a parameter — the caller
# (scanner_live.py, which already imports both min_scanner and hfis one
# level up, the only layer allowed to depend on both) computes it via
# hfis.narrate_deep_thesis() and passes it in. min_scanner.py no longer
# imports hfis at all.
...
# =========================================================================
# LIVE EVIDENCE ARCHIVE — persistence. Renamed from "Intelligence
# Database" (per chat): that name clashed with the new
# research_dataset.py/research_lab.py "Research Dataset"/"Research Lab"
# — a structurally different, offline, blind-to-outcome system. This
# archive is the continuously-updated, live-scan-attached store behind
# the Evidence & Research Department and the Markov regime model.
# Comment/prose only — file/constant names below are left unchanged
# (shadow_state.json etc.) so nothing already logged on disk gets
# orphaned, same discipline scanner_common.py's own banner rename used.
# MIN is the sole writer of these three files now (shadow_state.json,
# shadow_stats.json, shadow_trade_log.jsonl) — scanner_live.py only
# ever reads/queues.
# =========================================================================
def load_shadow_state():
    try:
        ...
        # backfill (per chat, 2026-08-20) — see REJECTED_LIVE_MIN_PRICE_DELTA_ATR_MULT
        # in scanner_common.py. family leg_id -> last logged entry price.
        state.setdefault("rejected_live_last_price", {})
        return state
    except Exception:
        return {"pending": [], "last_leg": {}, "seen_legs": {}, "drained_leg_ids": [],
                "rejected_live_last_price": {},
...
def save_shadow_state(shadow_state):
    # _persisted_at (per chat, WorldState Phase 1C): mtime is unreliable
    # for freshness here — GitHub Actions checks this repo out fresh every
    # run, resetting mtimes to checkout time, not last-content-write time.
    # Same fix load_state()/save_state() already use for state.json,
    # extended to this file. Harmless extra key — no reader here keys off
    # an exact field set.
    shadow_state = dict(shadow_state)
    shadow_state["_persisted_at"] = datetime.now(timezone.utc).isoformat()
    try:
...
def save_shadow_stats(shadow_stats):
    # _persisted_at — see save_shadow_state()'s comment on why this isn't
    # sourced from file mtime.
    shadow_stats["_persisted_at"] = datetime.now(timezone.utc).isoformat()
    try:
...
def build_shadow_setup(experiment, direction, entry, sl_raw, now_utc,
    ...
    return {
        ...
        "bars_open": 0,
        "max_r_reached": 0,
        # ADDED (2026-08-29, per chat — Trade Investigation Bureau,
        # reshaped from the Failure Investigation Bureau per Vally's
        # "trade lifecycle" dimension: MFE alone can't distinguish a
        # clean winner from a difficult one, or an immediate failure
        # from a delayed one — need the adverse side too). Continuous
        # (not discrete-R-level like max_r_reached above), updated every
        # bar in the pending loop below — see that loop for why the two
...
def classify_trade_path(max_r_reached, min_r_reached, r_achieved,
                         stall_threshold=0.3, adverse_threshold=-0.5):
    """
    ADDED (2026-08-29, per chat — Trade Investigation Bureau, directly
    building Vally's point: "A trade shouldn't just end as WIN/LOSS...
    Do losing trades usually fail immediately? versus do they initially
    behave correctly and then deteriorate? Those are completely
    different failure modes." Same distinction on the winning side —
    a clean winner and a difficult winner are not the same phenomenon
    even though both resolve WIN.

    Four labels, using only max_r_reached (MFE) / min_r_reached (MAE) /
    final r_achieved — no new detection logic, just a classification of
    data that now exists:

      clean_winner     — won, and never went meaningfully against
                          (min_r_reached above adverse_threshold).
      difficult_winner — won, but went meaningfully against first.
      immediate_failure — lost, and never got going in its favor
                           (max_r_reached below stall_threshold).
      delayed_failure   — lost, but WAS working first (max_r_reached
                           cleared stall_threshold) before failing.

    Deliberately NOT more granular than this yet (no "structural
    deterioration", "second rejection", etc. — see Trade Investigation
    Bureau module note for what's not captured yet and why). This is
    the honest ceiling of what MFE/MAE alone can classify; anything
    finer needs new detection logic, not a smarter label function.

    Returns None if inputs are missing (older records predate MFE/MAE
    tracking, or MAE tracking specifically — never fabricates a label
    from incomplete data).
    """
    if max_r_reached is None or min_r_reached is None or r_achieved is None:
        return None
    if r_achieved > 0:
        return "clean_winner" if min_r_reached > adverse_threshold else "difficult_winner"
    else:
        return "immediate_failure" if max_r_reached < stall_threshold else "delayed_failure"


def peek_environment_at_resolution():
    """
    ADDED (2026-08-29, per chat — Trade Investigation Bureau, Vally's
    "Market Environment" dimension: was volatility compressed before
    entry, was this a clean or unstable trend, breakout magnitude
    relative to ATR). All four fields already existed — compute_
    market_state() (scanner_observation.py) computes them every scan
    for Thesis's own use — this only reads the CURRENT persisted
    values, same read-only/best-effort/self-contained discipline as
    peek_conditions_at_resolution() above (a second independent file
    peek, not threaded through the resolution call chain, to avoid a
    much larger and riskier change to update_pending_shadow_setups()'s
    signature for this pass).

    THESIS-level, not leg-level — complements peek_conditions_at_
    resolution()'s leg-formation-time snapshot with whatever this
    SCAN's 15-minute read was, which is more recent (same-scan) even
    though still not moment-exact at this specific 5-minute candle.

    Returns None on any failure/missing state — never raises, never
    fabricates a value.

    FIX (2026-08-29, same pass): these four fields are NOT individual
    flat state.json keys — compute_market_state()'s output gets stored
    wholesale as the nested dict market_thesis_mtf_15m (see scanner_
    live.py's persistence block: "market_thesis_mtf_15m": thesis.
    mtf_15m). Checked this directly against how brain.py's own
    relate_recent_momentum() already reads the exact same nested field,
    rather than assuming a flat key existed.
    """
    try:
        state = load_state()
        mtf_15m = state.get("market_thesis_mtf_15m") or {}
        return {
            "trend_strength_atr_mult": mtf_15m.get("trend_strength_atr_mult"),
            "volatility_state": mtf_15m.get("volatility_state"),
            "compression_ratio": mtf_15m.get("compression_ratio"),
            "pullback_depth_pct": mtf_15m.get("pullback_depth_pct"),
        }
    except Exception:
        return None


def peek_conditions_at_resolution():
    """
    Read-only peek at the currently-open leg_obs record's formation_
    state, captured at trade-resolution time — see both call sites
    (_append_shadow_trade_log() below, _append_live_trade_log() in
    scanner_live.py). Best-effort: any failure returns None rather than
    raising, so a read hiccup here can never prevent the trade itself
    from being logged — that write is the important one, this is a
    bonus field on top of it.

    WHAT THIS IS AND ISN'T (2026-08-29, per chat): this is still the
    leg's FORMATION-time snapshot (market_phase/regime/etc as of when
    the leg opened, not a fresh read at this exact bar) — that
    limitation doesn't go away, capturing it here doesn't make it
    moment-exact. What DOES go away: no more guessing which leg was
    open via after-the-fact timestamp matching in a separate script,
    hours or weeks later, against a leg that may not have even
    resolved yet. This reads the real currently-open record directly,
    at the moment it matters, so it's never wrong about WHICH leg —
    only still approximate about how fresh that leg's own snapshot is.
    A genuinely moment-exact capture (Brain's own market_assessment,
    rebuilt fresh right here) is a bigger, separate decision — not
    ...
    try:
        ...
        if open_rec is None:
            return None
        return open_rec.get("formation_state")
    except Exception:
        return None
...
def _append_shadow_trade_log(setup, outcome, r_achieved, now_utc):
    ...
    record = {
        ...
        # ADDED (2026-08-29, per chat — Trade Investigation Bureau, per
        # Vally's "trade lifecycle" dimension). max_r_reached already
        # existed (MFE, discrete R-levels) but was never persisted to
        # the permanent record — only used transiently to size the
        # trailing stop. min_r_reached (MAE, continuous) is new — see
        # its own comment at the setup-dict/tracking-loop level above.
        # Together these are what let a resolution be classified as
        # more than WIN/LOSS — see classify_trade_path() below.
        "max_r_reached": setup.get("max_r_reached"),
        "min_r_reached": setup.get("min_r_reached"),
        "path_classification": classify_trade_path(
            setup.get("max_r_reached"), setup.get("min_r_reached"), r_achieved),
        ...
        # LIVE CONDITION CAPTURE (2026-08-29, per chat — real gap: the
        # investigation tooling (shadow_investigation.py) was
        # reconstructing conditions RETROACTIVELY, matching a resolved
        # trade's timestamp against leg_obs_log.jsonl after the fact —
        # approximate, and incomplete for any trade whose enclosing leg
        # hadn't closed/logged yet (this is exactly why the real report
        # showed 28% "no enclosing leg"). This captures the ACTUAL
        # currently-open leg's conditions AT THE MOMENT this trade
        # resolves — ground truth, not reconstruction, going forward.
        # Best-effort/read-only (same peek pattern already proven safe
        # in Market Event tagging) — see peek_conditions_at_resolution()
        # for what this is and still isn't (leg-formation-time snapshot,
        # not a fresh read at this exact bar — that part is unchanged).
        "conditions_at_resolution": peek_conditions_at_resolution(),
    }
...
# =========================================================================
_SHADOW_RESOLVE_MAX_ERRORS = 5  # consecutive per-setup failures before quarantine
# NOTE: an earlier version of this fix used a _SHADOW_LOSS_DIGEST_THRESHOLD
# constant here to switch between individual pushes and one digest message
# for shadow losses. Superseded (2026-09-08, per chat) — routine shadow
# loss resolutions aren't pushed to Telegram at all anymore; see the
# SUPPRESSED note near the end of update_pending_shadow_setups() below.
_SHADOW_QUARANTINE_LOG_FILE = os.path.join(BASE_DIR, "shadow_quarantine_log.jsonl")
# NOTE: this file is NOT yet in either workflow's "Save live-owned files"
# git-add list (checked directly against live-scan.yml, same discipline as
# every other *_LOG_FILE addition in this codebase) — it will currently be
# lost at the end of every ephemeral Actions run. Add it to both live-scan.yml
# and min-scan.yml's commit steps the same turn this ships, or quarantine
# records will only ever be visible for the single run that wrote them.


def _quarantine_pending_setup(setup, reason):
    """Best-effort permanent record of a pending shadow setup that could not
    be resolved after _SHADOW_RESOLVE_MAX_ERRORS consecutive attempts (see
    update_pending_shadow_setups' docstring for why this exists). Failure to
    write here must never raise — the setup is being force-dropped from
    "pending" either way; losing the quarantine record is a lesser evil than
    re-poisoning the resolution loop by keeping it around."""
    try:
        record = {
        ...
        with open(_SHADOW_QUARANTINE_LOG_FILE, "a") as f:
            f.write(json.dumps(record, default=_json_default) + "\n")
    except Exception as e:
        print("[SHADOW QUARANTINE WRITE ERROR] " + str(e))


def _resolve_one_pending_setup(setup, high, low, close, candle_time, shadow_stats,
                                logged_id_set, logged_ids):
    """The per-setup resolution body, extracted purely so
    update_pending_shadow_setups() can wrap ONE setup's processing in its
    own try/except instead of the whole shared loop.

    Returns (resolved, loss_note):
      resolved  — True if this setup resolved (should be dropped from
                  pending), False if it's still open (should stay in
                  pending).
      loss_note — a dict ({"note", "experiment", "variant", "r_final"}) for
                  every loss (note = format_case_note()'s output), else
                  None for a win. Not sent from here — the caller decides
                  what (if anything) reaches Telegram (as of 2026-09-08,
                  per chat: nothing — see the SUPPRESSED note near the end
                  of update_pending_shadow_setups()). The case itself is
                  ALWAYS permanently logged via open_trade_case() above
                  regardless of what this dict is used for — that's what
                  /cases and /case <n> read from."""
    setup["bars_open"] += 1
    direction = setup["direction"]
    ...
    if direction == "BUY":
        ...
        for r_level, r_val in ((setup["r3"], 3), (setup["r2"], 2), (setup["r1"], 1)):
            if high >= r_level:
                ...
                break
        # MAE tracking (2026-08-29, per chat — Trade Investigation
        # Bureau). Continuous, not discrete-R-level like MFE above,
        # because the discrete levels (r1/r2/r3) are only defined
        # on the FAVORABLE side by construction — there's no
        # equivalent "adverse r1/r2/r3" to snap to, and a rounded
        # continuous value is exactly what "how far against did
        # this go" needs. Worst (most negative) excursion so far.
        excursion_r = (low - setup["entry"]) / setup["risk"]
        setup["min_r_reached"] = round(min(setup["min_r_reached"], excursion_r), 2)
    else:
        ...
        for r_level, r_val in ((setup["r3"], 3), (setup["r2"], 2), (setup["r1"], 1)):
            if low <= r_level:
                setup["max_r_reached"] = max(setup["max_r_reached"], r_val)
                break
        excursion_r = (setup["entry"] - high) / setup["risk"]
        setup["min_r_reached"] = round(min(setup["min_r_reached"], excursion_r), 2)
    ...
    # RENAMED/EXPANDED (2026-08-29, per chat — Trade Investigation
    # Bureau). open_trade_case() now runs for EVERY resolution,
    # win or loss — a case file gets permanently logged either
    # way, unconditionally, regardless of Telegram push behavior.
    #
    # PUSH HISTORY (per chat, 2026-09-08): originally called
    # send_telegram(format_case_note(_case)) directly here, one message
    # per loss. That spammed 50+ messages the moment a backlog of stuck
    # pending trades (see the isolation fix above) all resolved in one
    # catch-up pass. A digest-above-threshold version replaced it, then
    # was itself dropped as "still spamming" — routine loss resolutions
    # now push NOTHING to Telegram; /cases and /case <n> are the on-demand
    # way to see them, reading from the exact same permanent log this
    # writes to either way. Formatted note still built and handed back to
    # the caller (not sent here) purely for the console-only visibility
    # print at the end of update_pending_shadow_setups().
    loss_note = None
    try:
        _case = open_trade_case(setup, outcome_label, r_final, resolved_at)
        if _case is not None and r_final <= 0:
            loss_note = {
                "note": format_case_note(_case),
                "experiment": setup.get("experiment"),
                "variant": setup.get("variant"),
                "r_final": r_final,
            }
    except Exception as e:
        print("[TRADE INVESTIGATION BUREAU ERROR] " + str(e))

    return True, loss_note
...
def update_pending_shadow_setups(shadow_state, shadow_stats, df_5m, now_utc):
    ...
    ISOLATION (audit fix, 2026-09-08 — found via "EXP8 logging but never
    resolving for days"): every setup is now processed inside its own
    try/except, via _resolve_one_pending_setup() above. Before this fix,
    ALL pending setups across EVERY experiment were processed in one shared
    loop with a single try/except around the entire update_pending_shadow_
    setups() call (see the caller in run_min_pass()). One malformed pending
    entry — bad/missing field, a corrupt tag, anything — raised out of that
    shared loop and killed resolution for every experiment that pass. Worse:
    since the exception happened before shadow_state["pending"]/["last_
    processed_candle"] were ever reassigned, the NEXT pass reloaded the
    exact same unresolved queue and replayed the exact same candles against
    it, crashing at the exact same spot — a silent, permanent stall, visible
    only as a console "[SHADOW ERROR] update_pending: ..." line nobody was
    watching, until someone happened to notice a "resolved" count frozen
    while "logged" kept climbing.

    This generalizes the isolation drain_rejected_live_queue() already has
    for its own per-line loop ("one bad line shouldn't sink the whole
    drain") to the resolution path, which never got the same treatment.

    A setup that keeps failing is retried up to _SHADOW_RESOLVE_MAX_ERRORS
    times (transient failures — a bad candle read, a momentary data gap —
    should get a few more scans to sort themselves out, same spirit as the
    trade-log write retry above) before being force-quarantined: removed
    from pending, permanently recorded via _quarantine_pending_setup(), and
    alerted on Telegram exactly once. This avoids the two failure modes on
    either side of the old bug: blocking everything forever, or silently
    retrying a structurally-broken record every scan for the rest of time.
    """
    ...
    loss_notes_this_pass = []  # console-visibility only now — see SUPPRESSED note below
    ...
    for candle_time, candle in candles.iterrows():
        ...
        for setup in pending:
            try:
                resolved, loss_note = _resolve_one_pending_setup(
                    setup, high, low, close, candle_time, shadow_stats,
                    logged_id_set, logged_ids,
                )
            except Exception as e:
                err_count = setup.get("_resolve_error_count", 0) + 1
                setup["_resolve_error_count"] = err_count
                setup["_resolve_last_error"] = f"{type(e).__name__}: {e}"
                print(
                    f"[SHADOW RESOLVE ERROR] {setup.get('experiment')}/"
                    f"{setup.get('variant')} id={setup.get('id')} "
                    f"(attempt {err_count}/{_SHADOW_RESOLVE_MAX_ERRORS}): "
                    f"{type(e).__name__}: {e}"
                )
                if err_count >= _SHADOW_RESOLVE_MAX_ERRORS:
                    _quarantine_pending_setup(setup, setup["_resolve_last_error"])
                    quarantined_this_pass.append(setup)
                    # Deliberately NOT re-appended to still_pending — this is
                    # what actually stops the poison-pill effect. Everything
                    # else in `pending` this candle keeps processing normally
                    # regardless of what this one setup did.
                    continue
            ...
            if not resolved:
                still_pending.append(setup)
            elif loss_note is not None:
                loss_notes_this_pass.append(loss_note)
    ...
    if quarantined_this_pass:
        try:
            names = ", ".join(
                f"{s.get('experiment')}/{s.get('variant')} (`{s.get('id')}`)"
                for s in quarantined_this_pass
            )
            send_telegram(
                f"🚫 *{len(quarantined_this_pass)} shadow setup(s) quarantined* "
                f"after {_SHADOW_RESOLVE_MAX_ERRORS} failed resolve attempts "
                f"each — {names}.\n"
                "These would previously have silently blocked resolution for "
                "*every* experiment, every scan, indefinitely. They've now "
                "been removed from pending and logged to "
                "shadow_quarantine_log.jsonl for inspection; resolution for "
                "everything else continues normally."
            )
        except Exception:
            pass  # alerting is best-effort — must never block the resolution loop

    # SUPPRESSED (2026-09-08, per chat — "let it not tell me at all"):
    # routine shadow loss resolutions are no longer pushed to Telegram at
    # all, individually or as a digest (superseding the digest-threshold
    # approach immediately above, which was still "spamming" a smaller
    # burst). Nothing about the permanent record changes — open_trade_case()
    # above already wrote every resolution, win or loss, to the case log
    # unconditionally, regardless of what happens here. That's the on-demand
    # side: /cases (recent summaries) and /case <n> (full detail on one)
    # already read straight from that same permanent log, so this data was
    # never actually dependent on being pushed — it just also got pushed,
    # which is the part being turned off. Kept as a console-only print (zero
    # Telegram effect) purely so a catch-up burst is still visible in the
    # Action logs if ever needed for debugging.
    if loss_notes_this_pass:
        n = len(loss_notes_this_pass)
        by_exp = {}
        for item in loss_notes_this_pass:
            by_exp.setdefault(item["experiment"] or "UNKNOWN", 0)
            by_exp[item["experiment"] or "UNKNOWN"] += 1
        print(
            f"[SHADOW] {n} loss(es) resolved this pass, not pushed to "
            f"Telegram (see /cases): {dict(by_exp)}"
        )
...
# ---- Experiment 3: SLOT REUSED as Support/Resistance (per chat,
# 2026-08-15). The old EXP3_POI (Order Block vs quality FVG comparison)
# is RETIRED here, not just renamed — FVG graduated to a live Tier 1
# fallback POI (see _tier1_poi_evaluate()'s 2026-08-15 fix), which is
# exactly what EXP3_POI existed to validate before promotion. Unlike the
# EXP4_LIQUIDITY -> EXP4_POLICY_LAB slot reuse (2026-08-11), EXP3_POI had
# real accumulated history, so it was archived and reported rather than
# silently left in place — see flush_exp3.py, run once, manually, before
# this code ever runs. This is a genuinely different idea from EXP3_POI,
# not a continuation: not a range-trading strategy, structural S/R that
# assists the CURRENT trend via flip+retest. Full design rationale lives
# on discover_sr_candidates()/update_sr_level() (scanner_observation.py)
# and the SR_* constants (scanner_common.py). Shadow-only — same
# graduation path FVG itself just completed; nothing here reads these
# constants from a live gate, and scanner_live.py doesn't import any of
# this at all.
SR_LEVELS_STATE_FILE = os.path.join(BASE_DIR, "sr_levels_state.json")


def load_sr_levels_state():
    try:
        with open(SR_LEVELS_STATE_FILE, "r") as f:
            state = json.load(f)
        if state.get("methodology_version") != SR_METHODOLOGY_VERSION:
            return {"levels": [], "next_id": 1, "methodology_version": SR_METHODOLOGY_VERSION}
        return state
    except Exception:
        return {"levels": [], "next_id": 1, "methodology_version": SR_METHODOLOGY_VERSION}


def save_sr_levels_state(sr_state):
    try:
        atomic_write_json(SR_LEVELS_STATE_FILE, sr_state)
    except Exception as e:
        print("[SR STATE SAVE ERROR] " + str(e))


def _sr_bar_index(df_15m):
    """Monotonic counter for cooldown/age math — just the running 15M
    candle count. Doesn't need to mean anything outside this tracker,
    only needs to strictly increase scan-to-scan, which len() already
    guarantees as more candles accumulate."""
    return int(len(df_15m))


def run_sr_tracking(facts, phase_result, macro_bias, current_atr_pips, now_utc,
                     sr_state, shadow_state, shadow_stats):
    """
    Driver — same shape as every other stateful tracker in this file
    (run_leg_observation, run_market_intent_tracking): the PURE
    functions in scanner_observation.py compute, this persists.

    Each scan: advances every tracked level one step via
    update_sr_level(), discovers new candidates via
    discover_sr_candidates() (deduped against already-tracked levels by
    rounded anchor price — a candidate anchored on a price already being
    tracked is not a new level), and logs a shadow trade to the new
    EXP3_SR key whenever update_sr_level() reports a RETEST_REACTION —
    the flip+retest entry this whole detector exists to find.
    phase_result here is a read-only view of LIVE's already-committed
    market_phase/market_phase_age_bars (per chat, physiology fix — MIN
    no longer independently recomputes phase; see run_min_pass()), not
    a fresh MIN-side computation. This still can't be influenced by
    which live tier fired this scan, and for a stronger reason than
    before: LIVE commits phase strictly BEFORE it ever evaluates a tier
    (see scanner_live.py's pass order), so the value read here was
    already locked in prior to any signal decision, regardless of which
    organ reads it afterward — the same "can't cheat by seeing the
    outcome" property Market Intent has, for the same reason (see
    build_market_intent()'s docstring).

    Never raises — caller wraps this the same as every other tracker in
    this file; a bug here affects a shadow log entry, never the live
    bot, which doesn't import any of this.
    """
    bar_index = _sr_bar_index(facts.df_15m)

    updated_levels = []
    for level in sr_state.get("levels", []):
        new_level, event = update_sr_level(level, facts, macro_bias, bar_index)
        if event == "FLIPPED":
            print(f"  [SR] Level #{level['id']} flipped -> {new_level['role']} "
                  f"(flip {new_level['flip_count']}/{SR_MAX_FLIPS})")
        elif event == "RETEST_REACTION":
            print(f"  [SR] Level #{level['id']} retest + reaction, "
                  f"role={new_level['role']}, bias-aligned")
            side = bias_to_side(macro_bias)
            entry = float(facts.last_candle_5m()["Close"])
            # SL beyond the FAR side of the zone — same convention as
            # OB/FVG: for a BUY the invalidation is a close back below
            # the (now-support) zone; for a SELL, back above it.
            sl_raw = new_level["zone_low"] if side == "BUY" else new_level["zone_high"]
            leg_id = f"sr|{new_level['id']}|{new_level['flip_count']}"
            setup = build_shadow_setup(
                "EXP3_SR", side, entry, sl_raw, now_utc,
                variant=new_level["role"],
                tags={
                    "flip_count": new_level["flip_count"],
                    "zone_width_pips": round((new_level["zone_high"] - new_level["zone_low"]) / PIP_SIZE, 1),
                    "age_bars": bar_index - new_level["born_bar"],
                },
                note=f"S/R flip+retest — level #{new_level['id']} flipped to "
                     f"{new_level['role']} (flip {new_level['flip_count']}), bias-aligned reaction",
                atr_pips=current_atr_pips,
            )
            log_shadow_setup(shadow_state, shadow_stats, setup, leg_id)
        elif event == "INVALIDATED":
            print(f"  [SR] Level #{level['id']} invalidated (flip_count={level['flip_count']})")
        updated_levels.append(new_level)

    known_anchors = {round(lvl["anchor_price"], 5) for lvl in updated_levels}
    for cand in discover_sr_candidates(facts, phase_result):
        if round(cand["anchor_price"], 5) in known_anchors:
            continue
        new_id = sr_state.get("next_id", 1)
        sr_state["next_id"] = new_id + 1
        level = {
            "id": new_id, "role": cand["role"],
            "zone_low": cand["zone_low"], "zone_high": cand["zone_high"],
            "anchor_price": cand["anchor_price"],
            "flip_count": 0, "last_flip_bar": None,
            "born_bar": bar_index, "state": "ACTIVE",
            "retested_since_flip": False,
        }
        updated_levels.append(level)
        known_anchors.add(round(cand["anchor_price"], 5))
        print(f"  [SR] New level #{new_id} discovered — {cand['role']} @ {cand['anchor_price']:.5f}")

    # Prune terminal levels beyond a retention window — same
    # SR_MAX_AGE_CANDLES-based idea as the age-based invalidation itself,
    # so this state file doesn't grow forever with dead levels.
    sr_state["levels"] = [
        lvl for lvl in updated_levels
        if lvl["state"] != "INVALIDATED" or (bar_index - lvl["born_bar"]) <= SR_MAX_AGE_CANDLES * 2
    ]
    return sr_state
...
# /accounts lists them, /use switches the active one, /setaccount adds
# or updates one. This is genuinely persistent (unlike Bank's own
# simulation below, which is deliberately stateless) — a balance the
# user sets has to survive across scans, so ACCOUNT_PROFILES_FILE MUST
# be in live-scan.yml's commit step (checked directly, not just
# commented — same lesson learned twice already this session).
# =========================================================================

def load_account_profiles():
    try:
        with open(ACCOUNT_PROFILES_FILE, "r") as f:
            state = json.load(f)
        if state.get("methodology_version") != ACCOUNT_PROFILES_METHODOLOGY_VERSION:
            return {"methodology_version": ACCOUNT_PROFILES_METHODOLOGY_VERSION, "active": None, "profiles": {}}
        return state
    except Exception:
        return {"methodology_version": ACCOUNT_PROFILES_METHODOLOGY_VERSION, "active": None, "profiles": {}}


def save_account_profiles(state):
    try:
        atomic_write_json(ACCOUNT_PROFILES_FILE, state)
    except Exception as e:
        print("[ACCOUNT PROFILES SAVE ERROR] " + str(e))


def active_account_profile():
    """
    Returns (balance, risk_pct, display_name) for the currently active
    account profile. display_name is None when no profile is active —
    the caller (Bank's format functions) uses that to decide whether to
    say "against your Live account" or "no account set, $10,000
    placeholder." Falls back to BANK_STARTING_CAPITAL/BANK_RISK_PCT so
    Bank keeps working exactly as before for anyone who never sets up a
    profile at all — this is additive, not a requirement.
    """
    state = load_account_profiles()
    active = state.get("active")
    profiles = state.get("profiles", {})
    if active and active in profiles:
        p = profiles[active]
        return p["balance"], p.get("risk_pct", BANK_RISK_PCT), p.get("name", active)
    return BANK_STARTING_CAPITAL, BANK_RISK_PCT, None


def format_position_size_line(risk_pips):
    """
    Builds the one Telegram line the live fire alert adds for real $ risk
    and a lot-size recommendation (per chat, 2026-08-16 — the Risk Engine
    piece of the friend's Structural Stop Engine -> Risk Engine -> Bank
    layering). Always against the ACTIVE account profile via
    active_account_profile() — this codebase's existing single-active-
    account design (/use), not the friend's original "show every account
    at once" mockup, which /use already superseded.

    Degrades honestly rather than guessing: no account profile set ->
    says so and points at /setaccount, using the BANK_STARTING_CAPITAL
    placeholder so the line still shows a number. below_min_lot/
    sanity_capped both get their own explicit wording — never silently
    rounds a warning away.
    """
    balance, risk_pct, name = active_account_profile()
    sizing = calculate_position_size(risk_pips, balance, risk_pct)
    if sizing is None:
        return "💰 *Position size:* _couldn't be calculated (bad stop distance)_"

    label = name if name else f"${balance:,.0f} placeholder — `/setaccount` for a real one"

    if sizing["sanity_capped"]:
        return (f"💰 *Position size ({label}):* \u26a0\ufe0f stop is implausibly tight — "
                f"sizing suppressed, check the setup before trading it")
    if sizing["below_min_lot"]:
        return (f"💰 *Position size ({label}):* risking `${sizing['risk_dollars']:,.0f}` "
                f"at this stop needs less than the `{MIN_LOT_SIZE}` lot floor — "
                f"either skip it or accept risking more than {risk_pct*100:.2f}% at `{MIN_LOT_SIZE}` lot")
    return (f"💰 *Position size ({label}):* `{sizing['lots']}` lot — "
            f"risking `${sizing['risk_dollars']:,.0f}` ({risk_pct*100:.2f}% of `${balance:,.0f}`)")
...
def format_accounts_list():
    ...
    if not profiles:
        return (
            "\U0001F464 *Accounts*\n" + "\u2500" * 25 + "\n"
            f"_No account profiles set up yet \u2014 /bank is using a "
            f"${BANK_STARTING_CAPITAL:,.0f} placeholder, not a real balance. "
            f"Add one: `/setaccount live 5000` (name, balance, optional risk% "
            f"e.g. `/setaccount live 5000 0.5`)._"
        )
    lines = ["\U0001F464 *Accounts*", "\u2500" * 25]
    for key, p in profiles.items():
        marker = "\U0001F7E2" if key == active else "\u26AA"
        risk = p.get("risk_pct", BANK_RISK_PCT) * 100
        lines.append(f"{marker} *{p.get('name', key)}*")
        lines.append(f"   Balance `${p['balance']:,.0f}`   Risk `{risk:.2f}%`")
    lines.append("")
    lines.append("_`/use <name>` to switch \u2014 `/setaccount <name> <balance> [risk%]` "
                 "to add/update \u2014 `/delaccount <name>` to remove._")
    return "\n".join(lines)


def use_account_profile(name):
    """/use <name>"""
    state = load_account_profiles()
    key = name.strip().lower()
    if key not in state.get("profiles", {}):
        return f"\U0001F464 _No account named `{name}`. `/accounts` to see what's set up._"
    state["active"] = key
    save_account_profiles(state)
    p = state["profiles"][key]
    return (f"\U0001F464 Switched to *{p.get('name', key)}* \u2014 "
            f"balance `${p['balance']:,.0f}`, risk `{p.get('risk_pct', BANK_RISK_PCT)*100:.2f}%`. "
            f"`/bank` now uses this account.")


def set_account_profile(name, balance, risk_pct=None):
    """/setaccount <name> <balance> [risk_pct as a percent, e.g. 0.5 for 0.5%]"""
    state = load_account_profiles()
    key = name.strip().lower()
    profile = {
        "name": name.strip(),
        "balance": float(balance),
        "risk_pct": (float(risk_pct) / 100.0) if risk_pct is not None else BANK_RISK_PCT,
    }
    state.setdefault("profiles", {})[key] = profile
    became_active = not state.get("active")
    if became_active:
        state["active"] = key
    save_account_profiles(state)
    return (
        f"\U0001F464 Saved *{profile['name']}* \u2014 balance `${profile['balance']:,.0f}`, "
        f"risk `{profile['risk_pct']*100:.2f}%`."
        + (" Set as active (first account)." if became_active else " `/use " + key + "` to make this the active one.")
    )


def delete_account_profile(name):
    """/delaccount <name>"""
    state = load_account_profiles()
    key = name.strip().lower()
    if key not in state.get("profiles", {}):
        return f"\U0001F464 _No account named `{name}`._"
    del state["profiles"][key]
    if state.get("active") == key:
        state["active"] = next(iter(state["profiles"]), None)
    save_account_profiles(state)
    return f"\U0001F464 Removed `{name}`."


# =========================================================================
# BANK — simulated capital per experiment. Per chat, 2026-08-16: "The Bank
# doesn't control the other departments. It gives them a financial
# environment in which their decisions have consequences." Turns each
# experiment's raw R-multiple outcomes into a simulated equity curve, so
# /bank can show P&L / drawdown / return% per experiment — capital
# consequence, not just win rate — and rank them by how they "repay
# confidence," directly per chat.
#
# Deliberately STATELESS. No bank_state.json, nothing persisted between
# runs at all — every number here is replayed fresh from the PERMANENT
# shadow_trade_log.jsonl each time /bank is called, same "recompute from
# the source of truth" discipline compute_evidence() already uses. This
# is a deliberate choice, not an oversight: a persisted running-equity
# state file is exactly the failure mode found TWICE already this same
# session (Market Intent's and S/R's state silently never surviving a
# GitHub Actions run because nothing added them to a workflow's commit
# step). Bank has nothing to forget to commit, because replaying a few
# hundred resolved trades costs nothing at today's log sizes.
#
# Does NOT reallocate capital between experiments, and never will
# automatically — per the friend's own framing, Bank OBSERVES and
# financially contextualizes; it doesn't control. If "EXP3_SR deserves
# more capital" ever becomes a real decision, that's a human reading
# this report and choosing, not this code doing it.
# =========================================================================

def _apply_bank_trade(equity, r_achieved, risk_pct, cost_r):
    """PURE. ONE trade's worth of the Bank compounding formula — risk_pct
    of CURRENT equity, minus the flat cost_r. Factored out of
    simulate_bank_account()'s loop so settle_bank_transactions() (the
    permanent ledger) applies the exact same arithmetic instead of a
    second, driftable copy of it. Returns (new_equity, risk_dollars) —
    the ledger needs risk_dollars too, for the transaction record;
    simulate_bank_account() only needs new_equity."""
    risk_dollars = equity * risk_pct
    effective_r = r_achieved - cost_r
    return equity + effective_r * risk_dollars, risk_dollars


def simulate_bank_account(records, starting_capital=BANK_STARTING_CAPITAL,
                           risk_pct=BANK_RISK_PCT, cost_r=BANK_COST_R):
    """
    PURE. Replays a list of resolved outcomes (each needing only
    r_achieved and a timestamp to sort by) against a simulated,
    compounding equity curve: risk_pct of CURRENT equity risked per
    trade — 0.5%, the user's real risk tolerance, so this stays
    apples-to-apples with a real account if one ever plugs into the same
    model — minus a flat per-trade cost in R terms (cost_r) bundling
    spread/slippage/commission/allocation fee into one number, per the
    friend's "Bank fee: 0.05R" example, rather than fabricating three
    separately-precise cost lines this codebase has no real execution
    data to back up.

    "records" is intentionally generic, not shadow-experiment-specific —
    per chat, 2026-08-16: "the bank department isn't tied to exps."
    Anything that can honestly express its outcomes as a sequence of
    (r_achieved, resolved_at) pairs can get a Bank account; see
    _leg_obs_as_bank_records() below for Forward Observation's version
    of this, which is NOT a shadow-pipeline record at all.

    Records are sorted by resolved_at (falling back to opened_at) before
    replay — order matters for a compounding curve; an out-of-order
    input would silently produce a different, wrong curve.

    Returns a dict: starting_capital, equity, realized_pl, return_pct,
    peak_equity, max_drawdown_dollars, max_drawdown_pct, n_trades,
    win_rate, current_risk_dollars (0.5% of CURRENT equity — what the
    next trade would risk if this were a real, live account), plus
    profit_factor and expectancy_r (both via _r_stats() — reused, not
    reimplemented, exact same PF/expectancy math the Failure Bureau and
    /calibration already use elsewhere in this file).
    """
    ...
    sorted_records = sorted(records, key=lambda r: r.get("resolved_at") or r.get("opened_at") or "")
    for r in sorted_records:
        r_achieved = r.get("r_achieved")
        if r_achieved is None:
            continue
        equity, _ = _apply_bank_trade(equity, r_achieved, risk_pct, cost_r)
    ...
    r_stats = _r_stats([r for r in sorted_records if r.get("r_achieved") is not None]) or {}
    n = r_stats.get("n", 0)

    return {
        "starting_capital": round(starting_capital, 2),
        "equity": round(equity, 2),
        "realized_pl": round(equity - starting_capital, 2),
        "return_pct": round(100 * (equity - starting_capital) / starting_capital, 1) if starting_capital else None,
        "peak_equity": round(peak, 2),
        "max_drawdown_dollars": round(max_dd_dollars, 2),
        "max_drawdown_pct": round(100 * max_dd_pct, 1),
        "n_trades": n,
        "win_rate": round(100 * r_stats["win_rate"], 1) if r_stats.get("win_rate") is not None else None,
        "profit_factor": r_stats.get("pf"),
        "expectancy_r": r_stats.get("avg_r"),
        "current_risk_dollars": round(equity * risk_pct, 2),
    }


def _read_leg_obs_log():
    """Reads the PERMANENT leg_obs_log.jsonl. Pure, safe to call from a
    Telegram command. Same pattern as _read_shadow_trade_log() /
    _read_market_intent_log() — added here since no standalone reader
    existed before (format_leg_obs_summary() read the file inline)."""
    try:
...
def _leg_obs_as_bank_records():
    """
    Converts Forward Observation's resolved leg records into the generic
    {r_achieved, resolved_at} shape simulate_bank_account() needs. Per
    chat, 2026-08-16: "the bank department isn't tied to exps, it can
    also help show realistic calibration of forward observation... the
    outcomes the bot tagged as high probability and predicted."

    Deliberately simple, explicitly labeled PROXY — not a fabricated R
    multiple, and this is NOT a trade:
      CONTINUED (the leg extended further in the SAME direction Market
                 Thesis was tracking)  -> +1.0
      REVERSED  (a new leg formed in the OPPOSITE direction)  -> -1.0
      INVALIDATED (origin violated or a 78.6%+ retrace, nothing new
                 forming) -> EXCLUDED entirely. Not a clean directional
                 resolution either way; forcing it into +-1 would be
                 exactly the kind of manufactured precision this
                 codebase avoids everywhere else (EVIDENCE_MIN_N gating,
                 banded not numeric failure_risk, etc.).

    No entry, no stop, no position size — this answers a narrower,
    honest question: "if you'd bet WITH the bias every time a leg
    formed, how would flat +-1 betting have done?" /legobs scenario
    already reports P(REVERSED) by (phase, cause, bucket) off this same
    resolved population — this is that population read through a
    capital-consequence lens instead of a probability lens.
    """
    out = []
    for r in _read_leg_obs_log():
        fate = r.get("fate")
        if fate == "CONTINUED":
            r_achieved = 1.0
        elif fate == "REVERSED":
        ...
        out.append({
            ...
            "leg_id": r.get("leg_id"),   # ledger dedup key (see _bank_txn_key) — not read by simulate_bank_account
        })
    return out


_BANK_DISPLAY_NAME = {
    "EXP4_POLICY_LAB::RISK_AT_HAND":        "EXP4 \u2014 Risk-at-Hand (live policy)",
    "EXP4_POLICY_LAB::CONVICTION_SWEEP_ON":  "EXP4 \u2014 Conviction Gate ON",
    "EXP4_POLICY_LAB::CONVICTION_SWEEP_OFF": "EXP4 \u2014 Conviction Gate OFF",
    "FORWARD_OBSERVATION":                   "Forward Observation (bias calibration)",
}

def _bank_aliases():
    """Merges Bank's own aliases with _SHADOW_ALIASES (defined later in
    this file, near /shadow's dispatch) — built lazily, on each call,
    rather than as a module-level constant, since _SHADOW_ALIASES isn't
    defined yet at the point this module executes top-to-bottom. Cheap;
    both dicts are tiny."""
    aliases = dict(_SHADOW_ALIASES)
    aliases.update({
        "forward": "FORWARD_OBSERVATION", "fo": "FORWARD_OBSERVATION",
        "forwardobservation": "FORWARD_OBSERVATION", "legobs": "FORWARD_OBSERVATION",
        "policyrah": "EXP4_POLICY_LAB::RISK_AT_HAND", "riskathand": "EXP4_POLICY_LAB::RISK_AT_HAND",
        "convictionon": "EXP4_POLICY_LAB::CONVICTION_SWEEP_ON",
        "convictionoff": "EXP4_POLICY_LAB::CONVICTION_SWEEP_OFF",
    })
    return aliases


def _bank_records_by_source():
    """Groups every resolved record Bank knows about into its SOURCE
    ("account") key — EXP4_POLICY_LAB split by policy tag, every other
    shadow experiment by its own name, Forward Observation as its own
    non-shadow department. Extracted out of compute_bank_accounts() (per
    chat, 2026-08-16) so settle_bank_transactions() groups records
    identically instead of reimplementing this same key logic a second
    time — the two would silently drift apart otherwise. Returns
    {source_key: [records]}, unfiltered by MIN_N — callers gate on
    sample size themselves."""
    by_key = {}
    for r in _read_shadow_trade_log():
        if r.get("r_achieved") is None:
            continue
        exp = r.get("experiment")
        if exp == "EXP4_POLICY_LAB":
            policy = r.get("tags", {}).get("policy")
            key = f"EXP4_POLICY_LAB::{policy}" if policy else None
        else:
            key = exp
        if key:
            by_key.setdefault(key, []).append(r)

    fo_records = _leg_obs_as_bank_records()
    if fo_records:
        by_key["FORWARD_OBSERVATION"] = fo_records

    return by_key


def compute_bank_accounts(min_n=BANK_MIN_N, starting_capital=BANK_STARTING_CAPITAL,
                           risk_pct=BANK_RISK_PCT):
    """
    Drives simulate_bank_account() across every SOURCE with enough
    resolved history — not just Shadow Pipeline experiments (per chat,
    2026-08-16, "the bank department isn't tied to exps"). Currently
    two source families:

      - Every Shadow Pipeline experiment (EXP4_POLICY_LAB split by its
        `policy` tag into three separate accounts — RISK_AT_HAND /
        CONVICTION_SWEEP_ON / CONVICTION_SWEEP_OFF — so the three
        alternate policies from the 2026-08-11 refactor replay against
        the identical population of setups and are directly comparable
        head-to-head, not blended into one number).
      - Forward Observation, via _leg_obs_as_bank_records() — a
        genuinely different DEPARTMENT, not a shadow experiment at all.

    starting_capital/risk_pct default to the BANK_* placeholders but are
    meant to be overridden with active_account_profile()'s values by the
    caller — this is what makes /bank show real numbers once an account
    profile exists, without this function needing to know anything about
    profiles itself.

    Same MIN_N-gating discipline as everywhere else in this file — an
    account simulated off a handful of trades is noise dressed up as a
    P&L statement.

    Returns an ordered list of (account_name, stats_dict) tuples ranked
    by return_pct descending — "how they repay confidence," per chat.
    """
    by_key = _bank_records_by_source()
    accounts = []
    for key, records in by_key.items():
        if len(records) < min_n:
            continue
        accounts.append((key, simulate_bank_account(
            records, starting_capital=starting_capital, risk_pct=risk_pct)))

    accounts.sort(key=lambda kv: kv[1]["return_pct"], reverse=True)
    return accounts


def _bank_account_name(key):
    return _BANK_DISPLAY_NAME.get(key, _SHADOW_DISPLAY_NAME.get(key, key))


# =========================================================================
# BANK LEDGER — permanent, append-only transaction record. Per chat,
# 2026-08-16 (later same day as Bank/Account Profiles shipped): "let
# every process and transaction be permanently logged... a bank id would
# be nice in case we need something in the future." See scanner_common.
# py's BANK section for the full "why" (simulate_bank_account() alone
# silently rewrites all of history if BANK_RISK_PCT/BANK_COST_R ever
# change — there was no durable record of what actually happened at the
# time). This section is ADDITIVE: format_bank_home/performance/
# account_detail below are UNCHANGED, still reading the live recompute.
# =========================================================================

def _bank_txn_key(source, record):
    """Stable natural key for one resolved record within one Bank
    source — what settle_bank_transactions() dedups against so the same
    underlying outcome never becomes a second permanent transaction.
    Shadow-pipeline records already carry a unique id (build_shadow_
    setup's uuid, permanent through to shadow_trade_log.jsonl); Forward
    Observation records carry leg_id instead (see
    _leg_obs_as_bank_records) — paired with resolved_at since a leg_id
    CAN recur after a reopen (see LEG_OBS_CLOSED_MAX elsewhere in this
    file), so leg_id alone isn't safely unique on its own."""
    rid = record.get("id")
    if rid:
        return f"{source}|{rid}"
    return f"{source}|{record.get('leg_id')}|{record.get('resolved_at')}"


def _read_bank_transactions():
    """Reads the PERMANENT bank_transactions.jsonl. Same pattern as
    _read_leg_obs_log()/_read_shadow_trade_log() — filtered to the
    current methodology version, safe to call from a Telegram command."""
    try:
        with open(BANK_TRANSACTIONS_FILE, "r") as f:
            return [
                rec for rec in (json.loads(ln) for ln in f if ln.strip())
                if rec.get("methodology_version") == BANK_LEDGER_METHODOLOGY_VERSION
            ]
    except FileNotFoundError:
        return []
    except Exception:
...
def _append_bank_transaction(record):
    """Permanently appends ONE settled Bank transaction to
    BANK_TRANSACTIONS_FILE. Append-only — never truncated or rewritten.
    Same write-hardening as every other permanent log in this file
    (retries SHADOW_TRADE_LOG_WRITE_RETRIES times before giving up, same
    shared retry constants — same file-write failure modes, no reason
    for different tuning). Returns True/False so the caller
    (settle_bank_transactions) knows whether it's actually safe to treat
    this record as settled."""
    line = json.dumps(record, default=_json_default) + "\n"
    last_error = None
    for attempt in range(1, SHADOW_TRADE_LOG_WRITE_RETRIES + 1):
        try:
            with open(BANK_TRANSACTIONS_FILE, "a") as f:
                f.write(line)
                f.flush()
                os.fsync(f.fileno())
            return True
            ...
            if attempt < SHADOW_TRADE_LOG_WRITE_RETRIES:
                time.sleep(SHADOW_TRADE_LOG_WRITE_RETRY_DELAY_SEC)
    print(f"[BANK LEDGER ERROR] gave up after {SHADOW_TRADE_LOG_WRITE_RETRIES} "
          f"attempts: {last_error}")
    return False


def settle_bank_transactions():
    """
    Turns every newly-resolved Bank-eligible outcome into a PERMANENT
    ledger entry — one bank_id per settled record, written once and
    never rewritten. Called every MIN pass (see run_min_pass()),
    independent of candle freshness, same reasoning as
    drain_rejected_live_queue: this only replays already-resolved
    permanent-log entries (shadow_trade_log.jsonl / leg_obs_log.jsonl
    via _bank_records_by_source()), no fresh candles needed.

    Each source's running equity carries forward from the LAST
    transaction already on file for that source (or BANK_STARTING_
    CAPITAL if this source has never settled before) — genuine
    compounding across scans, using the risk_pct/cost_r actually in
    effect AT SETTLEMENT TIME, not whatever they happen to be set to the
    next time someone recomputes. Uses BANK_STARTING_CAPITAL as the
    anchor (matching compute_bank_accounts()'s own default) — NOT the
    active account profile's real balance; Bank's simulation and Account
    Profiles' real balance are still two deliberately separate things,
    per the existing BANK_RISK_PCT comment in scanner_common.py.

    Does NOT change what /bank displays today — format_bank_home/
    performance/account_detail still call compute_bank_accounts(),
    unchanged, still a stateless recompute. This is the durable record
    sitting underneath it, not a replacement for it (yet).

    Returns the number of NEW transactions settled this pass — 0 most
    passes, since most scans have nothing newly resolved to settle.
    """
    existing = _read_bank_transactions()
    already_settled = set()
    last_equity = {}
    for txn in existing:
        already_settled.add(txn.get("txn_key"))
        # Append-only, and always written in resolved_at order PER
        # SOURCE (sorted below before writing) — so simply overwriting
        # as we scan through in file order leaves the LAST value seen
        # for a source as that source's most recent equity.
        last_equity[txn["source"]] = txn["equity_after"]

    by_key = _bank_records_by_source()
    settled_count = 0
    now_iso = datetime.now(timezone.utc).isoformat()

    for source, records in by_key.items():
        sorted_records = sorted(records, key=lambda r: r.get("resolved_at") or "")
        for r in sorted_records:
            r_achieved = r.get("r_achieved")
            if r_achieved is None:
                continue
            key = _bank_txn_key(source, r)
            if key in already_settled:
                continue

            equity_before = last_equity.get(source, BANK_STARTING_CAPITAL)
            equity_after, risk_dollars = _apply_bank_trade(
                equity_before, r_achieved, BANK_RISK_PCT, BANK_COST_R)

            txn = {
                "bank_id": uuid.uuid4().hex[:12],
                "methodology_version": BANK_LEDGER_METHODOLOGY_VERSION,
                "txn_key": key,
                "source": source,
                "r_achieved": r_achieved,
                "risk_pct": BANK_RISK_PCT,
            ...
            if _append_bank_transaction(txn):
                already_settled.add(key)
                last_equity[source] = equity_after
                settled_count += 1
            else:
                # Write failed — do NOT mark this record settled; next
                # pass retries it. Same "don't mark it done unless the
                # disk confirms it" discipline as every other permanent
                # log's append function in this file. Stop this SOURCE
                # here (order matters for compounding) but let other
                # sources still settle this pass.
                break

    if settled_count:
        print(f"[BANK LEDGER] settled {settled_count} new transaction(s)")
    return settled_count


def format_bank_home(min_n=BANK_MIN_N):
    """
    /bank (no argument) — the home screen. Brief, not exhaustive: total
    accounts, combined P/L, biggest winner/loser at a glance, then a
    pointer to /bank performance and /bank <account> for depth. Mirrors
    the Shadow Pipeline's own /shadow (summary) vs /shadow <experiment>
    (drill-down) split, per chat: "/shadow shows as it always has buh
    /bank shows the financial side of things."

    Uses the active account profile (see active_account_profile(),
    2026-08-16) if one is set — real balance, real risk% — and falls
    back to the BANK_* placeholder otherwise. Either way, the header
    says plainly which one this is; nothing here silently presents a
    placeholder as though it were real.
    """
    balance, risk_pct, profile_name = active_account_profile()
    accounts = compute_bank_accounts(min_n=min_n, starting_capital=balance, risk_pct=risk_pct)
    header = "\U0001F3E6 *Bank*\n" + "\u2500" * 25
    basis = (f"against your *{profile_name}* account (${balance:,.0f}, {risk_pct*100:.2f}% risk)"
             if profile_name else
             f"simulated \u2014 no account set, ${balance:,.0f} placeholder (`/accounts` to add one)")
    if not accounts:
        return (
            f"{header}\n_No account has {min_n}+ resolved outcomes yet \u2014 nothing to "
            f"simulate. Would run {basis}, {BANK_COST_R:.2f}R cost/trade, once there's "
            f"enough history._"
        )

    total_pl = sum(s["realized_pl"] for _, s in accounts)
    profitable = [k for k, s in accounts if s["realized_pl"] > 0]
    best_key, best_s = accounts[0]
    worst_key, worst_s = accounts[-1]

    lines = [
        header,
        f"_{basis.capitalize()}, {len(accounts)} accounts \u2014 shadow-only, never gates "
        f"anything live. Combined P/L across all accounts: `{total_pl:+,.0f}` "
        f"({len(profitable)}/{len(accounts)} profitable)._",
        "",
        f"\U0001F7E2 Best: *{_bank_account_name(best_key)}* "
        f"`{best_s['return_pct']:+.1f}%` (`{best_s['realized_pl']:+,.0f}`)",
        f"\U0001F534 Worst: *{_bank_account_name(worst_key)}* "
        f"`{worst_s['return_pct']:+.1f}%` (`{worst_s['realized_pl']:+,.0f}`)",
        "",
        "*All accounts:*",
    ]
    for key, s in accounts:
        emoji = "\U0001F7E2" if s["realized_pl"] > 0 else "\U0001F534"
        lines.append(f"  {emoji} {_bank_account_name(key)}  `{s['return_pct']:+.1f}%`  (n=`{s['n_trades']}`)")

    lines.append("")
    lines.append("_`/bank performance` for aggregate stats \u2014 `/bank <name>` to drill in "
                  "(e.g. `/bank forward`, `/bank exp7`)._")
    return "\n".join(lines)


def format_bank_performance(min_n=BANK_MIN_N):
    """
    /bank performance — capital performance, not individual trades.
    Combined-portfolio stats (every account's outcomes pooled into one
    replay) plus a best/worst capital-efficiency callout — per chat and
    the friend's own framing, "capital consequence, not conviction."

    Uses the active account profile if one is set — see
    format_bank_home()'s docstring for the same real-vs-placeholder
    labeling rule, applied here too.
    """
    balance, risk_pct, profile_name = active_account_profile()
    accounts = compute_bank_accounts(min_n=min_n, starting_capital=balance, risk_pct=risk_pct)
    if not accounts:
        return f"\U0001F3E6 *Bank Performance*\n_No account has {min_n}+ resolved outcomes yet._"

    all_records = []
    for r in _read_shadow_trade_log():
        if r.get("r_achieved") is not None:
            all_records.append(r)
    all_records.extend(_leg_obs_as_bank_records())
    combined = simulate_bank_account(all_records, starting_capital=balance, risk_pct=risk_pct)

    best_key, best_s = accounts[0]
    worst_key, worst_s = accounts[-1]
    basis = (f"your *{profile_name}* account" if profile_name else
             "no account set \u2014 placeholder")

    lines = [
        "\U0001F3E6 *Bank Performance \u2014 All Time, Combined*",
        "\u2500" * 25,
        f"_Basis: {basis}. Every account's outcomes pooled into ONE replay \u2014 not a sum "
        "of the individual equity curves, which would double-count compounding. This is "
        f"what one ${balance:,.0f} account would show if it had taken every resolved "
        "outcome across every department, in time order._",
        "",
        f"Starting Capital   `${combined['starting_capital']:,.0f}`",
        f"Current Equity     `${combined['equity']:,.0f}`",
        f"Net P/L            `{combined['realized_pl']:+,.0f}`",
        f"Return             `{combined['return_pct']:+.1f}%`",
        "",
        f"Peak Equity        `${combined['peak_equity']:,.0f}`",
        f"Max Drawdown       `-${combined['max_drawdown_dollars']:,.0f}` "
        f"(`{combined['max_drawdown_pct']}%`)",
        "",
        f"Profit Factor      `{combined['profit_factor'] if combined['profit_factor'] is not None else '—'}`",
        f"Win Rate           `{combined['win_rate']}%`",
        f"Expectancy         `{combined['expectancy_r']:+.2f}R`" if combined['expectancy_r'] is not None else "Expectancy         `\u2014`",
        "",
        "\u2500" * 25,
        f"\U0001F7E2 *Best capital user:* {_bank_account_name(best_key)}",
        f"   `{best_s['realized_pl']:+,.0f}` ({best_s['return_pct']:+.1f}%) \u2014 Max DD `{best_s['max_drawdown_pct']}%`",
        "",
        f"\U0001F534 *Worst capital user:* {_bank_account_name(worst_key)}",
        f"   `{worst_s['realized_pl']:+,.0f}` ({worst_s['return_pct']:+.1f}%) \u2014 Max DD `{worst_s['max_drawdown_pct']}%`",
    ]
    return "\n".join(lines)


def format_bank_account_detail(key):
    """/bank <name> — drill-down on one account, resolved via
    _BANK_ALIASES (same alias-resolution pattern as /shadow <experiment>).
    Uses the active account profile if one is set — same real-vs-
    placeholder rule as format_bank_home()."""
    resolved = _bank_aliases().get(key.strip().lower(), key)
    balance, risk_pct, profile_name = active_account_profile()
    accounts = dict(compute_bank_accounts(min_n=1, starting_capital=balance, risk_pct=risk_pct))
    # min_n=1 here — a thin account should still be drillable on request,
    # even though it's excluded from the ranked home screen; the "not
    # enough data" framing below still applies at the real BANK_MIN_N
    # threshold.
    s = accounts.get(resolved)
    if s is None:
        return (
            f"\U0001F3E6 _No account matches `{key}`, or it has fewer than "
            f"{BANK_MIN_N} resolved outcomes. Try `/bank` to see what's available._"
        )

    name = _bank_account_name(resolved)
    thin = s["n_trades"] < BANK_MIN_N
    lines = [f"\U0001F3E6 *{name}*", "\u2500" * 25]
    lines.append(f"_{'Against your ' + profile_name + ' account' if profile_name else 'Placeholder — no account set (`/accounts`)'}._")
    if thin:
        lines.append(f"_Only {s['n_trades']} resolved \u2014 below the {BANK_MIN_N} floor shown "
                      f"on the home screen. Numbers below are real, just noisy._")
    lines.append("")

    lines.extend([
        f"Starting Capital   `${s['starting_capital']:,.0f}`",
        f"Equity             `${s['equity']:,.0f}`",
        f"Net P/L            `{s['realized_pl']:+,.0f}`",
        f"Return             `{s['return_pct']:+.1f}%`",
        "",
        f"Peak Equity        `${s['peak_equity']:,.0f}`",
        f"Max Drawdown       `-${s['max_drawdown_dollars']:,.0f}` (`{s['max_drawdown_pct']}%`)",
        "",
        f"Trades             `{s['n_trades']}`",
        f"Win Rate           `{s['win_rate']}%`",
        f"Profit Factor      `{s['profit_factor'] if s['profit_factor'] is not None else '—'}`",
        f"Expectancy         `{s['expectancy_r']:+.2f}R`" if s['expectancy_r'] is not None else "Expectancy         `\u2014`",
        "",
        f"Current risk/trade `${s['current_risk_dollars']:,.0f}` "
        f"({risk_pct*100:.2f}% of current equity{', if this were live' if not profile_name else ''})",
    ])

    if resolved == "FORWARD_OBSERVATION":
        lines.append("")
        lines.append(
            "_Not trades \u2014 CONTINUED/REVERSED legs replayed as flat +-1. "
            "See /legobs scenario for the probability view of this same population._"
        )
    return "\n".join(lines)
        ...
        for policy_name, decision, score_used in variants:
            ...
            setup = build_shadow_setup(
                ...
                tags={
                    ...
                    "risk_band": risk_band,
                    # Explicit shared setup identity: every policy variant
                    # logged from this same leg/tier iteration carries the
                    # same leg_key, since they all share one peek/entry/
                    # now_utc computed above the variants loop. Lets any
                    # future paired-policy comparison (e.g. does SWEEP_ON
                    # vs RISK_AT_HAND change the outcome for the SAME
                    # market event) join on an explicit field instead of
                    # inferring identity from tier_number+opened_at.
                    "leg_key": leg_key,
                    **(peek.breakdown or {}),
                },
...
def get_policy_variants_for_leg(leg_key, tier_label=None):
    """Returns every resolved EXP4_POLICY_LAB variant sharing this leg_key,
    grouped for paired comparison (same underlying market setup, different
    simulated policy). Read-only, no min-N gate — this is a lookup, not a
    statistical claim.

    Deliberately does NOT aggregate across legs (e.g. "does SWEEP_ON beat
    SWEEP_OFF overall") — with very few multi-variant legs resolved so far
    there is nothing to aggregate yet, and building that report before the
    data exists would just be another premature-architecture problem. Once
    enough legs have >=2 resolved variants to say something, that becomes
    its own scoped follow-up (e.g. investigate_policy_sensitivity), not an
    expansion of this function.
    """
    rows = [
        r for r in _read_shadow_trade_log(experiment="EXP4_POLICY_LAB")
        if r.get("tags", {}).get("leg_key") == leg_key
        and (tier_label is None or r.get("tags", {}).get("tier_label") == tier_label)
    ]
    return sorted(rows, key=lambda r: r.get("tags", {}).get("policy", ""))
...
def drain_rejected_live_queue(shadow_state, shadow_stats):
    ...
    PRICE-POINT GRANULARITY (per chat, 2026-08-20 — supersedes the original
    fix's exact-leg_id-forever gate): `leg_id` (side|swing_high|swing_low|
    tier) identifies a FAMILY — the same standing leg being rejected by the
    same tier — not a single record. Gating purely on leg_id membership in
    a permanent set meant a leg sitting rejected for days produced exactly
    ONE shadow log for its entire lifetime, frozen at whatever the first
    rejection's price/context happened to be — real evidence about how that
    situation evolved was silently discarded, not merely deduped.
    Fix: shadow_state["rejected_live_last_price"] tracks the last LOGGED
    entry price per family. A new record for a known family is logged again
    once price has moved >= REJECTED_LIVE_MIN_PRICE_DELTA_ATR_MULT * ATR
    (or the pip floor, if ATR's unusable) from that last logged price —
    genuinely a different setup/R-profile, not just "still sitting there."
    This SUBSUMES the original replay-protection: a merge=union-resurrected
    duplicate of an already-logged line has the exact same entry price as
    what's on record for its family, so its delta is 0 — always below the
    floor, always skipped. drained_leg_ids/DRAINED_LEG_ID_CAP is kept
    exactly as before as a second, independent guard (defense in depth,
    zero cost) but no longer the sole gate."""
    ...
    last_price = shadow_state.setdefault("rejected_live_last_price", {})
    ...
    for line in lines:
        ...
        # Composite replay key: leg_id ALONE now identifies a whole family
        # that can legitimately produce multiple logged records over its
        # lifetime (see price-point note below) — so drained_set must key
        # on (leg_id, price), not leg_id alone, or every record after the
        # family's first would be wrongly caught here before the
        # price-delta check below ever runs.
        replay_key = (
            "{}@{:.5f}".format(leg_id, entry)
            if (leg_id is not None and entry is not None) else None
        )

        if replay_key is not None and replay_key in drained_set:
            # Byte-for-byte replay: same family, same price already logged —
            # a merge=union queue-file resurrection or genuine re-queue at
            # an unchanged price. Kept as a fast-path guard; the price-delta
            # check below would also catch this (delta=0), so this is
            # belt-and-suspenders, not load-bearing on its own.
            skipped_dupe += 1
            continue

        if leg_id is not None and entry is not None:
            prev_price = last_price.get(leg_id)
            if prev_price is not None:
                atr_pips = record.get("atr_pips")
                if atr_pips:
                    min_delta_pips = max(
                        REJECTED_LIVE_MIN_PRICE_DELTA_ATR_MULT * atr_pips,
                        REJECTED_LIVE_MIN_PRICE_DELTA_PIPS_FLOOR,
                    )
                else:
                    min_delta_pips = REJECTED_LIVE_MIN_PRICE_DELTA_PIPS_FLOOR
                delta_pips = abs(entry - prev_price) / PIP_SIZE
                if delta_pips < min_delta_pips:
                    skipped_unmoved += 1
                    continue  # same standing setup, price hasn't moved meaningfully
        ...
        if replay_key is not None:
            drained_set.add(replay_key)
            drained_leg_ids.append(replay_key)
        if leg_id is not None and entry is not None:
            last_price[leg_id] = entry
    ...
    if skipped_unmoved:
        print(f"[MIN] skipped {skipped_unmoved} record(s) whose price hadn't "
              f"moved enough since that family's last logged entry — same "
              f"standing setup, not a new price point")
    ...
    # Bound rejected_live_last_price the same way — same rationale as
    # DRAINED_LEG_ID_CAP above, prevents unbounded growth from families
    # that keep producing genuinely new price points over the bot's
    # lifetime. Oldest-inserted families are dropped first (dict
    # insertion order, Python 3.7+); a family dropped here just starts
    # fresh (treated as never-logged) if it's ever seen again, same as
    # any brand-new family — not a correctness issue, only means a very
    # old, long-cold family loses its "last known price" memory.
    if len(last_price) > DRAINED_LEG_ID_CAP:
        for k in list(last_price.keys())[: len(last_price) - DRAINED_LEG_ID_CAP]:
            del last_price[k]

    # Truncate — best-effort only. Even if a concurrent union-merge rebase
    # resurrects these lines on main, drained_leg_ids/rejected_live_last_price
    # (committed in the same shadow_state.json save as this function's
    # caller) are what actually prevent re-logging next pass, not this
    # truncation.
    try:
    ...
    for tier_label, tier_fn in TIER_REGISTRY.items():
        ...
        setup = build_shadow_setup(
            ...
            tags={# ATR/risk-ceiling facts only (per chat, 2026-08-11) —
                  ...
                  # score: raw peek.score, restored 2026-08-14. NOT the same
            # thing as the would_have_fired_live/conviction_score/
            # risk_band cluster removed 2026-08-11 — this is a plain
            # structural fact (same category as break_count,
            # atr_percentile_15m below), not a policy judgment. Needed
            # because compute_conviction_buckets() and format_conviction_
            # audit()'s "below floor vs at/above floor" section both
            # require EXP7's FULL population — every activated peek,
            # including below-floor ones — which EXP4_POLICY_LAB's
            # CONVICTION_SWEEP_ON arm can never provide by construction
            # (a policy variant only logs when ITS OWN gate already
            # fired, so "below floor" trades don't exist in that arm at
            # all). Discovered this distinction while fixing the
            # 2026-08-11 stale-tag bug elsewhere in this file — redirecting
            # these two consumers to EXP4_POLICY_LAB the same way the
            # other consumers were fixed would have silently zeroed out
            # the below-floor half of both reports forever.
            "score": peek.score,
...
def record_markov_transition(state, markov_data, current_state_label):
    ...
    ⚠  SERIAL-CORRELATION / EFFECTIVE-N — FIXED, 2026-08-14  ⚠
    Previously documented, not fixed: the raw cell-count / calibration-
    predictions totals conflate genuinely independent transition
    observations with the same persistent regime counted once per scan
    for however many scans it lasted (GBPUSD can hold e.g. BULLISH_FRESH
    for dozens of consecutive scans). A regime screenshot from
    2026-08-13's /calibration output showed the symptom directly: a
    Brier score of 0.012 against a 0.160 random baseline, and 50-60%/
    60-70% stated-probability buckets resolving at a literal 100% actual
    rate on n=11/n=20 — both artifacts of counting the same held regime
    dozens of times, not genuine forecasting skill.

    The fix does NOT touch the transition COUNT matrix (`transitions`)
    itself — that's the correct cumulative sufficient statistic and
    stays as-is; averaging over many real regime episodes is exactly
    what it's for. It touches ONLY `calibration_predictions`, the flat
    list format_calibration_report() reads: each entry now also carries
    `from_state` (the regime this prediction was made FROM). The report
    then collapses consecutive entries sharing the same from_state — a
    single held regime's whole persistence run — into ONE effective
    observation (see _effective_calibration_predictions() there),
    keeping the FIRST entry of each run (computed with the least
    within-run count inflation) rather than every repeat. Entries
    written before this fix have no `from_state` and are treated as
    always-boundary (never collapsed) — the old, inflated behavior for
    old data; correct behavior going forward for new data. No backfill,
    no fabricated from_state for historical rows.
    """
...
def compute_transition_probabilities(markov_data, from_state, exclude_self=False):
    ...
    `exclude_self` (added per chat, friend's review of a live screenshot
    showing "BULLISH_FRESH (n=1291) -> most likely next: BULLISH_FRESH
    99%"): the raw `transitions` matrix is recorded every 5-minute scan
    (record_markov_transition), so a regime held for dozens of
    consecutive scans contributes dozens of self-transition counts —
    the SAME artifact already diagnosed and fixed for the calibration
    report on 2026-08-14 (see record_markov_transition's docstring),
    but that fix only touched `calibration_predictions`, never this
    function. A high self-transition share is not a bug — trends
    genuinely persist scan to scan — but showing it next to a live
    signal invites reading "99%" as edge when it is closer to a
    tautology (5-minute regimes rarely flip on the very next 5-minute
    bar). exclude_self=True answers the question that's actually
    decision-relevant instead: "of the times this regime DID change,
    what did it usually change into" — computed by dropping the
    self-transition cell (and its pseudo-count) from both numerator
    and denominator before normalizing. n returned in that mode is the
    count of genuine regime-change observations only, never conflated
    with the self-persistence count. Still PURE; still Dirichlet-
    smoothed over the remaining k-1 states.
    """
    row = (markov_data.get("transitions", {}) or {}).get(from_state, {})
    if exclude_self:
        states = [s for s in MARKOV_STATES if s != from_state]
        row = {s: v for s, v in row.items() if s != from_state}
    else:
        states = MARKOV_STATES
    n = sum(row.values())
    k = len(states)
    denom = n + k * MARKOV_PRIOR_ALPHA
    probs = {s: (row.get(s, 0) + MARKOV_PRIOR_ALPHA) / denom for s in states}
    return probs, n


def format_markov_report(markov_data, current_state_label):
    """Full regime report for /markov. Shows TWO separate numbers,
    deliberately not blended into one (per chat, friend's review):

      1. Self-persistence — raw P(next scan = same state), straight
         off the full scan-by-scan matrix. Included for transparency,
         explicitly labeled as a 5-minute-granularity artifact, not a
         forecast.
      2. Conditional-on-change distribution (exclude_self=True) — of
         the times this regime actually ended, what it became. This is
         the number worth reading as a regime-evolution forecast.
    """
    self_probs, self_n = compute_transition_probabilities(markov_data, current_state_label)
    change_probs, change_n = compute_transition_probabilities(
        markov_data, current_state_label, exclude_self=True)
    self_pct = self_probs.get(current_state_label, 0.0) * 100
    ...
    lines = [
        ...
        "",
        f"*Self-persistence* (raw, per-scan): `{self_pct:.0f}%` (n=`{self_n}` scan-to-scan "
        f"observations FROM this state — inflated by however long a single regime run "
        f"lasts; not a forecast on its own).",
        "",
        f"*When it changes* (n=`{change_n}` genuine regime-change observations):",
    ]
    if change_n == 0:
        lines.append("_No observed regime change FROM this state yet — probabilities below are pure prior._\n")
    for s, p in sorted(change_probs.items(), key=lambda kv: -kv[1]):
        lines.append(f"`{s:<15}` `{p * 100:5.1f}%`")
    lines.append("")
...
def format_markov_line(markov_data, current_state_label):
    ...
    Reworked per chat, friend's review: previously reported the raw
    self-persistence stat (e.g. "BULLISH_FRESH (n=1291) -> most likely
    next: BULLISH_FRESH 99%"), which is a 5-minute-scan artifact, not
    trade-relevant information — see compute_transition_probabilities'
    docstring. Now reports the change-conditional distribution instead
    (exclude_self=True): what this regime has actually turned into,
    on the occasions it stopped being this regime. Returns None when
    no genuine regime-change has been observed FROM this state yet —
    a pure-prior 20% isn't worth printing next to an actual signal.
    """
    probs, n = compute_transition_probabilities(markov_data, current_state_label, exclude_self=True)
    if n == 0:
        return None
    top_state, top_p = max(probs.items(), key=lambda kv: kv[1])
    return (f"🔀 *Regime:* `{current_state_label}` — when it changes (n=`{n}`), "
            f"usually becomes: `{top_state}` `{top_p * 100:.0f}%`")
...
def load_market_event_pending():
    """
    Read the full pending-events queue (see MARKET_EVENT_PENDING_FILE's
    own docstring in scanner_common.py for why this is a dedicated file
    rather than a state.json key). Empty list on missing/corrupt file —
    same defensive discipline as load_leg_obs_state() above; a bad read
    here should never crash a scan, just start the queue fresh.
    """
    try:
        records = []
        with open(MARKET_EVENT_PENDING_FILE, "r") as f:
            for line in f:
                line = line.strip()
                if line:
...
def save_market_event_pending(records):
    """
    Rewrites the full pending-events queue. Both LIVE (appending new
    tagged events) and MIN (draining resolved ones) call this with the
    complete list they want on disk after their change — read-modify-
    rewrite, not a true append-only log, since entries need to be
    removable (drained) as well as addable. File size here is always
    tiny (a capped queue, not a permanent research log like shadow_
    trade_log.jsonl), so rewriting the whole file each time is fine.

    ATOMICITY FIX (Fix Spec batch 4, item #9): this was the one save
    path in the codebase still using a raw overwrite-mode `open(path,
    "w")` instead of the atomic writer, on the one file both LIVE and
    MIN jobs actually touch — a crash mid-write, or the two jobs
    racing each other, could truncate or interleave-corrupt this
    exact queue. Swapped for atomic_write_jsonl() (see its own
    docstring in scanner_common.py), the line-delimited sibling of
    atomic_write_json() built specifically for this fix.
    """
    try:
        atomic_write_jsonl(MARKET_EVENT_PENDING_FILE, records)
    except Exception as e:
        print("[MARKET EVENT PENDING SAVE ERROR] " + str(e))


def save_leg_obs_state(obs_state):
    # _persisted_at — see save_shadow_state()'s comment on why this isn't
    # sourced from file mtime.
    obs_state["_persisted_at"] = datetime.now(timezone.utc).isoformat()
    try:
...
def _summarize_pending_events(matched):
    """
    Composes the "Earlier: ..." line for a Market Event resolution
    follow-up. FIX (2026-08-27, per chat — production output was "quite
    repetitive and incoherent": the same headline appeared twice,
    non-consecutively, because the market genuinely oscillated near a
    state-family boundary multiple times over a 266-bar leg). Naively
    joining every pending headline verbatim (the original version of
    this) breaks down for any long-lived leg near a boundary — each
    individual push WAS a real, significance-gated transition at the
    time (see evaluate_market_event()), so this isn't about suppressing
    real events. It's about not re-narrating all of them verbatim once
    there are more than a few.

    Consecutive identical (category, headline) pairs are collapsed
    first (guards the exact case seen in production — same headline
    twice with nothing distinct in between). If more than a handful of
    DISTINCT events remain after that, summarizes to first -> ... ->
    last plus a count, instead of dumping the full list.
    """
    if not matched:
        return ""
    deduped = []
    for p in matched:
        key = (p.get("category"), p.get("headline"))
        if deduped and (deduped[-1].get("category"), deduped[-1].get("headline")) == key:
            continue
        deduped.append(p)

    if len(deduped) <= 3:
        return "; ".join(p.get("headline", "") for p in deduped if p.get("headline"))

    first = deduped[0].get("headline", "")
    last = deduped[-1].get("headline", "")
    n_between = len(deduped) - 2
    return (
...
def _compose_resolution_text(headlines_summary, direction, fate, bars_open):
    """
    LOCAL composer for the leg-resolution follow-up — deliberately NOT
    routed through hfis.py (2026-09-15, per chat + CI: min_scanner.py
    and hfis.py run in separate GitHub Actions jobs — min-scan.yml vs
    live-scan.yml — and hfis.py is architecturally restricted to
    scanner_live.py only; see check_layer_imports.py's FORBIDDEN dict
    and its own "organism-agnostic" comment on why. An earlier version
    of this fix imported hfis directly from here — caught by CI's
    layer-boundary check, correctly, and reverted).

    Same fix as HFIS's own narrate_resolution() otherwise gets you —
    varied, register-appropriate phrasing instead of a flat "Earlier: X"
    f-string — just owned locally by MIN instead of crossing the
    boundary. A second small phrase bank, not the "one narrator"
    ideal the friend proposed, but the actual constraint (two separate
    runtimes) wasn't one his review had visibility into; this is the
    compliant version of the same intent. Never raises; falls back to
    the plain f-string on any error, same discipline as every other
    composer in this codebase.
    """
    import random
    try:
        openers = {
...
def _close_leg_obs(obs_state, open_rec, fate, now_utc, add_to_closed=False):
    ...
    MARKET EVENT LIFECYCLE FOLLOW-UP (2026-08-27, per chat, CORRECTED
    2026-08-27 per audit): if this leg has any Market Events tagged
    against it (see load_market_event_pending()/_evaluate_and_push_
    market_event() in scanner_live.py — that's the only writer), send
    ONE follow-up push naming what fired earlier and how this leg
    resolved, then drain those entries from the queue. This used to
    take an optional `state` param and read/write state["market_event_
    pending"] — REMOVED, because state.json is exclusively LIVE-owned
    (see live-scan.yml's commit list) and min-scan.yml never commits it,
    so every write this function made was silently discarded the moment
    the runner tore down. Now reads/writes MARKET_EVENT_PENDING_FILE
    directly — see that constant's docstring in scanner_common.py for
    the full story and why it's a dedicated file, not a state.json key.
    Wrapped in its own try/except and runs strictly AFTER the existing
    write-retry logic below — a bug here must never affect whether the
    leg record itself gets closed and logged correctly.
    """
    open_rec["fate"] = fate
    ...
    # MARKET EVENT LIFECYCLE FOLLOW-UP — additive, best-effort, never
    # affects the close above (already committed).
    try:
        pending = load_market_event_pending()
        this_leg_id = open_rec.get("id")
        matched = [p for p in pending if p.get("leg_obs_id") == this_leg_id]
        if matched:
            headlines = _summarize_pending_events(matched)
            direction = (open_rec.get("formation_state") or {}).get("macro_leg_direction", "")
            push_text = _compose_resolution_text(headlines, direction, fate, open_rec['bars_open'])
            send_telegram("🔁 *Market Event — Resolved*\n\n" + push_text)
            remaining = [p for p in pending if p.get("leg_obs_id") != this_leg_id]
            save_market_event_pending(remaining)
    except Exception as e:
        print("[MARKET EVENT FOLLOW-UP ERROR] " + str(e))
        ...
        if current_leg_id not in closed_ids:
            ...
            obs_state["open"] = {
                ...
                "timeline": [{
                    "bar": 0, "kind": "formed",
                    "detail": (
                        f"Leg formed — {(formation.get('macro_leg_direction') or '?').title()} "
                        f"({'via CHoCH' if formation.get('macro_was_choch') else 'as continuation'})"
                    ),
                }],
...
def _effective_calibration_predictions(cal_data):
    """
    Fix, 2026-08-14 — see record_markov_transition()'s docstring for the
    full problem statement (a 2026-08-13 /calibration report showed a
    Brier score of 0.012 and 50-60%/60-70% buckets resolving at a
    literal 100% actual rate on n=11/n=20, both artifacts of counting
    the same persistent regime once per scan for however many scans it
    lasted, rather than once per genuinely new transition attempt).

    Collapses `calibration_predictions` down to one effective observation
    per maximal run of consecutive scan-groups sharing the same
    from_state. record_markov_transition() appends exactly
    len(MARKOV_STATES) entries per scan, all sharing that scan's
    from_state — this groups the flat list back into those chunks, then
    keeps only the FIRST chunk of each run of consecutive chunks whose
    from_state matches the one before it (the entry computed with the
    LEAST within-run count inflation, since later chunks in the same run
    were computed from counts already nudged by earlier scans in that
    same run).

    Chunks with from_state=None — entries written before this fix
    existed — are NEVER collapsed; each is always treated as its own
    boundary, exactly the old (uncorrected) behavior. Old data isn't
    silently reinterpreted; only new data going forward gets the fix.

    Pure. Returns a flat list in the same shape as the input, just
    shorter — every consumer below (bucket table, Brier score) just
    reads it the same way it read the raw list before this fix.
    """
    chunk_size = len(MARKOV_STATES)
    if chunk_size == 0:
        return list(cal_data)
    chunks = [cal_data[i:i + chunk_size] for i in range(0, len(cal_data), chunk_size)]

    kept = []
    prev_from_state = object()   # sentinel — can never equal a real from_state or None
    for chunk in chunks:
        from_state = chunk[0].get("from_state") if chunk else None
        if from_state is not None and from_state == prev_from_state:
            continue   # same persistence run as the previous chunk — skip, not independent
        kept.append(chunk)
        prev_from_state = from_state

    return [entry for chunk in kept for entry in chunk]
...
def format_calibration_report():
    ...
    try:
        ...
        cal_data_raw = markov_data.get("calibration_predictions", [])
        n_raw = len(cal_data_raw)
        # Fix, 2026-08-14 — see record_markov_transition()'s docstring:
        # collapses consecutive same-regime scans (one held regime
        # counted once per scan it lasted) down to one effective
        # observation per persistence run, so the report doesn't read a
        # single multi-hour trend as 30 independent forecasts.
        cal_data = _effective_calibration_predictions(cal_data_raw)
        n_cal = len(cal_data)
        if n_cal < CALIBRATION_MIN_N:
            lines.append(
                f"*1. Markov (regime transitions):* "
                f"_`{n_cal}` effective predictions (`{n_raw}` raw scan-rows) "
                f"— need `{CALIBRATION_MIN_N}+` effective_"
            )
        else:
            buckets = {}
            ...
            lines.append(
                ...
                f"(`{n_cal}` effective predictions, `{n_raw}` raw scan-rows "
                f"before de-correlation)"
            )
            lines.append(
                "_`stated%` → `actual%` (n). n counts one persistence run "
                "as ONE observation, not one per scan. Perfect forecast "
                "lies on the diagonal._"
            )
            ...
            if not any_bucket_shown:
                lines.append(f"  _No bucket has {CALIBRATION_MIN_N}+ effective predictions yet_")

            # Brier score: mean squared error (0 = perfect, 0.25 = random).
            # Computed on the EFFECTIVE (de-correlated) list — the raw list
            # would still understate this score exactly the way the
            # 2026-08-13 report's 0.012 did.
            brier = sum(
    ...
    # FIX, 2026-08-14 (same stale-tag bug as open_failure_case(), found
    # while checking for duplication with a Rejected-Live change): this
    # read EXP7_TIER_ATR's would_have_fired_live / conviction_score tags,
    # both of which the 2026-08-11 policy refactor moved OFF of EXP7 and
    # onto EXP4_POLICY_LAB — so n_fired has been frozen at whatever it
    # was right before that refactor ever since (this is very likely
    # exactly the "0 resolved fired signals" the 2026-08-13 screenshot
    # showed). The conviction gate itself now lives as EXP4_POLICY_LAB's
    # CONVICTION_SWEEP_ON arm (the old score>=CONVICTION_MIN_BY_TIER
    # gate, unmodified) — a record only gets logged for that variant when
    # ITS OWN fire condition was met, so every logged CONVICTION_SWEEP_ON
    # row already IS a "fired" signal; no would_have_fired_live filter
    # needed at all. band_label is already frozen on each row at
    # prediction time (see experiment_4_policy_lab), so this reads that
    # directly instead of re-deriving a band from a raw score.
    try:
        fired = [
            r for r in _read_shadow_trade_log(experiment="EXP4_POLICY_LAB")
            if r.get("tags", {}).get("policy") == "CONVICTION_SWEEP_ON"
        ]
        n_fired = len(fired)
        if n_fired < CALIBRATION_MIN_N:
            ...
            band_data = {}
            for r in fired:
                band = r.get("tags", {}).get("band_label")
                if band is None:
                    continue
            ...
            lines.append(
                ...
                f"(n=`{n_fired}` EXP4 CONVICTION_SWEEP_ON fired signals)"
            )
            lines.append(
...
# (EXP4_POLICY_LAB, tags.policy == "RISK_AT_HAND") resolves as a loss —
...
#
# FIX, 2026-08-14 (found while checking for duplication with a Rejected-
# Live change): originally gated on EXP7_TIER_ATR + tags.would_have_
# fired_live. The 2026-08-11 policy refactor that created EXP4_POLICY_LAB
# moved that exact tag OFF of EXP7_TIER_ATR's records (see experiment_7_
# tier_atr_mirror()'s own docstring: "SCOPED TO ATR ONLY... would_have_
# fired_live moved to EXP4_POLICY_LAB") without updating this trigger to
# match — so this has been checking a tag that no longer exists on the
# experiment it filters for, silently returning None every single call
# since that refactor. EXP4_POLICY_LAB's RISK_AT_HAND variant is the
# correct modern source: it's the control arm — activated + risk-gate-
# passed, unconditional fire, flat RR_RATIO — i.e. exactly "what today's
# live policy would have done." Checked EXP7_TIER_ATR is NOT a silent
# duplicate source for this: it logs every activated tier regardless of
# risk_gate_pass (a full-ATR-range research population, by design), so
# using it here would have opened cases on setups that never had a real
# expectation of firing live at all.
#
# Steps 1-4 and 7, as originally proposed, are implemented close to as
# specified:
#   1. Open a Case       — open_failure_case() below.
#   2. Expectation vs Reality — "expected WIN" (policy == RISK_AT_HAND)
...
#   4. Compare against historical cases — same-tier EXP4_POLICY_LAB
#      RISK_AT_HAND records, split by outcome, read from the existing
#      permanent log.
#
...
def _with_merged_tags(records):
    """
    GENERALIZATION (2026-08-29, per chat — user: "is a win/loss actively
    investigated across the bot"). Merges each record's conditions_at_
    resolution (see peek_conditions_at_resolution(), added same day) into
    its tags dict, prefixed cond_ to avoid colliding with any real tag
    key of the same name — so the comparison engine below sees BOTH the
    experiment's own setup parameters (fib level, swing source, HTF
    authority, whatever a given experiment tags) AND broader market
    condition (phase, regime, session) in one comparison, without
    _compare_tag() itself needing to know the difference. Returns NEW
    dicts — never mutates the records read from disk.
    """
    merged = []
    for r in records:
        combined = dict(r.get("tags") or {})
        for k, v in (r.get("conditions_at_resolution") or {}).items():
            combined[f"cond_{k}"] = v
        r2 = dict(r)
        r2["tags"] = combined
        merged.append(r2)
    return merged
...
def investigate_failure_case(tier_number, formation_tags, experiment="EXP4_POLICY_LAB",
                              policy_filter="RISK_AT_HAND", exclude_trade_id=None):
    """
    Steps 3-6. Reads this tier's OWN permanent history for `experiment`
    (tier-isolated, same "jury, not judge" isolation as compute_
    evidence() — Tier 1 only ever learns from Tier 1), optionally
    filtered to a specific policy tag, splits into winners/losers,
    compares every formation tag on this trade against both groups, and
    ranks the comparisons that cleared CASE_MIN_GROUP_N by effect size.

    GENERALIZED (2026-08-29, per chat): `experiment`/`policy_filter` used
    to be hardcoded to EXP4_POLICY_LAB/RISK_AT_HAND — meaning this
    engine, despite being fully generic internally, only ever actually
    ran for that one narrow population. EXP4_POLICY_LAB/RISK_AT_HAND
    behavior is UNCHANGED (still the default, still means exactly what
    it meant before — the live-policy-parity check). `policy_filter=
    None` skips the policy-tag filter entirely, for experiments that
    don't have an equivalent concept. Winners/losers now also carry
    conditions_at_resolution merged into their tags (_with_merged_
    tags() above) — market condition is compared alongside experiment-
    specific setup parameters, not just the setup parameters alone.

    FIX, 2026-08-14: previously read EXP7_TIER_ATR here, which stayed
    consistent with open_failure_case()'s old (broken) EXP7_TIER_ATR
    trigger but was ALSO wrong on its own terms even before that bug —
    EXP7_TIER_ATR logs every activated tier regardless of risk_gate_pass
    (deliberately, for ATR-range research), so it includes setups that
    were never real live candidates at all. Comparing a real RISK_AT_HAND
    loss against that broader population would silently mix in trades
    with no expectation of firing live. Now reads the same population
    open_failure_case() gates on.
    ...
    """
    records = [
        r for r in _read_shadow_trade_log(experiment=experiment)
        if r.get("tier_number") == tier_number
        and r.get("trade_id") != exclude_trade_id
        and (policy_filter is None or r.get("tags", {}).get("policy") == policy_filter)
    ]
    winners = _with_merged_tags([r for r in records if r.get("r_achieved", 0.0) > 0])
    losers  = _with_merged_tags([r for r in records if r.get("r_achieved", 0.0) <= 0])
...
def open_trade_case(setup, outcome, r_achieved, now_utc):
    """
    RENAMED from open_failure_case() (2026-08-29, per chat — user: "let's
    change the name to Trade Investigation Bureau, the department
    obsessed with everything there is to know about a trade" — credit
    to Vally for the framing: "Failure Investigation Bureau: Why did the
    system fail? already biases the subsystem toward pathology. A
    winning trade can be just as informative as a losing one.").

    WHAT CHANGED, PRECISELY:
      - Every resolution now opens a case — wins included, not just
        losses (see the removed `if r_achieved > 0: return None` gate).
        Vally's Trade Case File concept: "Every resolved trade gets a
        forensic record" — not just the failures.
      - The case schema now carries max_r_reached/min_r_reached (MFE/
        MAE — see classify_trade_path()) and environment atoms (see
        peek_environment_at_resolution()) alongside the existing tag
        comparison, moving toward Vally's Market Environment / Trade
        Lifecycle dimensions using data that already existed elsewhere
        in the bot, not new detection.
      - Telegram PUSH remains LOSS-ONLY — this is a deliberate, stated
        choice, not an oversight. EXP2_FIB alone resolved ~2,250 trades
        across 6 weeks; pushing a message for every win would be
        overwhelming noise, not "obsessive" in a useful sense. Every
        trade still gets a permanent case file either way (see
        _append_failure_case() below, file name unchanged — see that
        constant's own note on why). If you want win pushes too, that's
        a one-line change (see PUSH_ON_WIN note below) — not something
        I'm doing by default given the volume.

    Two comparison paths, UNCHANGED from the generalization pass:
    PATH 1 — EXP4_POLICY_LAB / RISK_AT_HAND: "expected WIN" live-parity
    framing, only meaningful for a loss (a win here isn't a discrepancy
    to explain — it's just what was expected. Still gets a plain case
    file for completeness, just without the parity language).
    PATH 2 — everything else: no parity claim, just "how does this
    resolution compare to this tier+experiment's own history."

    WHAT THIS STILL DOESN'T CAPTURE (per chat — stated plainly, not
    silently implied to be covered): Vally's Setup Anatomy and Entry
    Characteristics dimensions (sweep quality, retest quality, breakout
    acceptance, wick/body ratios, rejection behaviour) need NEW
    detection logic this pass does not add — they'd require analyzing
    the actual candles around entry, not just reading atoms that already
    exist elsewhere. Path classification (classify_trade_path()) is
    intentionally coarse (4 labels from MFE/MAE/outcome only) — Vally's
    richer "clean winner / difficult winner / immediate failure /
    delayed failure" anatomy is the ceiling of what's honestly
    supportable without that new detection work.
    ...
    experiment = setup.get("experiment")
    raw_tags = dict(setup.get("tags") or {})
    # Merge THIS trade's own market conditions into its comparison tags
    # too — see _with_merged_tags()'s docstring for why the historical
    # winners/losers get the same treatment. Best-effort peek, same as
    # _append_shadow_trade_log()'s identical call — a read hiccup here
    # must never prevent the case from opening with at least the
    # experiment's own tags.
    formation_tags = dict(raw_tags)
    try:
        for k, v in (peek_conditions_at_resolution() or {}).items():
            formation_tags[f"cond_{k}"] = v
    except Exception:
        pass

    is_win = r_achieved > 0
    if experiment == "EXP4_POLICY_LAB" and raw_tags.get("policy") == "RISK_AT_HAND":
        investigation = investigate_failure_case(
            tier_number, formation_tags, experiment="EXP4_POLICY_LAB",
            policy_filter="RISK_AT_HAND", exclude_trade_id=setup.get("id"))
        expected = None if is_win else "WIN"
    elif experiment == "EXP4_POLICY_LAB":
        # Any OTHER EXP4_POLICY_LAB policy arm (CONVICTION_SWEEP_ON/OFF,
        # etc.) — hypothetical alternate policy, not what live actually
        # would have done, so no "expected WIN" parity claim. Still
        # worth investigating like every other experiment (Path 2).
        investigation = investigate_failure_case(
            tier_number, formation_tags, experiment="EXP4_POLICY_LAB",
            policy_filter=raw_tags.get("policy"), exclude_trade_id=setup.get("id"))
        expected = None
    else:
        investigation = investigate_failure_case(
            tier_number, formation_tags, experiment=experiment,
            policy_filter=None, exclude_trade_id=setup.get("id"))
        expected = None
    ...
    case = {
        ...
        # Trade Lifecycle dimension (2026-08-29, per chat) — see
        # classify_trade_path()'s own docstring for what the 4 labels
        # mean and why they're the honest ceiling for now.
        "max_r_reached": setup.get("max_r_reached"),
        "min_r_reached": setup.get("min_r_reached"),
        "path_classification": classify_trade_path(
            setup.get("max_r_reached"), setup.get("min_r_reached"), r_achieved),
        # Market Environment dimension (2026-08-29, per chat) — THESIS-
        # level (this scan), complements formation_tags' cond_* fields
        # (leg-level, formation-time) above.
        "environment": peek_environment_at_resolution(),
        # FIX, 2026-08-14: formation_tags now comes from EXP4_POLICY_LAB
        # (see investigate_failure_case()'s docstring) — conviction_score
        # was EXP7_TIER_ATR's field name; EXP4 calls the same thing
        # score_used. predicted_win_prob has no EXP4_POLICY_LAB
        # equivalent at all (it was frozen only on EXP7_TIER_ATR rows) —
        # explicitly None here rather than silently reading a key that
        # will never exist on this population.
        "conviction_score": raw_tags.get("score_used"),
...
# Old name kept as an alias — anything still calling open_failure_case()
# (there shouldn't be, after this pass, but this costs nothing and
# avoids a silent breakage if something was missed) continues to work
# identically.
open_failure_case = open_trade_case
...
def format_case_note(case):
    ...
    GENERALIZED (2026-08-29, per chat): the header used to unconditionally
    render "Expected: WIN -> Observed: X" — accurate only for EXP4_
    POLICY_LAB/RISK_AT_HAND's live-parity cases (see open_failure_case()'s
    Path 1/Path 2 docstring). Cases opened via Path 2 have expected=None
    (no parity claim to make), so this now renders a plain outcome line
    for those instead of a broken/misleading "Expected: None" string.
    Also names the experiment now, since this fires across all of them.
    """
    lines = [
        f"🕵️ *Trade Investigation — Case #{case['case_number']}*",
        "─────────────────────",
        f"Experiment: `{case.get('experiment', 'EXP4_POLICY_LAB')}` | "
    ...
    if case.get("expected") is not None:
        ...
        lines.append(f"Outcome: `{case['observed']}` (`{case['r_achieved']}R`)")
...
def format_recent_cases(n=3):
    ...
    if not cases:
        return "🕵️ *Trade Investigation Bureau*\n_No cases opened yet._"
    recent = list(reversed(cases[-n:]))
    lines = [
        "🕵️ *Trade Investigation Bureau — Recent Cases*",
        "─────────────────────",
...
# =========================================================================
# MARKET INTENT — tracking, calibration, failure cases
# =========================================================================
# Per chat, 2026-08-13. build_market_intent() (scanner_observation.py)
# answers "given what I'm seeing, what kind of setup would I actually be
# interested in?" — pre-tier, read-only, never a signal. Everything below
# is what happens to that answer AFTER it's produced: tracked forward to
# a resolution, logged permanently, and — only on a FAILED resolution —
# investigated the same way the Failure Investigation Bureau investigates
# a tier's own losses. Kept in its OWN state/log/case files rather than
# folded into Forward Observation or the Bureau (explicit choice, same
# chat) — see MARKET_INTENT_* constants' own comment in scanner_common.py
# for why.
# =========================================================================

def load_market_intent_state():
    try:
        with open(MARKET_INTENT_STATE_FILE, "r") as f:
            state = json.load(f)
        if state.get("methodology_version") != MARKET_INTENT_METHODOLOGY_VERSION:
            return {"open": None, "methodology_version": MARKET_INTENT_METHODOLOGY_VERSION}
        return state
    except Exception:
        return {"open": None, "methodology_version": MARKET_INTENT_METHODOLOGY_VERSION}


def save_market_intent_state(mi_state):
    # _persisted_at — see save_shadow_state()'s comment (min_scanner.py,
    # INTELLIGENCE DATABASE section) on why this isn't sourced from file
    # mtime. Note this file is Live-owned (Live is the only caller of this
    # function, despite it being defined here) — see world_state_schema.md.
    mi_state["_persisted_at"] = datetime.now(timezone.utc).isoformat()
    try:
        atomic_write_json(MARKET_INTENT_STATE_FILE, mi_state, indent=2)
    except Exception as e:
        print("[MARKET INTENT SAVE ERROR] " + str(e))


def _read_market_intent_log(limit=None):
    """Reads the PERMANENT market_intent_log.jsonl. Pure, safe to call
    from a Telegram command. Same pattern as _read_failure_cases /
    _read_shadow_trade_log."""
    records = []
    try:
        with open(MARKET_INTENT_LOG_FILE, "r") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                try:
                    rec = json.loads(line)
                    if rec.get("methodology_version") == MARKET_INTENT_METHODOLOGY_VERSION:
                        records.append(rec)
                except Exception:
                    continue
    except FileNotFoundError:
...
def _append_market_intent_log(record):
    """Permanently appends one resolved Market Intent record. Same
    write-hardening pattern as _append_leg_obs_log — retries, fsync,
    ...
    for attempt in range(1, SHADOW_TRADE_LOG_WRITE_RETRIES + 1):
        try:
            with open(MARKET_INTENT_LOG_FILE, "a") as f:
                f.write(line)
                f.flush()
                os.fsync(f.fileno())
            return True
            ...
            if attempt < SHADOW_TRADE_LOG_WRITE_RETRIES:
                time.sleep(SHADOW_TRADE_LOG_WRITE_RETRY_DELAY_SEC)
    print(f"[MARKET INTENT LOG ERROR] gave up after {SHADOW_TRADE_LOG_WRITE_RETRIES} "
          f"attempts: {last_error}")
    return False


def _mi_check_materialized(code, facts, phase_result, thesis, macro_bias):
    """Whether a single WatchCode's condition is true THIS scan. One
    branch per enumerated code — closed vocabulary, see WatchCode
    (scanner_observation.py). Mirrors the exact same fact checks
    build_market_intent() used to propose the condition in the first
    place, just evaluated fresh each subsequent scan."""
    if code == WatchCode.PULLBACK_TO_OB.value:
        return facts.has_order_block() and facts.price_in_order_block()
    if code == WatchCode.PULLBACK_TO_FIB_POCKET.value:
        return facts.price_in_fib_pocket()
    if code == WatchCode.SWEEP_AND_RECLAIM.value:
        return phase_result is not None and phase_result.swept_boundary is not None
    if code == WatchCode.FRESH_DISPLACEMENT.value:
        return facts.has_fresh_bos_aligned_with_bias()
    if code == WatchCode.FIVE_M_ALIGNMENT.value:
        mtf_5m = thesis.mtf_5m if thesis else None
        return bool(mtf_5m and mtf_5m.get("m5_direction") == macro_bias)
    return False


def run_market_intent_tracking(intent, facts, ctx, phase_result, thesis, macro_bias,
                                bias_stale, now_utc, mi_state):
    """
    Driver — mirrors run_leg_observation()'s lifecycle mechanics (above)
    but tracks Market Intent's OWN separate record. One record open at a
    time. Opens when build_market_intent() reports has_content=True with
    at least one watch condition, and nothing is currently open.

    Each scan, checks every not-yet-materialized watch condition via
    _mi_check_materialized(). Closes when the underlying leg resolves —
    this MIRRORS (does not share state with) the same leg-fate logic
    run_leg_observation() uses: leg_id unchanged + not stale -> still
    open; leg_id changed -> CONTINUED if direction held else REVERSED;
    bias_stale with nothing replacing it -> INVALIDATED. That logic is
    intentionally duplicated in a few lines here rather than reached into
    Forward Observation's own state — the two records are independent by
    design (2026-08-13 chat) and each must stay resolvable even if the
    other tracking system is ever disabled.

    Final fate, written to the permanent log:
      NO_MATERIALIZATION — leg resolved, nothing on the watch list ever
                            became true. Not investigated — nothing
                            happened to explain.
      WORKED             — at least one condition materialized AND the
                            leg's own fate was CONTINUED (the direction
                            Market Intent was watching for held).
      FAILED             — at least one condition materialized but the
                            leg's own fate was REVERSED or INVALIDATED.
                            The ONLY fate that opens a Market Intent
                            failure case (open_market_intent_case()).

    Never raises — caller (scan()) wraps this exactly like Forward
    Observation; a bug here must never affect the live bot.
    """
    if macro_bias == "CONSOLIDATION":
        return mi_state
    ...
    current_leg_id = compute_leg_id(macro_bias, facts.swing_high, facts.swing_low)
    open_rec = mi_state.get("open")

    if open_rec is not None:
        still_same_leg = _same_leg(open_rec["leg_id"], current_leg_id)

        if still_same_leg and not bias_stale:
            _advance_bars(open_rec)
            for w in open_rec.get("watch_codes", []):
                if w not in open_rec.get("materialized_codes", []):
                    if _mi_check_materialized(w, facts, phase_result, thesis, macro_bias):
                        open_rec.setdefault("materialized_codes", []).append(w)
                        open_rec.setdefault("materialized_at_bar", {})[w] = open_rec["bars_open"]
                        print(f"  [MARKET INTENT] Materialized — {w} (bar {open_rec['bars_open']})")
            mi_state["open"] = open_rec
        else:
            old_dir = open_rec.get("macro_bias")
            if bias_stale and still_same_leg:
                leg_fate = "INVALIDATED"
            else:
                leg_fate = "CONTINUED" if old_dir == macro_bias else "REVERSED"

            materialized = bool(open_rec.get("materialized_codes"))
            if not materialized:
                mi_fate = "NO_MATERIALIZATION"
            elif leg_fate == "CONTINUED":
                mi_fate = "WORKED"
            else:
                mi_fate = "FAILED"

            open_rec["fate"] = mi_fate
            open_rec["leg_fate"] = leg_fate
            open_rec["resolved_at"] = now_utc.isoformat()

            if _append_market_intent_log(open_rec):
                mi_state["open"] = None
                print(f"  [MARKET INTENT] Closed — {mi_fate} (leg {leg_fate})")
                if mi_fate == "FAILED":
                    try:
                        open_market_intent_case(open_rec, now_utc)
                    except Exception as e:
                        print("[MARKET INTENT CASE ERROR] " + str(e))
            # else: write failed — leave "open" in place, retry next scan
            # (same pattern as _close_leg_obs's write-failure handling).

    if mi_state.get("open") is None and intent is not None and intent.has_content and intent.watching_for:
        mi_state["open"] = {
            "id": uuid.uuid4().hex[:12],
            "methodology_version": MARKET_INTENT_METHODOLOGY_VERSION,
            "leg_id": current_leg_id,
            "opened_at": now_utc.isoformat(),
            "bars_open": 0,
            "last_processed_candle": facts.df_5m.index[-1].isoformat(),
            ...
            "phase": intent.phase,
            "watch_codes": [w["code"] for w in intent.watching_for],
            "caution_codes": [c["code"] for c in intent.not_interested_in],
            ...
            "leg_fate": None,
            "resolved_at": None,
            # Snapshot for the raw-gap tag comparison in
            # investigate_market_intent_case() — same idea as Forward
            # Observation's Facet 2, deliberately small: only fields
            # that already exist elsewhere, nothing computed fresh.
            "tags": {
                "phase": intent.phase,
                "atr_5m_pips": facts.current_atr_5m_pips(),
                "break_count": getattr(phase_result, "break_count", None),
                "campaign_continuation_count": (thesis.campaign or {}).get("continuation_count") if thesis and thesis.campaign else None,
                "num_watch_codes": len(intent.watching_for),
                "num_caution_codes": len(intent.not_interested_in),
                "very_low_atr_warning": bool(ctx.very_low_atr_warning),
            },
        }
        print(f"  [MARKET INTENT] Opened — watching {[w['code'] for w in intent.watching_for]}")

    return mi_state


def _read_market_intent_cases(limit=None):
    cases = []
    try:
        with open(MARKET_INTENT_CASE_LOG_FILE, "r") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                try:
                    case = json.loads(line)
                    if case.get("methodology_version") == MARKET_INTENT_METHODOLOGY_VERSION:
                        cases.append(case)
                except Exception:
                    continue
    except FileNotFoundError:
...
def _append_market_intent_case(case):
    line = json.dumps(case, default=_json_default) + "\n"
    last_error = None
    for attempt in range(1, SHADOW_TRADE_LOG_WRITE_RETRIES + 1):
        try:
            with open(MARKET_INTENT_CASE_LOG_FILE, "a") as f:
                f.write(line)
                f.flush()
                os.fsync(f.fileno())
            return True
            ...
            if attempt < SHADOW_TRADE_LOG_WRITE_RETRIES:
                time.sleep(SHADOW_TRADE_LOG_WRITE_RETRY_DELAY_SEC)
    print(f"[MARKET INTENT CASE LOG ERROR] gave up after {SHADOW_TRADE_LOG_WRITE_RETRIES} "
          f"attempts: {last_error}")
    return False


def _next_mi_case_number():
    return len(_read_market_intent_cases()) + 1


def investigate_market_intent_case(tags, exclude_id=None):
    """
    Mirrors investigate_failure_case() (Failure Investigation Bureau,
    above) exactly — same _compare_tag() / Cohen's d / Cohen's h
    machinery, same raw-gap-not-percentage, min-N-gated, correlational-
    only discipline — pointed at Market Intent's OWN resolved log
    (WORKED vs FAILED, excluding NO_MATERIALIZATION — nothing to compare
    if the setup never formed) rather than the tier Bureau's.
    """
    records = [
        r for r in _read_market_intent_log()
        if r.get("id") != exclude_id and r.get("fate") in ("WORKED", "FAILED")
    ]
    winners = [r for r in records if r.get("fate") == "WORKED"]
    losers = [r for r in records if r.get("fate") == "FAILED"]

    comparisons = []
    for tag_key, this_value in (tags or {}).items():
        cmp_result = _compare_tag(
            tag_key, this_value,
            [{"tags": r.get("tags", {})} for r in winners],
            [{"tags": r.get("tags", {})} for r in losers],
            min_n=MARKET_INTENT_CASE_MIN_GROUP_N,
        )
        if cmp_result is not None:
            comparisons.append(cmp_result)
    ...
    if not comparisons:
        conclusion = (
            f"Not enough resolved history yet to identify a driving factor "
            f"(need {MARKET_INTENT_CASE_MIN_GROUP_N}+ worked AND "
            f"{MARKET_INTENT_CASE_MIN_GROUP_N}+ failed per tag; have "
            f"{len(winners)} worked / {len(losers)} failed total)."
        )
    else:
        top = comparisons[0]
        es_note = f", effect size `{top['effect_size']:+.2f}`" if top.get("effect_size") is not None else ""
        if top["type"] == "numeric":
            conclusion = (
                f"Largest measured effect: `{top['tag']}` — failed setups averaged "
                f"`{top['losers_mean']}` vs worked setups' `{top['winners_mean']}` "
                f"(this setup: `{top['this_value']}`, n={top['winners_n']}W/"
                f"{top['losers_n']}L{es_note}). Correlational, not causal — "
                f"worth researching, not acting on alone."
            )
        else:
            conclusion = (
                f"Largest measured effect: `{top['tag']}` — failed setups were usually "
                f"`{top['losers_mode']}` ({int(top['losers_mode_share']*100)}% of failed), "
                f"worked setups usually `{top['winners_mode']}` "
                f"({int(top['winners_mode_share']*100)}% of worked); this setup was "
                f"`{top['this_value']}` (n={top['winners_n']}W/{top['losers_n']}L{es_note}). "
                f"Correlational, not causal — worth researching, not acting on alone."
            )
...
def open_market_intent_case(resolved_record, now_utc):
    """
    Called only when a tracked Market Intent record resolves as FAILED
    (see run_market_intent_tracking()) — at least one watched condition
    materialized, but the leg then reversed or invalidated instead of
    continuing. Never called for NO_MATERIALIZATION (nothing formed, so
    nothing to investigate) or WORKED (no discrepancy to explain).
    """
    tags = resolved_record.get("tags") or {}
    investigation = investigate_market_intent_case(tags, exclude_id=resolved_record.get("id"))
    case = {
        "case_number": _next_mi_case_number(),
        "methodology_version": MARKET_INTENT_METHODOLOGY_VERSION,
        "id": resolved_record.get("id"),
        "opened_at": now_utc.isoformat(),
        "macro_bias": resolved_record.get("macro_bias"),
        "phase": resolved_record.get("phase"),
        "watch_codes": resolved_record.get("watch_codes"),
        "materialized_codes": resolved_record.get("materialized_codes"),
        "leg_fate": resolved_record.get("leg_fate"),
        "comparisons": investigation["comparisons"][:5],
        "conclusion": investigation["conclusion"],
        "n_worked_compared": investigation["n_winners"],
        "n_failed_compared": investigation["n_losers"],
    }
    _append_market_intent_case(case)
    return case


def format_market_intent_report(state, mi_state, intent_hypothesis=None):
    """
    Telegram rendering for /marketintent. Does not recompute anything,
    same pattern as format_market_thesis(). Appends the currently-tracked
    instance (if any) and a lightweight track record from the resolved
    log, gated by MARKET_INTENT_CALIBRATION_MIN_N the same as every other
    statistic in this codebase.

    PHASE 0 AUTHORITY ROUTING (2026-08-27, per audit — same pass as
    format_market_thesis()). Before this change, the narrative line came
    from state["market_intent_narrative"] — scanner_observation.stitch_
    market_intent_narrative()'s own independent prose. That function
    genuinely had no Brain-routed equivalent before now (build_market_
    intent_hypothesis() in brain.py already existed and already did the
    locations/confirmations/cautions grouping, but nothing rendered it as
    prose or wired it to a live command — see brain.format_intent_
    narrative()'s own docstring). Fixed: `intent_hypothesis` is Brain's
    build_market_intent_hypothesis(world_state) output, built by the same
    caller right before this is invoked (see _answer_deferred_reports()
    in scanner_live.py), rendered via brain.format_intent_narrative().

    `intent_hypothesis` is optional and falls back to the OLD state-
    sourced narrative when None — same defensive discipline as format_
    market_thesis(): never regress below the pre-Phase-0 answer.

    Tracking status (open_rec) and track-record stats below are UNTOUCHED
    by this change — they're live telemetry about a specific tracked
    instance and a resolved-log statistic, not a market interpretation,
    so they were never part of the two-competing-conclusions problem this
    pass exists to fix.

    BUG FIX (2026-08-17): this used to also gate on
    `state.get("status") == "NONE"` — a field ONLY ever written by
    load_state()'s empty/stale fallback in scanner_common.py, never
    cleared afterward by anything (apply_state_updates() never touches
    it). Once state.json hit that fallback a single time, "status"
    stayed "NONE" forever and this check always returned "No scan data
    yet." even with a fresh market_intent_narrative sitting right there
    in the same state dict. format_market_thesis() never had this
    problem — it only ever checked its own field, never this stale
    top-level sentinel. Do NOT re-add a state["status"] check here.

    CHANGED (2026-08-26, per chat — user named /marketintent as one of
    the three market-explanatory commands to keep, meaning conversational
    like /understand and /thesis now are). This used to also print the
    raw watch_codes/caution_codes as backtick-quoted internal enum values
    (`PULLBACK_TO_FIB_POCKET`, `ATR_INCOMPATIBLE`) plus a separate
    "Reasons" bullet dump underneath the narrative paragraph that already
    said the same thing in prose. Both are dropped — the codes are
    internal identifiers meant for log-matching/calibration, not for a
    human to read. Tracking status and track record stats are kept as-is
    (already plain language, not raw labels).
    """
    if "market_intent_narrative" not in state and not intent_hypothesis:
        return "🔎 *Market Intent*\n_No scan data yet._"

    lines = ["🔎 *Market Intent*", ""]
    if intent_hypothesis is not None:
        lines.append(format_intent_narrative(intent_hypothesis))
    else:
        lines.append(state.get("market_intent_narrative") or "_Nothing computed this scan._")

    open_rec = (mi_state or {}).get("open")
    if open_rec:
        mats = open_rec.get("materialized_codes") or []
        lines.append("")
        lines.append(
            f"_Currently tracking — opened {open_rec.get('bars_open', 0)} bars ago"
            + (f", materialized: {', '.join(mats)}" if mats else ", nothing materialized yet")
            + "._"
        )

    resolved = _read_market_intent_log()
    total_resolved = len(resolved)
    worked = sum(1 for r in resolved if r.get("fate") == "WORKED")
    failed = sum(1 for r in resolved if r.get("fate") == "FAILED")
    no_mat = sum(1 for r in resolved if r.get("fate") == "NO_MATERIALIZATION")

    lines.append("")
    if total_resolved < MARKET_INTENT_CALIBRATION_MIN_N:
        lines.append(
            f"_Track record: {total_resolved} resolved — need "
            f"{MARKET_INTENT_CALIBRATION_MIN_N}+ before a materialization/"
            f"success rate is shown._"
        )
    else:
        materialized_n = worked + failed
        mat_rate = round(100 * materialized_n / total_resolved, 1) if total_resolved else None
        work_rate = round(100 * worked / materialized_n, 1) if materialized_n else None
        lines.append(
            f"_Track record: {total_resolved} resolved — {materialized_n} materialized "
            f"({mat_rate}%), of which {worked} worked ({work_rate}%), {failed} failed, "
            f"{no_mat} never materialized._"
        )
...
_SHADOW_ALIASES = {
    ...
    "exp3": "EXP3_SR", "sr": "EXP3_SR", "supportresistance": "EXP3_SR", "exp3_sr": "EXP3_SR",
...
def _classify_context_block(tags):
    ...
    ADDED (2026-08-16): _blocked_by_risk_gate_width — technically not a
    "market context" block (it fires downstream, in Trade Management,
    after a tier genuinely structurally fired), but reused here rather
    than duplicating this same override-precedence mechanism: without
    it, these records carry no per-tier `activated` dict at all (see
    log_risk_gate_rejection_to_queue in scanner_live.py), so they'd fall
    through to _classify_rejection_reason("") below and land in "Other /
    Unclassified" — hiding the exact population the friend's Structural
    Stop Engine discussion needs visibility into. Mutually exclusive
    with the three checks above by construction (this only ever fires
    after context checks already passed), so precedence doesn't matter
    here."""
    if tags.get("_blocked_by_risk_gate_width"):
        return "Risk Gate — stop too wide"
...
def _humanize_token(token):
    """Same underlying bug as _md_safe_variant() (per chat, 2026-08-19 —
    found via /thesis showing 'high_vol · london_early' with visible
    underscores and 'was_choch=False' in Recent leg history) — but for
    values that sit INSIDE prose ('_regime: ..._') rather than a compact
    tag list, so words-with-spaces reads more naturally than a middle-dot
    identifier. 'high_vol' -> 'High Vol'. Deliberately a separate helper
    from _md_safe_variant() rather than unifying them: variant names
    (EXP2's 'int_no_htf_618') are technical identifiers better left
    compact; these are prose words. Purely a display transform — never
    changes what's stored, never invents a word that isn't already in
    the token."""
    if token is None:
        return None
...
def _plain_first(text):
    """Lowercases the first character of a sentence-fragment field
    (e.g. market_thesis_invalidation, market_thesis_expected_next_event,
    failure_risk_reasons) so it reads naturally after a lead-in clause
    like 'What I'd expect next:' instead of starting a new sentence with
    a capital. Purely cosmetic — same spirit as brain.py's _plain_clause,
    duplicated here rather than imported: brain.py deliberately imports
    nothing from this codebase (see its module docstring) and this is a
    two-line formatter, not worth breaking that boundary for."""
...
def format_rejected_live_detail(shadow_stats):
    ...
    if winners_tagged:
        ...
        for r in winners_tagged:
            # Risk-gate-width records carry a flat tier_label STRING (see
            # log_risk_gate_rejection_to_queue), not a per-tier dict —
            # handled separately here rather than forcing that shape to
            # match the other case artificially. Added 2026-08-16.
            if r["tags"].get("_blocked_by_risk_gate_width"):
                tl = r["tags"].get("tier_label")
                if tl:
                    tier_wins[tl] = tier_wins.get(tl, 0) + 1
                continue
            for tier_label, v in r["tags"].items():
                if isinstance(v, dict) and v.get("activated"):
...
# ---- MARKET INTELLIGENCE NETWORK: Trade Investigation Bureau --------------
# RENAMED (2026-08-29, per chat) from Failure Investigation Bureau — see
# open_trade_case()'s docstring for what changed and why.
...
def compute_conviction_buckets(tier_label, min_n=CONVICTION_AUDIT_MIN_BUCKET_N):
    ...
    score tag (frozen at signal time — see experiment_7_tier_atr_mirror,
    not recomputed after the fact), using this tier's own
    CONVICTION_MIN_BY_TIER floor plus the CONVICTION_MANAGEMENT_BANDS
    floors (which start at 0 — genuinely below-floor) as edges — the
    exact floors already gating live eligibility and sizing, so this
    checks whether THOSE floors earn their keep rather than inventing
    new ones to bucket on.

    Needs EXP7's FULL population (every activated peek, floor-clearing
    or not) — EXP4_POLICY_LAB's CONVICTION_SWEEP_ON arm only ever logs
    once its OWN gate has already fired, so it structurally cannot
    contain a below-floor row; see the `score` tag's own comment in
    experiment_7_tier_atr_mirror for why this reads EXP7 rather than
    EXP4 despite the nearby stale-tag fixes doing the opposite.
    ...
    for i, floor in enumerate(edges):
        ...
        group = [r for r in records
                 if r.get("tags", {}).get("score") is not None
                 and r["tags"]["score"] >= floor
                 and (ceiling is None or r["tags"]["score"] < ceiling)]
...
def format_conviction_audit(tier_label):
    ...
    # Needs EXP7's FULL population (every activated peek, floor-clearing
    # or not) for the same reason compute_conviction_buckets() does —
    # see the `score` tag's comment in experiment_7_tier_atr_mirror.
    records = [r for r in _read_shadow_trade_log(experiment="EXP7_TIER_ATR")
               if r.get("variant") == tier_label and r.get("r_achieved") is not None
               and r.get("tags", {}).get("score") is not None]
    if isinstance(tier_min, (int, float)):
        below = [r for r in records if r["tags"]["score"] < tier_min]
        at_above = [r for r in records if r["tags"]["score"] >= tier_min]
...
def format_failure_investigation(trade_id):
    ...
    lines = [f"🕵️ *Trade Investigation — `{trade_id}`* (Tier {tier_number})",
             "─────────────────────",
...
def format_market_thesis(state, understanding=None, deep_thesis_text=None):
    """
    Market Thesis Engine. Pure formatter — computes nothing itself, only
    renders. Runs ZERO experiments, touches ZERO API, same as every other
    formatter imported into scanner_live.py for instant replies.

    PHASE 0 AUTHORITY ROUTING (2026-08-27, per audit — friend's review of
    the /understand reacceleration case, then the follow-up architecture
    discussion). Before this change, /thesis's narrative body came
    straight from stitch_narrative() (scanner_observation.py, persisted
    to market_thesis_narrative) while /understand's came from brain.py —
    two independently-computed interpretations of the same scan, able to
    disagree on the exact same market (the actual bug that started this
    whole review). Fix: the narrative body is now brain.format_
    understanding_narrative(understanding) — the SAME function /understand
    calls, on the SAME understanding object, built by the SAME caller
    right before this is invoked (see _answer_deferred_reports() /
    _maybe_send_daily_thesis_update() in scanner_live.py). /thesis and
    /understand can no longer say different things on the same scan.

    `understanding` is optional and falls back to the OLD field-dump-free
    behaviour (reading market_thesis_* directly) when None or when Brain
    doesn't have enough to say yet (no directional HTF bias) — same
    defensive discipline as every other formatter here: never crash,
    never go silent, degrade to the best available source instead.

    `deep_thesis_text` is optional, pre-rendered prose for the "Deeper
    read" section (the caller computes this via
    hfis.narrate_deep_thesis(state.get("mil_understanding")) — this
    function does NOT call hfis itself, deliberately, to keep this
    module's import graph one-way; see the note at this file's hfis
    import site, now removed, for why). None/empty omits the section
    entirely, same as before this was made an injected parameter.

    STILL NO FREE INFERENCE in either branch, same discipline as brain.py.
    This function only chooses order/omission, never wording.

    NOT MOVED TO BRAIN (deliberately, Phase 0 scope — see chat): evidence_
    prose and invalidation stay state-sourced. They're closer to stated
    facts/falsification-conditions than to competing conclusions, and
    Brain doesn't yet have its own equivalent for either — showing them
    isn't a second contradicting verdict, just supporting/boundary detail.
    failure_risk is DROPPED from display here (was: a second, independently-
    computed LOW/MEDIUM/HIGH verdict sitting right next to Brain's own
    structural read — exactly the two-verdicts-in-one-paragraph problem
    this pass exists to remove). Still computed and persisted unchanged
    for EXP7/backend consumers (see market_thesis_failure_risk's other
    call sites) — only removed from what a human reads here.
    ...
    if market_assessment:
        sentences.append(format_understanding_narrative(understanding))
    else:
        # FALLBACK — Brain has nothing yet for this scan (no directional
        # HTF bias, or `understanding` wasn't passed at all, e.g. an
        # older caller). Unchanged from the pre-Phase-0 behaviour so this
        # never regresses to a worse answer than before.
        opener = (state.get("market_thesis_narrative")
                  or state.get("market_thesis_transition_narrative")
                  or current_state)
        sentences.append(opener)

        trend_health = state.get("market_thesis_trend_health")
        if trend_health and trend_health not in opener:
            sentences.append(trend_health)

    evidence_prose = state.get("market_thesis_evidence_prose") or []
    if evidence_prose:
        sentences.append("Supporting that: " + "; ".join(evidence_prose) + ".")

    invalidation = state.get("market_thesis_invalidation")
    if invalidation:
        sentences.append(f"This would be invalidated by {_plain_first(invalidation)}.")
    ...
    body = " ".join(sentences)
    footer = f"_{confidence_line} · updated {state.get('market_thesis_updated_at', '?')}_"

    # DEEP THESIS SECTION (2026-09-11, per chat — "the in-depth thesis/
    # analysis where there's primary thesis, counter thesis, failure
    # conditions, etc."). Everything above this point is Brain's older,
    # separate market_thesis_* narrative (current_state/evidence_prose/
    # invalidation) — it never touched MIL's structured belief at all.
    # The 9/12/3 overview push is deliberately light and stays that way
    # (narration_library.light_summary(), as of the 2026-09-16 Library
    # redesign — see that module's docstring); THIS command is the
    # surface meant to go deep, via narrate_deep_thesis()'s own narrated
    # rendering. Appended as its own section, not woven into `body`
    # above, so a missing/unpopulated MIL understanding degrades to
    # exactly the old /thesis output rather than a broken or truncated
    # one.
    # LAYER-BOUNDARY FIX (2026-09-14): this used to call
    # narrate_deep_thesis(state.get("mil_understanding")) directly, which
    # required importing hfis into min_scanner.py — see the import-site
    # comment near the top of this file for why that's a one-way-
    # dependency violation. The caller now renders it and passes the
    # result in; None/empty behaves identically to the old "nothing to
    # show" path.
    deep_section = deep_thesis_text
    deep_block = f"\n\n🔬 *Deeper read:*\n{deep_section}" if deep_section else ""

    return f"📖 *Market Thesis*\n\n{body}\n\n{footer}{deep_block}"
...
# =========================================================================
# WORLD STATE — Phase 1A/1C (per chat, 2026-08-18): a read-only unification
# layer over the specialist engines that already exist (Thesis, Phase,
# Intent, Structure, Live/Shadow setups, Evidence, Bank). Builds NOTHING
# new — every field is either already persisted somewhere, or explicitly
# reserved as None (conversation/trade_reasoning — Phase 3, not designed
# yet). structure.system_observation and setups.live_tiers were once in
# that reserved category too, but both shipped 2026-08-20 and have been
# populated every scan since — see their own comments below. Detects
# nothing, gates nothing, decides nothing — same discipline as
# MarketThesis/MarketIntent. Full design doc: world_state_schema.md
# (v0.2, per chat with Vally's collaborator).
#
# NOTE on freshness: deliberately never uses file mtime — see
# save_shadow_state()'s comment above for why that's unsafe on GitHub
# Actions. Every source's freshness comes from a timestamp embedded IN
# the file's own content.
# =========================================================================

# Live's own cron-job.org interval is confirmed in scanner_live.py's module
# docstring (every 5 min). MIN's own repeat interval — confirmed 2026-08-19
# against min-scan.yml directly: same 5-minute cadence as Live, offset by
# ~2-5 min ("live at :00/:05/:10..., this one at :02/:07/:12..."). No
# longer a placeholder guess.
# WorldState (build_world_state/format_world_state) moved to worldstate.py.
# Per chat (physiology work): MIN never called these itself (only
# scanner_live.py did, for Telegram commands) — living in this file
# wrongly implied MIN was authoritative over the organism's shared
# state. Pure relocation, no logic changed. Import from worldstate.py
# if you need these here.


def format_integrity_command(state):
    try:
        import scanner_integrity
        report = scanner_integrity.run_integrity_pass(state)
        return scanner_integrity.format_integrity_report(report)
    except Exception as e:
        return f"_/integrity error: {e}_"
...
def run_min_pass():
    ...
    if df_5m is not None:
        try:
            ...
            # DEFENSE IN DEPTH (audit fix, 2026-09-08): per-setup isolation
            # now lives inside update_pending_shadow_setups() itself (see
            # its docstring), so this should realistically never fire —
            # but if something outside that per-setup loop breaks (a bad
            # df_5m shape, a corrupt candle index), this is the last line
            # of defense before it goes back to being a silent, print-only
            # stall with no trace anywhere Telegram-facing. Escalated, not
            # just printed, precisely because a print-only catch here is
            # the exact pattern that let the original EXP8 bug run for days.
            print("[SHADOW ERROR — STRUCTURAL] update_pending: " + str(e))
            try:
                send_telegram(
                    f"⚠️ *update_pending_shadow_setups crashed* outside its "
                    f"own per-setup isolation: {type(e).__name__}: {e}\n"
                    "Resolution did not advance this pass for ANY experiment "
                    "— check console/Action logs, this is a structural bug."
                )
            except Exception:
                pass
    ...
    # Bank ledger settles regardless of cache freshness too — same
    # reasoning as the drain above: only replays already-resolved
    # permanent-log entries, no fresh candles needed. Per chat,
    # 2026-08-16 — "let every process and transaction be permanently
    # logged."
    try:
        settle_bank_transactions()
    except Exception as e:
        print("[BANK LEDGER ERROR] settle_bank_transactions: " + str(e))

    # System Overview Ledger — same "no fresh candles needed" reasoning
    # as settle_bank_transactions() immediately above: this only
    # aggregates already-permanently-logged records from the four MIN-
    # owned logs (shadow_trade_log.jsonl, bank_transactions.jsonl,
    # leg_obs_log.jsonl, failure_case_log.jsonl), never touches candles.
    # Self-bootstrapping (see system_ledger.py's own docstrings) — cheap
    # no-op on the vast majority of passes; only does real work once a
    # day/week/month has actually completed. Per chat, 2026-08-23.
    try:
        ledger_result = run_scheduled_ledger_maintenance(now_utc)
        if ledger_result["daily_built"] or ledger_result["weekly_built"] or ledger_result["monthly_built"]:
            print(f"[LEDGER] built: daily={ledger_result['daily_built']} "
                  f"weekly={ledger_result['weekly_built']} "
                  f"monthly={ledger_result['monthly_built']}")
        if ledger_result["errors"]:
            print(f"[LEDGER ERROR] {ledger_result['errors']}")
    except Exception as e:
        print("[LEDGER ERROR] run_scheduled_ledger_maintenance: " + str(e))

    if df_5m is None or len(df_1h) < HTF_BIAS_MIN_BARS:
        return  # nothing candle-dependent left we can safely do this pass

    # ── Read LIVE's authoritative bias — NEVER recompute it here ──
    # Per chat (physiology work): MIN independently calling
    # compute_macro_bias() against its own read-only snapshot was the
    # original "two brains" bug this whole exercise was built to find —
    # a private version of a fact LIVE already has authoritative,
    # committed state for. It was also strictly WORSE than reading the
    # committed field: MIN's cache can be staler than LIVE's last real
    # commit, so a "fresh" recompute here could silently produce an
    # OLDER bias than what's already sitting in state.json, while this
    # pass's downstream experiments treat it as current. Reading the
    # authoritative field is simultaneously more correct AND simpler.
    macro_bias = state.get("macro_bias_confirmed")
    bias_stale = state.get("macro_bias_stale", False)
    if macro_bias is None:
        print("[MIN] no authoritative macro_bias in state yet — skipping this pass.")
        return
    ...
    # ── Read LIVE's authoritative phase — NEVER recompute it here ──
    # Per chat (physiology work): same fix as macro_bias above, applied
    # to phase. discover_sr_candidates() (the only consumer of
    # phase_result in this whole pass) only ever reads .phase and
    # .age_bars — both of which LIVE already persists directly as
    # state["market_phase"] / state["market_phase_age_bars"], computed
    # strictly BEFORE any tier/signal decision fires this scan. That
    # timing is exactly what run_sr_tracking()'s own docstring wants
    # ("a RETEST_REACTION can never have been influenced by which live
    # tier fired this scan") — reading the committed field satisfies
    # that property just as well as recomputing did, since phase is
    # locked in before any tier evaluation regardless of which organ
    # reads it. The old recompute here also silently lost LIVE's
    # EXHAUSTION exit-hysteresis (PHASE_EXHAUSTION_EXIT_FRACTION) on
    # every MIN pass, since MIN never had a prev_phase to hand back in —
    # reading the authoritative field doesn't have that gap either.
    _phase_str = state.get("market_phase")
    _phase_age_bars = state.get("market_phase_age_bars")
    phase_result = None
    if _phase_str is not None:
        try:
            class _AuthoritativePhaseView:
                """Minimal read-only stand-in for PhaseResult — carries only
                the two fields discover_sr_candidates() actually touches.
                Never construct a full PhaseResult here; that would tempt a
                future edit into reading more fields than were verified
                safe, quietly reintroducing the duplication this fix
                removes."""
                def __init__(self, phase, age_bars):
                    self.phase = phase
                    self.age_bars = age_bars
            phase_result = _AuthoritativePhaseView(Phase(_phase_str), _phase_age_bars)
        except ValueError as e:
            # Bad/unrecognized market_phase value in state — expected,
            # non-fatal case.
            print(f"[MIN PHASE ERROR] unrecognized market_phase value in state: {e}")
            phase_result = None
        except Exception as e:
            # ADDED (2026-09-08, per audit): anything else here (e.g. a
            # NameError from a missing import, an AttributeError from an
            # upstream refactor) must NOT be allowed to propagate and
            # kill the whole MIN pass — the entire point of reading the
            # authoritative field instead of recomputing was to make
            # this path safe/optional. Fail this one component, keep
            # scanning.
            print(f"[MIN PHASE ERROR] unexpected error building phase_result: {type(e).__name__}: {e}")
            phase_result = None
    ...
    sr_state = load_sr_levels_state()
    _sr_state_holder = [sr_state]   # run_sr_tracking returns a NEW dict
                                     # (pure-style, not mutate-in-place
                                     # like shadow_state/shadow_stats) —
                                     # this holder is just how that
                                     # return value survives the
                                     # lambda-dispatch loop below.
    experiments = [
        ...
        ("Experiment 2 (Fib)", lambda: experiment_2_fib(
            facts, current_atr_pips, shadow_state, shadow_stats, now_utc)),
        ("Experiment 3 (Support/Resistance)", lambda: _sr_state_holder.__setitem__(0, run_sr_tracking(
            facts, phase_result, macro_bias, current_atr_pips, now_utc,
            _sr_state_holder[0], shadow_state, shadow_stats))),
    ...
    save_shadow_state(shadow_state)
    save_shadow_stats(shadow_stats)
    save_sr_levels_state(_sr_state_holder[0])
