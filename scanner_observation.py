...
def build_market_intent(phase_result, ctx, facts, thesis, macro_bias):
    ...
    if ob_live and not facts.price_in_order_block():
        watching_for.append({
            ...
            "sentence": (
                ...
                f"({zone_word} zone) rather than chasing price in the "
                f"middle of the range."
            ),
    ...
    if fib_live:
        ...
        watching_for.append({
            ...
            "sentence": (
                f"A {macro_bias_word} retracement into a Fibonacci "
                f"pullback zone inside the current leg — not a fresh "
                f"{zone_word} zone — rather than chasing price in the "
                f"middle of the range. What would count as a reaction: a "
                f"{macro_bias_word} rejection candle forming while price "
                f"is still inside that zone."
            ),
