...
def reconcile(prev: Optional[MarketUnderstanding], observed: ObservedLegState,
    ...
    try:
        ...
        #
        # MESSAGE FIX (2026-09-10, per chat): this used to always say
        # "Direction changed (X -> Y)" regardless of what validate_
        # transition() actually rejected — including the exact stuck-loop
        # case above, where direction had NOT changed (X and Y were the
        # same value) and the real problem was an illegal (from_state,
        # to_state) edge, not a direction flip. Now states the attempted
        # edge and the validator's own reason directly, so a future
        # unresolved_question is actually diagnosable from the text alone
        # instead of always looking like the same one specific scenario.
        new_unresolved_questions.append(
            f"Could not complete {transition.from_state.value} -> "
            f"{transition.to_state.value} ({e}); frozen at TRANSITIONING, "
            f"at {now_iso}."
        )
