...
from brain import (
    ...
    format_understanding_narrative,
...
def _group_report_text(state, now_utc):
    ...
    Mutates state (group_report_last_current_state) — caller is
    responsible for save_state() after calling this, same convention as
    every other state-mutating helper in this file."""
    ws = build_world_state(state)
    briefing = build_market_briefing(ws)
    th = (briefing or {}).get("thesis") or {}
    current_state = th.get("current_state") or "No directional read yet."
    prev = state.get("group_report_last_current_state")
    changed = prev is not None and prev != current_state
    header = now_utc.strftime("🕐 *GBPUSD — %H:%M UTC Report*")
    lines = [header, ""]
    if changed:
        lines.append("Something changed since the last report:")
        ...
        lines.append(current_state)
        lines.append("")
        lines.append("_Nothing significant changed since the previous report._")
    lines.append("")
    lines.append("_System status: data received and processed._")
    state["group_report_last_current_state"] = current_state
    return "\n".join(lines)
            ...
            if surface == "group":
                ...
                try:
                    if cmd_bare == "/status":
                        ...
                        save_state(state)  # persists group_report_last_current_state
...
def _scan_once():
    ...
    if phase_result is not None:
        ...
        try:
            ...
            if _mil_understanding is not None:
                try:
    ...
    if GROUP_CHAT_ID:
        ...
        _group_signal_text = (
            f"🟡 *GBPUSD — {result.tier_label} setup*\n\n"
            + _group_warn_block +
