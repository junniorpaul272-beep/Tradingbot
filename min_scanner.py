...
def _close_leg_obs(obs_state, open_rec, fate, now_utc, add_to_closed=False, state=None):
    ...
    `state` (2026-08-27, per chat — Market Event lifecycle) is OPTIONAL
    and additive: if this leg has any Market Events tagged against it in
    state["market_event_pending"] (see _evaluate_and_push_market_event()
    in scanner_live.py — that's the only writer of this key), send ONE
    follow-up push naming what fired earlier and how this leg resolved,
    then clear those entries. This is the actual "event lifecycle" —
    EVENT -> NOTIFY -> ... -> CONFIRMED/INVALIDATED/EVOLVED -> FOLLOW-UP
    — implemented by hooking the resolution Forward Observation ALREADY
    computes, not by building a second leg-tracking mechanism. Wrapped
    in its own try/except and runs strictly AFTER the existing
    ...
    # MARKET EVENT LIFECYCLE FOLLOW-UP (per chat, 2026-08-27) — additive,
    # best-effort, never affects the close above (already committed).
    if state is not None:
        try:
            pending = state.get("market_event_pending") or []
            this_leg_id = open_rec.get("id")
            matched = [p for p in pending if p.get("leg_obs_id") == this_leg_id]
            if matched:
                headlines = "; ".join(p.get("headline", "") for p in matched if p.get("headline"))
                ...
                state["market_event_pending"] = remaining
                save_state(state)
        except Exception as e:
        ...
        if _same_leg(open_leg_id, current_leg_id):
            ...
            if bias_stale:
                ...
                obs_state = _close_leg_obs(
                    obs_state, open_rec, "INVALIDATED", now_utc, add_to_closed=True, state=state)
            else:
            ...
            obs_state = _close_leg_obs(
                obs_state, open_rec, fate, now_utc, add_to_closed=False, state=state)
