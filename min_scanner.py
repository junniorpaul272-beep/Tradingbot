...
from scanner_common import (
    ...
    CONVICTION_MANAGEMENT_BANDS, CONVICTION_MIN_BY_TIER,
...
_SHADOW_STATS_EXPERIMENT_KEYS = [
    "EXP1_STRUCTURE", "EXP2_FIB", "EXP3_POI",
...
# untouched by this list going forward) rather than scrubbed — no
# reason to touch historical files for a key that logged nothing.
...
# ---- Experiment 4: Liquidity Sweep -- REMOVED (per chat, 2026-08-11) -----
# See the comment above _SHADOW_STATS_EXPERIMENT_KEYS for the root-cause
# writeup (structural window collision between sweep detection and 15M
# confirmation, never confirmed against live data before removal).
# Dispatch entry removed from run_min_pass()'s experiments list too.
    ...
    for tier_label, tier_fn in TIER_REGISTRY.items():
        ...
        setup = build_shadow_setup(
            ...
            tags={"conviction_score": peek.score,
                  # BUG FIX (found continuing this same change): peek.fired
                  # / live_peek.fired are now ALWAYS True once a tier is
                  # activated with a structural sl_raw (conviction no
                  # longer gates `fired` — see classify_tierN_risk). These
                  # two tags exist specifically to answer "would the OLD
                  # score gate have fired this" for retrospective research
                  # (compute_conviction_buckets, the Failure Investigation
                  # Bureau's would_have_fired_live=True check, etc.) — they
                  # MUST keep reading the conviction decision directly, not
                  # .fired, or every mirrored trade silently reads as
                  # "the live engine expected a win" regardless of score.
                  "would_have_fired_pre_context": bool(
                      peek.conviction and peek.conviction.get("decision") == "FIRE"),
                  "would_have_fired_live": bool(
                      ctx.tradeable and risk_gate_pass and
                      live_peek.conviction and live_peek.conviction.get("decision") == "FIRE"),
                  "risk_gate_pass": risk_gate_pass,
                  # NEW tags (same change): risk_band is what EXP7 needs
                  # tagged to eventually answer the actual point of this
                  # feature — "does LOW/MEDIUM/HIGH separate winners from
                  # losers within each tier" — without this the risk-at-
                  # hand labels have no research data behind them at all.
                  # fires_live_now is DIFFERENT from would_have_fired_live
                  # above: that one is the OLD score-gated population
                  # (retrospective only); this one is today's REAL live-
                  # fire population under the new structural-only gate
                  # (live_peek.fired already correctly reflects stale-bias
                  # suppression via _gate_stale_bias, just no longer
                  # reflects a conviction decision).
                  "risk_band": peek.risk["band"] if peek.risk else None,
                  "fires_live_now": bool(ctx.tradeable and risk_gate_pass and live_peek.fired),
...
# ---- MARKET INTELLIGENCE NETWORK: Regime-Conditional Performance ---------
...
def run_min_pass():
    ...
    experiments = [
        ...
        # Experiment 4 (Liquidity) removed — see comment above
        # _SHADOW_STATS_EXPERIMENT_KEYS.
