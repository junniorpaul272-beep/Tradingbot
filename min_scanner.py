...
def _resolve_one_pending_setup(setup, high, low, close, candle_time, shadow_stats,
    ...
    """The per-setup resolution body, unchanged in logic from before the
    isolation fix — extracted purely so update_pending_shadow_setups() can
    wrap ONE setup's processing in its own try/except instead of the whole
    shared loop. Returns True if this setup resolved (should be dropped
    from pending), False if it's still open (should stay in pending)."""
    setup["bars_open"] += 1
    direction = setup["direction"]
...
def update_pending_shadow_setups(shadow_state, shadow_stats, df_5m, now_utc):
    ...
    for candle_time, candle in candles.iterrows():
        ...
        for setup in pending:
            try:
                resolved = _resolve_one_pending_setup(
