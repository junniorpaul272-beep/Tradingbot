...
def synthesize_market_understanding(relationships, current_condition):
    ...
    a next_watch resolved HERE, once, in one place:
      ...
      - Branch didn't mention next_watch at all -> filled from
        ...
        Thesis's own expected_next_event used to use — same content,
        different authority: Brain is now the one deciding to use it,
        not passively inheriting Thesis's already-committed conclusion.
    ...
    if "next_watch" not in result:
        result["next_watch"] = (current_condition or {}).get("default_next_event")
    return result
...
def build_event_snapshot(understanding, intent_hypothesis):
    """
...
def evaluate_market_event(understanding, intent_hypothesis, prev_snapshot):
    """
    ...
    THREE CATEGORIES (Vally's taxonomy):
      ...
      - opportunity: a new location or confirmation entered Brain's
        intent_hypothesis that wasn't there last scan — something newly
        relevant in the opportunity landscape, tier-agnostic (tiers 1-3
        all flow through intent_hypothesis's locations/confirmations —
        see build_market_intent_hypothesis() above). Cautions
        appearing/disappearing are deliberately NOT event-worthy on
        their own — lower-signal than something becoming relevant.
    ...
    if not structure_fired:
        ...
        if prev_family and curr_family and prev_family != curr_family:
            events.append({
                "category": "state",
                "headline": (
                    f"The market's condition has shifted from "
                    f"{_STATE_FAMILY_LABELS.get(prev_family, prev_family)} "
                    f"to {_STATE_FAMILY_LABELS.get(curr_family, curr_family)}."
                ),
            })
    ...
    new_locations = curr_locations - set(prev_snapshot.get("location_codes") or [])
    new_confirmations = curr_confirmations - set(prev_snapshot.get("confirmation_codes") or [])
    if new_locations or new_confirmations:
        events.append({
