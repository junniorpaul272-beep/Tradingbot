...
def _evaluate_and_push_market_event(state, now_utc):
    ...
    try:
        ...
        events = evaluate_market_event(understanding, intent_hypothesis, prev_snapshot)
        ...
        if events:
            ...
            # fate. Read-only peek at leg_obs's own state file — same
            # established pattern already used for Market Story's
            # timeline peek above (_answer_deferred_reports() /
            # build_market_thesis() call site) — NOT run_leg_observation()
            # itself, which only runs from min_scanner.py's run_min_pass().
            # No open leg (e.g. mid-transition, or bias just went stale)
            # -> nothing to tag against; the event still gets pushed, it
            # just won't have a resolution follow-up later.
            try:
        ...
        state["market_event_prev_snapshot"] = build_event_snapshot(understanding, intent_hypothesis)
        save_state(state)
    except Exception as e:
