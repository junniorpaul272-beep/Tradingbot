...
from scanner_common import EXPECTED_NEXT_EVENT_MAP, load_markov_data
from min_scanner import (
    ...
    # ADDED (2026-09-08, bug fix): both load_markov_data() and
    # compute_bank_accounts() were called below (lines ~238, ~251) but
    # never imported. build_world_state() has no internal try/except,
    # so this was a guaranteed NameError on every single call — every
    # caller (/world, /understand, /thesis, /marketintent, /overview,
    # and the proactive Market Event evaluation) was failing on every
    # invocation. Command call sites surfaced it as a Telegram error
    # message; the Market Event call site swallowed it silently inside
    # its own broad except Exception.
    compute_bank_accounts,
)
