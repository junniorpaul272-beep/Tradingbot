...
def load_shadow_state():
    try:
        ...
        if state.get("methodology_version") != SHADOW_METHODOLOGY_VERSION:
            return {"pending": [], "last_leg": {}, "drained_leg_ids": [],
...
def drain_rejected_live_queue(shadow_state, shadow_stats):
    ...
    log_shadow_setup's own last_leg dedup does NOT catch this: it only
    remembers the single most recently logged leg_id per experiment/variant
    key, so any leg that isn't the literal last one in a replayed batch
    gets silently re-logged (inflates shadow_stats['logged'], duplicates
    shadow_state['pending']).
    Fix: shadow_state["drained_leg_ids"] is the actual source of truth for
    "have I handled this leg_id" — it lives in shadow_state.json, which MIN
    exclusively writes (no cross-workflow contention, no union driver on
    that file), so it survives regardless of what happens to the queue
    file's git history."""
...
def format_scenario_summary():
    ...
    legs by their formation-time (market_phase, measured_move_bucket) and
    reports the empirical P(REVERSED) per bucket via the same
    Beta-Binomial posterior used for tier evidence (_beta_posterior /
    EVIDENCE_MIN_N). Called by `/legobs scenario`.
    ...
    same trending run tend to share BOTH phase and extension bucket, so
    consecutive samples in the same bucket are correlated draws, not
    independent ones. More bucket dimensions (phase × extension) means
    more ways to slice a still-limited, still-correlated sample — this
    does not fix that problem, it just gives more buckets to misread with
    false confidence. Treat every percentage below as directional, and
    cross-check against compute_tier_sharpe()/the Markov transition
    ...
    for r in records:
        ...
        if phase is None or ext_bucket is None:
            continue
        tagged_n += 1
        key = (phase, ext_bucket)
        b = buckets.setdefault(key, {"wins": 0, "losses": 0, "n": 0})
        b["n"] += 1
        if r.get("fate") == "REVERSED":
            b["wins"] += 1
        else:
            b["losses"] += 1
    ...
    if not buckets:
        return (
            "🔭 *Scenario Forecast*\n"
            "_No resolved legs tagged with phase/extension yet — this "
            "field was only just added; give it time to accumulate._"
        )

    lines = [
        "🔭 *Scenario Forecast — P(Reversal) by Phase + Leg Extension*",
        "─────────────────────",
        f"Tagged resolved legs: `{tagged_n}` (of `{len(records)}` total resolved)",
        "",
    ]
    for (phase, ext_bucket), b in sorted(buckets.items(), key=lambda kv: -kv[1]["n"]):
        n = b["n"]
        if n < EVIDENCE_MIN_N:
            lines.append(
                f"  `{phase}` / `{ext_bucket}`: N=`{n}` — "
                f"_below EVIDENCE_MIN_N ({EVIDENCE_MIN_N}), not shown_"
            )
            continue
        post_mean, ci_lo, ci_hi = _beta_posterior(b["wins"], b["losses"])
        lines.append(
            f"  `{phase}` / `{ext_bucket}`: N=`{n}` — P(reversal) "
