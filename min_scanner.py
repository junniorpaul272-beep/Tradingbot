...
def format_market_intent_report(state, mi_state):
    ...
    watch_codes = state.get("market_intent_watch_codes") or []
    caution_codes = state.get("market_intent_caution_codes") or []
    reasons = state.get("market_intent_reasons") or []

    if watch_codes:
        lines.append("")
        lines.append("*Watching for:*")
        for w in watch_codes:
            # w is now a structured dict (per chat, 2026-08-24 —
            # WorldState/Brain review), not a bare code string —
            # unwrap here so this raw dev-facing view keeps working
            # unchanged. This IS the "code manifest" the redesign is
            # replacing for the human-facing briefing; deliberately
            # left as-is here since /marketintent stays the raw
            # dev/audit view (per chat), not deleted.
            lines.append(f"  • `{w['code']}`")
    if caution_codes:
        lines.append("")
        lines.append("*Not interested in:*")
        for c in caution_codes:
            lines.append(f"  • `{c['code']}`")
    if reasons:
        lines.append("")
        lines.append("*Reasons (from Market Thesis):*")
        for r in reasons[:6]:
            lines.append(f"  • {r}")
...
def format_market_thesis(state):
    ...
    if mtf:
        ...
        if parts:
            ...
            if regime_bits:
                lines.append(f"  _regime: {' · '.join(_humanize_token(b) for b in regime_bits)}_")
            lines.append("")
