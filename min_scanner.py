...
def format_market_intent_report(state, mi_state):
    """
    Telegram rendering for /marketintent. Reads the ALREADY-PERSISTED
    Market Intent snapshot from state (state["market_intent_*"], written
    by scan() the same scan build_market_intent() ran) — does not
    recompute anything, same pattern as format_market_thesis(). Appends
    the currently-tracked instance (if any) and a lightweight track
    record from the resolved log, gated by MARKET_INTENT_CALIBRATION_MIN_N
    the same as every other statistic in this codebase.
    ...
    human to read; market_intent_narrative already IS "what I'm watching
    for and why" in one paragraph. Tracking status and track record stats
    are kept as-is (already plain language, not raw labels).
    """
    if "market_intent_narrative" not in state:
...
def format_market_thesis(state):
    ...
    REWRITTEN (2026-08-26, per chat — user: "it should talk to me", and
    explicitly named /thesis as one of the three market-explanatory
    commands to keep, meaning conversational, not a field dump). This
    used to render ~15 separately-headed sections (Current State,
    Transition, Trend Health, Since Last Scan, 5M Read, 15M Read,
    Campaign Extension, Evidence, Weaknesses, Failure Risk, Expected
    Next Event, Invalidation, Confidence) even though market_thesis_
    narrative already stitches most of that into one paragraph
    (stitch_narrative(), scanner_observation.py) — the old version just
    printed the paragraph AND all its own ingredients separately underneath
    it. Now: one paragraph. Every clause is still sourced directly from a
    state field already computed elsewhere (STILL NO FREE INFERENCE, same
    discipline as brain.py) — this function only chooses order/omission.
    ...
    # Opener: the narrative stitch already covers transition cause,
    # extension, 5M read, delta-since-last-scan and campaign in one go
    # (see stitch_narrative()) — falls back to the plain transition
    # sentence, then to the bare current_state label, if narrative wasn't
    # built this scan (e.g. no timeline yet).
    ...
    if weaknesses_prose:
        sentences.append("Weighing against it: " + "; ".join(weaknesses_prose) + ".")

    failure_risk = state.get("market_thesis_failure_risk")
    failure_risk_reasons = state.get("market_thesis_failure_risk_reasons") or []
    if failure_risk and failure_risk.upper() != "LOW":
        risk_txt = f"Failure risk is {failure_risk.lower()}"
        if failure_risk_reasons:
            risk_txt += " — " + "; ".join(_plain_first(r) for r in failure_risk_reasons)
        sentences.append(risk_txt + ".")

    expected = state.get("market_thesis_expected_next_event")
    if expected:
        sentences.append(f"What I'd expect next: {_plain_first(expected)}.")
