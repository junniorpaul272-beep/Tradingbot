...
TWO WAYS TO RUN THIS, MATCHING TWO DIFFERENT NEEDS:
  ...
  2. --auto MODE (2026-08-30, per chat — Vally: "I'd consider making
     ...
     writes the results to disk (report file, edge_finder_log.jsonl,
     candidate_registry.json). This is the mode a scheduled workflow
     (edge-research.yml) uses.

     SUPPRESSED (2026-09-09, per chat — "let it not tell me at all"):
     --auto no longer pushes a Telegram summary automatically, in any
     form — see run_auto_mode()'s own comment for the history (a
     2026-09-02 fix gated the push to only fire when something was
     actually notable, but one real event anywhere still dragged every
     other scope's boring filler along with it, which was still
     spamming). --telegram is kept as a working flag for a deliberate,
     manual send, but --auto never calls it on its own anymore.
...
def run_auto_mode(args):
    ...
    # SUPPRESSED (2026-09-09, per chat — same "let it not tell me at all"
    # as the shadow-loss digest fix in min_scanner.py). The gate added
    # 2026-09-02 (see comment history below) only stopped pushing when
    # EVERY scope was boring — one real NEW/STRENGTHENED/WEAKENED/
    # DISAPPEARED event ANYWHERE still fired the whole multi-scope
    # summary, dragging every other scope's "no candidates" filler along
    # with it. That's exactly what the screenshot showed. Rather than
    # tighten the gate further, this now sends nothing, ever, regardless
    # of --telegram — report file (args.out_report), edge_finder_log.jsonl,
    # and candidate_registry.json are written unconditionally above and
    # unaffected by this. --telegram is kept as a real, working flag
    # (send_telegram_message() is untouched) rather than deleted, in case
    # a specific investigation run ever wants to push deliberately — it
    # just isn't called automatically from --auto anymore.
    #
    # ORIGINAL GATE (per chat, 2026-09-02): previously sent this summary to
    ...
    # nothing" was still pinging Telegram every cycle.
    any_notable = any(
        etype != "UNCHANGED"
    ...
    if args.telegram:
        print(
            "\nTelegram: not sent — --auto no longer pushes automatically "
            "(report file + registry + log are still written above; "
            f"{'a notable event fired this cycle' if any_notable else 'nothing notable fired this cycle'} "
            "for reference, but that no longer triggers a push)."
        )
