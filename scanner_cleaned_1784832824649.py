...
class MarketFacts:
    ...
    def price_in_fib_pocket(self, near=FIB_ZONE_NEAR, far=FIB_ZONE_FAR,
        ...
        return observed_low <= high_bound + tol and observed_high >= low_bound - tol
    ...
    def price_in_zone(self, level, tolerance_pips=ZONE_TOLERANCE_PIPS):
        ...
        observed_low = min(c["Low"], c_prev["Low"])
        observed_high = max(c["High"], c_prev["High"])
        return observed_low <= level + tol and observed_high >= level - tol
    ...
    def fib_distance_pips(self, near=FIB_ZONE_NEAR, far=FIB_ZONE_FAR):
        """Signed distance from the last close to the adaptive fib zone
        level, in pips. Convention (both bias directions):
          > 0  price is on the FAR side of the zone (overshot / deep)
          = 0  price is exactly at the zone
          < 0  price has not reached the zone yet
        Shadow logger and Validation Engine pull this directly so a
        Step-1 rejection still gets a concrete fib_distance_pips reading,
        not just 'Failed at Step 1'."""
        zone = self.fib_zone(near=near, far=far)
        close = float(self.last_candle_5m()["Close"])
        raw = close - zone
        # For BEARISH bias the zone is above price; invert so positive =
        # price has pushed into/beyond the zone in both directions.
        if self.macro_bias == "BEARISH":
            raw = -raw
        return raw / PIP_SIZE
    ...
    def price_in_order_block(self, tolerance_pips=ZONE_TOLERANCE_PIPS):
        """True iff price is touching the order block within tolerance.
        Derived from ob_penetration_pct(): the block is "touched" when
        penetration >= -tolerance% (price is within tolerance of the near
        edge) AND penetration < 100%+tolerance% (not yet blown through it).
        Falls back to the range-based check when ob_penetration_pct() cannot
        be computed (no valid OB)."""
        ob = self.order_block()
        if ob is None or ob.get("mitigated", False):
            return False
        tol = tolerance_pips * PIP_SIZE
