...
whose [opened_at, resolved_at) window contains it), and produces:
  ...
  4. INTEGRITY / COVERAGE LAYER (2026-08-29, per chat — Vally's review
     of the FIRST report with outcome-conditioned analysis: "no_
     matching_leg isn't a market phase. unknown measured move isn't
     necessarily a market phase either. They're data lineage states...
     you need to formally distinguish MARKET CONDITION vs CONTEXT
     AVAILABILITY"). Every experiment's rise/decay comparison now leads
     with: outcome record count, context coverage per window, and a
     GOOD/PARTIAL/LOW comparability rating — BEFORE any condition-mix
     percentage, so missing context can never again be misread as a
     market finding (this is exactly what happened to the original
     EXP2 "unknown measured move" conclusion — see measured_move_
     bucket's relabel to no_prior_leg in investigation_lib.py for the
     specifics). Also adds the "what did the UNMATCHED trades actually
     do" table Vally flagged as the single most important missing
     piece — a condition-mix breakdown alone can't tell you that.
  5. DETERIORATION GATE (2026-08-29, per chat — Vally's four-question
     check, made explicit: sufficient trades in both windows? a
     meaningful decline, not noise? enough context coverage to
     attribute conditions at all? both windows actually populated?).
     A/B/C only run if all four clear — an experiment with a real
     peak-then-trough pattern but too small a decline or too little
     coverage gets an explicit "not investigated, here's why" instead
     of a confident-looking statistical autopsy of noise (his EXP7
     example: 1.95R decline across 29/21 trades). An experiment with NO
     peak-then-trough pattern at all (still rising) is explicitly
     reported as "currently positive, nothing to investigate" — not
     forced through a decay narrative it doesn't have.
...
from investigation_lib import (
    ...
    compute_integrity_report, format_integrity_report,
    unmatched_performance, format_unmatched_performance,
    should_investigate_deterioration,
)
...
def investigate_experiment(experiment_key, all_trades, leg_intervals, csv_writer, outcome_detail=False, min_n=5):
    ...
    if peak_idx is not None and trough_idx is not None and trough_idx > peak_idx:
        ...
        # INTEGRITY LAYER (2026-08-29, per chat — Vally: "no_matching_leg
        # isn't a market phase... right now the investigation output
        # mixes them in the same condition distribution. I think you
        # need to formally distinguish MARKET CONDITION vs CONTEXT
        # AVAILABILITY"). Reported FIRST, before any condition-mix
        # percentage, so missing context can never masquerade as a
        # market finding — see compute_integrity_report()'s own
        # docstring for what GOOD/PARTIAL/LOW mean.
        integrity = compute_integrity_report(enriched, rise_rows, decay_rows)
        lines.append("\n" + format_integrity_report(integrity, f"{experiment_key} — investigation integrity"))
        ...
        lines.append(format_unmatched_performance(
            "  no_matching_leg performance during rise", unmatched_performance(rise_rows), min_n=min_n))
        ...
        lines.append(format_unmatched_performance(
            "  no_matching_leg performance during decay", unmatched_performance(decay_rows), min_n=min_n))

        # GATE (2026-08-29, per chat — Vally's four-question check: "The
        # movement is too small relative to the sample... this is where
        # the investigation engine needs a gate... otherwise you'll end
        # up doing elaborate statistical autopsies on noise"). A/B/C
        # only run if this passes — a peak-then-trough pattern existing
        # is necessary but not sufficient.
        proceed, gate_reasons = should_investigate_deterioration(
            curve, peak_idx, trough_idx, rise_rows, decay_rows)

        if not proceed:
            lines.append("\n### Deterioration not investigated\n")
            lines.append("This experiment shows a peak-then-trough pattern, but did not "
                         "clear the gate for a full investigation:")
            for r in gate_reasons:
                lines.append(f"  - {r}")
            lines.append("Treat this as noise, not edge decay, until more data changes the above.")
        elif outcome_detail:
        ...
        lines.append("\nCurrently positive; no observed peak-to-trough deterioration to "
                     "investigate. (Per chat — this is the correct output for an "
                     "experiment whose cumulative R is still rising, not a gap in the "
                     "tool: forcing a decay narrative onto a genuinely still-rising "
                     "curve would be worse than saying nothing.)")
        if outcome_detail:
            lines.append("\n(Level A still runs on all matched trades below — it doesn't "
                         "need a peak/trough window.)\n")
            matched_all = [r for r in enriched if r.get("_context")]
            lines.append(format_outcome_by_dimension(
                f"{experiment_key} — win rate / expectancy / PF per condition value",
                outcome_by_dimension(matched_all), min_n=min_n))
