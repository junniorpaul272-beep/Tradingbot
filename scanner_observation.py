...
def build_market_thesis(phase_result, ctx, facts, macro_bias, bias_stale, state):
    """
    ...
    failure_risk, failure_risk_reasons = classify_failure_risk(
        phase, phase_result.aging_reason, break_count, dist_in_atr, ctx.low_atr_warning)
...
class TierResult:
    ...
    def __init__(self, activated=False, fired=False, direction=None,
        ...
        # HIGH, "reasons": [...]}. Descriptive only, same as
        # classify_failure_risk() elsewhere in this file — never gates
        # `fired`. Populated once a tier's mandatory conditions AND its
        # structural fire-trigger have already passed (same point
        # `conviction` used to be populated at, before it stopped gating).
        self.risk = risk
...
def _tier1_poi_evaluate(facts, ctx, state, now_utc):
    ...
    return TierResult(
        ...
        risk={"band": risk_band, "reasons": risk_reasons},
