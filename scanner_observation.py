...
def build_market_intent(phase_result, ctx, facts, thesis, macro_bias):
    ...
    if ob_live and not facts.price_in_order_block():
        watching_for.append({
            ...
            "sentence": (
                ...
                f"({zone_word} zone, {ob['low']:.5f}-{ob['high']:.5f}) "
                f"rather than chasing price in the middle of the range."
            ),
    ...
    if fib_live:
        ...
        watching_for.append({
            ...
            "sentence": (
                f"A retracement into the HTF fib pocket "
                f"({low:.5f}-{high:.5f}) rather than the middle of the range."
            ),
