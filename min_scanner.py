...
_SHADOW_LOSS_DIGEST_THRESHOLD = 5  # >5 losses in one pass -> one digest, not N messages
...
def _resolve_one_pending_setup(setup, high, low, close, candle_time, shadow_stats,
    ...
    Returns (resolved, loss_note):
      resolved  — True if this setup resolved (should be dropped from
                  ...
                  pending).
      loss_note — a dict ({"note", "experiment", "variant", "r_final"}) if
                  this resolution was a loss that would normally be pushed
                  to Telegram (note = format_case_note()'s output), else
                  None. Not sent here (DIGEST FIX, 2026-09-08) — the caller
                  batches these across the whole pass and decides
                  individual sends vs. one digest, precisely to avoid a
                  50+ message spam burst when a backlog of stuck trades
                  all resolve in one catch-up pass (see
                  update_pending_shadow_setups())."""
    setup["bars_open"] += 1
    direction = setup["direction"]
    ...
    # way. Telegram push stays LOSS-ONLY, deliberately (see
    # open_trade_case()'s own docstring for why) — wins are
    # still investigated and recorded, just not pushed.
    #
    # DIGEST FIX (2026-09-08, per chat — "why did I get 50+ messages"):
    # this used to call send_telegram(format_case_note(_case)) directly,
    # one message per loss, unconditionally. That's fine in normal
    # operation (losses trickle in one at a time) but turned into a
    # 50+ message spam burst the moment a backlog of stuck pending
    # trades (see the isolation fix above) all resolved in one catch-up
    # pass. Rather than send here, hand the formatted note back to the
    # caller, which decides whether to send individually or fold
    # everything from this pass into one digest — see
    # update_pending_shadow_setups().
    loss_note = None
    try:
...
def update_pending_shadow_setups(shadow_state, shadow_stats, df_5m, now_utc):
    ...
    loss_notes_this_pass = []  # DIGEST FIX (2026-09-08) — see docstring
    ...
    # DIGEST FIX (2026-09-08, per chat — "why did I get 50+ messages"):
    # under normal operation losses trickle in one at a time, so sending
    # each individually is fine and keeps the real-time feel. But when a
    # backlog clears in one catch-up pass (e.g. after the isolation fix
    # above unblocks a stall that had been silently piling up pending
    # trades), that same per-loss send turns into a 50+ message spam
    # burst — one bad UX problem replacing another. Below
    # _SHADOW_LOSS_DIGEST_THRESHOLD, behavior is unchanged (individual
    # messages, in resolution order). Above it, fold everything from this
    # pass into exactly one digest instead — full detail is unaffected,
    # since open_trade_case() already ran and permanently logged every
    # case regardless of how the Telegram side gets summarized.
    if not loss_notes_this_pass:
        pass
    elif len(loss_notes_this_pass) <= _SHADOW_LOSS_DIGEST_THRESHOLD:
        for item in loss_notes_this_pass:
            try:
                send_telegram(item["note"])
            except Exception:
                pass  # best-effort — one failed send shouldn't block the rest
    else:
        try:
            n = len(loss_notes_this_pass)
            total_r = sum(item["r_final"] for item in loss_notes_this_pass)
            avg_r = total_r / n
            by_exp = {}
            for item in loss_notes_this_pass:
                key = item["experiment"] or "UNKNOWN"
                by_exp.setdefault(key, []).append(item["r_final"])
            breakdown_lines = "\n".join(
                f"  {exp}: {len(rs)} loss(es), avg {sum(rs)/len(rs):.2f}R"
                for exp, rs in sorted(by_exp.items(), key=lambda kv: -len(kv[1]))
            )
            send_telegram(
                f"🗞 *{n} shadow losses resolved in one catch-up pass* "
                f"(avg {avg_r:.2f}R, total {total_r:.2f}R)\n\n"
                f"By experiment:\n{breakdown_lines}\n\n"
                "Individual case notes were folded into this digest to avoid "
                f"a {n}-message spam burst — full per-trade detail is still "
                "permanently recorded in the trade log and case files as "
                "usual, this only changes what got pushed to Telegram."
            )
        except Exception as e:
            print("[SHADOW LOSS DIGEST ERROR] " + str(e))
