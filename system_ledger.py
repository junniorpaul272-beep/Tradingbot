...
def build_daily_ledger(target_date, phase_history_events=None,
                        current_methodology_versions=None, day_number=None):
    """
    ...
    `day_number` — added 2026-08-25, per chat: a calendar-day index
    counting from the EARLIEST date across all five source logs (see
    all_dates_present_in_source_logs()), computed by the caller
    (maybe_build_daily_ledgers() below) and passed in here so this stays
    a pure function of its inputs — this module still doesn't read
    daily_ledger.jsonl itself to figure out its own sequence number.
    Deliberately calendar-day-based, not "count of entries written so
    far": if a day gets skipped (bot down, commit gap, whatever), the
    NEXT entry's day_number still jumps by the true gap size, so a skip
    is visible by inspection (day_number 45 followed by 47) rather than
    silently renumbering around it. None if the caller doesn't supply
    one — never fabricated, same as everything else in this file.
    """
    ...
    return {
        ...
        "day_number": day_number,  # None if caller didn't supply one — see docstring above
...
def maybe_build_daily_ledgers(now_utc):
    ...
    to_build = [d for d in all_dates if d <= yesterday and d not in already]

    # day_number's reference point — the earliest date across ALL source
    # logs, not just already-ledgered ones, so it stays stable even on a
    # fresh backfill run with an empty daily_ledger.jsonl to start from.
    first_date = datetime.fromisoformat(all_dates[0]).date() if all_dates else None

    built = []
    for date_str in to_build:
        day_number = (datetime.fromisoformat(date_str).date() - first_date).days + 1 if first_date else None
        ledger = build_daily_ledger(
            date_str, current_methodology_versions=CURRENT_METHODOLOGY_VERSIONS,
