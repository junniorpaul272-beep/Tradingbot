...
def _maybe_send_daily_thesis_update(state, now_utc):
    ...
    save_state(state)
    try:
        # PHASE 0 (per audit, 2026-08-27): this push calls the exact same
        # format_market_thesis() /thesis does — it was a fourth call site
        # of the old competing narrative, easy to miss since it's a
        # scheduled push rather than a command. Same fix, same reason.
        _ws_snapshot = build_world_state(state)
        _understanding = build_market_understanding(_ws_snapshot)
        send_telegram(
            "🗓️ *Daily Market Thesis*\n\n" + format_market_thesis(state, _understanding)
        )
    except Exception as e:
