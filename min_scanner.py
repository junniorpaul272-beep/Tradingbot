...
def format_tier_block_analysis(tier_label):
    ...
    exist in this codebase today —
      - ATR_DATA_INVALID: the ATR reading itself was unusable (NaN/0)
                           and blocked the whole scan before Rule of Law
                           ever ran (this tier never got a conviction
                           check at all that scan). NOTE (2026-08-11):
                           ATR pip LEVEL is no longer a gate at all — a
                           thin-but-valid ATR reading never lands in
                           this bucket anymore, only a genuinely broken
                           reading does, so expect this near-empty going
                           forward. Use tags["_atr_very_low"]/
                           ["_atr_low"] to see thin-volatility rejections.
