...
# new — every field is either already persisted somewhere, or (structure/
# live_tiers) reserved as None until the Phase 1B facts digest ships in
# scanner_live.py's scan loop. Detects nothing, gates nothing, decides
# nothing — same discipline as MarketThesis/MarketIntent. Full design doc:
# world_state_schema.md (v0.2, per chat with Vally's collaborator).
#
...
def build_world_state(state):
    ...
    # ---- setups: live tiers (⚠ Phase 1B) + shadow (already persisted) --
    setups = {
        "live_tiers": state.get("live_tier_digest"),   # ⚠ None until Phase 1B
...
def format_world_state(world_state):
    ...
    if sysobs is None:
        lines.append("Structure: _not available yet — Phase 1B (live facts digest) not built_")
    else:
