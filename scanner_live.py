...
def answer_deferred_reports(deferred_report_cmds, state):
    """
    Answers /thesis, /marketintent, /world, /understand — whichever got
    queued into deferred_report_cmds by check_result_commands() this
    pass. Extracted into its own function (per chat: generalize, don't
    patch each early-return site with slightly different inline code)
    specifically so it can be called from EVERY exit point of
    _scan_once(), not just the one at the bottom of the normal path.

    Bug this fixes: before extraction, a command queued here got
    silently dropped with NO reply at all — not even an error — if the
    scan hit any of _scan_once()'s several early returns first (forex
    weekend closure, data fetch/freshness/sanity failure, an open
    trade, consolidation phase, missing swing points). All of those
    happen AFTER commands are queued but were BEFORE where they used to
    get answered. /understand hit this immediately in testing because
    today's a Sunday and is_forex_weekend() returns True until ~22:00
    UTC — not a bug in /understand itself, a pre-existing gap in every
    deferred command.

    Safe to call on a state that's missing this pass's own
    market_thesis_*/market_intent_*/structure_digest keys (i.e. on an
    early-return path where this pass's own computation never ran) —
    format_market_thesis()/format_market_intent_report()/
    build_world_state() already fall back to whatever was last
    persisted, same as they always have; this just guarantees SOME
    reply reaches Telegram instead of none.
    """
    for _cmd in deferred_report_cmds:
        try:
            if _cmd == "thesis":
                ...
                send_telegram(format_market_understanding(
                    build_market_understanding(build_world_state(state))))
        except Exception as e:
...
def _scan_once():
    ...
    if is_forex_weekend(now_utc):
        ...
        answer_deferred_reports(deferred_report_cmds, state)
        save_stats(stats)
        save_state(state)
        return
