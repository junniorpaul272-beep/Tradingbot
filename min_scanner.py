...
# HFIS — ADDED (2026-09-15, per chat, friend's "one owner of Telegram
# prose" rule). Qualified import, same discipline as scanner_live.py's
# `import hfis` — used only by _close_leg_obs()'s resolution follow-up
# below, so that push goes through HFIS's register instead of a raw
# f-string (see narrate_resolution()'s own docstring in hfis.py for
# the full story on why this changed).
import hfis
...
def _close_leg_obs(obs_state, open_rec, fate, now_utc, add_to_closed=False):
    ...
    try:
        ...
        if matched:
            ...
            # ROUTED THROUGH HFIS (2026-09-15, per chat — friend's audit
            # caught this exact push as a second, un-narrated producer of
            # market-facing prose: a raw f-string bypassing hfis.py
            # entirely, in a flat register nothing else in the bot uses.
            # narrate_resolution() never raises; on any internal error it
            # returns None and this falls back to the original f-string
            # so a bug in HFIS can never silently drop a resolution push.
            push_text = hfis.narrate_resolution(headlines, direction, fate, open_rec['bars_open'])
            if not push_text:
                push_text = (
                    f"Earlier: {headlines}\n\n"
                    f"That {direction.lower()} leg has now resolved as *{fate}* "
                    f"after {open_rec['bars_open']} bars."
                )
