...
MARKOV_STATE_FILE = "markov_transitions.json"

# ---- FORWARD OBSERVATION + VALIDATION ENGINE --------------------------------
LEG_OBS_STATE_FILE = "leg_obs_state.json"   # current open per-leg record
LEG_OBS_LOG_FILE   = "leg_obs_log.jsonl"    # permanent resolved records (append-only)
LEG_OBS_CLOSED_MAX = 20   # rolling cap on closed_ids set — prevents reopen-after-invalidation bug

CALIBRATION_LOG_FILE = "calibration_log.jsonl"   # reserved for future per-event logging
...
FAILURE_CASE_LOG_FILE = "failure_case_log.jsonl"   # permanent, append-only
...
def save_markov_data(markov_data):
    try:
        with open(MARKOV_STATE_FILE, "w") as f:
...
def update_pending_shadow_setups(shadow_state, shadow_stats, df_5m, now_utc):
    ...
    for setup in shadow_state["pending"]:
        ...
        if hit_sl and setup["max_r_reached"] == 0:
            ...
            try:
                _case = open_failure_case(setup, "LOSS", -1.0, now_utc)
        ...
        if hit_sl or timed_out:
            ...
            r_final = float(r) if r > 0 else -1.0
            _append_shadow_trade_log(setup, outcome_label, r_final, now_utc)
            try:
                _case = open_failure_case(setup, outcome_label, r_final, now_utc)
                if _case is not None:
                    send_telegram(format_case_note(_case))
            except Exception as e:
                print("[FAILURE BUREAU ERROR] " + str(e))
...
def save_leg_obs_state(obs_state):
    try:
        with open(LEG_OBS_STATE_FILE, "w") as f:
...
def _update_zone_touches(rec, facts, macro_bias):
    ...
    if rec.get("tier2_touched_bar") is None and facts.price_in_zone(facts.fib_zone()):
...
def run_leg_observation(facts, ctx, state, macro_bias, bias_stale, now_utc, obs_state):
    ...
    current_leg_id = compute_leg_id(macro_bias, swing_high, swing_low)
    ...
    if open_rec is not None:
        ...
        if _same_leg(open_leg_id, current_leg_id):
            # Same leg — always increment bars first
            open_rec["bars_open"] += 1
    ...
    try:
        ...
        if n_cal < CALIBRATION_MIN_N:
            ...
            lines.append(
                ...
                f"(0.00=perfect, 0.25=random baseline)"
            )
        lines.append("")
    except Exception as e:
...
def investigate_failure_case(tier_number, formation_tags, exclude_trade_id=None):
    ...
    # Step 5: rank by raw gap size — largest first. No normalization
    # into a percentage; numeric and categorical gaps are different
    # units and deliberately NOT compared to each other beyond ordering
    # each list independently would over-claim precision, so they're
    # ranked together but the report always shows each gap in its own
    # native unit, never collapsed into one fake common scale.
    ...
    if not comparisons:
        ...
        if top["type"] == "numeric":
            conclusion = (
                f"Largest measured difference: `{top['tag']}` — losers averaged "
                f"`{top['losers_mean']}` vs winners' `{top['winners_mean']}` "
                f"(this trade: `{top['this_value']}`, n={top['winners_n']}W/{top['losers_n']}L). "
                f"Correlational, not causal — worth researching, not acting on alone."
            )
        else:
            conclusion = (
                f"Largest measured difference: `{top['tag']}` — losers were usually "
                ...
                f"(n={top['winners_n']}W/{top['losers_n']}L). "
                f"Correlational, not causal — worth researching, not acting on alone."
            )
