...
from scanner_common import (
    ...
    MIN_CONFIRMATION_WINDOW_SECONDS,
    THESIS_UPDATE_TZ_OFFSET_HOURS, THESIS_UPDATE_HOURS_LOCAL,
    LIVE_TRADE_LOG_FILE, TIER_PRIORITY, TIER_NUMBER,
    JOURNAL_MAX_ENTRIES,
    load_bias_ab_log, save_bias_ab_log,
    load_bias_minleg_log, save_bias_minleg_log,
    load_markov_data, save_markov_data,
    HTF_BIAS_MIN_BARS, ATR_WARN_PIPS, SWING_LOOKBACK_15, ATR_MIN_PIPS,
    SCAN_LOCK_FILE, SCAN_LOCK_MAX_AGE_SEC,
    atr_histogram_bucket,
    BRAIN_LOG_FILE, BRIEFING_LOG_FILE,
    # ADDED 2026-09-18, per chat — Research Lab groundwork (scan_id +
    # structure_digest_log + zone_log). See these constants'/functions'
    # own docstrings in scanner_common.py for the full story.
    SCAN_INDEX_LOG_FILE, STRUCTURE_DIGEST_LOG_FILE, ZONE_LOG_FILE, THESIS_CONTEXT_LOG_FILE,
    next_scan_id, append_permanent_jsonl,
    # ADDED (2026-08-27, per chat — Market Event lifecycle): cap for the
    # pending-events queue, written here, read/cleared by _close_leg_obs()
    # in min_scanner.py. CORRECTED same day, per audit: this used to be a
    # state.json key — moved to a dedicated file (MARKET_EVENT_PENDING_
    # FILE, see its docstring in scanner_common.py) because state.json is
    # LIVE-owned and min-scan.yml never commits it, so min_scanner.py's
    # writes were being silently discarded every pass.
...
from scanner_observation import (
    ...
    compute_zone_interaction_reset, observe_zone_interaction,
    format_timeline_diagnostics, build_diagnostic_report, new_diagnostic,
    diag_set, _gate_stale_bias, sl_multiplier_for_context,
    build_zone_observations,
)
# Brain (brain.py) — same one-way, read-only direction as the
# min_scanner import below: takes a WorldState dict and returns a
# Market Understanding, imports nothing else, runs zero experiments,
# touches zero API.
# CHANGED (2026-08-26, per chat): /understand is now conversational
# (format_understanding_narrative()) and is the ONLY brain.py-backed
# Telegram command left — /briefing was removed (see check_result_
# commands()) and format_market_understanding()'s dev/audit dump is no
# longer wired to any command (kept in brain.py, unused, in case a
# dev/audit surface is wanted again later). build_market_briefing()/
# format_market_briefing() are imported here but currently unused for
# the same reason — left available rather than deleted.
from brain import (
    build_market_understanding, format_market_understanding,
    format_understanding_narrative, _plain_clause,
    build_market_briefing, format_market_briefing,
    _refine_next_watch,
    # ADDED (2026-08-27, per audit — Phase 0 authority routing):
    # /thesis and /marketintent now build the same WorldState/
    # understanding/intent_hypothesis objects /understand already built,
    # and pass them into format_market_thesis()/format_market_intent_
    # report() so all three commands render one interpretation. See
    # _answer_deferred_reports()/_maybe_send_daily_thesis_update() below.
    build_market_intent_hypothesis,
    # ADDED (2026-08-27, per chat — Market Event layer, design credited
    # to Vally). See _evaluate_and_push_market_event() below and each
    # function's own docstring in brain.py.
    evaluate_market_event, build_event_snapshot, format_market_event,
)
...
from min_scanner import (
    format_integrity_command,
    load_shadow_stats, load_leg_obs_state,
    # ADDED (2026-08-29, per chat — live condition capture at resolution
    # time, so future trades don't need retroactive reconstruction).
    peek_conditions_at_resolution,
    # ADDED (2026-08-27, per audit — see MARKET_EVENT_PENDING_FILE's
    # docstring in scanner_common.py): replaces the old state["market_
    # event_pending"] key, which was silently discarded every MIN pass.
    load_market_event_pending, save_market_event_pending,
...
# WorldState moved out of min_scanner.py — per chat (physiology work),
# living there wrongly implied MIN was authoritative over the organism's
# shared state; MIN never called these itself. Pure relocation.
from worldstate import build_world_state, format_world_state
# MIL — per chat (physiology work). Imported as a module (not
# "from mil import ...") deliberately: every call below stays qualified
# (mil.reconcile, mil.ingest, ...) so it's never ambiguous at a glance
# which organ's function is being called at either insertion point.
import mil
# HFIS — per chat. Same qualified-import discipline as mil above.
# A local procedural narrator (no network, no cost) — see hfis.py's
# module docstring. narrate() returning None is a belt-and-suspenders
# case now, not an expected failure mode; every call site still falls
# back to format_market_event()'s template text regardless.
import hfis
# Story Interpreter — ADDED (2026-09-15, per chat, friend's
# architectural review). Sits between evaluate_market_event() above and
# hfis.narrate()/format_market_event() below: classifies each already-
# significant event's impact on the CURRENT story (support/challenge/
# break/opportunity/surprise vs. background/ignore) and builds the
# "what objectively happened" clause hfis.narrate() weaves in ahead of
# its own composed conclusion. Qualified import, same discipline as mil/
# hfis — see market_story.py's own module docstring for the full design.
import market_story
# The Library — ADDED (2026-09-16, per chat, the architecture redesign).
# Pure, deterministic text-rendering functions over the story dict
# market_story.build_market_story() produces. This file (scanner_live.py)
# calls these directly and assembles the overview push itself — the
# "Composition" layer from the redesign, whose only job is choosing
# ORDER, never inventing text. See narration_library.py's own module
# docstring for the full rationale.
import narration_library

# /overview (per chat) — local to scanner_live.py, not scanner_common.py,
# since this is a purely LIVE-owned, LIVE-only concept (unlike
# MARKET_EVENT_PENDING_MAX, which is shared with min_scanner.py's leg
# resolution follow-up and genuinely needs to live in the shared file).
OVERVIEW_PENDING_EVENTS_MAX = 20
...
# Coverage — per chat, 2026-08-23: one line per _scan_once() invocation,
# written UNCONDITIONALLY at the very top of the function, before any
# early-return path (weekend closure, data fetch failure, etc.) can skip
# it. This is the ONLY way to answer "did the bot observe nothing, or
# did it not observe" after the fact — every other signal (rejected-live
# queue entries, trade records) only gets written on SOME exit paths,
# not all of them, so none of them can serve as a scan-coverage proxy.
...
def _log_structure_digest(digest, scan_id, scanned_at_iso):
    """Appends one structure_digest_log.jsonl row for this scan. Thin
    wrapper around append_permanent_jsonl() (scanner_common.py) — exists
    mainly so the call site above reads as one named step rather than an
    inline dict-build, matching _log_new_zones()'s shape immediately
    below."""
    record = dict(digest)
    record["scan_id"] = scan_id
    record["scanned_at"] = scanned_at_iso
    append_permanent_jsonl(STRUCTURE_DIGEST_LOG_FILE, record)


def _log_thesis_context(thesis, phase_result, scan_id, scanned_at_iso):
    """Appends one thesis_context_log.jsonl row — added 2026-09-20, per
    chat (friend's "storing context" proposal). Joined to
    structure_digest_log.jsonl by scan_id, not merged into the same row
    — see THESIS_CONTEXT_LOG_FILE's comment in scanner_common.py for why
    (thesis is computed later in the scan than the structure digest, and
    the digest's unconditional-every-scan coverage guarantee would break
    if its write were moved later to wait for thesis fields).

    Every field here is something build_market_thesis()/compute_market_
    phase() already computed this scan for the live /thesis narrative —
    no new detection, same "capture what's already being thrown away"
    fix structure_digest_log.jsonl itself applied on 2026-09-18. Called
    only from inside the same try block that computes `thesis` — if
    thesis computation failed or was skipped this scan, no row is
    written, same as leaving a genuinely unknown value as absent rather
    than fabricating one.
    """
    record = {
        "scan_id": scan_id,
        "scanned_at": scanned_at_iso,
        "phase": thesis.current_state,
        ...
        "trend_health": thesis.trend_health,
        "failure_risk": thesis.failure_risk,
        "confidence": thesis.confidence,
        "mtf_15m": thesis.mtf_15m,
        "mtf_5m": thesis.mtf_5m,
        "campaign": thesis.campaign,
    }
    append_permanent_jsonl(THESIS_CONTEXT_LOG_FILE, record)


def _log_new_zones(state, facts, scan_id, scanned_at_iso):
    """
    Appends one permanent zone_log.jsonl row for each zone (FVG/order
    block) that is NEW this scan — i.e. its identity_key has never been
    logged before — and never again while it remains active. Uses
    state["zone_log_seen"] ({identity_key: first_seen_scan_id}) as the
    first-seen ledger, mutated in place (caller owns save_state()).

    Pruned to just the identity_keys still active THIS scan, every
    pass — so it can't grow unboundedly. This is safe (not lossy)
    because build_zone_observations()'s identity_keys are absolute-
    timestamp-based and zone formation is monotonic: a key dropping off
    the active list (aged out past FVG_MAX_AGE_CANDLES, or its leg's
    BOS no longer current) refers to the exact same 15M candle either
    way and can never legitimately reappear as a "new" zone — so
    forgetting it the moment it's gone loses nothing.
    """
    zones = build_zone_observations(facts)
    seen = state.get("zone_log_seen", {})
    still_active = {}
    for zone in zones:
        key = zone["identity_key"]
        still_active[key] = seen.get(key, scan_id)
        if key not in seen:
            record = dict(zone)
            record["scan_id"] = scan_id
            record["scanned_at"] = scanned_at_iso
            append_permanent_jsonl(ZONE_LOG_FILE, record)
    state["zone_log_seen"] = still_active
...
def append_ohlc_history(df_5m, df_15m, df_1h, state):
    """
    Permanently appends every NEW closed candle from this scan's
    fetch_ohlc() calls to an append-only per-timeframe log (never
    truncated or rewritten — same discipline as _append_brain_log
    below). Added 2026-09-02, per chat: candle_cache.json above is a
    ROLLING ~200-bar snapshot, fully OVERWRITTEN every scan — it exists
    purely for MIN's ephemeral-runner handoff and was never a permanent
    OHLC archive, despite being committed to git every 5 minutes.
    Nothing in this codebase could answer "what was the 1H ATR three
    weeks ago" before this — confirmed the hard way trying (and
    failing) to backtest the macro-bias leg-significance gate against
    real history instead of a synthetic series.

    Dedup strategy: a cursor (ISO timestamp of the last candle already
    logged, per timeframe) is read from `state` and returned as
    `updates` for the caller to apply_state_updates() — same additive-
    state pattern as macro_atr_1h elsewhere, rather than re-reading the
    tail of a growing jsonl file every scan to find where we left off.
    Only candles strictly newer than the cursor are appended, so the
    ~200-row overlap between consecutive fetch_ohlc() calls doesn't get
    re-written 200x over. A closed candle is immutable once fetched
    (Twelve Data only returns completed bars for a past interval), so
    plain append is correct — no update-in-place case exists.

    Growth estimate at outputsize=200/5-minute cron: ~1 new row/5min
    (5min), ~1/15min (15min), ~1/hour (1h) — roughly 300-400 new JSONL
    lines/day total across all three files combined. Small enough to
    commit to git indefinitely without the repo-bloat concern a
    full-snapshot-every-scan file like candle_cache.json would have if
    it were structured this way instead.
    """
    updates = {}
    for label, df in (("5min", df_5m), ("15min", df_15m), ("1h", df_1h)):
        if df is None or df.empty:
            continue
        cursor_key = f"ohlc_history_last_ts_{label}"
        last_ts = state.get(cursor_key)
        new_rows = df if last_ts is None else df[df.index > pd.Timestamp(last_ts)]
        if new_rows.empty:
            continue
        try:
            with open(OHLC_HISTORY_FILES[label], "a") as f:
            ...
            updates[cursor_key] = new_rows.index[-1].isoformat()
        except Exception as e:
            print(f"[OHLC HISTORY WRITE ERROR] {label}: {e}")
    return updates
...
def evaluate_rule_of_law(facts, ctx, state, stats, now_utc):
    ...
    # Zone interaction observation (Phase 2, per chat 2026-09-11) — same
    # reset-on-leg-change discipline as the Timeline above. Pure
    # observation: does not affect activated/fired for any tier, does not
    # feed risk or conviction. See compute_zone_interaction_reset() and
    # observe_zone_interaction() in scanner_observation.py for what this
    # tracks and why, and their docstrings for the two bugs already caught
    # and fixed by testing before this shipped.
    apply_state_updates(state, compute_zone_interaction_reset(state, leg_id))
    interaction_now = state.get("zone_interaction", {})
    interaction_updates = observe_zone_interaction(facts, interaction_now, now_utc)
    if interaction_updates:
        merged_interaction = dict(interaction_now)
        merged_interaction.update(interaction_updates)
        apply_state_updates(state, {"zone_interaction": merged_interaction})
...
def _append_live_trade_log(active, outcome, exit_price, r_achieved, closed_at, close_method):
    ...
    record = {
        ...
        # LIVE CONDITION CAPTURE (2026-08-29, per chat) — same fix,
        # same reasoning as _append_shadow_trade_log()'s identical
        # field in min_scanner.py; see peek_conditions_at_resolution()'s
        # own docstring there for what this is and still isn't.
...
def _append_brain_log(understanding, world_state):
    """
    Permanently appends ONE Brain interpretation to BRAIN_LOG_FILE
    (append mode — never truncated or rewritten). Added 2026-08-25, per
    chat: brain.py itself stays fully ephemeral/stateless on purpose (see
    its own module docstring) — but that meant every /understand reply
    silently overwrote the only record of what Brain said, unlike every
    other subsystem here, which all get a permanent append-only log.
    Written from the caller (this file), never from brain.py itself, so
    Brain's zero-import isolation is completely untouched by this.

    world_state_generated_at is logged alongside understanding so a
    later reviewer can tell exactly which WorldState snapshot (and
    therefore which coherence/freshness state — see world_state["coherence"])
    this interpretation was built from, without reconstructing it.
    """
    record = {
        "logged_at": datetime.now(timezone.utc).isoformat(),
        "world_state_generated_at": (world_state or {}).get("generated_at"),
    ...
    try:
        ...
        print("[BRAIN LOG ERROR] " + str(e))


def _append_briefing_log(briefing):
    """
    UNUSED as of 2026-08-26 (per chat) — /briefing was removed as a
    Telegram command (see the removed-command note in
    check_result_commands() where /briefing used to be routed). Left
    defined, not deleted, in case build_market_briefing()'s Thesis+
    Logic+Intent assembly is wanted again later behind a differently-
    designed surface; nothing currently calls this.

    Permanently appends ONE build_market_briefing() output to
    BRIEFING_LOG_FILE (append mode — never truncated or rewritten).
    Same reasoning as _append_brain_log() above, but for the briefing
    object specifically: briefing["reasoning_snapshot"]/["intent_snapshot"]
    already carry everything build_market_briefing()'s own docstring
    says is needed to answer a later "why did you say X" question — but
    only if something keeps the object around past this one Telegram
    reply. This is that persistence, written from the caller (this
    file), never from brain.py itself, so brain.py's zero-import
    isolation stays untouched.

    briefing["generated_at"] is already inside the object (Brain sets
    it), so it isn't duplicated at the top level here the way
    world_state_generated_at is in _append_brain_log() — there's only
    one timestamp to track for this record, not two objects with
    potentially different snapshot times.
    """
    record = {
        "logged_at": datetime.now(timezone.utc).isoformat(),
        "briefing": briefing,
    ...
    try:
        with open(BRIEFING_LOG_FILE, "a") as f:
            f.write(json.dumps(record, default=_json_default) + "\n")
    except Exception as e:
        print("[BRIEFING LOG ERROR] " + str(e))
...
def format_trade_query_response(stats, current_price, now_utc):
    ...
    if pending:
        ...
        return (
            ...
            "❓ Reply /taken or /skip — no reply within ~9 minutes = assumed taken."
        )
...
def manage_active_trade(stats, state, df_5m, now_utc):
    ...
    if outcome is not None:
        ...
        # GROUP MIRROR — factual result only (per chat: "when a tracked
        # signal resolves, report the result factually"). No signal-time
        # score (internal/dev metric, not beginner-facing).
        if GROUP_CHAT_ID:
            try:
                send_telegram(
                    f"{icon} *Trade closed — {outcome}*\n\n"
                    f"Entry: `{active['entry']:.5f}` → Exit: `{exit_price:.5f}`\n"
                    f"P/L: `{pl_pips:+.1f} pips`",
                    chat_id=GROUP_CHAT_ID,
                )
            except Exception as e:
                print("[GROUP TRADE CLOSE ERROR] " + str(e))
...
HELP_TEXT = (
    ...
    "`/thesis` — Market Thesis Engine: current-scan narrative, in plain language\n"
    "`/marketintent` — what I'm watching for right now, and why\n"
    "`/overview` — combined Market Understanding: what's changed since the last update, plus current conditions\n\n"
    "*Bank & accounts*\n"
    ...
    "`/understand` — Brain's interpretation of /world, in plain language\n"
    "`/integrity` — integrity/consistency diagnostics (read-only, never gates)\n\n"
    "`/help` — show this list"
)
...
# =========================================================================
# GROUP SURFACE (per chat, 2026-09-21 — group gateway work). Read-only
# observation surface for a beginner (Valentine's friend), separate from
# the personal control chat above. Per the explicit design constraint:
# this is NOT a second brain. Every function below only assembles
# strings from what build_world_state()/build_market_understanding()/
# build_market_briefing()/build_market_intent_hypothesis() ALREADY
# compute for the personal side — no new detection, no independent
# reading of raw state, no raw internal/dev metrics (calibration
# counts, resolved-after-N-bars stats, experiment IDs, etc.). If a
# value isn't already a human-facing sentence/field on one of those
# four objects, it does not belong in a group-facing string.
#
# GROUP_COMMANDS is the sole allowlist — check_result_commands() below
# dispatches these BEFORE the large personal elif chain, then `continue`s
# for a group-chat update either way (recognized or not), so nothing
# from the personal command set (`/taken`, `/skip`, `/shadow`,
# `/calibration`, etc.) is ever reachable from the group.
# =========================================================================
GROUP_COMMANDS = ("/status", "/watch", "/report", "/help")


def format_group_help():
    # CANONICAL allowed-command list for the group surface — this text
    # IS the spec for what the group can do; the dispatch branch below
    # (search `GROUP_CHAT_ID and chat_id == GROUP_CHAT_ID`) enforces it
    # in code, and set_group_commands.py at the repo root scopes
    # Telegram's own "/" menu to match it (a separate, Telegram-side
    # step — see that file's docstring and GROUP_CHAT_ID's comment in
    # scanner_common.py for the full picture). If this list ever
    # changes, update all three by hand — nothing keeps them in sync
    # automatically.
    return (
        "🤖 *ValleyBot — Group*\n\n"
        "`/status` — what's happening right now\n"
        "`/watch` — what the bot is waiting to see\n"
        "`/report` — the current hourly report\n"
        "`/help` — this list\n\n"
        "This is a read-only observation feed. Signals and hourly "
        "reports post here on their own — nothing sent in this chat "
        "changes what the bot does."
    )


def _group_status_text(state):
    """/status — 'what is the market doing right now'. Reuses Brain's
    own current_condition + format_understanding_narrative() (the exact
    plain-language sentence /understand already sends personally) —
    zero new wording, zero jargon dump."""
    ws = build_world_state(state)
    understanding = build_market_understanding(ws)
    ...
    narrative = format_understanding_narrative(understanding) if understanding else \
...
def _group_watch_text(state):
    """/watch — 'what is the bot looking for'. Reuses build_market_intent_
    hypothesis()'s locations/confirmations — the same already-narrated
    `sentence` fields format_market_briefing()'s Areas of Interest /
    What would confirm it sections use personally."""
    ws = build_world_state(state)
    intent_hypothesis = build_market_intent_hypothesis(ws)
    locations     = (intent_hypothesis or {}).get("locations") or []
    confirmations = (intent_hypothesis or {}).get("confirmations") or []
...
def _group_report_text(state, now_utc):
    """Hourly report body — used both by the scheduled wall-clock push
    (_maybe_send_group_hourly_report) and the on-demand /report pull, so
    the two can never disagree. Per chat, 2026-09-21 (correcting an
    earlier 'only send on change' idea): this ALWAYS produces content —
    a heartbeat that goes silent on a quiet market is indistinguishable
    from a dead bot from the outside. 'Nothing changed' is said
    explicitly, never represented by silence.

    REVISED (2026-09-22, per chat — friend's "teach a beginner like a
    mentor" idea, plus the gap the user caught in the friend's own
    mock-up: it showed a primary thesis and a counter-thesis but never
    said what would actually prove the whole thesis WRONG). This used to
    read build_market_briefing()['thesis']['current_state'] alone — the
    OLD MarketThesis engine's one-line summary, the exact "database
    heartbeat" the friend was complaining about. It now reads the same
    market_assessment object /status and /understand already narrate
    (build_market_understanding(ws)['market_assessment'] — primary_
    thesis/primary_threat/confirmation_needed/alternative/next_watch,
    Brain's own hand-written synthesis) and lays it out as separate,
    labeled sections instead of one paragraph — this function only
    chooses ORDER/LABELS/OMISSION among sentences Brain already wrote,
    same "no new wording" discipline as format_understanding_narrative()
    itself (see that function's docstring). Two more already-computed,
    already-narrated fields are added that neither /status nor the old
    report ever surfaced here:
      - market_thesis_evidence_prose  -> "Why" (the same evidence prose
        /thesis already narrates via format_market_thesis() in
        min_scanner.py — approved for narration, unlike thesis_
        weaknesses(_prose), which stays banned everywhere per brain.py's
        own no-narration rule for that field)
      - market_thesis_invalidation    -> "What would invalidate this" —
        the actual failure condition. Distinct from primary_threat:
        primary_threat is the immediate opposing force baked into THIS
        scan's read; invalidation is the structural line/condition that
        would break the thesis outright. The friend's mock-up only had
        room for the former; this is the piece it was missing.
    Change-detection now compares market_assessment['state'] (the same
    canonical short situation key market_story.py/build_event_snapshot()
    already use for their own dedup) instead of diffing the full prose,
    which could reword itself between scans without anything structural
    actually changing.

    'System status: data received and processed' is dropped from the
    body entirely (per chat) — a healthy scan doesn't need to say so
    every hour, and an UNhealthy one already gets its own interrupt via
    _maybe_send_group_system_alert(), a separate message class for
    exactly that.

    Mutates state (group_report_last_state_key) — caller is responsible
    for save_state() after calling this, same convention as every other
    state-mutating helper in this file."""
    ws = build_world_state(state)
    understanding = build_market_understanding(ws)
    ...
    if not ma:
        bias = (understanding.get("current_condition") or {}).get("macro_bias") if understanding else None
        ...
        state["group_report_last_state_key"] = None
        return "\n".join(lines)
    ...
    state["group_report_last_state_key"] = current_state_key
    ...
    if owner and owner.get("status") == "FIRED" and owner.get("tier"):
        ...
        tier_txt = f"Tier {tier_num}" if tier_num else owner["tier"]
        lines.append(f"*Signal:* A {tier_txt} setup is currently live — see the last signal alert.")
    else:
        lines.append("*Signal:* No new trade signal at this time.")

    if not changed and prev_state_key is not None:
        lines.append("")
...
_STORY_IMPACT_GROUP_LABEL = {
    # Per chat, 2026-09-22 — friend's "what just changed and why does it
    # matter" idea: story_impact (market_story.classify_events(), already
    # computed for the personal push) tells us exactly this, in a word —
    # this dict is the only new text in the whole function: a beginner-
    # facing phrase per already-decided classification, nothing re-judged.
...
def _group_market_event_text(events_to_push, mechanisms, story, state):
    """Beginner-facing group rendering of a Market Event push — the
    middle tier of the friend's Report -> Event -> Signal teaching flow
    (per chat, 2026-09-22). Sits alongside hfis.narrate()'s personal-chat
    push, never replacing it: same events_to_push / mechanisms / _story
    objects _evaluate_and_push_market_event() already built for that
    push, passed straight through — this function adds no new detection,
    no new classification, and no re-derivation of what happened. Its
    only job is choosing labels/order for a less experienced reader,
    same discipline as _group_report_text()/format_understanding_
    narrative() elsewhere in this file.

    Two sections:
      1. One line per event: icon + headline (evaluate_market_event()'s
         own sentence) + mechanism (build_mechanism()'s "what objectively
         happened" clause, when there is one) + a plain "effect on the
         thesis" phrase keyed off story_impact (see _STORY_IMPACT_GROUP_
         LABEL above) — this is the "what just changed and why does it
         matter" + "does this support/challenge/break the view" pairing
         from the friend's mock-up.
      2. narration_library.deep_read(story) verbatim — the SAME labeled
         block (Primary thesis / Primary threat / Counter thesis /
         Failure condition / What matters next) already used for
         /overview's deep section. Reused as-is rather than re-labeled
         here, so the group never sees different wording for the same
         fact than the personal deep-read would show — this is also
         where the friend's missing "what would prove this wrong" piece
         (Failure condition) lands in the Event tier, not just the
         Report tier.

    Returns None if there is nothing to say (events_to_push empty) —
    caller should skip sending rather than push an empty message.
    """
    ...
    deep = narration_library.deep_read(story)
    if deep:
        lines.append(deep)
        lines.append("")
...
def _maybe_send_group_hourly_report(state, now_utc):
    """Group hourly heartbeat — wall-clock (one per calendar hour UTC).
    No-op (returns False) if GROUP_CHAT_ID isn't configured, so this is a
    dead call, not a crash, on any deploy that hasn't set it up yet.

    AUDIT FIX (2026-09-22): hour_key used to be persisted BEFORE the send
    was attempted ("save-before-send"), on the theory that this prevents
    a duplicate if the process crashes between send and save. In practice
    it meant any failed send (wrong/misconfigured chat_id, a transient
    Telegram API error, etc.) still permanently marked that hour as
    delivered — the report would then silently skip every retry for the
    rest of the hour with zero visible symptom besides the missing
    message. That false-negative is far more likely and far more costly
    than the crash-window race it was guarding against (a process dying
    in the single tick between a successful send and this save call), so
    hour_key is now only persisted once send_telegram() actually confirms
    delivery. A failed attempt is retried on the next scan cycle within
    the same hour instead of going quiet."""
    if not GROUP_CHAT_ID:
        return False
    hour_key = now_utc.strftime("%Y-%m-%dT%H")
    if state.get("group_report_last_sent_hour_key") == hour_key:
        return False
    try:
        text = _group_report_text(state, now_utc)  # mutates state in-place
        delivered = send_telegram(text, chat_id=GROUP_CHAT_ID)
    except Exception as e:
        print("[GROUP HOURLY REPORT ERROR] " + str(e))
        delivered = False
    if delivered:
        state["group_report_last_sent_hour_key"] = hour_key
    save_state(state)
    return delivered


def _maybe_send_group_system_alert(state, now_utc, reason):
    """Group-facing SYSTEM ALERT — a separate message class from a normal
    report, so a failed/stale scan never quietly looks like 'no signal,
    quiet market' to a beginner. Pass a non-empty `reason` string on a
    failure (sends immediately on the first occurrence, then at most
    once/hour while it persists, so an extended outage doesn't spam the
    group every 5-minute cron tick); pass reason=None once data is
    confirmed healthy again to send a one-time recovery note — a no-op
    if no alert was active."""
    if not GROUP_CHAT_ID:
        return
    was_alerting = state.get("group_system_alert_active", False)
    if reason:
        hour_key = now_utc.strftime("%Y-%m-%dT%H")
        if was_alerting and state.get("group_system_alert_last_hour_key") == hour_key:
            return
        state["group_system_alert_active"] = True
        state["group_system_alert_last_hour_key"] = hour_key
        save_state(state)
        try:
            send_telegram(
                "⚠️ *ValleyBot System Alert*\n\n"
                f"{reason}\n\n"
                "No new signals or reports will be posted here until "
                "this clears.",
                chat_id=GROUP_CHAT_ID,
            )
        except Exception as e:
            print("[GROUP ALERT ERROR] " + str(e))
    elif was_alerting:
        state["group_system_alert_active"] = False
        save_state(state)
        try:
            send_telegram(
                "✅ *ValleyBot* — data is current again. Normal reporting resumed.",
                chat_id=GROUP_CHAT_ID,
            )
        except Exception as e:
            print("[GROUP ALERT ERROR] " + str(e))
...
def check_result_commands(stats, state=None):
    ...
    try:
        ...
        # AUDIT FIX (2026-09-08): this used to return completely silently on
        # ANY failure here — a network blip, a bad/non-JSON Telegram response,
        # anything — with no print, no counter, nothing. Not fatal on its
        # own (offset never advances, so nothing already sent is lost —
        # Telegram's getUpdates queue just gets picked up whole next cycle),
        # but a run of these with zero trace is indistinguishable from "the
        # bot is broken" from the outside. Now at least visible in the
        # console/Action log; see check_result_commands() docstring and its
        # own call site in _scan_once() for the escalation one level up,
        # which is what actually alerts on Telegram (this function can't
        # reliably alert about its own inability to reach Telegram).
        print(f"[TELEGRAM POLL ERROR] getUpdates failed: {type(e).__name__}: {e}")
        return stats, deferred_report_cmds

    # ROOT-CAUSE FIX (2026-09-22, per audit — "Telegram commands have been
    # dead for 3 months, no trace anywhere"): this used to be one combined
    # check — `if not resp.get("ok") or not resp.get("result"): return`.
    # That collapsed two completely different situations into one silent
    # early-return with ZERO print, ZERO Telegram alert, nothing:
    #   1. resp["ok"] is True and resp["result"] is just empty — the
    #      normal, expected case 99% of the time (no new messages since
    #      last poll). Silence here is correct.
    #   2. resp["ok"] is False — Telegram REJECTED the request. This is
    #      the exact failure a leftover/duplicate webhook registration
    #      causes: getUpdates and setWebhook are mutually exclusive, so if
    #      a webhook is (or ever becomes) registered on this bot token,
    #      every single getUpdates call returns HTTP 409 with
    #      {"ok": false, "description": "Conflict: can't use getUpdates
    #      method while webhook is active..."} — valid JSON, so it never
    #      hit the getUpdates-network-failure except block below (that one
    #      already got an AUDIT FIX on 2026-09-08 and prints+alerts
    #      correctly). This "ok": false branch had NO equivalent fix. It
    #      is indistinguishable from case 1 above from the console, and
    #      the offset never advances either way — so it fails the exact
    #      same way, forever, every 5 minutes, with nothing anywhere
    #      (console or Telegram) ever showing it. This is almost certainly
    #      the actual mechanism behind "commands stopped working, no error,
    #      no warning" persisting for months: a webhook conflict (or any
    #      other token/auth/API-level rejection) has been invisible by
    #      construction. Split the two cases and escalate #2 the same way
    #      every other command-adjacent failure in this file already is.
    if not resp.get("ok"):
        error_desc = resp.get("description", "no description in response")
        print(f"[TELEGRAM POLL ERROR] getUpdates rejected: {error_desc}")
        is_webhook_conflict = "webhook" in error_desc.lower()
        try:
            send_telegram(
                "\u26a0\ufe0f *TELEGRAM COMMANDS ARE NOT WORKING*\n\n"
                f"getUpdates was rejected by Telegram: `{error_desc}`\n\n"
                + (
                    "This is almost always caused by a webhook still being "
                    "registered on this bot token — getUpdates and a webhook "
                    "cannot both be active. Run:\n"
                    "`curl https://api.telegram.org/bot<TOKEN>/getWebhookInfo`\n"
                    "and if `url` is non-empty, clear it with:\n"
                    "`curl https://api.telegram.org/bot<TOKEN>/deleteWebhook`\n\n"
                    if is_webhook_conflict else
                    "Check the bot token/permissions — this is an API-level "
                    "rejection, not a network blip.\n\n"
                )
                + "Every /win, /loss, /undo, /shadow, etc. sent while this "
                "persists gets silently ignored with no reply, and this "
                "alert itself will repeat every scan until fixed."
            )
        except Exception:
            pass
        return stats, deferred_report_cmds
    ...
    for update in resp["result"]:
        ...
        cmd = "?"  # defined up front so the except block below can always
                   # reference it, even if the crash happens before command
                   # parsing gets a chance to run (see STRUCTURAL FIX note).

        # STRUCTURAL FIX (2026-09-10, per chat — "messages just collapse...
        # it just doesn't acknowledge them... basically almost everyone
        # I've used... a much general issue"). This used to advance
        # last_update_id HERE, unconditionally, before this update had been
        # processed at all — before the chat_id filter, before text
        # extraction, before the command even ran. Everything from here
        # down to the elif chain's own per-command try/except (now merged
        # into the one below) sat in that gap, meaning ANY future change to
        # this preamble — a new field being read, a stricter parse, exactly
        # the kind of thing that gets added over sessions as commands grow
        # — could throw here and permanently drop that update: Telegram
        # would never redeliver it (offset already past it), but its
        # command never actually ran. That's the general, structural
        # version of every specific "this one command doesn't acknowledge"
        # bug that's been chased and patched command-by-command across
        # multiple sessions — the individual patches (the /shadow crash
        # alert, the unrecognized-command reply, etc.) only ever covered
        # what happens to be inside the try below, never this preamble, and
        # never protected against a NEW bug appearing in either place going
        # forward.
        #
        # Fixed at the root instead of finding the one specific line that
        # broke this time: the ENTIRE per-update body — preamble included —
        # now runs inside one try, and last_update_id only advances in the
        # finally at the bottom, after a full processing attempt (success
        # OR a caught-and-Telegram-reported failure) either way. An update
        # is only ever marked consumed once something has actually been
        # attempted on it — never before, regardless of what that
        # something is or how it fails.
        try:
            ...
            if chat_id == TELEGRAM_CHAT_ID:
                surface = "personal"
            elif GROUP_CHAT_ID and chat_id == GROUP_CHAT_ID:
                surface = "group"
            else:
                continue
            ...
            # ── GROUP SURFACE — dispatched here, separately from the
            # personal elif chain below, and ALWAYS `continue`s (whether
            # or not the command was recognized) so nothing in the huge
            # personal chain (/taken, /skip, /shadow, /calibration, ...)
            # is ever reachable from the group chat_id. See GROUP_COMMANDS
            # and its section docstring above check_result_commands().
            if surface == "group":
                cmd_bare = cmd.split("@")[0]  # strip a possible @BotName suffix
                try:
                    if cmd_bare == "/status":
                        send_telegram(_group_status_text(state), chat_id=GROUP_CHAT_ID)
                    elif cmd_bare == "/watch":
                        send_telegram(_group_watch_text(state), chat_id=GROUP_CHAT_ID)
                    elif cmd_bare == "/report":
                        now_for_report = datetime.now(timezone.utc)
                        text = _group_report_text(state, now_for_report)
                        save_state(state)  # persists group_report_last_state_key
                        send_telegram(text, chat_id=GROUP_CHAT_ID)
                    elif cmd_bare in ("/help", "/start"):
                        send_telegram(format_group_help(), chat_id=GROUP_CHAT_ID)
                    # any other command from the group is silently ignored —
                    # no error text, no hint at commands that don't exist here.
                except Exception as e:
                    print("[GROUP COMMAND ERROR] " + type(e).__name__ + ": " + str(e))
                continue
            ...
            elif cmd in ("/overview", "overview"):
                # Deferred, same reasoning as /thesis — the combined
                # Market Understanding overview (per chat) needs this
                # pass's mil_understanding/thesis/intent finalized, not
                # last cycle's.
                deferred_report_cmds.append("overview")

            elif cmd in ("/marketintent", "marketintent"):
                # Deferred — see docstring.
                deferred_report_cmds.append("marketintent")

            elif cmd in ("/world", "world"):
                # Deferred — same reasoning as /thesis: WorldState reads
                # this pass's own market_thesis_*/market_intent_* keys,
                # which aren't finalized until later in _scan_once().
                deferred_report_cmds.append("world")

            elif cmd in ("/understand", "understand"):
                # Deferred — Brain reads a WorldState dict, which itself
                # isn't finalized until later in _scan_once() (same
                # reasoning as /world above).
                # CHANGED (2026-08-26, per chat): /understand sends the
                # human-facing narrative (format_understanding_
                # narrative()) — the dev/audit field dump
                # (format_market_understanding()) is no longer wired to
                # any command. Per chat: only /understand, /thesis,
                # /marketintent remain as market-explanatory commands;
                # /understandraw and /briefing (dev/audit surfaces) are
                # removed rather than kept as a parallel raw view — see
                # the removed-command note where /briefing used to be
                # routed, just below /integrity.
                deferred_report_cmds.append("understand")

            elif cmd in ("/integrity", "integrity"):
                # Deferred — same reasoning as /world/thesis: checks
                # THIS pass's market_thesis_*/market_intent_* keys, which
                # aren't finalized until later in _scan_once(). Read-only
                # diagnostic surface — see scanner_integrity.py's own
                # module docstring. Never gates anything live.
                deferred_report_cmds.append("integrity")

            # REMOVED (2026-08-26, per chat): /briefing and /understandraw.
            # Both were dev/audit field dumps (build_market_briefing()/
            # format_market_briefing() and format_market_understanding()
            # respectively) — screenshotted by the user as exactly the
            # "database -> Telegram" problem the friend's review
            # diagnosed. Per chat: only /understand, /thesis,
            # /marketintent should remain as commands that explain the
            # market to a human; /world and /integrity stay (explicitly
            # plumbing/diagnostic views, not market interpretation).
            # build_market_briefing()/format_market_briefing() are left
            # defined in brain.py (unused) rather than deleted, in case
            # the underlying Thesis+Logic+Intent assembly is wanted again
            # later behind a differently-designed surface — but nothing
            # in this file calls them anymore.
            ...
            try:
                send_telegram(
                    ...
                    if cmd != "?" else
                    f"\u26a0\ufe0f _Failed to process an incoming message before its command "
                    f"could even be parsed: {type(e).__name__}: {e}_\n"
                    "_(Logged to console -- this is a bug, not you. This message will NOT be "
                    "retried — it's being marked as seen either way, same as every other update "
                    "this scan, so one bad message can't jam the queue for everything after it.)_"
                )
            except Exception:
                pass
        finally:
            # See the STRUCTURAL FIX note at the top of this loop — this is
            # the only place last_update_id advances now, and it always
            # runs exactly once per update, after a full processing attempt
            # (clean success, the chat_id-mismatch `continue` above, or the
            # except block just above), never before one.
...
#      live there instead).
# =========================================================================
def _evaluate_and_push_market_event(state, now_utc):
    """
    Market Event layer entry point (per chat, 2026-08-27 — design
    credited to Vally: WorldState -> Brain -> Market Understanding ->
    Event Evaluation -> Significance -> Human Narrative -> Push, with
    the explicit principle that "the push is just another interface to
    the Brain, not another intelligence system"). Runs every scan,
    unconditionally — NOT gated behind a Telegram command, unlike
    /thesis /marketintent /understand. This is what makes it a proactive
    notify rather than a reactive reply.

    Deliberately separate from _maybe_send_daily_thesis_update() above:
    that's a fixed schedule (9am/12pm/3pm) regardless of whether
    anything changed; this fires on a real change, at whatever time it
    happens, and may fire zero or multiple times a day.

    Builds this scan's WorldState/understanding/intent_hypothesis (same
    calls _answer_deferred_reports() already makes for /understand and
    /marketintent — cheap, pure, no new detection), compares against
    the snapshot persisted last scan (market_event_prev_snapshot — a
    NEW state key, see build_event_snapshot()'s own docstring for why
    it does not touch market_thesis_*/market_intent_*), and pushes only
    if evaluate_market_event() decided something crossed the
    significance bar. The snapshot is updated EVERY scan regardless of
    whether an event fired, so the next comparison is always against
    the immediately preceding scan, not the last scan that happened to
    produce an event — this is what keeps a market oscillating near a
    boundary from silently skipping a real change just because an
    earlier oscillation already "used up" the comparison.

    Wrapped in its own try/except, same discipline as every other
    scan-cycle step in this file (see e.g. [MARKET INTENT ERROR] above)
    — a bug here must never be able to take down the rest of the scan.
    """
    try:
        ws_snapshot = build_world_state(state)
        understanding = build_market_understanding(ws_snapshot)
        intent_hypothesis = build_market_intent_hypothesis(ws_snapshot)

        # Read-only peek at leg_obs's own state file — same established
        # pattern already used for Market Story's timeline peek in
        # _answer_deferred_reports()/build_market_thesis()'s call site —
        # NOT run_leg_observation() itself, which only runs from min_
        # scanner.py's run_min_pass(). Used for two independent things
        # below: (1) Tier 3 detection input to evaluate_market_event()
        # (2026-08-27, per chat — closes the Tier 3 gap flagged last
        # pass — Tier 3 has no location to flow through intent_
        # hypothesis, see evaluate_market_event()'s own docstring), and
        # (2) lifecycle tagging further down. One peek, two uses — this
        # can legitimately be up to one pass stale, same caveat the
        # existing timeline peek already carries; fine for both uses.
        _obs_peek = load_leg_obs_state()
        _open_leg = _obs_peek.get("open")

        prev_snapshot = state.get("market_event_prev_snapshot")
        events = evaluate_market_event(
            understanding, intent_hypothesis, prev_snapshot, leg_obs_open=_open_leg,
            mil_understanding=state.get("mil_understanding"))

        # DEBOUNCE (2026-08-27, per chat — same production repetition
        # fix as _summarize_pending_events() in min_scanner.py, but at
        # the source). If this scan's event is identical (category +
        # headline) to the last thing pushed for the currently open leg,
        # suppress it — a genuine back-to-back re-fire of the same
        # transition is noise, not new information. Does NOT suppress
        # real oscillation (A -> B -> A is three distinct pushes, each
        # one real at the time) — only an exact immediate repeat.
        # Reads the file-backed queue (CORRECTED 2026-08-27 per audit —
        # see MARKET_EVENT_PENDING_FILE's docstring in scanner_common.py
        # for why this is no longer a state.json key).
        pending_so_far = load_market_event_pending()
        if events and _open_leg is not None:
            same_leg = [p for p in pending_so_far if p.get("leg_obs_id") == _open_leg.get("id")]
            last_for_leg = same_leg[-1] if same_leg else None
            if (last_for_leg and len(events) == 1
                    and events[0].get("category") == last_for_leg.get("category")
                    and events[0].get("headline") == last_for_leg.get("headline")):
                events = []

        # STORY IMPACT CLASSIFICATION (per chat, 2026-09-15 — friend's
        # architectural review, "story impact" taxonomy). Runs on every
        # event evaluate_market_event() returned, AFTER the debounce
        # above (a suppressed repeat never needs classifying). Does not
        # change whether evaluate_market_event() considers something a
        # candidate — only whether a candidate is worth interrupting the
        # user for. BACKGROUND/IGNORE-classified events are still logged
        # (below) so nothing about them is silently lost — they simply
        # never reach Telegram, the overview queue, or lifecycle tagging.
        classified_events = market_story.classify_events(
            events, understanding=understanding,
            mil_understanding=state.get("mil_understanding"),
            prev_snapshot=prev_snapshot)

        # SITUATION IDENTITY (2026-09-15, per chat — friend's third
        # review: "one event gets one narrative identity... unless
        # something materially changed about that same situation,
        # silence"). Complements the debounce above rather than
        # replacing it: the debounce catches an exact immediate repeat
        # (same category+headline back to back); this catches a "state"
        # event whose underlying situation (market_assessment.state — a
        # canonical string Brain already computes, see market_story.
        # situation_key()'s docstring) is the SAME as the last one
        # actually narrated, even if evaluate_market_event() rendered it
        # with different template wording or via a different code path.
        # Downgrades to BACKGROUND (never IGNORE) — still logged, never
        # silently vanished. Only touches "state" category events; see
        # is_repeat_situation()'s own docstring for why structure/
        # opportunity are out of scope here.
        _last_situation = state.get("last_narrated_situation")
        for ev in classified_events:
            if market_story.is_repeat_situation(ev, understanding, _last_situation):
                ev["story_impact"] = market_story.StoryImpact.BACKGROUND.value

        events_to_push = market_story.push_worthy(classified_events)
        if classified_events and not events_to_push:
            print("[MARKET STORY] filtered as background/ignore, no push: "
                  + str([(e.get("story_impact"), e.get("headline")) for e in classified_events]))

        if events_to_push:
            # Record the situation we're ABOUT to tell the user about,
            # so the next scan's is_repeat_situation() check compares
            # against what was actually said, not just computed.
            if any(e.get("category") == "state" for e in events_to_push):
                state["last_narrated_situation"] = market_story.situation_key(understanding)

            # THE MARKET STORY (2026-09-15, deep-clean pass — wires up
            # build_market_story(), which existed since the prior fix
            # but was never actually called anywhere; _compose_current_
            # conditions()/_compose_state_story() in hfis.py now read
            # this dict instead of reaching into market_assessment
            # directly). Built once here, passed to BOTH this push and
            # the overview push below, so live events and the ongoing
            # briefing draw on the identical schema.
            _story = market_story.build_market_story(
                understanding, mil_understanding=state.get("mil_understanding"),
                intent_hypothesis=intent_hypothesis)

            # THE "A" RUNG (per chat — closes the "jumping from A to B"
            # gap): one mechanism clause per pushed event, built ONLY
            # from facts already computed elsewhere (see market_story.
            # build_mechanism()'s own docstring). Passed alongside the
            # events themselves so hfis.narrate() can cite it ahead of
            # its own composed conclusion, rather than starting from the
            # conclusion the way format_market_event()'s headlines do.
            mechanisms = [
                market_story.build_mechanism(ev, understanding, state.get("mil_understanding"))
                for ev in events_to_push
            ]

            # HFIS (per chat, corrected): a local, zero-cost procedural
            # narrator — no network call, no API key, no cost. narrate()
            # still returns None on any unexpected internal error (belt
            # and suspenders, not the expected path) — the fallback to
            # the existing template text stays in place either way.
            push_text = hfis.narrate(
                events_to_push, understanding,
                mil_understanding=state.get("mil_understanding"),
                mechanisms=mechanisms, market_story=_story)
            if not push_text:
                push_text = format_market_event(events_to_push, understanding, intent_hypothesis)
            if push_text:
                send_telegram(push_text)

            # GROUP MARKET EVENT MIRROR (per chat, 2026-09-22 — friend's
            # Report -> Event -> Signal teaching flow, middle tier). Same
            # events_to_push/mechanisms/_story this personal push just
            # used — no second computation. See _group_market_event_text()'s
            # own docstring for why this is a separate, beginner-labeled
            # rendering rather than reusing hfis.narrate()'s output
            # (hfis.py's variety/phrase-bank behavior is deliberately kept
            # personal-chat-only, same reasoning as the Signal mirror
            # above dropping the personal message's account-specific
            # lines instead of reusing it verbatim).
            if GROUP_CHAT_ID:
                try:
                    _group_event_text = _group_market_event_text(
                        events_to_push, mechanisms, _story, state)
                    if _group_event_text:
                        send_telegram(_group_event_text, chat_id=GROUP_CHAT_ID)
                except Exception as e:
                    print("[GROUP MARKET EVENT ERROR] " + str(e))

            # /overview accumulation (per chat) — deliberately a SEPARATE
            # concept from MARKET_EVENT_PENDING_FILE above (that one
            # tracks leg-resolution follow-ups; this one is purely for
            # "what changed since the last time anything was sent,"
            # command OR scheduled push, whichever happened more
            # recently). A state.json key (not a file) since it's
            # LIVE-owned and short-lived — cleared the moment it's
            # actually reported, never grows unbounded even if
            # /overview goes unchecked for a while thanks to the cap.
            # Only events_to_push land here (2026-09-15, per chat, fixes
            # the "New spot worth watching just showed up" recap-of-a-
            # recap bug) — the overview's recap section renders this
            # queue as a RECAP of what's already been sent, so it must
            # only ever contain things that were, in fact, already sent.
            #
            # SNAPSHOT THE STATE-CATEGORY NARRATIVE AT PUSH TIME
            # (2026-09-16, found during a stress-test pass, not reported
            # by the user): narration_library.recap_line()'s "state"
            # branch reads story.what_price_is_doing_now — but the
            # RECAP is rendered later, from whatever the story is AT
            # THAT TIME, not from the story as it was when this specific
            # event fired. Two genuinely different state transitions
            # both pending in the same overview batch would both render
            # using the CURRENT (most recent) synthesis — collapsing two
            # real, different events into identical recap text. Same
            # root cause as the opportunity-recap duplicate found
            # earlier this week (a composer reaching for "current" state
            # instead of using what was captured when the event fired),
            # just in a different branch. Fixed here, not in narration_
            # library.py, because the snapshot has to be taken NOW,
            # while `_story` still reflects the moment this event
            # actually happened — recap_line() itself stays a pure
            # function of its inputs; this just makes sure it gets the
            # RIGHT input later.
            _events_for_pending = []
            for ev in events_to_push:
                ev_copy = dict(ev)
                if ev.get("category") == "state":
                    ev_copy["what_price_is_doing_now_at_push"] = _story.get("what_price_is_doing_now")
                _events_for_pending.append(ev_copy)

            _overview_pending = state.get("overview_pending_events", [])
            _overview_pending = (_overview_pending + _events_for_pending)[-OVERVIEW_PENDING_EVENTS_MAX:]
            apply_state_updates(state, {"overview_pending_events": _overview_pending})

            # LIFECYCLE TAGGING (per chat, 2026-08-27): tag each fired
            # event against whichever leg_obs record is currently open,
            # so _close_leg_obs() (min_scanner.py) can send a resolution
            # follow-up when that specific leg eventually closes with a
            # fate. No open leg (e.g. mid-transition, or bias just went
            # stale) -> nothing to tag against; the event still gets
            # pushed, it just won't have a resolution follow-up later.
            try:
                if _open_leg is not None:
                    pending = pending_so_far
                    for ev in events_to_push:
                        pending.append({
                            "leg_obs_id": _open_leg.get("id"),
                            "category": ev.get("category"),
                            "headline": ev.get("headline"),
                            "story_impact": ev.get("story_impact"),
                        })
                    save_market_event_pending(pending[-MARKET_EVENT_PENDING_MAX:])
            except Exception as e:
                print("[MARKET EVENT TAG ERROR] " + str(e))

        state["market_event_prev_snapshot"] = build_event_snapshot(understanding, intent_hypothesis, leg_obs_open=_open_leg)
        save_state(state)
    except Exception as e:
        print("[MARKET EVENT ERROR] " + str(e))


def _build_and_send_overview(state, now_utc, header):
    """
    Per chat — the combined Market Understanding overview: what changed
    since the last time anything was sent (command OR scheduled push,
    whichever happened more recently — deliberately ONE shared tracker,
    not two separate ones per channel) plus the current read on
    conditions regardless.

    "Last sent" is intentionally about delivery, not computation — using
    the last SCAN would mean two commands sent minutes apart with
    nothing scanned in between would have nothing to say about each
    other, which misses the actual question ("what have I not seen
    yet"). Mutates and saves `state` itself, save-before-send ordering
    (matches every other at-most-once Telegram send in this file) —
    but the TEXT is composed from state BEFORE state is reset, since
    the text describes what's about to be cleared.

    REBUILT (2026-09-16, per chat — the architecture redesign). This
    function IS the "Composition" layer now: it builds the story dict
    once, then calls narration_library's pure functions directly and
    assembles the three sections itself (recap, light summary, deep
    read) — no more calling into hfis.narrate_overview()/hfis.compose_
    deep_read_plain(), both retired. This is exactly what "a thin layer
    whose only job is choosing order, never inventing text" means in
    practice: everything below is string concatenation and a header
    label, nothing here composes a sentence.
    """
    try:
        _ws_snapshot = build_world_state(state)
        _understanding = build_market_understanding(_ws_snapshot)
        _intent_hypothesis = build_market_intent_hypothesis(_ws_snapshot)

        _last_sent_at = state.get("overview_last_sent_at")
        _elapsed = None
        if _last_sent_at:
            try:
                _elapsed = (now_utc - datetime.fromisoformat(_last_sent_at)).total_seconds()
            except Exception:
                _elapsed = None

        _pending_events = state.get("overview_pending_events", [])
        _mil_understanding = state.get("mil_understanding")

        # ONE story dict, built once, read by every section below — the
        # actual fix for "something is grabbing from another file": no
        # section past this line reaches into _understanding/_mil_
        # understanding/_intent_hypothesis directly again.
        _story = market_story.build_market_story(
            _understanding, mil_understanding=_mil_understanding,
            intent_hypothesis=_intent_hypothesis)
        _thesis_status = (_mil_understanding or {}).get("thesis_status")

        sections = []

        # Section 1 — what changed (recap of already-pushed events only;
        # see narration_library.recap_line()'s own docstring for why
        # that framing matters).
        if _pending_events:
            recap_lines = [narration_library.recap_line(e, _story) for e in _pending_events]
            sections.append(
                narration_library.elapsed_label(_elapsed) + " "
                + " ".join(l for l in recap_lines if l))
        else:
            sections.append("Nothing new to report since last time." if _last_sent_at
                             else "Here's where things stand:")

        # Section 2 — the light summary, always.
        light = narration_library.light_summary(_story, thesis_status=_thesis_status)
        if light:
        ...
        if not push_text:
            # Fallback: the old single-thesis renderer, same discipline
            # as every other HFIS call site in this file.
            push_text = format_market_thesis(state, _understanding)

        # Section 3 — the deep read, plain and labeled, appended after
        # whichever of the above produced push_text.
        _deep_section = narration_library.deep_read(_story)
        if _deep_section:
            push_text = push_text + "\n\n🔬 *Deeper read:*\n" + _deep_section

        apply_state_updates(state, {
            "overview_last_sent_at": now_utc.isoformat(),
            "overview_pending_events": [],
        })
        save_state(state)
        send_telegram(header + "\n\n" + push_text)
    except Exception as e:
        print("[OVERVIEW ERROR] " + str(e))
...
def _maybe_send_daily_thesis_update(state, now_utc):
    ...
    save_state(state)
    try:
        # CHANGED (per chat, /overview work): this used to call
        # format_market_thesis() directly (the same renderer /thesis
        # uses) — now goes through the shared overview composer so the
        # scheduled push gets the same delta-since-last-send treatment
        # as the on-demand /overview command, using whichever was sent
        # more recently (this scheduled push OR a manual /overview) as
        # its own "last sent" baseline. The 9/12/3 wall-clock SCHEDULE
        # is unchanged — only the CONTENT changed.
        _build_and_send_overview(state, now_utc, header="🗓️ *Market Overview*")
    except Exception as e:
        print("[DAILY THESIS ERROR] " + str(e))
    return True


def _answer_deferred_reports(deferred_report_cmds, state):
    """
    AUDIT FIX (2026-08-26): answers queued /thesis, /marketintent, /world,
    /understand, /integrity commands from check_result_commands(). (/briefing
    was removed from this list the same day, per chat — see the removed-
    command note in check_result_commands().)

    Previously this logic lived ONLY inline, right before
    evaluate_rule_of_law() in _scan_once(). But between where
    check_result_commands() queues these commands and that inline point,
    _scan_once() has NINE separate early `return`s (forex weekend, data
    fetch failure, data freshness failure, data sanity failure, active
    trade still open, trade just closed, insufficient 1H bars,
    CONSOLIDATION bias, no confirmed swing points yet). Any one of those
    firing while a deferred command was queued caused it to be silently
    dropped: check_result_commands() already advanced
    stats["last_update_id"] (set unconditionally per update, before any
    per-command handling) and stats gets saved at each of those early
    returns, so the Telegram update is marked consumed — the command
    doesn't just get delayed to next scan, it's gone for good, with no
    reply and no error. This is almost certainly what happened to the
    /understand sent at 2:02 PM on 2026-08-26: the pass that received it
    hit one of those nine early exits before ever reaching the old
    inline answer point, while the /marketintent sent 6 minutes later
    landed on a pass that ran clean through.

    Fix: this function is now called at EVERY early-return point in
    _scan_once() between the command-queue point and the end of the
    function, as well as at the original post-thesis/intent point for
    the normal-flow case. Each call site only ever fires once per scan
    (whichever return/end-of-function is actually taken), so commands
    are never answered twice. format_market_thesis()/
    format_market_intent_report()/etc. already fall back to whatever
    was last persisted when this pass's own state isn't finalized yet
    (e.g. an early-exit pass) — same as they always have.
    """
    for _cmd in deferred_report_cmds:
        try:
            if _cmd == "thesis":
                # PHASE 0 (per audit, 2026-08-27): same WorldState/
                # understanding /understand builds, so /thesis can't say
                # something /understand wouldn't on this exact scan.
                _ws_snapshot = build_world_state(state)
                _understanding = build_market_understanding(_ws_snapshot)
                send_telegram(format_market_thesis(
                    state, _understanding,
                    deep_thesis_text=hfis.narrate_deep_thesis(state.get("mil_understanding")),
                ))
            elif _cmd == "overview":
                # Per chat — shares the exact same composer and "last
                # sent" tracker as the scheduled 9/12/3 push, so whichever
                # happened more recently (manual or scheduled) is always
                # the baseline for "what changed." now_utc computed
                # locally rather than threading it through this
                # function's signature — _answer_deferred_reports() is
                # called from 9+ early-return sites in _scan_once(), and
                # this is the only branch that needs a clock.
                _build_and_send_overview(state, datetime.now(timezone.utc), header="📊 *Market Overview*")
            elif _cmd == "marketintent":
                _ws_snapshot = build_world_state(state)
                _intent_hypothesis = build_market_intent_hypothesis(_ws_snapshot)
                send_telegram(format_market_intent_report(
                    state, load_market_intent_state(), _intent_hypothesis))
            elif _cmd == "world":
                send_telegram(format_world_state(build_world_state(state)))
            elif _cmd == "understand":
                _ws_snapshot = build_world_state(state)
                _understanding = build_market_understanding(_ws_snapshot)
                _append_brain_log(_understanding, _ws_snapshot)
                send_telegram(format_understanding_narrative(_understanding))
            elif _cmd == "integrity":
                send_telegram(format_integrity_command(state))
        except Exception as e:
            try:
...
def _scan_once():
    ...
    # Coverage — deliberately the FIRST thing this function does, before
    # load_stats()/load_state() (which could themselves fail) and before
    # every other early-return path below. One line, timestamp only — no
    # outcome, no market data, nothing that could make this write fail.
    # This is what makes the ledger's per-day coverage honest rather than
    # inferred from side effects that only fire on SOME exit paths.
    try:
        with open(SCAN_LOG_FILE, "a") as f:
            f.write(json.dumps({"scanned_at": now_utc.isoformat()}) + "\n")
    except Exception as e:
        print("[SCAN LOG ERROR] " + str(e))

    stats = load_stats()
    state = load_state()

    # ── SCAN ID (Research Lab groundwork, per chat 2026-09-18) ─────────────
    # Minted here, not in the unconditional SCAN_LOG_FILE write above —
    # that one must survive load_state() itself failing, so it stays
    # timestamp-only by design (see its own comment). This one only fires
    # once state has actually loaded. Every permanent record the rest of
    # this pass writes (structure_digest_log, zone_log; ohlc history is
    # deliberately excepted — see append_ohlc_history()'s own docstring
    # on why timestamp-range joins already cover it) is stamped with this
    # scan_id, and the mapping is logged immediately below so it survives
    # even if this pass early-returns further down before reaching them.
    scan_id = next_scan_id(state)
    append_permanent_jsonl(SCAN_INDEX_LOG_FILE, {
        "scan_id": scan_id,
        "scanned_at": now_utc.isoformat(),
    })
    # Saved immediately, not deferred to the first "natural" save_state()
    # call further down (ctx_updates, ~40 lines below): several early-
    # return paths between here and there (e.g. the CONSOLIDATION branch)
    # never call save_state() at all. Without this, scan_id would be lost
    # on every such early return and reissued next scan — duplicate IDs,
    # silently. Cheap: save_state() does a full atomic overwrite, so this
    # doesn't conflict with the later saves, it's just the first of them.
    save_state(state)
    ...
    try:
        ...
        # AUDIT FIX (2026-09-08): this was the exact gap that could make a
        # command get NO reply with NO trace anywhere Telegram-facing.
        # check_result_commands() already isolates and alerts on a failure
        # inside any INDIVIDUAL command's handling (see its own "AUDIT FIX"
        # except block) — but anything that throws BEFORE that per-command
        # try (parsing the getUpdates response, building a closure, etc.)
        # only ever reached this bare print. The scan then carried on and
        # exited 0, so the Actions job showed green and "Alert on job
        # failure" never fired either — the only trace was a console line
        # nobody was watching. Escalated the same way every other
        # command-adjacent failure already is. This does NOT lose the
        # message: offset only advances inside check_result_commands()'s own
        # per-update loop, so anything that failed this early gets replayed
        # next cycle — the alert is about visibility/delay, not data loss.
        print("[COMMANDS ERROR — STRUCTURAL] " + type(e).__name__ + ": " + str(e))
        try:
            send_telegram(
                f"⚠️ *Command polling failed this cycle*: {type(e).__name__}: {e}\n"
                "Any message you sent this cycle wasn't dropped — it'll be "
                "picked up and answered next scan — but this failed before "
                "reaching normal per-command error handling, which means "
                "it's a structural bug worth checking, not routine."
            )
        except Exception:
            pass
    ...
    pending = stats.get("pending_confirmation")
    if pending:
        # MIN-WINDOW GATE (2026-09-03, per audit — see MIN_CONFIRMATION_
        # WINDOW_SECONDS's docstring in scanner_common.py). A pending
        # confirmation used to be fair game for auto-promote on the very
        # next scan regardless of how little real time that left the
        # user — a signal firing seconds before a cron tick gave almost
        # no window at all. Now: only auto-promote once at least
        # MIN_CONFIRMATION_WINDOW_SECONDS have actually elapsed since the
        # signal fired. If not enough time has passed yet, pending_
        # confirmation is left untouched for a later scan to re-check —
        # this does NOT extend indefinitely; the very next scan that DOES
        # clear the floor will promote it, same as before.
        since_raw = pending.get("awaiting_confirmation_since") or pending.get("opened_at")
        elapsed_ok = False
        if since_raw:
            try:
                since_dt = datetime.fromisoformat(since_raw)
                elapsed_ok = (now_utc - since_dt).total_seconds() >= MIN_CONFIRMATION_WINDOW_SECONDS
            except ValueError:
                elapsed_ok = True  # malformed timestamp — fail open to the OLD behavior rather than wedge forever
        else:
            elapsed_ok = True  # no timestamp at all (shouldn't happen going forward) — same fail-open reasoning
        ...
        if elapsed_ok:
            ...
            print(f"  [AUTO-TAKEN] Skipped — only "
                  f"{(now_utc - since_dt).total_seconds():.0f}s elapsed, "
                  f"floor is {MIN_CONFIRMATION_WINDOW_SECONDS}s. Leaving pending_confirmation open.")
    ...
    # ── GROUP HOURLY HEARTBEAT (wall-clock, not scan-count) ─────────────
    # Same reasoning as the daily thesis push above, separate cadence.
    # No-op if GROUP_CHAT_ID isn't configured.
    try:
        _maybe_send_group_hourly_report(state, now_utc)
    except Exception as e:
        print("[GROUP HOURLY SCHEDULER ERROR] " + str(e))

    if is_forex_weekend(now_utc):
        print("Forex market is closed for the weekend. Skipping market evaluation.")
        save_stats(stats)
        save_state(state)
        _answer_deferred_reports(deferred_report_cmds, state)
        return
    ...
    if df_5m is None or df_15m is None or df_1h is None:
        ...
        _maybe_send_group_system_alert(state, now_utc, "Market data could not be fetched.")
        save_stats(stats)
        _answer_deferred_reports(deferred_report_cmds, state)
        return
    ...
    if not (fresh_5m and fresh_15m and fresh_1h):
        ...
        _maybe_send_group_system_alert(state, now_utc, "Market data freshness could not be verified.")
        save_stats(stats)
        _answer_deferred_reports(deferred_report_cmds, state)
        return
    ...
    if not (data_looks_sane(df_5m, "5min") and
        ...
        _maybe_send_group_system_alert(state, now_utc, "Market data failed a sanity check.")
        save_stats(stats)
        _answer_deferred_reports(deferred_report_cmds, state)
        return

    # All three data checks just passed — clear any group alert that was
    # active from a prior failing pass (no-op if none was active).
    _maybe_send_group_system_alert(state, now_utc, None)

    # MARKET DATA FRESHNESS STAMP (Fix Spec batch 4, item #8 — per chat,
    # locked decision; audited commit point — this is the earliest point
    # in _scan_once() where fetch AND freshness AND sanity have ALL
    # succeeded, so it's the correct and only place this field is ever
    # set). Deliberately NOT "fetch() returned something" — a response
    # containing stale/incomplete/malformed/rejected candles must not
    # revive this clock, which is exactly why this sits after all three
    # gates above, not right after the fetch calls. Persisted whenever
    # the next save_state() call in this pass runs (below) — this line
    # only updates the in-memory dict, same convention as every other
    # state[...] = ... assignment in this file. See load_state()'s
    # comment in scanner_common.py for how this field is consumed and
    # why it's kept separate from "timestamp".
    state["market_data_timestamp"] = now_utc.isoformat()
    ...
    # Permanent OHLC archive — see append_ohlc_history()'s own docstring
    # for why this is separate from the rolling cache above. Same
    # "unconditional, before any early return" placement as the cache
    # write itself — a candle that closed this scan should be archived
    # regardless of what bias/tier logic decides afterward.
    apply_state_updates(state, append_ohlc_history(df_5m, df_15m, df_1h, state))
    ...
    # Added 2026-08-25, alongside prior_continuation_leg_* below — see
    # that block's comment for why this is captured separately from
    # _prev_leg_origin above.
    _prev_leg_latest_swing_origin = state.get("macro_leg_latest_swing_origin")
    _prev_continuation_count = state.get("campaign_continuation_count")
    ...
    # ── same idea, for the EXHAUSTION exit-hysteresis fix (2026-08-26,
    # per chat) — captured before compute_market_phase() below overwrites
    # market_phase, so it can tell "was this scan already EXHAUSTION" from
    # "just now becoming EXHAUSTION." See PHASE_EXHAUSTION_EXIT_FRACTION
    # in scanner_common.py. ──────────────────────────────────────────────
    _prev_phase = state.get("market_phase")
    ...
    # exact function's pass order. Placed here, immediately after bias/
    # leg-survival become authoritative and BEFORE the CONSOLIDATION/
    # no-swing-points early returns below, specifically so MIL runs on
    # EVERY scan including both of those — a bearish thesis collapsing
    # into "no edge" is itself a real belief transition, not something
    # to leave unrecorded. maintain_thesis()/evaluate_scenarios() (call
    # site 2, below, once thesis/intent exist this scan) enrich this
    # same understanding later — this call alone is what gets
    # thesis_status/direction/history right on every single scan.
    # MIL never opens state.json itself (see mil.py's module docstring)
    # — reconcile() returns a payload, this commits it exactly like
    # every other block in this function commits its own state.
    _mil_understanding = None
    # Fix Spec batch 5, item #11 (locked contract, Pass 9): call site 2
    # below must not treat a partial call-site-1 success (reconcile()
    # ok, track_counter_candidate() raised) as fully fresh. Flag is set
    # True only once, as the LAST line of the try block below — after
    # both calls AND their single combined persist have all succeeded —
    # so any failure anywhere in this block, including in the persist
    # itself, leaves it False.
    _mil_call_site_1_ok = False
    try:
        _prev_understanding = mil.understanding_from_dict(state.get("mil_understanding"))
        _mil_observed = mil.ingest(state)
        _mil_understanding = mil.reconcile(_prev_understanding, _mil_observed, now_utc.isoformat())
        # ADDED (per chat, JUINIOR — "the counter case is more in depth",
        # 2026-0X-XX). MUST run after reconcile() in this same try block —
        # track_counter_candidate() reads _mil_understanding.counter_thesis
        # to tell whether reconcile() just promoted this exact candidate
        # THIS scan (see track_counter_candidate()'s own docstring for why
        # that ordering matters). Same single-commit pattern as every other
        # block here — one apply_state_updates call for both functions'
        # combined effect on _mil_understanding, not two separate writes.
        _mil_understanding = mil.track_counter_candidate(_mil_understanding, _mil_observed, now_utc.isoformat())
        apply_state_updates(state, {"mil_understanding": mil.persist_reasoning_state(_mil_understanding)})
        _mil_call_site_1_ok = True
    except Exception as e:
        print("[MIL RECONCILE ERROR] " + str(e))
    ...
    # ── continuation-level bookkeeping (pure, additive, zero risk to the
    # bias path above — reads/writes only new prior_continuation_leg_*
    # keys). Added 2026-08-25, per chat: prior_macro_leg_* above is
    # gated on macro_leg_origin CHANGING, which — by detect_bos_impulse()'s
    # own contract — it deliberately does NOT do on a same-direction
    # continuation. That's correct for prior_macro_leg's own two
    # consumers (see capture_prior_continuation_snapshot()'s docstring),
    # but it meant a long-running campaign (e.g. 31 continuations deep)
    # had NO persisted fact at all for "the leg immediately before this
    # continuation" — only "the leg before the whole current swing."
    #
    # Gated on campaign_continuation_count CHANGING instead of
    # macro_leg_origin — that's the field that actually increments once
    # per genuinely-confirmed continuation (see _campaign_updates() in
    # scanner_observation.py), so this fires every continuation,
    # including the ones the block above structurally can't see. ────────
    if state.get("campaign_continuation_count") != _prev_continuation_count:
        apply_state_updates(state, capture_prior_continuation_snapshot(
            _prev_leg_direction, _prev_leg_latest_swing_origin, _prev_leg_extreme))
    ...
    # ── SHADOW: min-leg-size gate (per chat, 2026-09-01) ────────────────
    # Second, independent shadow rule — see MIN_LEG_SIZE_ATR_MULT in
    # scanner_common.py and compute_macro_bias_min_leg_gate()'s docstring
    # in scanner_observation.py for the gap this tests. Logged to its own
    # file (bias_minleg_log.json), NOT merged into bias_ab_log.json — that
    # file's schema is already owned by the old-rule A/B test above.
    # Read-only instrumentation: never touches macro_bias/tier gating.
    minleg_bias, minleg_updates = compute_macro_bias_min_leg_gate(df_1h, df_15m, state)
    apply_state_updates(state, minleg_updates)
    minleg_agree = (minleg_bias == macro_bias)
    if not minleg_agree:
        print(f"  [SHADOW-MINLEG] live={macro_bias}{' STALE' if bias_stale else ''} | "
              f"min-leg-gate={minleg_bias} | DIVERGE")

    try:
        bias_minleg_log = load_bias_minleg_log()
        bias_minleg_log.append({
            "time":         now_utc.strftime("%Y-%m-%d %H:%M UTC"),
            "live_bias":    macro_bias,
            "minleg_bias":  minleg_bias,
            "agree":        minleg_agree,
            "rejected_leg": state.get("shadow_minleg_last_rejected_leg"),
            "price":        float(df_5m["Close"].iloc[-1]),
        })
        save_bias_minleg_log(bias_minleg_log)
    except Exception as e:
        print("[BIAS MINLEG LOG ERROR] " + str(e))
    ...
    # ── STRUCTURE DIGEST (WorldState 1B, structure.system_observation —
    # the "sensor layer", per chat 2026-08-20) ─────────────────────────────
    # Captured HERE, immediately after `facts` exists and BEFORE any tier
    # evaluation — deliberately independent of whether a tier activates,
    # fires, or the risk gate does anything, unlike live_tier_digest
    # (which only makes sense post-decision). This is meant to run EVERY
    # scan, cheaply, off the exact same MarketFacts instance every tier
    # already reads — no second candle read, no new detection. Same
    # "capture, never feed back into a decision" discipline as
    # live_tier_digest; nothing downstream reads state["structure_digest"]
    # to gate anything.
    try:
        digest = build_structure_digest(facts)
        # Additive regime enrichment (per chat, friend's "context
        # collapse" critique of the Markov model): build_structure_digest()
        # stays PURE (facts only, per its own docstring) — this merges in
        # the two regime fields that are already local variables at this
        # exact call site, so the permanent per-scan record stops losing
        # them. Before this, macro_bias/staleness at any given historical
        # scan was NOT reconstructable from anywhere on disk — only ever
        # aggregated away into markov_transitions.json's cumulative count
        # matrix, which discards per-instance identity by design. This is
        # the actual prerequisite for the friend's proposed multi-condition
        # historical matcher ("1H bias=bullish AND fib 0.5-0.7 AND ..."):
        # without regime persisted per scan, no future query could ever
        # condition on it, no matter how good the query engine is.
        # Purely additive section — no methodology-version bump needed,
        # same bump policy SHADOW_METHODOLOGY_VERSION documents in
        # scanner_common.py. Nested as its own "regime" section (not
        # top-level keys) to match structure/location/liquidity's shape
        # — research_dataset.py's load_structure_digest_log() flattens
        # named sections only, so this also needs that loader updated
        # (see research_dataset.py) or these fields silently never reach
        # the analysis-ready DataFrame despite being logged correctly.
        digest["regime"] = {
        ...
        save_state(state)
        # Permanent, per-scan copy (Research Lab groundwork, per chat
        # 2026-09-18) — state["structure_digest"] above is a same-key
        # field the NEXT scan immediately overwrites, so despite this
        # function's own docstring saying it exists "so this scan's
        # MarketFacts stops being thrown away," it was still being
        # thrown away every single scan. This is what actually fixes
        # that: one permanent row per scan, joinable back to everything
        # else via scan_id.
        _log_structure_digest(digest, scan_id, now_utc.isoformat())
    except Exception as e:
        print("[STRUCTURE DIGEST ERROR] " + str(e))

    # ── ZONE LOG (Research Lab groundwork, per chat 2026-09-18) ────────────
    # First-detection-only permanent log of FVG (both directions) and
    # order-block zones — see build_zone_observations()'s own docstring
    # for why it deliberately does NOT reuse facts.significant_fvg()/
    # facts.order_block() output as-is. Isolated in its own try/except,
    # same discipline as structure digest above: a failure here must
    # never affect live tier evaluation below it.
    try:
        _log_new_zones(state, facts, scan_id, now_utc.isoformat())
        save_state(state)
    except Exception as e:
        print("[ZONE LOG ERROR] " + str(e))
    ...
    try:
        ...
        phase_result, phase_updates = compute_market_phase(
            ...
            prev_macro_bias=_prev_macro_bias, prev_phase=_prev_phase)
    ...
    if phase_result is not None:
        try:
            ...
            apply_state_updates(state, {
                ...
                "market_thesis_evidence_prose":        thesis.evidence_prose,
                "market_thesis_weaknesses_prose":      thesis.weaknesses_prose,
                # ADDED (2026-08-27, per chat — /understand repetition fix):
                # persisted alongside weaknesses_prose in lockstep, same
                # single-source-of-truth discipline as every other thesis_*
                # key here. See MarketThesis.weakness_categories docstring.
                "market_thesis_weakness_categories":   thesis.weakness_categories,
                ...
                "market_thesis_failure_risk_reasons_prose": thesis.failure_risk_reasons_prose,
            ...
            # Research Lab groundwork, second pass (per chat, 2026-09-20) —
            # see _log_thesis_context()'s own docstring. Inside this same
            # try block deliberately: if thesis computation above raised,
            # there's nothing genuine to log, so no row (never a fabricated
            # one) is the correct behavior, same as this whole block already
            # being skipped when phase_result is None.
            try:
                _log_thesis_context(thesis, phase_result, scan_id, now_utc.isoformat())
            except Exception as e:
                print("[THESIS CONTEXT LOG ERROR] " + str(e))
        except Exception as e:
        ...
        try:
            ...
            apply_state_updates(state, {
                ...
                # Structured, not just codes (per chat, 2026-08-24 —
                # WorldState/Brain review): the old `[w["code"] for w in
                # ...]` reduction discarded zone_low/zone_high/role
                # before this ever reached state.json, which meant
                # build_world_state()'s intent.watching_for silently had
                # NONE of the location data brain.py's
                # build_market_intent_hypothesis() needs — it only ever
                # had bare code strings. Carrying the structured dict
                # through is what makes "Brain reads watching_for's
                # zone directly, no re-detection" actually true instead
                # of aspirational. `sentence` kept too, for
                # backward-compat / dev-audit rendering (per chat) — NOT
                # meant to be the human-facing text once
                # build_market_briefing() is trusted; that's Brain's job.
                "market_intent_watch_codes":    [
                    {"code": w["code"], "role": w["role"],
                     "zone_low": w["zone_low"], "zone_high": w["zone_high"],
                     "sentence": w["sentence"]}
                    for w in intent.watching_for
                ],
                "market_intent_caution_codes":  [
                    {"code": c["code"], "role": c["role"], "sentence": c["sentence"]}
                    for c in intent.not_interested_in
                ],
            ...
            # ── MIL: enrich this scan's understanding — call site 2 of 2 ──
            # Per chat (physiology work). Reuses the SAME in-memory
            # _mil_understanding call site 1 already produced this scan
            # (guarded: only runs if that call succeeded) — this is
            # enrichment, not a second reconciliation. thesis/intent only
            # exist on scans that reach this point (phase_result not
            # None, thesis/intent builds both succeeded), which is
            # exactly why this can't be call site 1's job — see the
            # insertion-point trace in mil.py's own header for the full
            # reasoning on why this is two calls, not one.
            #
            # FIX (Fix Spec batch 5, item #11 — locked contract, Pass 9):
            # guard was `_mil_understanding is not None` alone, which
            # only proves reconcile() ran — it says nothing about
            # whether track_counter_candidate() (call site 1's second
            # call) also succeeded. Nothing in maintain_thesis()/
            # evaluate_scenarios()/generate_expectations() below depends
            # on track_counter_candidate()'s output, so this costs
            # nothing functionally on a genuine full success — but on a
            # partial one it would have enriched (and persisted, below)
            # a this-scan-fresh reconcile() result that never got its
            # counter-candidate tracking, all stamped under one fresh
            # last_reconciled_at — exactly the "partially successful
            # upstream stage appears fully fresh" failure this contract
            # forbids. _mil_call_site_1_ok is only True when BOTH calls
            # AND their persist succeeded (see call site 1, above).
            if _mil_understanding is not None and _mil_call_site_1_ok:
                try:
                    _mil_understanding = mil.maintain_thesis(
                        _mil_understanding, thesis.evidence_prose, thesis.weaknesses_prose)
                    _mil_understanding = mil.evaluate_scenarios(
                        _mil_understanding, intent.watching_for)
                    # AUDIT FIX (2026-09-11, per chat — "expected next
                    # event still says CHoCH or range"). _refine_next_watch()
                    # was added 2026-08-27 specifically to fix this exact
                    # ambiguity ("CHoCH or range" flattens a real structural
                    # signal and a much weaker stall into one either/or —
                    # see its docstring in brain.py) — but it was only ever
                    # applied to Brain's OWN next_watch field. This call
                    # passed thesis.expected_next_event straight through,
                    # completely bypassing that fix — MIL's expectations
                    # (and therefore anything reading mil_understanding
                    # directly) kept showing the raw, unrefined map text
                    # regardless of the brain.py fix ever having shipped.
                    # Reused the SAME function rather than duplicating its
                    # logic — it's pure and already handles "not in the
                    # refinement table -> return unchanged" on its own, so
                    # this is a strict improvement with no new failure mode
                    # for entries the table doesn't cover.
                    _direction_label = (
                        facts.macro_bias.title()
                        if facts.macro_bias in ("BULLISH", "BEARISH") else None
                    )
                    _mil_understanding = mil.generate_expectations(
                        _mil_understanding,
                        _refine_next_watch(thesis.expected_next_event, _direction_label),
                        intent.watching_for)
                    apply_state_updates(state, {"mil_understanding": mil.persist_reasoning_state(_mil_understanding)})

                    # AUTO-PUSH ON THESIS CHANGE (2026-09-12, per chat —
                    # "when there's a thesis change... let it automatically
                    # send the new thesis, but this time a brief summary
                    # of the prior thesis and why it changed"). Fires once
                    # per genuine change, comparing against `_prev_
                    # understanding` from call site 1 above (same
                    # enclosing scope, set before this try block runs —
                    # guarded defensively below in case that assignment
                    # never happened this scan for any reason). Excludes
                    # the UNKNOWN->SUPPORTED bootstrap transition — that's
                    # initialization, not a change with a "prior thesis"
                    # worth summarizing. Uses the SAME narrate_deep_thesis()
                    # /thesis now uses (see min_scanner.py) for the "new
                    # picture" half, so the two surfaces never say the deep
                    # section differently — only whether the "what changed"
                    # preamble is shown differs between them.
                    try:
                        _prior_status = getattr(_prev_understanding, "thesis_status", None)
                        _prior_status = _prior_status.value if hasattr(_prior_status, "value") else _prior_status
                        _new_status = _mil_understanding.thesis_status
                        _new_status = _new_status.value if hasattr(_new_status, "value") else _new_status
                        _prior_direction = (getattr(_prev_understanding, "primary_thesis", None) or {}).get("direction")
                        if _prior_status and _prior_status != "UNKNOWN" and _prior_status != _new_status:
                            _change_text = hfis.narrate_thesis_change(
                                mil.persist_reasoning_state(_mil_understanding),
                                _prior_direction, _prior_status)
                            if _change_text:
                                send_telegram("📖 *Thesis Update*\n\n" + _change_text)
                    except Exception as e:
                        print("[MIL THESIS CHANGE PUSH ERROR] " + str(e))
                except Exception as e:
                    print("[MIL ENRICH ERROR] " + str(e))
        except Exception as e:
    ...
    # ── MARKET EVENT — runs every scan, unconditionally (per chat,
    # 2026-08-27 — see _evaluate_and_push_market_event()'s own docstring
    # for why this sits here: same guarantee _answer_deferred_reports()
    # above needed — this pass's market_thesis_*/market_intent_* keys
    # are already finalized, and we're still before evaluate_rule_of_
    # law() so a downstream crash can't skip an event that already
    # happened this scan). ─────────────────────────────────────────────
    _evaluate_and_push_market_event(state, now_utc)
    ...
    if not result.fired:
        # WorldState 1B — capture BEFORE the early return, strictly
        # observational (result is already fully decided at this point;
        # this write influences nothing downstream of it).
        try:
            apply_state_updates(state, {
                "live_tier_digest": build_live_tier_digest(result, risk_result=None),
            })
            save_state(state)
        except Exception as e:
            print("[WORLD STATE DIGEST ERROR] " + str(e))
        save_stats(stats)
    ...
    # WorldState 1B — capture AFTER the risk gate has fully decided,
    # strictly observational (same discipline as the early-return capture
    # above). Placed here in _scan_once, not inside
    # apply_risk_gate_and_finalize() itself — kept that function
    # untouched rather than assume anything about who else might call it
    # later.
    try:
        apply_state_updates(state, {
            "live_tier_digest": build_live_tier_digest(result, risk_result=risk_result),
        })
        save_state(state)
    except Exception as e:
        print("[WORLD STATE DIGEST ERROR] " + str(e))
    ...
    try:
        ...
        position_size_line = ""
    # CAUTION SECTION (2026-09-12, per chat + advisor input — Q2: "should
    # skepticism reach the fired signal?"). Answer: yes, informational
    # only — never let it touch confidence/risk numerically without
    # empirical validation first (the same discipline this codebase
    # already learned the hard way once: "arbitrary gates bolted onto the
    # bot... can accidentally destroy the signal population while
    # believing you've made the system safer"). market_intent_caution_
    # codes was already being computed correctly every scan by build_
    # market_intent() — confirmed producing genuinely useful text
    # ("3 breaks into this leg — getting old", "Order block already
    # mitigated") — but nothing downstream ever read it again once a
    # tier actually fired. This is that missing wire, and nothing else:
    # setup validity (whether the signal fires at all) is completely
    # untouched by what this section says.
    #
    # Per the friend: "absence of cautions should also be meaningful" —
    # a clean signal states that plainly rather than just omitting the
    # section, so silence here always means "checked, found nothing,"
    # never "wasn't checked."
    _caution_codes = state.get("market_intent_caution_codes") or []
    if _caution_codes:
        _caution_lines = "\n".join(f"• {c['sentence']}" for c in _caution_codes)
        caution_line = f"\n⚠️ *Cautions:*\n{_caution_lines}\n"
    else:
        caution_line = "\n✅ _No major caution flags._\n"
    ...
    telegram_ok = send_telegram(
        ...
        + "_No reply within ~9 minutes = assumed taken._"
    )
    if not telegram_ok:
        stats.clear()
    ...
    # ── GROUP SIGNAL MIRROR (per chat, 2026-09-21 — group gateway work) ──
    # Same fired signal, restructured for a beginner audience: warnings
    # elevated ABOVE the action line (per chat: "the first thing your
    # friend sees is SELL... the warnings are subordinate" — this is a
    # presentation fix only, it does not touch whether/how the signal
    # fired). Deliberately drops what's personal-account-specific
    # (suggested size, $ position sizing, the /taken /skip control-plane
    # footer) and the Markov regime line (research-facing, not beginner-
    # facing) — everything else reuses the exact variables the personal
    # message above already computed, never a second calculation.
    #
    # EXTENDED (2026-09-22, per chat — friend's "why it triggered" idea):
    # result.reason is the tier's OWN already-written human-readable
    # explanation of what conditions were satisfied (see stitch_signal_
    # narrative()'s docstring in scanner_observation.py — "result.reason
    # ... ALREADY tier-appropriate human text, generated one call closer
    # to the actual detection by that tier's own evaluate()"). This adds
    # that one line and nothing else: deliberately NOT adding compute_
    # evidence()'s win-rate/CI statistics here — those are explicitly
    # "informational only" research content on the personal side (see
    # format_evidence_note()'s own docstring), and showing raw win-rate
    # percentages to a beginner audience is exactly the "unearned
    # numbers" trap this project's own philosophy already rejects
    # elsewhere. "What weakens it" is already covered by the warning
    # block above (cautions + low-ATR); there is no equally clean
    # already-computed "what supports it" field distinct from those two,
    # so nothing is invented to fill that slot.
    if GROUP_CHAT_ID:
        _group_warn_lines = []
        if low_atr_line:
            _group_warn_lines.append(low_atr_line.strip().lstrip("🔴⚠️ ").strip())
        for _c in _caution_codes:
            _group_warn_lines.append(_c["sentence"])
        _group_warn_block = (
            "⚠️ *Confirmation is incomplete:*\n"
            + "\n".join(f"• {ln}" for ln in _group_warn_lines) + "\n\n"
        ) if _group_warn_lines else ""
        _group_tier_num = TIER_NUMBER.get(result.tier_label)
        _group_tier_txt = f"Tier {_group_tier_num}" if _group_tier_num else result.tier_label
        _why_line = f"Why it triggered: {result.reason}\n" if result.reason else ""
        _group_signal_text = (
            f"🟡 *GBPUSD — {_group_tier_txt} setup*\n\n"
            + _group_warn_block +
            f"Direction: *{result.direction}*\n"
            f"Higher-timeframe bias: *{macro_bias}*\n"
            + _why_line +
            "─────────────────────\n"
            f"Entry: `{risk_result['entry']:.5f}`\n"
            f"Stop: `{risk_result['sl']:.5f}` _({risk_result['risk_pips']:.1f} pips)_\n"
            f"Target: `{risk_result['tp']:.5f}` _({risk_result['reward_pips']:.1f} pips)_\n"
            f"Risk:Reward: `1:{risk_result['target_r']}`\n\n"
            "_This is a qualifying system setup, not a guaranteed trade. "
            "Review it before treating it as actionable._"
        )
        try:
            send_telegram(_group_signal_text, chat_id=GROUP_CHAT_ID)
        except Exception as e:
            print("[GROUP SIGNAL ERROR] " + str(e))
...
def scan():
    ...
    try:
        ...
        # AUDIT FIX (per chat, 2026-08-25): this was the one place in the
        # whole call chain with NO exception handler at all. Every
        # send_telegram() call site is already safe (send_telegram()
        # itself never raises — see its own docstring), and the Telegram
        # command loop already isolates one bad command from crashing the
        # rest (see the "AUDIT FIX" comment on that except block). But
        # anything ELSE unexpected inside _scan_once() — a corrupted
        # market_intent_state.json, a KeyError deep in some formatter,
        # anything not already anticipated — had no floor to land on
        # except a bare traceback to stderr. On a Cronjob.com/Termux setup
        # not capturing stderr, that's a scan that silently never ran its
        # trading logic, with zero trace anywhere a person would see it.
        # This does NOT change behavior on any currently-working path —
        # it only catches what used to crash the process outright.
        import traceback
        tb = traceback.format_exc()
        print("[SCAN CRASH] " + type(e).__name__ + ": " + str(e) + "\n" + tb)
        try:
            send_telegram(
                f"\U0001f6a8 _Scan crashed — {type(e).__name__}: {e}_\n"
                "_(Full traceback in console/logs. This scan's trading "
                "logic did not run.)_"
            )
        except Exception:
            pass  # send_telegram() itself shouldn't raise, but this is
                   # the last line of defense in the process — never let
                   # the crash-reporter become a second crash.
    finally:
        try:
            os.close(lock_fd)
        except OSError:
