...
def investigate_failure_case(tier_number, formation_tags, exclude_trade_id=None):
    """
    Steps 3-6. Reads this tier's OWN permanent EXP4_POLICY_LAB history,
    filtered to the RISK_AT_HAND policy arm only (tier-isolated, same
    "jury, not judge" isolation as compute_evidence() — Tier 1 only ever
    learns from Tier 1), splits into winners/losers, compares every
    formation tag on this trade against both groups, and ranks the
    comparisons that cleared CASE_MIN_GROUP_N by raw gap size.
    ...
    records = [
        ...
        and r.get("tags", {}).get("policy") == "RISK_AT_HAND"
    ]
...
def open_failure_case(setup, outcome, r_achieved, now_utc):
    ...
    moment an EXP4_POLICY_LAB RISK_AT_HAND setup resolves. Only opens a
    case when the live engine actually expected a win — this trade's
    policy tag is RISK_AT_HAND, the control arm that mirrors today's
    live policy exactly (activated + risk-gate-passed, unconditional
    fire) — and reality diverged (outcome is a loss). A resolved loss on
    any other policy variant (CONVICTION_SWEEP_ON/OFF) isn't a case:
    those are hypothetical alternate policies, not what live actually
    would have done.
    ...
    """
    if setup.get("experiment") != "EXP4_POLICY_LAB":
        return None
    if setup.get("tags", {}).get("policy") != "RISK_AT_HAND":
        return None
    if r_achieved > 0:
    ...
    case = {
        ...
        "conviction_score": formation_tags.get("score_used"),
