...
def _maybe_send_group_hourly_report(state, now_utc):
    ...
    same save-before-send ordering as _maybe_send_daily_thesis_update()
    so a crash between send and save can't cause a duplicate. No-op
    (returns False) if GROUP_CHAT_ID isn't configured, so this is a
