...
# ADDED (2026-08-27, per chat — Market Event lifecycle). Cap on
# state["market_event_pending"] — Market Events tagged against the
# currently-open leg_obs record, awaiting that leg's resolution (see
# _close_leg_obs()'s state= handling in min_scanner.py, and
# _evaluate_and_push_market_event() in scanner_live.py, the only
# writer). Generous relative to how many events can realistically tag
# one open leg — this is a safety cap, not an expected steady-state size.
