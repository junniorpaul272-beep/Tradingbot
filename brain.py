...
_STATE_FAMILY_LABELS = {
    ...
    "pressure": "the leg coming under opposing pressure",
    "maturing": "an aging, maturing leg",
...
def evaluate_market_event(understanding, intent_hypothesis, prev_snapshot, leg_obs_open=None,
    ...
    if not structure_fired:
        ...
        #      an event" principle to the state category itself. A
        #      transition OUT of bias_only (we now know something we
        #      didn't) is still genuinely informative and still fires.
        ...
        if (prev_family and curr_family and prev_family != curr_family
                and curr_family != "bias_only"):
            template = random.choice(_STATE_TRANSITION_TEMPLATES)
            events.append({
                "category": "state",
                "headline": template.format(
                    prev=_STATE_FAMILY_LABELS.get(prev_family, prev_family),
                    curr=_STATE_FAMILY_LABELS.get(curr_family, curr_family),
                ),
            })
    ...
    if new_location_codes or new_confirmations:
        ...
        if _new_location_entries:
            ...
            headline = f"A new area of interest has entered the current opportunity landscape: {_zones}."
        else:
            headline = "A new area of interest has entered the current opportunity landscape."
        events.append({
