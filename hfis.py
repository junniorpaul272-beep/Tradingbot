...
def _compose_current_conditions(cc, mil_understanding, story=None):
    ...
    into a raw market_assessment dict itself.

    NARROWED BACK TO AN ACTUAL SUMMARY (2026-09-16, per chat): a prior
    same-day revision made Q3/Q4 ALSO show primary_threat, the full
    counter-case, and the hard failure level — the exact same four
    facts compose_deep_read_plain() (below) shows again, labeled, right
    after this in the same message. Every overview was saying everything
    twice. The original ask was always two DISTINCT parts — a short
    read here, the depth in "Deeper read" — not the same depth rendered
    twice in two registers. This function goes back to being genuinely
    light: bias, the leg move, and ONE line of Brain's own primary_
    thesis for texture. Primary threat, counter-thesis, and the failure
    condition now live ONLY in compose_deep_read_plain() — check there
    before adding a fact back here.
    """
    cc = cc or {}
    s = story or {}
    ...
    # Q3 — What's happening now, in ONE line (Brain's own primary_thesis
    # only — NOT primary_threat/counter-case/failure-condition, which
    # would duplicate compose_deep_read_plain() below). Volatility phrase
    # is the fallback only when Brain's synthesis isn't available.
    ...
    try:
        ...
        if counter and counter.get("direction"):
            ...
            if ecc and ecc.get("direction"):
                ...
                # Fizzle history (2026-09-16 — carried over from the now-
                # removed _compose_counter_case(), the one fact in that
                # narrated version worth keeping: whether a counter-case
                # in this direction has tried and failed before). Stated
                # plainly, not as a phrase-bank sentence.
                recent_fizzles = [
                    r for r in (mu.get("counter_candidate_history") or [])[-5:]
                    if r.get("resolution") == "fizzled"
                    and (r.get("direction") or "").lower() == ecc["direction"].lower()
                ]
                if recent_fizzles:
                    line += f" (a {ecc['direction'].lower()} case has fizzled here before)"
                lines.append(line)
...
def _compose_recap(event, current_condition, mil_understanding, intent_hypothesis, story=None):
    ...
    if category == "opportunity":
        # FIXED (2026-09-16, per chat — confirmed via a production
        # screenshot: two pending opportunity events in one overview
        # rendered as byte-identical duplicate lines). Root cause: this
        # branch was reaching into intent_hypothesis for the CURRENT
        # last location, ignoring which specific pending event it was
        # actually rendering — so every opportunity event in the same
        # batch got the same text. The event already carries its own
        # correct sentence (event["headline"], captured by evaluate_
        # market_event() at the moment THAT event fired) — using it
        # directly means two different pending events render as two
        # different lines, and two pending events that really are about
        # the same zone (a genuine re-flag) now correctly show as one.
        parts = [random.choice(_RECAP_OPPORTUNITY_OPENERS)]
        parts.append(headline if headline else "a new level")
        return " ".join(parts)
