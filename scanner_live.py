    ...
    try:
        ...
        if events_to_push:
            ...
            push_text = hfis.narrate(
                events_to_push, understanding, intent_hypothesis,
                mil_understanding=state.get("mil_understanding"),
            ...
            # recap bug) — narrate_overview() renders this queue as a
            # RECAP of what's already been sent (see its own docstring),
            # so it must only ever contain things that were, in fact,
            # already sent.
            _overview_pending = state.get("overview_pending_events", [])
            _overview_pending = (_overview_pending + events_to_push)[-OVERVIEW_PENDING_EVENTS_MAX:]
...
def _build_and_send_overview(state, now_utc, header):
    ...
    try:
        ...
        # Same schema as the live push (see _evaluate_and_push_market_
        # event()'s _story above) — built fresh here since this runs on
        # its own schedule/command, not necessarily right after a push.
        _story = market_story.build_market_story(
            _understanding, mil_understanding=state.get("mil_understanding"),
            intent_hypothesis=_intent_hypothesis)

        push_text = hfis.narrate_overview(
            state.get("mil_understanding"), _understanding.get("current_condition"),
            _pending_events, _elapsed, intent_hypothesis=_intent_hypothesis,
            market_story=_story)
        ...
        # SCOPE FIX (2026-09-15, per chat — "the market overview didn't
        # change, I said it should ALSO contain the detailed thesis...
        # primary thesis, counter thesis, failure conditions"). Original
        # request read (wrongly) as "keep the overview light, put the
        # deep analysis somewhere else on demand" — built that as an
        # addition to /thesis only. Actual request was one combined push:
        # basic summary FIRST, then the in-depth analysis, back to back,
        # every time — not two separate surfaces. This used to only
        # attach deep_thesis_text inside the FALLBACK branch above (when
        # narrate_overview() produces nothing) — but narrate_overview()
        # normally succeeds every time, so that branch almost never runs,
        # and the deep section was never actually reaching the real
        # 9/12/3 push or /overview in practice. Now appended
        # unconditionally, after whichever renderer produced push_text,
        # so both branches get it consistently instead of only the rare
        # one. Degrades silently (no second section, no error) if MIL's
        # understanding isn't populated yet — same as narrate_deep_
        # thesis()'s own None-on-nothing-to-say discipline.
        # SCOPE FIX (2026-09-15 → REVISED 2026-09-16, per chat — "not as
        # sweet words exactly how it is": the user does not want this
        # section narrated at all. compose_deep_read_plain() (hfis.py)
        # replaces narrate_deep_thesis() here — same fields (primary
        # thesis, primary threat, counter thesis, failure condition,
        # what matters next), stated as plain labeled lines instead of
        # combinatorial prose. narrate_deep_thesis() itself is untouched
        # and still serves /thesis — this call site only.
        _deep_section = hfis.compose_deep_read_plain(state.get("mil_understanding"), _story)
        if _deep_section:
            push_text = push_text + "\n\n🔬 *Deeper read:*\n" + _deep_section
