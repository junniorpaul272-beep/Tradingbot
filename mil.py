...
class ThesisStatus(str, Enum):
    UNKNOWN      = "UNKNOWN"       # bootstrap, or a thesis recovering from
                                    # INVALIDATED with no fresh directional
                                    # read yet (audit item #2)
...
LEGAL_TRANSITIONS = {
    # SELF-LOOP ADDED (audit item #2): UNKNOWN now also arrives here
    # post-invalidation, not just at bootstrap — it can sit here for
    # several scans waiting on a fresh, non-stale directional read, same
    # as the four self-loops below exist for the same reason.
    ThesisStatus.UNKNOWN:       {ThesisStatus.SUPPORTED, ThesisStatus.UNKNOWN},
    # SELF-LOOPS ADDED (2026-09-10, per chat — found via real stats.json
    # showing 44 scans stuck TRANSITIONING->TRANSITIONING, then confirmed
    # via direct testing that this was only the tip of it): every one of
    # these four states was missing itself from its own legal target set.
    # That meant staying SUPPORTED for two consecutive scans in a row —
    # the single most common outcome in a healthy market, "nothing
    # changed, thesis still holds" — was ALWAYS an illegal transition,
    # every single scan, forced through the except-block fallback in
    # reconcile() and bounced through a spurious TRANSITIONING detour
    # every time. Reproduced directly: five ordinary non-stale scans in a
    # row produced SUPPORTED/TRANSITIONING/SUPPORTED/TRANSITIONING/...
    # oscillation, not five quiet SUPPORTED scans. This wasn't unique to
    # the GBPUSD incident — it's structural, and has likely been
    # corrupting unresolved_questions/history on every healthy scan since
    # this file was first built.
    #
    # Also added: SUPPORTED -> TRANSITIONING and WEAKENING -> TRANSITIONING
    # (reconcile()'s own comment already documented "SUPPORTED/WEAKENING/
    # CONTESTED all jump here on first sight of a confirmed promotion" —
    # CONTESTED already had the edge, SUPPORTED and WEAKENING did not,
    # despite the code being written as if they did). And widened
    # TRANSITIONING's own targets to include WEAKENING/CONTESTED, not just
    # INVALIDATED/SUPPORTED — needed for the real GBPUSD case: a
    # challenger that fades without confirming should be able to land
    # back on WEAKENING/CONTESTED (if the reverted-to direction is itself
    # stale) exactly the same way SUPPORTED can, not be forced through an
    # artificially narrower exit than every other non-terminal state has.
    # WIDENED (2026-09-12, per chat + advisor input — Q1: "SUPPORTED ->
    # CONTESTED directly"). Answer: yes, but only via the credibility-gated
    # path in reconcile() above (atr_ok + vs_prior_ok) — WEAKENING is a
    # condition, not a mandatory waiting period, but a low-quality
    # counter-candidate still can't skip it. See reconcile()'s own comment
    # for the full reasoning and why a second, generic "any counter
    # candidate" check couldn't be allowed to also reach this edge.
    ThesisStatus.SUPPORTED:     {ThesisStatus.SUPPORTED, ThesisStatus.WEAKENING,
                                  ThesisStatus.CONTESTED, ThesisStatus.TRANSITIONING,
                                  ThesisStatus.INVALIDATED},
    ThesisStatus.WEAKENING:     {ThesisStatus.WEAKENING, ThesisStatus.SUPPORTED,
                                  ThesisStatus.CONTESTED, ThesisStatus.TRANSITIONING,
                                  ThesisStatus.INVALIDATED},
    # ADDED (Fix Spec batch 3, item #6 — locked architecture decision,
    # not a bug fix): CONTESTED had no direct path back to SUPPORTED —
    # a counter-candidate fizzling out could only resolve via an
    # intermediate TRANSITIONING detour. Gated the same way SUPPORTED ->
    # CONTESTED is gated (see the credible_counter_candidate check above
    # and the CONTESTED_RESOLVED_NO_CHALLENGE block below reconcile()'s
    # main branch) — this edge existing legally does not mean it's
    # reachable without evidence.
    ThesisStatus.CONTESTED:     {ThesisStatus.CONTESTED, ThesisStatus.WEAKENING,
                                  ThesisStatus.TRANSITIONING, ThesisStatus.INVALIDATED,
                                  ThesisStatus.SUPPORTED},
    ThesisStatus.TRANSITIONING: {ThesisStatus.TRANSITIONING, ThesisStatus.INVALIDATED,
                                  ThesisStatus.SUPPORTED, ThesisStatus.WEAKENING,
                                  ThesisStatus.CONTESTED},
...
REQUIRES_RECONCILIATION_IF_NOT_INVALIDATED = {
    (ThesisStatus.TRANSITIONING, ThesisStatus.SUPPORTED),  # new direction without invalidation
    # ADDED (2026-09-12, per chat — Q1 fix): the credibility-gated direct
    # jump also has to explain itself on the record, same discipline as
    # every other non-invalidation transition that skips a step.
    (ThesisStatus.SUPPORTED, ThesisStatus.CONTESTED),
    # ADDED (Fix Spec batch 3, item #6): the new CONTESTED -> SUPPORTED
    # edge must explain itself the same way — a bare "nothing happened
    # this scan" default is explicitly disallowed by the spec. See the
    # CONTESTED_RESOLVED_NO_CHALLENGE block in reconcile() for what
    # satisfies this.
    (ThesisStatus.CONTESTED, ThesisStatus.SUPPORTED),
}
...
def validate_transition(transition: ThesisTransition) -> None:
    ...
    PURE. Enforces the two hard rules from the transition table:
      ...
      3. TRANSITIONING -> SUPPORTED, and SUPPORTED -> CONTESTED, are legal
         without invalidation_fired but must carry a non-empty
         reconciliation naming what actually justified the change
         (superseding_evidence non-empty AND reconciliation is not
         None/blank) — see REQUIRES_RECONCILIATION_IF_NOT_INVALIDATED. If
         neither condition is satisfiable, the caller must NOT call this
         with that (from_state, to_state) pair at all — it should instead
         leave thesis_status where it was and append to unresolved_questions.
    ...
    if ((transition.from_state, transition.to_state) in REQUIRES_RECONCILIATION_IF_NOT_INVALIDATED
            and not transition.invalidation_fired):
        if not transition.superseding_evidence or not transition.reconciliation:
            raise TransitionError(
                f"{transition.from_state} -> {transition.to_state} without "
                "invalidation firing requires non-empty superseding_evidence "
                "AND a written reconciliation. If neither is available, do "
                "not complete this transition — leave status where it was "
                "and record an unresolved_question instead of forcing a "
                "confident-sounding narrative over an ambiguous case."
            )
...
def reconcile(prev: Optional[MarketUnderstanding], observed: ObservedLegState,
    ...
    mechanical paths traced out of compute_macro_bias():
        ...
        prior thesis was INVALIDATED     -> UNKNOWN, unconditionally, on
                                              the very next reconciliation
                                              (does not wait for
                                              invalidation_fired to read
                                              False — see that branch's
                                              own comment for why)
        prior thesis is UNKNOWN          -> stays UNKNOWN until a fresh,
                                              non-stale directional bias
                                              exists, then -> SUPPORTED
                                              with that direction
        ...
        direction unchanged, invalidation
          fields set this scan            -> * -> INVALIDATED (hard gate,
                                              only path in)
        direction CHANGED this scan       -> must reconcile via one of four
                                              named causes (audit item #4):
                                              invalidation_fired=True ->
                                              INVALIDATED, cause=
                                              INVALIDATION_NEW_LEG (recovery
                                              from there follows the UNKNOWN
                                              path above, not this line).
                                              Else promotion_confirmed (and
                                              NOT macro_leg_was_choch, to
                                              rule out a stale flag riding a
                                              CHoCH leg) -> TRANSITIONING ->
                                              SUPPORTED(new direction) on
                                              the following confirming scan,
                                              cause=PROMOTION_CONFIRMED.
                                              Else macro_leg_was_choch AND
                                              NOT macro_bias_stale (proof
                                              this is the CURRENT leg's own
                                              fresh event, not an old leg's
                                              birth-certificate attribute
                                              read alongside unrelated
                                              drift) -> same TRANSITIONING
                                              -> SUPPORTED delay, cause=
                                              CHOCH_REVERSAL. Otherwise,
                                              genuinely ambiguous — do NOT
                                              force a confident narrative —
                                              leave at TRANSITIONING, cause=
                                              BIAS_FLIP (the fallback ONLY,
                                              no longer every case), and
                                              record an unresolved_question
                                              instead.

    NOTE: prev.thesis_status == INVALIDATED or UNKNOWN is handled by its
    own dedicated pair of branches BELOW, before direction_changed is even
    computed — primary_thesis["direction"] is None the entire time a
    thesis sits in either state, so direction_changed would otherwise
    always read True here and misroute into the direction-changed cascade
    further down, which has no legal edge for either state.
    """
    ...
    # INVALIDATED -> UNKNOWN (audit item #2). Unconditional, exactly one
    # scan after arriving at INVALIDATED — deliberately does NOT re-check
    # invalidation_fired. Reason: the leg-birth clearing (audit item #1)
    # only fires once a fresh confirmed leg is actually born, which can be
    # many scans after the invalidation itself — invalidation_fired can
    # keep reading True the whole time nothing has replaced the dead leg
    # yet. Gating this step on it reading False would leave the thesis
    # stuck at INVALIDATED for that entire window — the exact "permanent
    # lock" bug this fix exists to close. Nothing below this point ever
    # routes back to INVALIDATED from UNKNOWN, so this can't turn into an
    # INVALIDATED <-> UNKNOWN oscillation either.
    if prev.thesis_status == ThesisStatus.INVALIDATED:
        transition = ThesisTransition(
            from_state=ThesisStatus.INVALIDATED, to_state=ThesisStatus.UNKNOWN, at=now_iso,
            transition_cause="UNKNOWN", invalidation_fired=invalidation_fired,
            invalidation_mechanism=invalidation_mechanism,
            reconciliation=("Prior thesis was invalidated last scan; belief reset to "
                            "UNKNOWN pending a fresh, non-stale directional read."),
        )
        validate_transition(transition)
        return replace(
            prev,
            thesis_status=ThesisStatus.UNKNOWN,
            primary_thesis=dict(prev.primary_thesis),  # direction stays None
            counter_thesis=None,
            history=prev.history + [transition],
            unresolved_questions=list(prev.unresolved_questions),
            last_reconciled_at=now_iso,
        )

    # UNKNOWN -> SUPPORTED (audit item #2), only once observed.macro_bias
    # _confirmed is a real direction AND not stale — a genuinely fresh (or
    # surviving) leg standing behind it, not just the same old stale hold
    # that was already ambient before/during the invalidation. UNKNOWN ->
    # UNKNOWN otherwise (self-loop while waiting) — deliberately never
    # re-examines invalidation_fired, same reasoning as the branch above.
    if prev.thesis_status == ThesisStatus.UNKNOWN:
        fresh_direction = (
            observed.macro_bias_confirmed in ("BULLISH", "BEARISH")
            and not observed.macro_bias_stale
        )
        to_state = ThesisStatus.SUPPORTED if fresh_direction else ThesisStatus.UNKNOWN
        reconciliation = (
            f"Fresh, non-stale {observed.macro_bias_confirmed} bias confirmed — "
            "thesis re-established from UNKNOWN."
        ) if fresh_direction else None
        transition = ThesisTransition(
            from_state=ThesisStatus.UNKNOWN, to_state=to_state, at=now_iso,
            transition_cause="UNKNOWN", invalidation_fired=False,
            reconciliation=reconciliation,
        )
        validate_transition(transition)
        new_primary_thesis = dict(prev.primary_thesis)
        new_counter_thesis = prev.counter_thesis
        if to_state == ThesisStatus.SUPPORTED:
            new_primary_thesis["direction"] = observed.macro_bias_confirmed
            new_counter_thesis = None
        # HISTORY GATE (Fix Spec batch 3, item #7 — locked contract:
        # "self-loops update last_reconciled_at but do not become
        # history entries." The audit's own finding was demonstrated
        # against the main return below; UNKNOWN -> UNKNOWN is the same
        # self-loop shape (LEGAL_TRANSITIONS' own comment documents it
        # as an explicit waiting self-loop) and would silently repeat
        # the identical defect if left ungated here — extending the
        # same gate for consistency with the contract as written, not
        # a separate finding.
        new_history = (prev.history if transition.from_state == transition.to_state
                       else prev.history + [transition])
        return replace(
            prev,
            thesis_status=to_state,
            primary_thesis=new_primary_thesis,
            counter_thesis=new_counter_thesis,
            history=new_history,
            unresolved_questions=list(prev.unresolved_questions),
            last_reconciled_at=now_iso,
        )
    ...
    if not direction_changed:
        # CREDIBLE COUNTER-CANDIDATE CHECK (2026-09-12, per chat + advisor
        # input — Q1: "should SUPPORTED -> CONTESTED be a legal direct
        # jump?"). Answer: yes, but gated — "WEAKENING should describe a
        # condition, not be a mandatory checkpoint... but don't let any
        # large opposing candle cause SUPPORTED -> CONTESTED... the
        # transition should be driven by the quality of contradiction, not
        # how many scans have passed." `has_counter_candidate` alone (any
        # candidate direction present at all) is exactly the loose check
        # that would let a single big candle manufacture a fake contest —
        # the same class of problem `atr_ok`/`vs_prior_ok` already exist
        # to prevent for genuine bias reversals (see compute_macro_bias in
        # scanner_observation.py). Reused those same two gates here rather
        # than inventing a new credibility bar — a counter-candidate only
        # counts as "credible enough to contest a currently-SUPPORTED
        # thesis outright" if it would ALSO have been credible enough to
        # flip the bias outright. Deliberately scoped to the SUPPORTED-
        # source case only: WEAKENING -> CONTESTED keeps the original
        # looser check, since arriving at WEAKENING already required some
        # justified concern first — the extra bar is specifically for
        # skipping that step.
        credible_counter_candidate = bool(
            has_counter_candidate
            and observed.macro_candidate_leg_atr_ok
            and observed.macro_candidate_leg_vs_prior_ok
        )
        # DEFENSIVE CROSS-CHECK (test_negative_space.py::
        # test_stale_invalidation_evidence_does_not_masquerade_as_current,
        # audit item #1/#2). A genuine same-scan invalidation always
        # leaves compute_macro_bias() with macro_bias_stale=True (the
        # "survived" path — the only one that can set stale=False — is
        # mutually exclusive with the invalidation path within a single
        # call). So requiring bias_stale here costs nothing in the real
        # pipeline; it exists so reconcile() itself doesn't trust a stale/
        # contradictory invalidation flag against an otherwise-healthy,
        # non-stale, directionally-confirmed leg, rather than depending
        # entirely on scanner_observation.py's leg-birth clearing to keep
        # the two fields in sync.
        if invalidation_fired and observed.macro_bias_stale:
            to_state = ThesisStatus.INVALIDATED
            cause = "FAILED_CONTINUATION"
            superseding = [invalidation_mechanism] if invalidation_mechanism else []
            reconciliation = None
        elif prev.thesis_status == ThesisStatus.SUPPORTED and observed.macro_bias_stale and has_counter_candidate:
            # SUPPORTED source: CONTESTED is ONLY reachable via the
            # credible path below — a loose has_counter_candidate alone
            # caps out at WEAKENING here, same as if no candidate existed
            # at all. This is the actual enforcement of the gate; without
            # this branch existing separately, the generic
            # "macro_bias_stale and has_counter_candidate" check a few
            # lines down would catch the SUPPORTED case too and reach
            # CONTESTED regardless of credibility, silently defeating the
            # whole point of adding the stricter check above.
            if credible_counter_candidate:
                to_state = ThesisStatus.CONTESTED
                cause = "CREDIBLE_COUNTER_CANDIDATE"
                superseding = ["macro_candidate_leg_atr_ok", "macro_candidate_leg_vs_prior_ok"]
                reconciliation = (
                    "A counter-candidate appeared that clears the same "
                    "magnitude bar a genuine bias reversal would need "
                    "(ATR-relative size and size versus the prior leg) — "
                    "credible enough to contest this thesis directly, "
                    "without first passing through WEAKENING as a waiting "
                    "period."
                )
            else:
                to_state = ThesisStatus.WEAKENING
                cause = "UNKNOWN"
                superseding, reconciliation = [], None
        ...
        # RECONCILIATION FIX (2026-09-10, per chat — found via real
        # stats.json showing thesis_status stuck TRANSITIONING ->
        # TRANSITIONING for 44 consecutive scans, ~4 hours, every one
        # logging a misleadingly-worded unresolved_question ("Direction
        # changed (BEARISH -> BEARISH)" — direction had NOT changed).
        #
        # Root cause: once the challenger direction parked in
        # counter_thesis faded WITHOUT ever confirming (direction reverted
        # to matching primary_thesis, i.e. direction_changed went back to
        # False) while observed.macro_bias_stale was True, the block above
        # computed WEAKENING/CONTESTED — a target LEGAL_TRANSITIONS now
        # permits from TRANSITIONING (see the table above), but rule 3 of
        # validate_transition() only requires reconciliation when the
        # target is specifically SUPPORTED, so that part was never the
        # issue for WEAKENING/CONTESTED. It WAS the issue on the (also
        # common) branch above landing on plain SUPPORTED: validate_
        # transition() correctly rejected TRANSITIONING -> SUPPORTED
        # without a reconciliation, which this generic branch never
        # supplied, forcing the except-block fallback every time. Fixed by
        # supplying it specifically for this one combination, rather than
        # forcing to_state to always be SUPPORTED regardless of what the
        # generic computation above actually found (that was this fix's
        # first draft — overcorrected by short-circuiting staleness
        # detection entirely; this version lets WEAKENING/CONTESTED still
        # apply when warranted, now that they're legal targets).
        if (prev.thesis_status == ThesisStatus.TRANSITIONING
                and to_state == ThesisStatus.SUPPORTED
                and not invalidation_fired):
            cause = "TRANSITIONING_RESOLVED_NO_CHALLENGE"
            superseding = ["challenger_direction_faded_without_confirming"]
            reconciliation = (
                "The challenger direction parked in counter_thesis never "
                "confirmed and the bias has reverted to matching the "
                "original primary_thesis direction — TRANSITIONING "
                "resolves back to SUPPORTED."
            )

        # CONTESTED_RESOLVED_NO_CHALLENGE (Fix Spec batch 3, item #6 —
        # locked decision, mirrors TRANSITIONING_RESOLVED_NO_CHALLENGE
        # immediately above). The generic branch above only reaches
        # to_state=SUPPORTED from a CONTESTED source via its final
        # `else` (observed.macro_bias_stale is False) — has_counter_
        # candidate may still be True or False at that point, but
        # either way the condition that made this thesis contestable
        # (staleness) is gone, so the counter-candidate's contest is
        # over: either it never regained staleness-backed credibility
        # (fizzled) or it's still present but no longer backed by a
        # stale primary bias (ceased to qualify). Both are "the
        # counter-candidate's case collapsed," not "nothing happened."
        if (prev.thesis_status == ThesisStatus.CONTESTED
                and to_state == ThesisStatus.SUPPORTED
                and not invalidation_fired):
            cause = "CONTESTED_RESOLVED_NO_CHALLENGE"
            superseding = (
                ["counter_candidate_no_longer_credible"] if has_counter_candidate
                else ["counter_candidate_fizzled"]
            )
            reconciliation = (
                "The counter-candidate contesting this thesis "
                + ("is no longer backed by a stale primary bias"
                   if has_counter_candidate else "fizzled without confirming")
                + " — the original bias is no longer stale, so CONTESTED "
                "resolves directly back to SUPPORTED."
            )
        ...
        # this silently.
        #
        # CAUSAL VOCABULARY (per chat — audit item #4 design decision).
        # transition_cause used to be hardcoded to "BIAS_FLIP" for every
        # branch below, conflating two different things under one label:
        # (1) "we genuinely don't know what mechanism caused this" and
        # (2) "we know exactly what happened, we just hadn't named it."
        # Each known mechanism now gets its own cause; BIAS_FLIP is the
        # fallback ONLY for the genuinely-ambiguous case at the bottom.
        if invalidation_fired:
            ...
            cause = "INVALIDATION_NEW_LEG"
        elif observed.promotion_confirmed and not observed.macro_leg_was_choch:
            # DEFENSIVE CROSS-CHECK (test_negative_space.py::
            # test_stale_promotion_confirmed_does_not_masquerade_as_current,
            # audit item #4). A genuine 15M promotion always sets
            # macro_leg_was_choch=False in compute_macro_bias() — it's
            # explicitly NOT a CHoCH, by design (see that function's own
            # comment). A fresh 1H CHoCH-based leg-birth (was_choch=True)
            # is a DIFFERENT mechanism entirely, and item #4's leg-birth
            # clearing already stops a stale promotion_confirmed flag
            # from surviving into that scan in the real pipeline — this
            # check is the same belt-and-suspenders precaution as the
            # invalidation one above: costs nothing when the two fields
            # already agree (the normal case), stops reconcile() from
            ...
            cause = "PROMOTION_CONFIRMED"
        elif observed.macro_leg_was_choch and not observed.macro_bias_stale:
            # CHOCH_REVERSAL (per chat — audit item #4 design decision,
            # settled after test_stale_promotion_confirmed... exposed that
            # the spec was incomplete, not that the promotion fix was
            # broken). A genuine 1H CHoCH is this function's OWN top-line
            # "flip gate" ("ONLY a confirmed 1H BOS/CHoCH can change the
            # held direction" — compute_macro_bias()'s own docstring) —
            # the system's highest-authority direction-determining event,
            # not a mechanical bar that might be noise. It deserves its
            # own causal label, distinct from the generic "we don't know
            # what caused this" BIAS_FLIP fallback.
            #
            # Proof condition, not just field presence (per chat — "the
            # CHoCH path should only activate when the evidence actually
            # says this is a fresh CHoCH-originated reversal, rather than
            # merely because the leg happens to contain CHoCH-related
            # data"): macro_leg_was_choch is written in exactly two places
            # in compute_macro_bias() — freshly, from bos_1h.get(
            # "was_choch"), at a genuine 1H leg-birth; forced False at a
            # genuine 15M promotion. It is NEVER touched while a leg
            # merely survives, so on its own it proves "the CURRENTLY
            # HELD leg's birth was a CHoCH" for that leg's entire
            # lifetime — accurate, but not proof THIS scan is the fresh
            # event, vs. an old CHoCH leg's birth-certificate attribute
            # being read alongside direction drift from something
            # unrelated. macro_bias_stale=False is the same freshness
            # proof invalidation's cross-check above already relies on
            # (a genuine leg-birth/survival both set it False; nothing
            # else does) — pairing the two is the minimal corroboration
            # that this is a live, current event, not a stale artifact.
            #
            # Same one-scan confirmation delay as the promotion branch
            # above, deliberately kept — a confirmed 1H CHoCH is high-
            # authority, but this is a NAMING fix (item #4's stated
            # scope), not a separate decision to fast-track CHoCH past
            # the existing TRANSITIONING confirmation step. Revisit
            # separately if immediate SUPPORTED-on-first-sight is wanted.
            reconciliation = (
                "A confirmed 1H CHoCH reversed the held direction — "
                "compute_macro_bias()'s own flip gate, the system's "
                "highest-authority direction-determining event."
            )
            superseding = ["choch_reversal"]
            to_state = (
                ThesisStatus.SUPPORTED if prev.thesis_status == ThesisStatus.TRANSITIONING
                else ThesisStatus.TRANSITIONING
            )
            cause = "CHOCH_REVERSAL"
        else:
        ...
        transition = ThesisTransition(
            from_state=prev.thesis_status, to_state=to_state, at=now_iso,
            transition_cause=cause, invalidation_fired=invalidation_fired,
            invalidation_mechanism=invalidation_mechanism,
            superseding_evidence=superseding, reconciliation=reconciliation,
        )

    new_unresolved_questions = list(prev.unresolved_questions)
    if cause == "BIAS_FLIP":
        # FIX (per chat) — this branch's own comment above ("do NOT force
        # a confident narrative over an ambiguous case") always implied
        # surfacing that ambiguity, but nothing ever appended it. BIAS_FLIP
        # is now exclusively this one fallback (see the causal-vocabulary
        # comment above it) — every other branch in the direction-changed
        # cascade sets its own named cause instead — so gating on it here
        # can't misfire on a transition this scan's evidence actually did
        # explain.
        new_unresolved_questions.append(
            f"Direction changed ({prev.primary_thesis.get('direction')} -> "
            f"{observed.macro_bias_confirmed}) but neither invalidation, "
            "promotion, nor a fresh CHoCH reversal explains it — left at "
            f"TRANSITIONING pending clearer evidence, at {now_iso}."
        )
    try:
        validate_transition(transition)
    except TransitionError as e:
        # CONFIRMED BUG (2026-09-17, per chat — friend's Pass 7 audit,
        # traced against the actual code and verified reachable):
        # this used to unconditionally force transition.to_state =
        # TRANSITIONING here, then return it WITHOUT re-validating.
        # TRANSITIONING is only legal from 4 of the 6 states (see
        # LEGAL_TRANSITIONS above) — it is NOT legal from INVALIDATED
        # or UNKNOWN. INVALIDATED is a genuinely reachable prev.
        # thesis_status (see line ~830's thesis_status=transition.
        # to_state, and line ~815's INVALIDATED branch) — so INVALIDATED
        # -> TRANSITIONING was a real, demonstrable illegal transition
        # this exact code could produce and persist, exactly as the
        # validator it just ran was supposed to prevent. UNKNOWN shares
        # the identical defect but is currently unreachable as a prev.
        # thesis_status in this codebase (nothing ever assigns it —
        # bootstrap hardcodes SUPPORTED) — a latent landmine, not a live
        # bug today, but the same code path and same root cause, so
        # fixed here too rather than left for whenever INVALIDATED ->
        # UNKNOWN (already legal per the table) is ever actually wired up.
        #
        # MINIMAL FIX, NOT A BEHAVIOR REWRITE: keep freezing at
        # TRANSITIONING for the 4 states where it's actually legal (the
        # original, presumably-intended behavior — confirmed safe: no
        # (X, TRANSITIONING) edge is ever gated by REQUIRES_RECONCILIATION_
        # IF_NOT_INVALIDATED, so hardcoding it never trips a SECOND
        # validator rule once the edge itself is legal). Only fall back
        # to "leave thesis_status where it was" for the two states where
        # TRANSITIONING itself is illegal — which is exactly what this
        # file's OWN documented contract already specifies for an
        # unjustifiable transition (validate_transition()'s docstring:
        # "the caller must NOT call this with that pair at all — it
        # should instead leave thesis_status where it was"). Not a new
        # design decision; making the code match the contract already
        # written for it, while changing as little existing behavior as
        # the fix actually requires.
        can_freeze_at_transitioning = ThesisStatus.TRANSITIONING in LEGAL_TRANSITIONS.get(transition.from_state, set())
        new_unresolved_questions.append(
            f"Could not complete {transition.from_state.value} -> "
            f"{transition.to_state.value} ({e}); "
            + (f"frozen at TRANSITIONING, at {now_iso}."
               if can_freeze_at_transitioning
               else f"left at {transition.from_state.value}, at {now_iso}.")
        )
        if not can_freeze_at_transitioning:
            # No transition is recorded, no history entry is appended,
            # and thesis_status/primary_thesis/counter_thesis are all
            # returned exactly as they were in `prev`.
            return replace(
                prev,
                unresolved_questions=new_unresolved_questions,
                last_reconciled_at=now_iso,
            )
        transition.to_state = ThesisStatus.TRANSITIONING

    # IMMUTABILITY FIX (2026-09-12, per chat — found via testing the Q1
    # credibility gate: a 3-scenario test using the SAME bootstrapped
    # MarketUnderstanding as `prev` for two independent scenarios showed
    # scenario A's call silently changing what scenario B saw as its
    # starting thesis_status, even though scenario B never touched
    # scenario A's result. Root cause: this function was NOT pure despite
    # being documented as reconciling "prior belief" — it mutated `prev`
    # directly (prev.thesis_status = ..., prev.history.append(...), then
    # `return prev`) and handed back the SAME object it was given instead
    # of a new one. That's a real, general hazard, not just a test
    # artifact: ANY caller that keeps its own reference to the pre-
    # reconcile() object (to compare old vs new, e.g. scanner_live.py's
    # thesis-change-push feature, which needs to diff `_prev_understanding`
    # against the freshly reconciled result) would silently see them
    # collapse into the same object the instant reconcile() ran — making
    # an old-vs-new comparison always report "nothing changed," always,
    # regardless of what actually happened. Fixed by building a genuinely
    # new MarketUnderstanding via dataclasses.replace() instead — `prev`
    # is now never written to, anywhere in this function. Also copies
    # primary_thesis into a NEW dict before editing it, for the same
    # reason (a dict is mutable — replace() alone would still share the
    # original dict by reference otherwise).
    new_primary_thesis = dict(prev.primary_thesis)
    new_counter_thesis = prev.counter_thesis
    if transition.to_state == ThesisStatus.SUPPORTED:
        ...
        new_primary_thesis["direction"] = None
        ...
        new_counter_thesis = {"direction": observed.macro_bias_confirmed,
    ...
    # HISTORY GATE (Fix Spec batch 3, item #7 — locked contract).
    # Confirmed by direct execution during the audit: a self-loop scan
    # (e.g. SUPPORTED -> SUPPORTED, nothing actually changed) was still
    # appending a full ThesisTransition to history every single scan.
    # Contract: self-loops update last_reconciled_at but do not become
    # history entries — only genuine status changes do. Gated here
    # rather than upstream (e.g. skipping transition construction
    # entirely) because `transition` is still needed above for
    # unresolved_questions bookkeeping (cause == "BIAS_FLIP") even on
    # a self-loop-shaped path through the direction-changed branch.
    new_history = (prev.history if transition.from_state == transition.to_state
                   else prev.history + [transition])

    return replace(
        prev,
        thesis_status=transition.to_state,
        primary_thesis=new_primary_thesis,
        counter_thesis=new_counter_thesis,
        history=new_history,
        unresolved_questions=new_unresolved_questions,
...
def evaluate_scenarios(understanding: MarketUnderstanding,
    ...
    MarketUnderstanding via replace(). The ONLY true no-op path left is
    no counter_thesis at all — nothing exists yet to attach watching_for
    to, so there's nothing to isolate.

    STALENESS FIX (audit item #5). Used to also treat zero fresh LOCATION
    entries as a no-op, on the theory that "nothing to add" meant
    "nothing to do" — but that let a LOCATION zone MarketIntent stopped
    watching survive in counter_thesis indefinitely, since nothing ever
    told it the zone was gone. watching_for is rebuilt from
    intent_watching_for EVERY call now, empty list included — it mirrors
    the current MarketIntent, it does not accumulate past ones.
    """
