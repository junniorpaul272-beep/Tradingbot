...
from investigation_lib import (
    ...
    build_leg_intervals, find_enclosing_leg, is_gap_label,
)
...
# DERIVED DIMENSIONS (2026-08-30, per chat — user/Vally: "among exhausted
# markets, does reacceleration separate profitable trades from losing
# ones? If reacceleration isn't currently represented as a dimension,
# I'd absolutely want it added"). NOT a new capture field — computed on
# the fly from environment.volatility_state/trend_strength_atr_mult,
# both of which are ALREADY captured (see peek_environment_at_
# resolution() in min_scanner.py). Reuses brain.py's OWN canonical
# definition and threshold directly (RECENT_MOMENTUM_STRENGTH_ATR_MULT)
# rather than re-deriving a second, potentially-drifting copy of "what
# counts as reaccelerating" — brain.py is pure stdlib (see its own "no
# imports from scanner_common/scanner_observation/min_scanner" module
# rule; nothing stops OTHER files importing FROM it), so this is a
# single source of truth, not a parallel guess.
try:
    from brain import RECENT_MOMENTUM_STRENGTH_ATR_MULT
except ImportError:
    RECENT_MOMENTUM_STRENGTH_ATR_MULT = 2.0  # brain.py's current value, as of 2026-08-27 — fallback only if brain.py isn't checked out alongside this script

DERIVED_DIMENSIONS = ["is_reaccelerating"]


def _compute_is_reaccelerating(env):
    """Exact same gate as brain.py's _reaccelerating(): both conditions,
    not just one — an expanding-but-small push, or a large-but-flat
    one, isn't the specific combination this concept describes. Returns
    None (not False) when the underlying fields aren't available —
    absence preserved, not fabricated as "not reaccelerating"."""
    if not env:
        return None
    vol_state = env.get("volatility_state")
    trend_mult = env.get("trend_strength_atr_mult")
    if vol_state is None or trend_mult is None:
        return None
    return bool(vol_state == "expanding" and trend_mult >= RECENT_MOMENTUM_STRENGTH_ATR_MULT)
...
def _value_for_dimension(trades, idx, dim, continuous_buckets):
    ...
    if dim in DERIVED_DIMENSIONS:
        if dim == "is_reaccelerating":
            return _compute_is_reaccelerating(t.get("environment"))
    return None
...
def _dimension_contrast(trades, dim, continuous_buckets):
    ...
    largest expectancy gap between any two GENUINE values that BOTH
    clear MIN_GROUP_N.

    GAP VALUES EXCLUDED FROM THE COMPARISON (2026-08-30, per chat —
    user: "if [no_prior_leg] is a production gap it's quite odd it's
    being treated as a market environment" — correct catch). Values
    where is_gap_label(dim, value) is True (no_prior_leg, generic
    "unknown") are data-coverage gaps, not market readings — comparing
    them against expansion/exhaustion as if they were peers implicitly
    claims "no data" is itself a regime, which it isn't. They're
    computed and reported separately (see "gap_population" in the
    returned dict) so the coverage information isn't thrown away, but
    they can NEVER become best_value/worst_value, can never drive a
    dimension to SIGNAL/CANDIDATE_EDGE, and are excluded from confound_
    scan()/subpopulation_drilldown() for the same reason.
    ...
    for i, t in enumerate(trades):
        ...
        if val is None:
            continue
        if is_gap_label(dim, val):
            gap_rows.append(t)
        else:
            by_value[val].append(t)

    gap_population = _r_stats(gap_rows) if gap_rows else None
...
def confound_scan(trades, dim, best_value, worst_value, all_dimensions, continuous_buckets):
    ...
    # GAP-AWARE (2026-08-30, per chat): a dominant "unknown"/gap value
    # on some other_dim is meaningless as a confound signal — "both
    # groups are mostly missing this field" isn't a market-structure
    # explanation, it's just more missing data. Excluded from these
    # counts the same way _dimension_contrast() excludes gaps from its
    # own comparisons — see is_gap_label()'s docstring in
    # investigation_lib.py for why this applies generically, not just
    # to measured_move_bucket/no_prior_leg specifically.
    for other_dim in all_dimensions:
        ...
        for i in best_idx:
            ...
            if v is not None and not is_gap_label(other_dim, v):
                best_other[v] += 1
        for i in worst_idx:
            v = _value_for_dimension(trades, i, other_dim, continuous_buckets)
            if v is not None and not is_gap_label(other_dim, v):
...
def run_edge_finder_cycle(trades, dimensions=None):
    ...
    """
    dimensions = dimensions or (LEG_DIMENSIONS + ENV_CATEGORICAL + ENV_CONTINUOUS + DERIVED_DIMENSIONS)
...
def append_cycle_record(log_path, trade_count, level1_results, level2_results, scope="ALL"):
    ...
    record = {
        ...
        # ADDED (2026-08-30, per chat — Vally: "different experiments
        # may represent different policies, pooling could create fake
        # edges... I'd consider making eligibility per-experiment").
        # "ALL" is a real, distinct scope value (pooled across every
        # experiment) — not a placeholder; --experiment runs get their
        # own scope string, --all/no --experiment gets "ALL" explicitly
        # so the two are never confused when read back.
        "scope": scope,
...
# ---------------------------------------------------------------------
# PER-EXPERIMENT ELIGIBILITY (2026-08-30, per chat — Vally: "I'd
# consider making [eligibility] per-experiment... otherwise one
# experiment could accumulate data and cause another experiment to be
# repeatedly analyzed with no new evidence"). load_last_cycle_count()
# above is now scope-BLIND (reads the single most recent cycle
# regardless of what it analyzed) — kept as-is for direct single-
# experiment/single-scope CLI usage (see edge_finder.py's --experiment
# flag), but the automated multi-experiment path below needs its own
# per-scope lookup, which this section provides.
# ---------------------------------------------------------------------
...
def last_count_per_scope(edge_log_path):
    """Reads edge_finder_log.jsonl once and returns {scope: most_recent_
    trade_count} — the LAST cycle recorded for each distinct scope,
    since cycles are appended in chronological order and a later
    record for the same scope should win. {} if the log doesn't exist
    yet (first run ever, same as load_last_cycle_count())."""
    cycles = _load_jsonl_soft(edge_log_path)
    last = {}
    for c in cycles:
        last[c.get("scope", "ALL")] = c.get("trade_count", 0)
    return last


def check_eligibility_per_experiment(shadow_log_path, edge_log_path,
                                      experiments=None, min_new_trades=25):
    """
    CHEAP check — counts only, no analysis (per chat: "the cron should
    probably check eligibility per experiment... Its job is not
    necessarily to always run research — just check whether research is
    due"). Returns {experiment: {"eligible": bool, "current": int,
    "last": int, "reason": str}} for each experiment independently, so a
    scheduled workflow can decide which ones actually warrant paying for
    a full run this cycle, per Vally's explicit warning against pooling
    experiments automatically ("different experiments may represent
    different policies, and pooling them could create fake edges").
    """
    experiments = experiments or ALL_EXPERIMENTS
    all_trades = load_jsonl(shadow_log_path)
    counts_by_exp = defaultdict(int)
    for t in all_trades:
        if parse_ts(t.get("resolved_at")) is not None:
            counts_by_exp[t.get("experiment")] += 1

    last_counts = last_count_per_scope(edge_log_path)
    result = {}
    for exp in experiments:
        current = counts_by_exp.get(exp, 0)
        last = last_counts.get(exp, 0)
        eligible, reason = should_run_cycle(current, last, min_new_trades)
        result[exp] = {"eligible": eligible, "current": current, "last": last, "reason": reason}
    return result


# ---------------------------------------------------------------------
# CANDIDATE REGISTRY (2026-08-30, per chat — Vally: "I would NOT have
# the Edge Finder automatically feed questions into the Trade
# Investigation Bureau every single scan without a gate... Stops the
# same finding from repeatedly spamming the Bureau"). A single JSON
# object (not append-only JSONL like edge_finder_log.jsonl) — entries
# get UPDATED in place across cycles (first_seen/last_seen/cycles_
# observed/last_gap), which an append-only log can't represent without
# re-deriving "current state" from history every time. Same read-
# modify-write pattern as shadow_state.json/leg_obs_state.json
# elsewhere in this project, not a new persistence idiom.
# ---------------------------------------------------------------------

def load_candidate_registry(path):
    try:
        with open(path, "r") as f:
            return json.load(f)
...
def save_candidate_registry(path, registry):
    try:
        with open(path, "w") as f:
            json.dump(registry, f, indent=2, default=str)
    except Exception as e:
        print(f"[CANDIDATE REGISTRY SAVE ERROR] {e}")


def make_finding_id(scope, dimension, best_value, worst_value):
    return f"{scope}::{dimension}::{best_value}_vs_{worst_value}"


def update_candidate_registry(registry, scope, level1_results, run_at):
    """
    Updates `registry` IN PLACE given one scope's Level 1 results from
    this cycle. Returns a list of (finding_id, event_type, result_dict)
    tuples — event_type is NEW / STRENGTHENED / WEAKENED / UNCHANGED /
    DISAPPEARED. Per chat: "Then the Bureau doesn't receive the same
    question 15 times. It receives: 'New candidate investigation
    opened.' And later perhaps: 'Existing candidate strengthened.'"

    A finding only produces NEW/STRENGTHENED/WEAKENED/DISAPPEARED
    events — those are the ones worth surfacing prominently (Telegram,
    a future Bureau dispatch). UNCHANGED events are still recorded (so
    cycles_observed/last_seen stay accurate) but are exactly the
    "temporary statistical wiggle" noise Vally warned against
    surfacing every cycle — callers should treat UNCHANGED as
    background, not a notification-worthy event.

    STRENGTHENED/WEAKENED threshold: >25% relative change in gap size
    since last seen — arbitrary but stated plainly, not hidden; tune if
    it proves too noisy or too quiet in practice.
    """
    events = []
    seen_ids = set()
    for r in level1_results:
        if r["status"] not in ("CANDIDATE_EDGE", "ROBUST_CANDIDATE"):
            continue
        fid = make_finding_id(scope, r["dimension"], r["best_value"], r["worst_value"])
        seen_ids.add(fid)
        gap = r["gap"]

        if fid not in registry:
            registry[fid] = {
                "finding_id": fid, "scope": scope, "dimension": r["dimension"],
                "best_value": r["best_value"], "worst_value": r["worst_value"],
                "status": r["status"], "first_seen": run_at, "last_seen": run_at,
                "cycles_observed": 1, "last_gap": gap,
                "investigation_dispatched": False, "disappeared": False,
            }
            events.append((fid, "NEW", r))
            continue

        entry = registry[fid]
        prev_gap = entry.get("last_gap") or gap
        event_type = "UNCHANGED"
        if prev_gap and abs(gap - prev_gap) / abs(prev_gap) > 0.25:
            event_type = "STRENGTHENED" if abs(gap) > abs(prev_gap) else "WEAKENED"
        entry.update({
            "last_seen": run_at, "status": r["status"], "last_gap": gap,
            "disappeared": False,
            "cycles_observed": entry.get("cycles_observed", 1) + 1,
        })
        events.append((fid, event_type, r))

    # Findings that WERE candidates in this scope but didn't reappear
    # this cycle — flagged once (disappeared=True guards against
    # re-flagging every subsequent cycle it stays gone).
    for fid, entry in registry.items():
        if entry.get("scope") == scope and fid not in seen_ids and not entry.get("disappeared"):
            entry["disappeared"] = True
            events.append((fid, "DISAPPEARED", None))
