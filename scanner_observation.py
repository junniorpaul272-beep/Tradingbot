...
from scanner_common import (
    ...
    detect_bos_impulse,
    refine_next_event_for_display,
    INVALIDATION_RETRACE, HTF_STRUCTURE_WING, HTF_EMA_SLOPE_BARS,
    HTF_CONSOLIDATION_ATR_MULT, HTF_EMA_FLAT_THRESHOLD,
    PROMOTION_MIN_BREAK_COUNT, MIN_LEG_SIZE_ATR_MULT,
    MIN_REVERSAL_VS_PRIOR_LEG_FRACTION, SESSION_WINDOWS_UTC,
...
def _track_extreme_rejections(state, prefix, window_df, anchor_dir, anchor_extreme):
    """
    PURE. Per chat ("add rejections") — the structured field HFIS needed
    but didn't have: a persisted count of how many times price has
    tested and failed to clear the leg's own extreme, so the narrator
    can say "third time it's tried and failed at X" from a real number
    instead of needing free-text evidence to already say so.

    Deliberately narrower than MarketFacts.rejection_candle() (a
    single-candle wick/engulf pattern used for signal-firing) — this
    tracks REPEATED failure against ONE SPECIFIC LEVEL (the leg's own
    extreme) across scans, which is a different question: not "was this
    bar a rejection candle" but "has price been stuck at this level."

    A genuine test-and-fail on the latest closed bar: the bar's
    high/low reached or exceeded the CURRENT extreme intrabar, but its
    close failed to hold beyond it. Bars that never reach the extreme
    are not evidence either way — the count is untouched, not reset —
    reserving reset for either a genuine extension (close beyond it) or
    a fresh leg forming (handled at leg-birth above, not here).
    """
    if window_df.empty or anchor_dir not in ("BULLISH", "BEARISH") or anchor_extreme is None:
        return {}

    last = window_df.iloc[-1]
    if anchor_dir == "BULLISH":
        attempted = last["High"] >= anchor_extreme
        extended  = last["Close"] > anchor_extreme
        rejected_price = last["High"]
    else:
        attempted = last["Low"] <= anchor_extreme
        extended  = last["Close"] < anchor_extreme
        rejected_price = last["Low"]

    if extended:
        return {prefix + "_extreme_rejection_count": 0}
    if attempted:
        count = state.get(prefix + "_extreme_rejection_count", 0) + 1
        return {
            prefix + "_extreme_rejection_count": count,
            prefix + "_last_rejection_at": last.name.isoformat() if hasattr(last.name, "isoformat") else str(last.name),
            prefix + "_last_rejection_price": float(rejected_price),
        }
    return {}
...
def check_leg_anchor_survival(state, prefix, window_df, invalidation_retrace=INVALIDATION_RETRACE):
    ...
    if origin_violated or retrace_violated:
        # Per chat (MIL design) — this used to be computed and discarded.
        # MIL's FailureCondition/invalidation_mechanism needs to know
        # WHICH condition fired, not just that the leg died. Persisted
        # under a stable key (not prefix + "_...") so it survives the
        # anchor removal above and is readable after the fact by
        # reconcile() even once the anchor fields themselves are gone.
        # Deliberately records BOTH booleans, not just a winner label —
        # both can be true simultaneously (a violent close-through-origin
        # bar is very often ALSO >=78.6% retrace), and collapsing that to
        # one string would silently discard which conditions were
        # actually true, the exact discipline this fix exists to fix.
        return False, {
            prefix + "_direction": _REMOVE,
            prefix + "_origin": _REMOVE,
            prefix + "_extreme": _REMOVE,
            prefix + "_origin_time": _REMOVE,
            prefix + "_last_invalidation_origin_violated": bool(origin_violated),
            prefix + "_last_invalidation_retrace_violated": bool(retrace_violated),
            prefix + "_last_invalidation_at": datetime.now(timezone.utc).isoformat(),
        }
...
def compute_macro_bias(df_1h, df_15m, state):
    ...
    # Additive telemetry (per chat, 2026-09-01) — atr_now was already
    # computed here every scan and thrown away. Persisting it as
    # macro_atr_1h is what lets macro_leg_is_significant() (used by the
    # three tier fire-gates below, NOT by this function) judge leg size
    # in ATR terms without re-fetching/recomputing df_1h itself. Does
    # NOT touch macro_bias_confirmed/macro_leg_origin/macro_leg_extreme
    # or any branch below — zero risk to the direction decision.
    if not (pd.isna(atr_now) or atr_now == 0):
        updates["macro_atr_1h"] = float(atr_now)
    ...
    # SOURCE-LEVEL FIX (per chat, 2026-09-01, Vally): detect_bos_impulse()
    # is PURE and stateless — it recomputes was_choch from ONLY the
    # window of df_1h_x it's handed, with zero memory of what THIS
    # function already confirmed as dominant in a prior scan (it doesn't
    # even take `state` as a parameter). A violent move can knock its
    # own local tracking of the PRIOR dominant leg out (origin/retrace
    # invalidated → dominant=None), and if the very next confirmed break
    # goes the other way, was_choch comes back False — not because there
    # was no real prior trend, but because detect_bos_impulse's local
    # walk never re-registered one. Concrete case: a 75-pip bounce off a
    # crash low held macro_bias_confirmed == BULLISH for 37+ hours inside
    # an otherwise-bearish multi-day 1H downtrend, was_choch=False,
    # while `state` itself still held a confirmed BEARISH bias from real
    # prior history that this function simply overwrote without ever
    # checking it.
    #
    # Fix: only gate the REVERSAL case — a fresh leg whose direction
    # differs from state's already-confirmed prior bias. Same-direction
    # continuations and genuine cold starts (no prior confirmed bias at
    # all) are completely unaffected — this is deliberately NOT a
    # blanket pip/ATR filter on every leg, only on legs proposing to
    # supersede a context that's already established. "Absence of
    # context should not manufacture authority" — an unqualified
    # reversal candidate is tracked separately (macro_candidate_leg_*)
    # so Brain/thesis can still describe it honestly as a corrective/
    # countertrend move, but it does NOT overwrite macro_bias_confirmed.
    # The prior confirmed direction is preserved, matching this
    # function's own documented contract above ("a HELD direction, not
    # a fresh per-scan reading").
    _challenge_evidence = None
    _candidate_episode_written = False
    if bos_1h is not None and confirmed in ("BULLISH", "BEARISH") and bos_1h["direction"] != confirmed:
        leg_range = abs(bos_1h["impulse_end"] - bos_1h["impulse_start"])
        atr_ok = (
            not (pd.isna(atr_now) or atr_now == 0) and
            leg_range >= (MIN_LEG_SIZE_ATR_MULT * atr_now)
        )
        # Second criterion (per chat, 2026-09-01, Vally) — significance
        # relative to what this candidate is trying to supersede, not
        # just current volatility. If the prior confirmed leg's own
        # range isn't available (e.g. it predates this field being
        # tracked), this check can't run — don't let missing telemetry
        # silently make qualification easier, so it counts as NOT met
        # rather than skipped.
        prior_origin = state.get("macro_leg_origin")
        prior_extreme = state.get("macro_leg_extreme")
        prior_leg_range = (
            abs(prior_extreme - prior_origin)
            if prior_origin is not None and prior_extreme is not None else None
        )
        vs_prior_ok = (
            prior_leg_range is not None and prior_leg_range > 0 and
            leg_range >= (MIN_REVERSAL_VS_PRIOR_LEG_FRACTION * prior_leg_range)
        )
        qualifies = atr_ok and vs_prior_ok

        # ── Structural Challenge evidence (per chat) — PURE TELEMETRY,
        # zero effect on `qualifies` or the acceptance decision below.
        # Per chat: the existing atr_ok/vs_prior_ok gate answers "is this
        # move big enough," never "is this real" — a single large gap/
        # news candle can clear both magnitude checks without
        # representing genuine multi-bar conviction. Rather than guess a
        # threshold on top of this (explicitly flagged as the wrong move
        # without real data to calibrate against — same discipline
        # already used for CONVICTION_MIN_BY_TIER/EXP7 elsewhere in this
        # codebase), this records the extra evidence a future rule would
        # need, on BOTH the accepted and rejected paths, so the next
        # incident like this one is diagnosable from state.json instead
        # of being reconstructed from memory. Nothing below reads these
        # fields to gate anything yet.
        _bos_origin_idx = bos_1h.get("origin_idx")
        _bos_break_idx = bos_1h.get("break_idx")
        _bar_span = (
            (_bos_break_idx - _bos_origin_idx)
            if _bos_origin_idx is not None and _bos_break_idx is not None
            else None
        )
        _challenge_evidence = {
            "was_choch":              bos_1h.get("was_choch", False),
            "bar_span":               _bar_span,
            "campaign_direction":     state.get("campaign_direction"),
            "campaign_continuation_count": state.get("campaign_continuation_count"),
        }
        ...
        if not qualifies:
            ...
            required_atr_pips = (
                round((MIN_LEG_SIZE_ATR_MULT * atr_now) / PIP_SIZE, 1)
                if not (pd.isna(atr_now) or atr_now == 0) else None
            )
            required_prior_pips = (
                round((MIN_REVERSAL_VS_PRIOR_LEG_FRACTION * prior_leg_range) / PIP_SIZE, 1)
                if prior_leg_range is not None else None
            )
            updates.update({
                "macro_candidate_leg_direction":     bos_1h["direction"],
                "macro_candidate_leg_origin":         bos_1h["impulse_start"],
                "macro_candidate_leg_extreme":        bos_1h["impulse_end"],
                "macro_candidate_leg_origin_time":    origin_time,
                "macro_candidate_leg_was_choch":      bos_1h.get("was_choch", False),
                "macro_candidate_leg_range_pips":     round(leg_range / PIP_SIZE, 1),
                "macro_candidate_leg_required_atr_pips":   required_atr_pips,
                "macro_candidate_leg_required_prior_pips": required_prior_pips,
                "macro_candidate_leg_atr_ok":         atr_ok,
                "macro_candidate_leg_vs_prior_ok":    vs_prior_ok,
                "macro_candidate_leg_vs_confirmed":   confirmed,
                # Structural Challenge evidence (per chat) — telemetry only.
                "macro_candidate_leg_bar_span":                    _challenge_evidence["bar_span"],
                "macro_candidate_leg_campaign_direction":          _challenge_evidence["campaign_direction"],
                "macro_candidate_leg_campaign_continuation_count": _challenge_evidence["campaign_continuation_count"],
            })
            _candidate_episode_written = True
            print(f"  [BIAS] candidate reversal {bos_1h['direction']} ({leg_range / PIP_SIZE:.1f}p) "
                  f"did not qualify to supersede held {confirmed} "
                  f"[atr_ok={atr_ok} (need {required_atr_pips}p) | "
                  f"vs_prior_ok={vs_prior_ok} (need {required_prior_pips}p) | "
                  f"was_choch={_challenge_evidence['was_choch']} | bar_span={_challenge_evidence['bar_span']} | "
                  f"campaign={_challenge_evidence['campaign_direction']}x{_challenge_evidence['campaign_continuation_count']}] "
                  f"— preserving prior direction")
            # Fall through exactly as if detect_bos_impulse() had found
            # nothing this scan — the prior confirmed leg's own state
            # (origin/extreme) is untouched, so its own survival/stale
            # logic below runs completely normally against fresh price.
            bos_1h = None

    if not _candidate_episode_written:
        # Audit item #3 (per chat — locked decision: leg-owned evidence
        # clears at leg-birth; candidate episodes resolve on
        # disappearance, as their own independent lifecycle). This
        # covers every path that does NOT produce a fresh non-qualifying
        # candidate write above this scan: no reversal attempt at all,
        # the break agreeing with the held bias, or this scan's reversal
        # attempt just qualified and is becoming a real leg instead of
        # staying a candidate. All of these mean any macro_candidate_leg_*
        # evidence still sitting in state belongs to an episode that's
        # over — fizzled or promoted, either way gone — and must not be
        # read by reconcile()'s has_counter_candidate/
        # credible_counter_candidate checks as if it were still current.
        # _REMOVE on an absent key is a no-op (apply_state_updates does
        # state.pop(k, None)), so this is safe to run every scan.
        updates.update({
            "macro_candidate_leg_direction": _REMOVE,
            "macro_candidate_leg_origin": _REMOVE,
            "macro_candidate_leg_extreme": _REMOVE,
            "macro_candidate_leg_origin_time": _REMOVE,
            "macro_candidate_leg_was_choch": _REMOVE,
            "macro_candidate_leg_range_pips": _REMOVE,
            "macro_candidate_leg_required_atr_pips": _REMOVE,
            "macro_candidate_leg_required_prior_pips": _REMOVE,
            "macro_candidate_leg_atr_ok": _REMOVE,
            "macro_candidate_leg_vs_prior_ok": _REMOVE,
            "macro_candidate_leg_vs_confirmed": _REMOVE,
            "macro_candidate_leg_bar_span": _REMOVE,
            "macro_candidate_leg_campaign_direction": _REMOVE,
            "macro_candidate_leg_campaign_continuation_count": _REMOVE,
        })
    ...
    if bos_1h is not None:
        ...
        updates.update({
            ...
            # Rejection tracking (per chat — "add rejections"). A fresh
            # leg means a fresh extreme to test, so the streak resets
            # regardless of whether this was a continuation or a
            # reversal — see _track_extreme_rejections() below for where
            # this actually increments.
            "macro_leg_extreme_rejection_count": 0,
            "macro_leg_last_rejection_at": _REMOVE,
            "macro_leg_last_rejection_price": _REMOVE,
            # Audit item #1: this new leg has no relationship to whatever
            # invalidation evidence the PREVIOUS leg left behind — that
            # evidence belongs to the leg that died, not this one. Same
            # lifecycle boundary as the rejection-tracking fields above.
            "macro_leg_last_invalidation_origin_violated": _REMOVE,
            "macro_leg_last_invalidation_retrace_violated": _REMOVE,
            "macro_leg_last_invalidation_at": _REMOVE,
            # Audit item #4: a promotion belongs to the leg it promoted —
            # same reasoning, same lifecycle boundary as the invalidation
            # fields just above. macro_leg_promotion_reason travels with
            # it (same write site, same evidence pair).
            "macro_leg_promotion_confirmed": _REMOVE,
            "macro_leg_promotion_reason": _REMOVE,
        })
        # Structural Challenge evidence (per chat) — captured at leg BIRTH,
        # same pattern as macro_leg_was_choch above, but only populated
        # when this acceptance came from a qualifying REVERSAL candidate
        # (_challenge_evidence is None for a same-direction continuation
        # or a cold start, where there was no confirmed direction to
        # challenge in the first place — nothing to record). This is the
        # permanent audit trail: if a future acceptance turns out wrong
        # the way Valentine's did, state.json/leg history shows exactly
        # what evidence it was accepted on, rather than needing to be
        # reconstructed from memory. Still pure telemetry — nothing above
        # this line was changed, `qualifies` still decides acceptance
        # alone.
        if _challenge_evidence is not None:
            updates.update({
                "macro_leg_formation_bar_span":                _challenge_evidence["bar_span"],
                "macro_leg_formation_campaign_direction":       _challenge_evidence["campaign_direction"],
                "macro_leg_formation_campaign_continuation_count": _challenge_evidence["campaign_continuation_count"],
            })
            print(f"  [BIAS] ACCEPTED reversal {bias} as new confirmed direction "
                  f"[was_choch={_challenge_evidence['was_choch']} | bar_span={_challenge_evidence['bar_span']} | "
                  f"overriding campaign={_challenge_evidence['campaign_direction']}x"
                  f"{_challenge_evidence['campaign_continuation_count']}]")
    ...
    if survived:
        # Rejection tracking (per chat) — must compare against the
        # extreme's value BEFORE this scan's update, not after. If this
        # bar's own high is what just extended the extreme,
        # anchor_updates already reflects that new value — comparing
        # this bar's close against ITS OWN high would always look like
        # a "rejection" (a close can never exceed its own bar's high),
        # incorrectly flagging a genuine extension as a failure.
        _extreme_before_this_scan = state.get("macro_leg_extreme")
        updates.update(anchor_updates)
        bias = state.get("macro_leg_direction")
        updates["macro_bias_confirmed"] = bias
        updates["macro_bias_stale"] = False
        updates.update(_track_extreme_rejections(
            state, "macro_leg", df_1h_x, bias, _extreme_before_this_scan))
    ...
    # FIX (audit item #1 prerequisite): anchor_updates was being silently
    # discarded on this path. check_leg_anchor_survival() is PURE and
    # returns its dict on EVERY call, survived or not — the "survived"
    # branch above already honors that contract (updates.update(...) at
    # the top); this branch didn't, so a genuine invalidation's own
    # evidence (macro_leg_last_invalidation_origin_violated/
    # _retrace_violated/_at, plus the macro_leg_direction/_origin/
    # _extreme/_origin_time _REMOVE entries marking the dead leg's
    # identity gone) never reached apply_state_updates() on the exact
    # scan it happened. mil.ingest() could never see invalidation_fired
    # =True as a result — this is the fix that lets it.
    updates.update(anchor_updates)
    ...
    if confirmed in ("BULLISH", "BEARISH"):
        ...
        if early_bos is not None and early_bos.get("direction") not in (None, bias):
            ...
            # Per chat (MIL design) — previously only used inline to decide
            # this if-branch, then discarded. reconcile() needs this as a
            # direct persisted fact, not something inferred indirectly from
            # macro_leg_was_choch==False (which is also True in this branch
            # but conflates "not a CHoCH" with "promotion happened").
            updates["macro_leg_promotion_confirmed"] = promo_ok
            updates["macro_leg_promotion_reason"] = promo_reason
            if promo_ok:
...
def compute_macro_bias_min_leg_gate(df_1h, df_15m, state):
    """
    SHADOW ONLY — never used for trading decisions. PURE, same
    (result, updates) shape as compute_macro_bias(). Reads/writes only
    shadow_minleg_-prefixed keys.

    Gap this tests (per chat, 2026-09-01): compute_macro_bias() accepts
    ANY detect_bos_impulse() leg as macro_bias_confirmed regardless of
    the leg's size — a shallow post-spike bounce gets identical
    authority to a genuine multi-hundred-pip trend leg, because nothing
    currently checks leg magnitude before promoting it to "the ONE
    authority for direction." Concrete case that surfaced this: a
    75-pip bounce off a crash low (origin == the crash's own bottom
    candle) held macro_bias_confirmed == BULLISH for 37+ hours inside
    an otherwise-bearish multi-day 1H downtrend, because it never
    technically violated its own (small) origin or hit 78.6% retrace
    of its own (small) range.

    Identical to compute_macro_bias() in every branch EXCEPT: when a
    fresh bos_1h leg is found, its origin-to-extreme range must be
    >= MIN_LEG_SIZE_ATR_MULT * ATR_1H to be accepted. An undersized leg
    is treated as if detect_bos_impulse() had returned None for gating
    purposes this scan — falls through to leg-anchor survival / held
    prior shadow_minleg_ direction / raw EMA fallback, same as the
    "no fresh break" path below. This does NOT block the leg from
    growing across scans — if it later continues enough to clear the
    ATR multiple, a later scan accepts it normally, same size check.

    Does not modify the 15M stale-bias-promotion path — that path
    already has its own gate (_promotion_confirmed: break_count +
    EMA agreement) and wasn't implicated in the case above.
    """
    updates = {}
    df_1h_x = df_1h.copy()
    df_1h_x["EMA_100"] = df_1h_x["Close"].ewm(span=100, adjust=False).mean()
    ...
    ema_now = df_1h_x["EMA_100"].iloc[-1]
    atr_now = df_1h_x["ATR_1H"].iloc[-1]
    confirmed = state.get("shadow_minleg_bias_confirmed")
    ...
    if is_flat:
        updates["shadow_minleg_bias_confirmed"] = "CONSOLIDATION"
        updates["shadow_minleg_bias_stale"] = False
        return "CONSOLIDATION", updates

    bos_1h = detect_bos_impulse(df_1h_x, wing=HTF_STRUCTURE_WING)
    leg_big_enough = False
    if bos_1h is not None and not (pd.isna(atr_now) or atr_now == 0):
        leg_range = abs(bos_1h["impulse_end"] - bos_1h["impulse_start"])
        leg_big_enough = leg_range >= (MIN_LEG_SIZE_ATR_MULT * atr_now)

    if bos_1h is not None and leg_big_enough:
        bias = bos_1h["direction"]
        origin_idx = bos_1h.get("origin_idx")
        origin_time = (
            df_1h_x.index[origin_idx].isoformat()
        ...
        updates.update({
            "shadow_minleg_bias_confirmed": bias,
            "shadow_minleg_bias_stale": False,
            "shadow_minleg_leg_direction": bias,
            "shadow_minleg_leg_origin": bos_1h["impulse_start"],
            "shadow_minleg_leg_extreme": bos_1h["impulse_end"],
            "shadow_minleg_leg_origin_time": origin_time,
        })
        return bias, updates

    if bos_1h is not None and not leg_big_enough:
        # Leg exists structurally but is too small relative to current
        # ATR to earn macro-bias authority under this shadow rule — do
        # NOT flip/reconfirm on it. Record it as a rejected candidate
        # (visible in state.json / the shadow log) so it's obvious
        # WHY the shadow rule is diverging from live, then fall through
        # to the same survival/hold/fallback path as "no fresh break."
        updates["shadow_minleg_last_rejected_leg"] = {
            "direction": bos_1h["direction"],
            "range_pips": round(abs(bos_1h["impulse_end"] - bos_1h["impulse_start"]) / PIP_SIZE, 1),
            "required_pips": round((MIN_LEG_SIZE_ATR_MULT * atr_now) / PIP_SIZE, 1) if not pd.isna(atr_now) else None,
        }

    survived, anchor_updates = check_leg_anchor_survival(state, "shadow_minleg_leg", df_1h_x)
    if survived:
        updates.update(anchor_updates)
        updates["shadow_minleg_bias_confirmed"] = state.get("shadow_minleg_leg_direction")
        updates["shadow_minleg_bias_stale"] = False
        return state.get("shadow_minleg_leg_direction"), updates

    if confirmed in ("BULLISH", "BEARISH"):
        updates["shadow_minleg_bias_stale"] = True
        return confirmed, updates

    raw_bias = "BULLISH" if close_now > ema_now else "BEARISH"
    updates["shadow_minleg_bias_confirmed"] = raw_bias
    updates["shadow_minleg_bias_stale"] = True
    return raw_bias, updates
...
class MarketPhase:
    ...
    aging_reason: Optional[str] = None      # "break_count" / "break_count+ema_distance" / None
                                             # (2026-08-26, per chat: "ema_distance" alone is no
                                             # longer produced — dist_in_atr can only compound an
                                             # already-justified break_count read now, never trigger
                                             # EXHAUSTION on its own. See compute_market_phase()'s
                                             # inline SEMANTIC FIX comment for why.)
...
def compute_market_phase(df_1h, macro_bias, bias_stale, facts, state, extension=None,
    ...
    if macro_bias == "CONSOLIDATION":
        ...
        # SEMANTIC FIX (2026-08-26, per chat with friend — user caught this
        # from a live screenshot: fresh CHoCH, 0-1 breaks, but 2.3 ATR from
        # EMA100 was independently enough to flip phase to EXHAUSTION).
        # dist_in_atr is a LOCATION fact, not a structural-aging fact — see
        # scanner_common.py's PHASE_EXHAUSTION_EMA_DIST_ATR_MULT docstring
        # and the friend's own correction earlier in this same chat
        # thread: "Price is stretched away from its average... that's
        # legitimate supporting evidence [for a pullback]. It doesn't
        # mean the trend is exhausted." A strong FRESH move can easily
        # be 2.5+ ATR from a 100-period EMA with zero structural breaks —
        # that's normal early-expansion behavior, not aging.
        # break_count (repeated same-direction 1H BOS) is the only thing
        # that can independently justify EXHAUSTION now. dist_in_atr can
        # only COMPOUND an already-justified read (aging_reason becomes
        # "break_count+ema_distance" — both are extended AND aged, a
        # stronger signal) — it can no longer manufacture one on its own.
        # See classify_failure_risk()/format_market_thesis() for where
        # dist_in_atr is still surfaced independently — it never
        # disappears, it just stops overriding the phase label by itself.
        # Friend's open semantic question — is break_count alone really
        # "exhaustion" vs. just "structural maturity"? — is DELIBERATELY
        # left unresolved here (see this function's module-level docstring
        # note below the phase branch): fixing the dist_in_atr bug doesn't
        # require settling that, and a Phase-enum rename/redesign to split
        # "mature" from "exhausted" touches every consumer of Phase.
        # EXHAUSTION across brain.py/scanner_live.py/min_scanner.py
        # (leg_maturity, trend_health, failure_risk, EXPECTED_NEXT_EVENT_MAP,
        # narrative templates) — too large a change to bundle into this
        # fix without the user explicitly scoping that separately.
        if break_count >= break_threshold:
            aging_reason = "break_count"
            if dist_in_atr >= dist_threshold:
                aging_reason = "break_count+ema_distance"
...
def classify_failure_risk(phase, aging_reason, break_count, dist_in_atr, low_atr_warning):
    ...
    Returns (band, reasons, reasons_prose) — reasons is always populated,
    so a HIGH read is exactly as auditable as a LOW one.

    ADDED reasons_prose (2026-08-26, per chat — human-facing /understand
    and /thesis): same facts as `reasons`, phrased as a natural clause
    instead of a dev-facing "(aging threshold)"/"(exhaustion threshold)"
    fragment — same pattern as evidence_prose/weaknesses_prose just above
    in build_market_thesis(), built in lockstep so they can't drift apart.

    WORDING FIX (2026-08-26, per chat with friend — the same correction
    that removed dist_in_atr as an independent EXHAUSTION trigger in
    compute_market_phase()): the dist_in_atr reason used to say
    "approaching the exhaustion threshold," which frames EMA distance as
    inherently progressing toward exhaustion — exactly the causal story
    just established as wrong. dist_in_atr is a location/extension fact:
    legitimate support for "a pullback becomes more attractive," not "the
    leg is dying." Reworded to describe it as extension/pullback-risk
    rather than exhaustion-in-progress. This is cosmetic (the MEDIUM/HIGH
    banding logic itself is untouched — extension is still allowed to
    contribute risk, since a stretched leg failing to continue is a real
    failure mode) — only the words describing WHY changed.
    """
    if phase not in (Phase.EXPANSION, Phase.EXHAUSTION):
        return "N/A", ["No confirmed directional leg to assess yet."], ["there's no confirmed directional leg to assess yet"]
    ...
    if phase == Phase.EXHAUSTION:
        ...
        if aging_reason and "break_count" in aging_reason:
            ...
            reasons_prose.append(f"the leg is {break_count} breaks in, past the aging threshold")
        if aging_reason and "ema_distance" in aging_reason:
            reasons.append(f"{dist_in_atr:.1f} ATR from EMA_100 (exhaustion threshold)")
            reasons_prose.append(
                f"on top of that, price is {dist_in_atr:.1f} ATR from the 100 EMA, "
                f"meaningfully extended")
        if low_atr_warning:
            reasons.append("volatility below the warning floor")
            reasons_prose.append("volatility is running below the warning floor")
        if not reasons:
            reasons.append("leg held-over/stale — founding structure already invalidated")
            reasons_prose.append("the leg is held-over/stale — the structure that founded it already invalidated")
        return "HIGH", reasons, reasons_prose
    ...
    if break_count is not None and break_count >= max(
        ...
        reasons_prose.append(
            f"the leg is already {break_count} breaks in, getting close to the "
            f"{PHASE_EXHAUSTION_MIN_BREAK_COUNT}-break aging threshold")
    if dist_in_atr is not None and dist_in_atr >= PHASE_EXHAUSTION_EMA_DIST_ATR_MULT * FAILURE_RISK_APPROACHING_FRACTION:
        reasons.append(
            f"{dist_in_atr:.1f} ATR from EMA_100 — extended enough that a pullback "
            f"becomes more attractive"
        )
        reasons_prose.append(
            f"price is {dist_in_atr:.1f} ATR from the 100 EMA, extended enough that "
            f"a pullback becomes more attractive")
    if low_atr_warning:
        reasons.append("volatility below the warning floor — thin conditions can fail continuation")
        reasons_prose.append("volatility is below the warning floor, and thin conditions can fail a continuation")
    if reasons:
        return "MEDIUM", reasons, reasons_prose
    return ("LOW",
            ["fresh leg, break count and EMA distance both well inside the aging thresholds"],
            ["it's a fresh leg — break count and EMA distance are both well inside the aging thresholds"])
...
class MarketThesis:
    ...
    # ---- additive field (2026-08-27, per chat — fixing /understand's
    # repetition bug): a category tag per weaknesses/weaknesses_prose
    # entry, same index, same lockstep-append discipline as those two.
    # EXISTS SO A DOWNSTREAM CONSUMER (brain.py) CAN TELL WHEN A
    # WEAKNESS IS THE SAME UNDERLYING SIGNAL IT HAS ALREADY SYNTHESIZED
    # ELSEWHERE, WITHOUT STRING-MATCHING PROSE. Root cause this fixes:
    # leg_maturity's is_aging (brain.py) and this function's break_count/
    # dist_in_atr weaknesses both key off PHASE_EXHAUSTION_MIN_BREAK_COUNT
    # / PHASE_EXHAUSTION_EMA_DIST_ATR_MULT, but at DIFFERENT thresholds
    # (this function uses FAILURE_RISK_APPROACHING_FRACTION of the full
    # exhaustion bar). Since the fraction is <1, is_aging=True guarantees
    # (not just "often") that the break_count weakness below has also
    # fired, and both_signals guarantees the ema_extension weakness has
    # too — so brain.py was printing the same fact twice on every single
    # aging leg, not as an occasional edge case. This field lets brain.py
    # drop the categories it already said something about, while leaving
    # every other weakness (bias_stale, ob_mitigated, atr_floor) alone.
    # None-safe / order-preserving: never re-derive category from the
    # prose string, never assume list is present (older state.json).
    weakness_categories: Optional[list] = None
    # Same idea, one field later (2026-08-26, per chat): failure_risk_
    # reasons's prose twin. Built in lockstep inside classify_failure_
    # risk() itself (returns (band, reasons, reasons_prose) as one
    # tuple), not here — so this is just plumbing it through.
    failure_risk_reasons_prose: Optional[list] = None
...
def classify_thesis_delta(prev, new, direction_label=None):
    """
    ...
    DISPLAY FIX (2026-09-11, per chat — "expected next event still says
    CHoCH or range"): the comparison on expected_next_event below has
    always been (and stays) keyed on the RAW map text, for stability —
    but the bullet TEXT shown to a human now goes through
    refine_next_event_for_display() (scanner_common.py) first. This is a
    separate bug from the one fixed the same day in scanner_live.py's MIL
    call site — both independently bypassed the original 2026-08-27 fix,
    which only ever touched brain.py's own next_watch narrative. This
    file has no import access to that fix at all (brain.py's own header
    enforces a hard rule against scanner_observation importing it, or
    vice versa) — hence the separate, shared copy in scanner_common.py.
    `direction_label` is optional and purely cosmetic (only used inside
    the refined sentence template) — omitting it just renders the
    template's {d} placeholder empty, never breaks.
    """
    ...
    if prev.get("expected_next_event") != new.get("expected_next_event"):
        bullets.append(
            f"Expected next event: {refine_next_event_for_display(prev.get('expected_next_event'), direction_label) or 'none'} → "
            f"{refine_next_event_for_display(new.get('expected_next_event'), direction_label) or 'none'}"
        )
...
def build_market_thesis(phase_result, ctx, facts, macro_bias, bias_stale, state, prev=None,
    ...
    if phase == Phase.EXPANSION:
        ...
        reason_txt = {
            ...
            # "ema_distance" alone kept here defensively even though
            # compute_market_phase() no longer produces it standalone
            # (2026-08-26 fix) — cheap insurance against this dict
            # silently falling through to "held-over/stale" if that ever
            # changes again, rather than something meant to be reachable
            # today.
    ...
    if facts.has_fresh_bos_aligned_with_bias():
        ...
        evidence_prose.append("a fresh 15-minute break of structure lines up with that bias")
    if facts.has_choch_15m():
        evidence.append("15M CHoCH confirming direction")
        evidence_prose.append("a 15-minute change of character is confirming the direction")
    if facts.has_order_block():
        evidence.append("Order block confirmed, unmitigated")
        evidence_prose.append("there's a confirmed, still-unmitigated order block backing it")
    if phase_result.volatility_hint == "expanding":
        evidence.append("1H volatility expanding")
        evidence_prose.append("1-hour volatility is expanding")

    if bias_stale:
        weaknesses.append("Bias marked stale — founding leg already invalidated")
        weaknesses_prose.append("the bias is technically stale — the leg that founded it already invalidated")
        weakness_categories.append("bias_stale")
    ob = facts.order_block()
    if ob is not None and ob.get("mitigated"):
        weaknesses.append("Order block already mitigated")
        weaknesses_prose.append("the order block backing this has already been mitigated")
        weakness_categories.append("ob_mitigated")
    if ctx.very_low_atr_warning:
        ...
        weaknesses_prose.append(
            f"volatility is very low, at {ctx.current_atr_pips:.1f} pips against a {ATR_MIN_PIPS}p floor")
        weakness_categories.append("atr_floor")
    elif ctx.low_atr_warning:
        weaknesses.append(f"Low-ATR warning — {ctx.current_atr_pips:.1f}p, below the {ATR_WARN_PIPS}p floor")
        weaknesses_prose.append(
            f"volatility is running low, at {ctx.current_atr_pips:.1f} pips against a {ATR_WARN_PIPS}p floor")
        weakness_categories.append("atr_floor")
    if break_count is not None and break_count >= max(
            1, round(PHASE_EXHAUSTION_MIN_BREAK_COUNT * FAILURE_RISK_APPROACHING_FRACTION)):
        weaknesses.append(f"{break_count} breaks into this leg — getting old")
        weaknesses_prose.append(f"the leg is already {break_count} breaks in and starting to age")
        weakness_categories.append("leg_break_count")
    if dist_in_atr is not None and dist_in_atr >= PHASE_EXHAUSTION_EMA_DIST_ATR_MULT * FAILURE_RISK_APPROACHING_FRACTION:
        weaknesses.append(f"{dist_in_atr:.1f} ATR from EMA_100 — extended")
        # WORDING FIX (2026-09-14, per chat — "why is it giving me the
        # calculations 4.4 ATR, like what am I doing with that"). The old
        # prose here ("price is 4.4 ATR away from the 100 EMA, meaningfully
        # extended") technically satisfied the 2026-08-26 "phrase it as a
        # natural clause" bar — it's a grammatical sentence — but never
        # translated what the number MEANS. Leads with the plain-English
        # read (stretched a long way from its own average, the kind of
        # extension that tends to run out of room) and keeps the real
        # multiple as supporting detail rather than the entire point — no
        # invented threshold, no new claim, same underlying fact.
        weaknesses_prose.append(
            f"price has stretched a long way from the 100 EMA relative to "
            f"its usual range ({dist_in_atr:.1f}x ATR) — legs extended that "
            f"far tend to be running low on room"
        )
        weakness_categories.append("ema_extension")
    ...
    # ---- failure risk ----------------------------------------------------
    failure_risk, failure_risk_reasons, failure_risk_reasons_prose = classify_failure_risk(
    ...
    try:
        ...
        _direction_label = macro_bias.title() if macro_bias in ("BULLISH", "BEARISH") else None
        delta = classify_thesis_delta(prev, new_snapshot, _direction_label)
    except Exception:
        delta = None
    ...
    return MarketThesis(
        ...
        weaknesses_prose=weaknesses_prose,
        weakness_categories=weakness_categories,
        expected_next_event=expected_next_event,
        failure_risk=failure_risk,
        failure_risk_reasons=failure_risk_reasons,
        failure_risk_reasons_prose=failure_risk_reasons_prose,
...
class WatchCode(str, Enum):
    """Closed, enumerated vocabulary of things Market Intent can say it's
    watching FOR. Adding a new one is a deliberate code change, not a
    free-text option — same discipline as TransitionCause / Phase."""
    PULLBACK_TO_OB          = "PULLBACK_TO_OB"
    PULLBACK_TO_FIB_POCKET  = "PULLBACK_TO_FIB_POCKET"
    SWEEP_AND_RECLAIM       = "SWEEP_AND_RECLAIM"
    FRESH_DISPLACEMENT      = "FRESH_DISPLACEMENT"
    FIVE_M_ALIGNMENT        = "FIVE_M_ALIGNMENT"


class CautionCode(str, Enum):
    """Closed, enumerated vocabulary of things Market Intent is
    deliberately NOT interested in right now. Same discipline as
    WatchCode above — a caution is a judgment about setup QUALITY
    ("would this make sense"), which is the one place Market Intent is
    explicitly allowed to editorialize (unlike the Market Thesis
    Engine's extension/campaign fields, which stay strictly descriptive
    — see build_market_thesis()'s own docstring on why). What it must
    NOT do is attach a probability or conviction number to that
    judgment; every sentence below is qualitative on purpose."""
    CHASING_MID_RANGE  = "CHASING_MID_RANGE"
    AGING_CAMPAIGN     = "AGING_CAMPAIGN"
    UNSWEPT_CHOCH      = "UNSWEPT_CHOCH"
    ATR_INCOMPATIBLE   = "ATR_INCOMPATIBLE"


class IntentRole(str, Enum):
    """Per chat (2026-08-24, brain.py Market Intent redesign): a closed
    semantic role for every WatchCode/CautionCode, decided HERE — where
    the vocabulary itself is owned — rather than in brain.py.

    WHY THIS LIVES HERE, NOT IN brain.py: brain.py's own file-level rule
    is that it imports nothing from this module (or any scanner file)
    and only ever consumes facts WorldState already contains. A role
    lookup table keyed on WatchCode/CautionCode would either force
    brain.py to import that enum (breaking the boundary) or duplicate
    the table in two files that could silently drift apart as codes are
    added. Tagging the role onto each watching_for/not_interested_in
    entry AT CREATION TIME (see build_market_intent() below) avoids
    both: the role becomes a WorldState fact like any other, and Brain
    just groups by it.

    LOCATION      — a candidate zone/place where a scenario could be
                    tested (PULLBACK_TO_OB, PULLBACK_TO_FIB_POCKET).
                    Multiple LOCATION codes CAN be open at once (e.g.
                    both an OB and a Fib pocket unmitigated
                    simultaneously) — per chat, Brain must present these
                    as alternatives, never silently pick one as "the"
                    scenario. No priority order is declared here on
                    purpose; if a real case ever needs one, it must be
                    added explicitly to this docstring, not inferred
                    from enum/list order.
    CONFIRMATION  — a condition that, if it materializes, would
                    strengthen whatever scenario is currently being
                    watched (SWEEP_AND_RECLAIM, FRESH_DISPLACEMENT,
                    FIVE_M_ALIGNMENT). These are not mutually exclusive
                    either — aging_phase alone can open both
                    SWEEP_AND_RECLAIM and FRESH_DISPLACEMENT together
                    (see build_market_intent() below) — so CONFIRMATION
                    is always presented as a list, never collapsed to
                    one.
    CAUTION       — every CautionCode. Independent of LOCATION/
                    CONFIRMATION; not a competing interpretation, a
                    parallel one (e.g. "here's what would confirm
                    continuation" and "here's why the current extension
                    is already suspect" can both be true at once).

    THE RULE THIS ENFORCES: Brain may GROUP by role (that's a stated
    fact, not a guess). Brain may NOT decide that one LOCATION code
    outranks another, or that a LOCATION code is "the scenario" and a
    CONFIRMATION code is "how it resolves," beyond what this table
    already says — that would be inventing a relationship the tracking
    layer never asserted, the exact mistake this table exists to
    prevent."""
    LOCATION     = "LOCATION"
...
WATCH_CODE_ROLE = {
    WatchCode.PULLBACK_TO_OB.value:         IntentRole.LOCATION.value,
    WatchCode.PULLBACK_TO_FIB_POCKET.value: IntentRole.LOCATION.value,
    WatchCode.SWEEP_AND_RECLAIM.value:      IntentRole.CONFIRMATION.value,
    WatchCode.FRESH_DISPLACEMENT.value:     IntentRole.CONFIRMATION.value,
    WatchCode.FIVE_M_ALIGNMENT.value:       IntentRole.CONFIRMATION.value,
}
# CautionCode has exactly one role — every caution is CAUTION — so no
# per-code table is needed; not_interested_in entries are tagged
# directly with IntentRole.CAUTION.value in build_market_intent().


@dataclass
class MarketIntent:
    """Per chat, 2026-08-13 ("shower idea"): the answer to "given what
    I'm seeing right now, what kind of setup would I actually be
    interested in?" — NOT a signal, NOT a prediction, NOT conviction.
    See build_market_intent()'s docstring for the full contract."""
    has_content: bool
    watching_for: list          # [{"code","sentence","zone_low","zone_high"}]
    not_interested_in: list     # [{"code","sentence"}]
    reasons: list                # reused directly from thesis.evidence + thesis.weaknesses
    narrative: str
    phase: Optional[str] = None
    macro_bias: Optional[str] = None


def build_market_intent(phase_result, ctx, facts, thesis, macro_bias):
    """
    PURE. Answers a different question than either the Market Thesis
    Engine or the Tiers, per chat 2026-08-13:
      Market Thesis  -> "what is the market doing, and why?"
      Market Intent  -> "given that, what kind of setup would make sense?"
      Tiers          -> "does an ACTUAL defined setup exist right now?"
    A market can fail every tier threshold while still having an
    obvious location worth waiting for — that gap is the entire reason
    this function exists.

    Runs where the Market Thesis Engine runs — before
    evaluate_rule_of_law() ever executes — and reads ONLY
    MarketPhase / MarketContext / MarketFacts / MarketThesis. Never a
    tier function, never a tier RESULT. This is the same timing
    constraint build_market_thesis() documents on itself (Tier/Evidence/
    RiskGate output doesn't exist yet at this point in the scan) — here
    it is load-bearing rather than incidental: it is what lets
    run_market_intent_tracking() (min_scanner.py) later check whether a
    watched-for condition actually materialized with zero risk of this
    function having peeked at which tier eventually fired. "Because it's
    generated before the tiers, it can't cheat by seeing which setup
    eventually fires" — direct requirement from chat.

    Closed, enumerated vocabulary only (WatchCode / CautionCode above) —
    same discipline as PHASE_TRANSITION_CAUSES / EXPECTED_NEXT_EVENT_MAP:
    every condition below is a fixed check against a fact that ALREADY
    exists on MarketFacts/MarketContext/MarketPhase/MarketThesis. No new
    detector, no new threshold, no freeform-generated text. A market
    that matches nothing in the vocabulary returns has_content=False —
    "nothing specific to watch for" is a valid, plain answer, not a gap
    to be papered over with a forced description.

    NEVER raises — caller (scan()) wraps this the same as every other
    narrative layer; a bug here must not affect anything Tiers/Risk
    Gate/Trade Management read.
    """
    macro_bias_word = "bullish" if macro_bias == "BULLISH" else "bearish"
    zone_word = "demand" if macro_bias == "BULLISH" else "supply"
    ...
    # ---- WATCH FOR ----------------------------------------------------
    ob = facts.order_block()
    ob_live = facts.has_order_block()
    if ob_live and not facts.price_in_order_block():
        # WORDING FIX (2026-09-15, per chat — friend's second review:
        # "rather than chasing price in the middle of the range" is the
        # detector explaining its own classification logic, not a market
        # fact — and the sentence never even cited the actual zone
        # bounds, which sit right there in ob["low"]/ob["high"]. Both
        # fixed: real levels now in the sentence, meta-justification
        # clause dropped.
        watching_for.append({
            "code": WatchCode.PULLBACK_TO_OB.value,
            "role": WATCH_CODE_ROLE[WatchCode.PULLBACK_TO_OB.value],
            "sentence": (
                f"A pullback into the {macro_bias_word} order block "
                f"({zone_word} zone) at {ob['low']:.5f}\u2013{ob['high']:.5f}."
            ),
            "zone_low": ob["low"], "zone_high": ob["high"],
        })

    fib_bounds = facts.fib_pocket_bounds()
    fib_live = fib_bounds is not None and not facts.price_in_fib_pocket()
    if fib_live:
        low, high = fib_bounds
        # WORDING FIX (2026-09-11, per chat — "HTF fib pocket isn't really
        # much to go by", plus this entry never said bullish/bearish at
        # all, unlike its sibling PULLBACK_TO_OB right above, which
        # already includes macro_bias_word/zone_word). Three specific
        # complaints, all addressed here rather than downstream: (1) no
        # direction stated — added, matching PULLBACK_TO_OB's pattern;
        # (2) "HTF fib pocket" names the zone but not what it structurally
        # IS relative to what a reader already knows (an order block) —
        # spelled out as a Fibonacci retracement zone inside the current
        # leg, explicitly distinguished from a fresh zone; (3) no stated
        # condition for what would actually count as the reaction firing
        # — added the real one (a rejection candle forming while price is
        # still inside these bounds), matching what _tier2_fib_evaluate()
        # actually checks, not an invented "close above/below" framing
        # that isn't what the code does.
        #
        # WORDING FIX #2 (2026-09-15, per chat — friend's second review:
        # "not a fresh {zone_word} zone — rather than chasing price in
        # the middle of the range" is the SAME meta-justification problem
        # as PULLBACK_TO_OB above, just still present here — replaced
        # with "retracement of the current leg" (states what the zone
        # actually IS relative to the move, a market fact, not why the
        # detector picked it) and the real price bounds are now in the
        # sentence instead of only sitting in zone_low/zone_high unused.
        watching_for.append({
            "code": WatchCode.PULLBACK_TO_FIB_POCKET.value,
            "role": WATCH_CODE_ROLE[WatchCode.PULLBACK_TO_FIB_POCKET.value],
            "sentence": (
                f"A {macro_bias_word} retracement into the Fibonacci "
                f"pullback zone of the current leg, around "
                f"{low:.5f}\u2013{high:.5f}. What would count as a "
                f"reaction: a {macro_bias_word} rejection candle forming "
                f"while price is still inside that zone."
            ),
            "zone_low": low, "zone_high": high,
        })

    aging_phase = phase_result is not None and phase_result.phase in (Phase.EXHAUSTION, Phase.TRANSITION)

    if aging_phase and phase_result.swept_boundary is None:
        watching_for.append({
            "code": WatchCode.SWEEP_AND_RECLAIM.value,
            "role": WATCH_CODE_ROLE[WatchCode.SWEEP_AND_RECLAIM.value],
            "sentence": (
                "A sweep and reclaim of the nearest untaken local liquidity "
                "before treating any reversal as meaningful evidence."
            ),
            "zone_low": None, "zone_high": None,
        })

    if aging_phase and not facts.has_fresh_bos_aligned_with_bias():
        watching_for.append({
            "code": WatchCode.FRESH_DISPLACEMENT.value,
            "role": WATCH_CODE_ROLE[WatchCode.FRESH_DISPLACEMENT.value],
            "sentence": (
                f"Fresh {macro_bias_word} displacement — a new BOS in the "
                f"current direction — to re-confirm continuation rather "
                f"than assuming the leg is still healthy."
            ),
            "zone_low": None, "zone_high": None,
        })

    mtf_5m = thesis.mtf_5m if thesis else None
    if mtf_5m and mtf_5m.get("m5_direction") and mtf_5m["m5_direction"] != macro_bias:
        watching_for.append({
            "code": WatchCode.FIVE_M_ALIGNMENT.value,
            "role": WATCH_CODE_ROLE[WatchCode.FIVE_M_ALIGNMENT.value],
            "sentence": (
                "5M structure to align with the 15M read before treating "
                "any reaction as confirmed."
            ),
            "zone_low": None, "zone_high": None,
        })

    # ---- NOT INTERESTED IN ---------------------------------------------
    if not ob_live and not fib_live:
        # No defined zone within reach at all — whatever price is doing
        # right now is, by definition, happening away from a location.
        not_interested_in.append({
            "code": CautionCode.CHASING_MID_RANGE.value,
            "role": IntentRole.CAUTION.value,
            "sentence": "Chasing price away from any defined zone, in the middle of the current range.",
        })

    campaign = thesis.campaign if thesis else None
    if campaign and campaign.get("continuation_count", 0) >= 2 and phase_result.phase == Phase.EXHAUSTION:
        not_interested_in.append({
            "code": CautionCode.AGING_CAMPAIGN.value,
            "role": IntentRole.CAUTION.value,
            "sentence": (
                f"Buying/selling the current expansion outright — this "
                f"campaign is already {campaign['continuation_count']} "
                f"continuation leg(s) deep."
            ),
        })

    if facts.has_choch_15m() and phase_result is not None and phase_result.swept_boundary is None:
        not_interested_in.append({
            "code": CautionCode.UNSWEPT_CHOCH.value,
            "role": IntentRole.CAUTION.value,
            "sentence": "Treating the current CHoCH as strong reversal evidence — no liquidity sweep preceded it.",
        })

    if ctx.low_atr_warning or ctx.very_low_atr_warning:
        not_interested_in.append({
            "code": CautionCode.ATR_INCOMPATIBLE.value,
            "role": IntentRole.CAUTION.value,
            "sentence": "A wide invalidation here — current ATR is compressed relative to a typical structural stop.",
        })

    has_content = bool(watching_for) or bool(not_interested_in)

    reasons = []
    if thesis is not None:
        reasons = list(thesis.evidence or []) + list(thesis.weaknesses or [])

    phase_label = phase_result.phase.value if phase_result is not None else None
    narrative = stitch_market_intent_narrative(
        macro_bias_word, phase_label, watching_for, not_interested_in)
...
def stitch_market_intent_narrative(macro_bias_word, phase_label, watching_for, not_interested_in):
    """PURE. Same fixed-template stitching discipline as stitch_narrative()
    / stitch_signal_narrative() above — concatenates the already-built
    sentence list; does not generate new text or judge anything itself."""
    if not watching_for and not not_interested_in:
        return (
            f"Nothing specific to watch for right now — {phase_label or 'current'} "
            f"{macro_bias_word} conditions with no notable location or caution to flag."
        )
    parts = []
    if watching_for:
        parts.append("What I'd be watching for: " + " ".join(w["sentence"] for w in watching_for))
    if not_interested_in:
        parts.append("Not interested in: " + " ".join(c["sentence"] for c in not_interested_in))
    return " ".join(parts)
...
# =========================================================================
# SUPPORT / RESISTANCE — shadow-only, EXP3 slot reuse. Per chat,
# 2026-08-15. Two PURE functions: discovery (birth a new candidate) and
# update (advance one tracked level by one scan). Both take everything
# they need as arguments and return data — the STATEFUL part (a
# persistent list of tracked levels across scans) lives in min_scanner.py
# (run_sr_tracking()), same split as everywhere else in this file: this
# module computes, min_scanner.py persists.
# =========================================================================

def discover_sr_candidates(facts, phase_result):
    """
    PURE. Birth condition: a leg that has been aging (EXHAUSTION or
    TRANSITION, per the Market Phase Engine) for at least
    SR_MIN_CONTAINMENT_BARS bars IS, by construction, a leg whose
    swing_high and swing_low have both held — that's what "aging
    without a fresh break" already means in Phase Engine terms. No new
    pivot-detection math: reuses facts.swing_high / facts.swing_low (the
    leg's own already-computed extremes) and phase_result.age_bars
    (already computed) rather than reinventing a two-sided-rejection
    detector from raw candles.

    Deliberately NOT range-restricted — EXHAUSTION/TRANSITION happen
    inside trends too (see the Phase enum's own docstring: EXHAUSTION is
    "still trending... but aging/extended"). This is exactly the literal
    example from chat: price trending down, failing to take out the low
    AND failing to take out the high, is a two-sided rejection around
    swing_low/swing_high — not a range.

    Returns a list of 0-2 candidate dicts (resistance from swing_high,
    support from swing_low), zone-sized off current 15M ATR — a zone,
    never an exact price, same adaptive-sizing pattern as
    fib_pocket_bounds(). Caller (run_sr_tracking()) is responsible for
    dedup against already-tracked levels; this function has no state and
    doesn't know what's already being tracked.
    """
    if phase_result is None or phase_result.phase not in (Phase.EXHAUSTION, Phase.TRANSITION):
        return []
    if phase_result.age_bars is None or phase_result.age_bars < SR_MIN_CONTAINMENT_BARS:
        return []

    atr_now = facts.current_atr_15m()
    if not atr_now or pd.isna(atr_now):
        return []
    half_width = SR_ZONE_ATR_MULT * atr_now

    candidates = []
    if facts.swing_high is not None:
        candidates.append({
            "role": "RESISTANCE",
            "anchor_price": float(facts.swing_high),
            "zone_low": float(facts.swing_high - half_width),
            "zone_high": float(facts.swing_high + half_width),
        })
    if facts.swing_low is not None:
        candidates.append({
            "role": "SUPPORT",
            "anchor_price": float(facts.swing_low),
            "zone_low": float(facts.swing_low - half_width),
            "zone_high": float(facts.swing_low + half_width),
        })
    return candidates


def update_sr_level(level, facts, macro_bias, bar_index):
    """
    PURE. Advances ONE tracked level by one scan. Returns
    (updated_level_dict, event) where event is one of:

      None              — nothing changed. Includes the case that
                           matters most for this design: price touched,
                           or even fully wicked through, the zone
                           WITHOUT a 15M CLOSE beyond it. Mitigation
                           doesn't kill an S/R level the way it kills an
                           OB — a touch alone never changes state, only
                           a decisive close does (see `broke_up`/
                           `broke_down` below).
      "FLIPPED"         — a 15M candle closed decisively beyond the
                           zone, the cooldown was clear, role swapped
                           (RESISTANCE<->SUPPORT). flip_count += 1.
      "RETEST_REACTION" — price has returned to a level that flipped in
                           a BIAS-ALIGNED direction and produced an
                           actual rejection candle there — this is the
                           one tradeable moment this whole detector
                           exists to find (see run_sr_tracking(),
                           min_scanner.py, for what happens to this
                           event). Never fires on a level's ORIGINAL
                           role, only after it has flipped at least
                           once — an untested S/R level being "at
                           support" isn't the entry; the flip having
                           already proven the market accepted the new
                           side is what makes the retest meaningful.
      "INVALIDATED"     — terminal. Either the flip cap (SR_MAX_FLIPS)
                           was exceeded, or the level aged past
                           SR_MAX_AGE_CANDLES with nothing happening.

    Level dict shape (all fields caller-owned, this function only reads/
    copies them): {id, role, zone_low, zone_high, anchor_price,
    flip_count, last_flip_bar, born_bar, state, retested_since_flip}.
    Never mutates the input dict — always returns a new one.
    """
    if level["state"] == "INVALIDATED":
        return level, None

    if bar_index - level["born_bar"] > SR_MAX_AGE_CANDLES:
        return {**level, "state": "INVALIDATED"}, "INVALIDATED"

    c = facts.last_candle_15m()
    close = float(c["Close"])
    zone_low, zone_high = level["zone_low"], level["zone_high"]
    cooldown_clear = (level["last_flip_bar"] is None or
                       (bar_index - level["last_flip_bar"]) >= SR_FLIP_COOLDOWN_BARS)

    broke_up = level["role"] == "RESISTANCE" and close > zone_high
    broke_down = level["role"] == "SUPPORT" and close < zone_low

    if (broke_up or broke_down) and cooldown_clear:
        flip_count = level["flip_count"] + 1
        if flip_count > SR_MAX_FLIPS:
            return {**level, "state": "INVALIDATED"}, "INVALIDATED"
        new_role = "SUPPORT" if broke_up else "RESISTANCE"
        updated = {
            **level, "role": new_role, "flip_count": flip_count,
            "last_flip_bar": bar_index, "state": "ACTIVE",
            "retested_since_flip": False,
        }
        return updated, "FLIPPED"

    in_zone = zone_low <= close <= zone_high
    if level["flip_count"] >= 1 and not level.get("retested_since_flip") and in_zone:
        bias_aligned = (
            (level["role"] == "SUPPORT" and macro_bias == "BULLISH") or
            (level["role"] == "RESISTANCE" and macro_bias == "BEARISH")
        )
        if bias_aligned and facts.rejection_candle(macro_bias):
            return {**level, "retested_since_flip": True}, "RETEST_REACTION"
...
class MarketFacts:
    ...
    def last_candle_15m(self):
        """Added for the S/R tracker (per chat, 2026-08-15) — breaks are
        evaluated on a 15M close, same timeframe BOS/CHoCH already use,
        rather than 5M noise. Mirrors last_candle_5m() exactly."""
        return self.df_15m.iloc[-1]
    ...
    def current_atr_15m(self):
        """Added for the S/R tracker — zone sizing needs 15M ATR, same
        series atr_percentile_15m() below already reads; mirrors
        current_atr_5m() exactly, just the other timeframe."""
        return self._atr_15m_series.iloc[-1]

    def atr_15m_series(self):
        """Public accessor for the full 15M ATR series — added so
        build_zone_observations() (Research Lab groundwork, per chat
        2026-09-18) doesn't have to reach into self._atr_15m_series
        from outside the class or recompute atr(df_15m) a second time.
        Read-only by convention (callers must not mutate the returned
        Series in place — same expectation as every other facts.*
        accessor on this class)."""
        return self._atr_15m_series
    ...
    # FVG facts — promoted from Shadow Pipeline Experiment 3 (POI) to a
    # live Tier 1 fact, per chat 2026-08-14. Previously this exact spot
    # said FVG was "intentionally NOT exposed" — Tier 1 traded the Order
    # Block only, and FVG reactions were shadow-only research (EXP3_POI's
    # variant="fvg" rows). That research is now graduating: the live bot
    # can react to a quality-filtered FVG the same way it reacts to an
    # OB, using the IDENTICAL calculation EXP3 already validated —
    # detect_significant_fvg(), zero new math. See _tier1_poi_evaluate()
    # for how this and order_block() combine (OB first, FVG as a
    # fallback POI when no OB reaction is live).
    #
    # Confirms, for the record, a question raised while planning this
    # move: detect_order_block() has NO dependency on FVG presence at
    # all — it is derived purely from the BOS impulse candle (see
    # detect_order_block()'s own docstring). The two have always been
    # fully independent detectors; exposing FVG here does not change OB
    # criteria in any way.
    def significant_fvg(self):
        """Lazy-cached. Returns the nearest/most-recent quality-filtered
        FVG (same selection EXP3_POI used: quality_gaps[-1]) or None.
        "Quality" = detect_significant_fvg()'s existing size/freshness
        filter (FVG_MIN_SIZE_ATR_MULT / FVG_MAX_AGE_CANDLES) — unchanged,
        not loosened or tightened for this move."""
        if self._fvg_cache == "UNSET":
            gaps = detect_significant_fvg(self.df_15m, self.macro_bias, self._atr_15m_series)
            self._fvg_cache = gaps[-1] if gaps else None
        return self._fvg_cache

    def has_significant_fvg(self):
        return self.significant_fvg() is not None

    def _fvg_distance_pips_raw(self):
        """Unrounded core — same boundary-safety reasoning as
        _ob_distance_pips_raw()/_fib_distance_pips_raw()."""
        gap = self.significant_fvg()
        if gap is None:
            return None
        c = self.last_candle_5m()
        if c["Low"] > gap["high"]:
            return (c["Low"] - gap["high"]) / PIP_SIZE
        if c["High"] < gap["low"]:
            return (gap["low"] - c["High"]) / PIP_SIZE
        return 0.0

    def fvg_distance_pips(self):
        d = self._fvg_distance_pips_raw()
        return None if d is None else round(d, 1)

    def price_in_fvg(self, tolerance_pips=ZONE_TOLERANCE_PIPS):
        """Same zone-tolerance discipline as price_in_order_block() —
        reuses the shared ZONE_TOLERANCE_PIPS constant rather than
        inventing an FVG-specific one."""
        d = self._fvg_distance_pips_raw()
...
# =========================================================================
def build_structure_digest(facts):
    """
    PURE. WorldState 1B, structure.system_observation — the "sensor
    layer" (per chat, 2026-08-20). STRICTLY OBSERVATIONAL: flat, typed
    primitives read off the SAME `facts` (MarketFacts) instance the live
    tiers already evaluated against this scan — no new detection, no
    recomputation, no interpretation, no confidence/probability fields
    (those belong to Brain/Evidence, not here — see project overview §9
    review). Nothing reads this output to make a decision; it exists
    purely so this scan's MarketFacts stops being thrown away at the end
    of the scan, per the roadmap's long-standing structure.system_observation
    placeholder.

    Deliberately narrow for v1 (per chat): STRUCTURE + LOCATION +
    LIQUIDITY sensors only. Movement (choch_magnitude, ATR ratio) and
    pressure (rejection_metrics) sensors are explicitly deferred until
    this narrower slice has been checked against real charts — the
    stated goal is proving the pattern works before widening it, not
    exposing every MarketFacts method that exists.

    Every field here is a measurement or an event, never a label like
    "renewed_buying_pressure" — that kind of concept belongs to Brain,
    built FROM these primitives, not computed here.

    Returns a plain dict. Never raises: any single sensor failing (e.g.
    a level-dependent liquidity calc on a degenerate swing range) is
    caught locally and reported as None, not allowed to take down the
    whole digest — same "never fabricate, prefer None" discipline used
    ...
    bos = _safe(facts.bos_15m)

    structure = {
        "bos_direction":            bos["direction"] if bos else None,
        "bos_aligned_with_bias":    _safe(facts.has_fresh_bos_aligned_with_bias, False),
        "choch_detected":           _safe(facts.has_choch_15m, False),
    }

    ob = _safe(facts.order_block)

    location = {
        "order_block_present":     _safe(facts.has_order_block, False),
        "order_block_mitigated":   bool(ob.get("mitigated", False)) if ob else None,
        "price_in_order_block":    _safe(facts.price_in_order_block, False),
        "ob_distance_pips":        _safe(facts.ob_distance_pips),
        "ob_penetration_pct":      _safe(facts.ob_penetration_pct),
        "fvg_present":             _safe(facts.has_significant_fvg, False),
        "price_in_fvg":            _safe(facts.price_in_fvg, False),
        "fvg_distance_pips":       _safe(facts.fvg_distance_pips),
        "price_in_fib_pocket":     _safe(facts.price_in_fib_pocket, False),
        "fib_pocket_distance_pips": _safe(facts.fib_pocket_distance_pips),
    }

    # Liquidity is parametrized on a level (per MarketFacts' own design —
    # a level is a fact input, not a hardcoded field). The two levels
    # that mean something at the "what does the market currently know"
    # level, without reaching for tier-specific context, are the same
    # swing_high/swing_low facts already form the structural range.
    sweep_high = _safe(lambda: facts.has_liquidity_sweep(facts.swing_high), False)
    sweep_low = _safe(lambda: facts.has_liquidity_sweep(facts.swing_low), False)

    liquidity = {
        "swing_high_swept":        sweep_high,
        "swing_high_sweep_distance_pips":
            _safe(lambda: facts.sweep_distance_pips(facts.swing_high)) if sweep_high else None,
        "swing_low_swept":         sweep_low,
        "swing_low_sweep_distance_pips":
            _safe(lambda: facts.sweep_distance_pips(facts.swing_low)) if sweep_low else None,
    }
...
def build_zone_observations(facts):
    """
    PURE. Research Lab groundwork (per chat, 2026-09-18). Returns a
    list of flat dicts, one per FVG/order-block zone MarketFacts has
    direct evidence for THIS scan.

    Deliberately NOT filtered to macro_bias, unlike facts.
    significant_fvg() (which only exposes the one relevant to the
    live tiers this scan): a research population needs every FVG in
    both directions, not just whichever one the bot is currently
    gated on — otherwise any reaction-rate study built on top of this
    would silently be studying "FVGs the live bot cared about," not
    "FVGs," which is exactly the kind of self-confirming contamination
    a baseline/control comparison is supposed to catch.

    Each dict carries an "identity_key" built from
    (zone_type, direction, formation_time) — the formation candle's
    ABSOLUTE timestamp, not its position in any rolling window.
    Positions (`idx` / `origin_idx`) shift every scan as the window
    slides forward; absolute timestamps don't. That's what lets the
    caller (see _log_new_zones() in scanner_live.py) dedup a zone
    against scans arbitrarily far apart instead of just consecutive
    ones.

    Never raises — one zone type failing to compute (e.g. no BOS yet,
    so no order block) just means it's absent from the returned list,
    same "prefer absence over blowing up the scan" discipline as
    build_structure_digest() above.
    """
    def _safe(fn):
        try:
            return fn()
    ...
    # FVG — both directions. detect_significant_fvg() already carries
    # "time" as gap's formation candle absolute timestamp (straight
    # from detect_fvg()'s window.index[i]) — no translation needed.
    for direction in ("BULLISH", "BEARISH"):
        gaps = _safe(lambda d=direction: detect_significant_fvg(
            facts.df_15m, d, facts.atr_15m_series())) or []
        for gap in gaps:
            zones.append({
                "zone_type":      "FVG",
                "direction":      direction,
                ...
                "high":           gap["high"],
                "low":            gap["low"],
                "formation_time": gap["time"],
                "size_atr_ratio": gap.get("size_atr_ratio"),
                "age_candles":    gap.get("age_candles"),
                "identity_key":   f"FVG:{direction}:{gap['time']}",
            })

    # Order block — the one behind the current confirmed BOS, if any.
    # origin_idx is a position inside `lookback_15m` below (see detect_
    # order_block()'s own docstring on why it's bounded to bos["origin_
    # idx"] onward within that same window) — translated to an absolute
    # timestamp here so identity_key survives the window sliding
    # forward scan to scan.
    ob = _safe(facts.order_block)
    if ob is not None and ob.get("origin_idx") is not None:
        lookback_15m = facts.df_15m.tail(SWING_LOOKBACK_15)
        formation_time = None
        try:
            formation_time = lookback_15m.index[ob["origin_idx"]].isoformat()
        except Exception:
            pass
        if formation_time is not None:
            zones.append({
                ...
                "identity_key":   f"OB:{ob['direction']}:{formation_time}",
            })
...
def compute_signal_timeline_updates(facts, timeline, now_utc):
    ...
    TIMELINE FIX (2026-09-11, per chat — a live signal showed "Engulf"
    stamped BEFORE "Fib entered," which made a real, correctly-fired
    trade look like it fired on a two-hour-old stale confirmation).
    Root cause: this stage used to be checked as a sibling `if`,
    independent of price_in_fib_pocket() — unlike `sweep`, which was
    already correctly nested. That meant it locked onto the FIRST
    rejection-shaped candle anywhere in the leg's life, whether or not
    price had ever been near the pocket yet, rather than the candle
    that actually confirmed THIS entry. Fixed by nesting it the same
    way `sweep` already was — it can now only ever stamp while price is
    genuinely inside the pocket, so the Timeline reflects the same
    live, same-scan check `_tier2_fib_evaluate()` actually fires on,
    not an unrelated earlier candle.

    Also renamed (per chat, point 8 of the advisor conversation): the
    underlying signal here is `facts.rejection_candle()`, not an actual
    bullish/bearish engulfing-pattern check — calling it "Engulf" in
    the human-facing message implies a specific candle shape that
    isn't what's being measured, and creates a false mental model.
    Stage key renamed engulf -> rejection; format_timeline_diagnostics()
    below still reads the old engulf_at key as a fallback so a timeline
    already in flight at deploy time (stamped under the old key) keeps
    displaying correctly rather than showing a false "—".
    """
    updates = {}
...
def format_timeline_diagnostics(timeline, now_utc):
    ...
    RENAME (2026-09-11, per chat): "Engulf" -> "Rejection", matching the
    stage-key rename in compute_signal_timeline_updates() above — see
    that function's docstring. `engulf_at` is still read as a fallback
    for any timeline already in flight under the old key at deploy time.
    """
    def _parse(iso):
        try:
    ...
    stages = [
        ...
        ("Rejection",   _parse(timeline.get("rejection_at") or timeline.get("engulf_at")), None),
...
# ---- ZONE INTERACTION OBSERVATION (Phase 2, per chat 2026-09-11) -----------
# Built from an advisor conversation about why a bullish TIER_2_FIB signal
# felt wrong against broader context. The agreed direction (NOT built here:
# parent/child leg context, MIL thesis-health fields, MIN research on which
# relationships actually predict continuation, or any SIGNAL-side gating —
# all deliberately deferred, see the build-order discussion) was to stop
# inventing per-object rules (resistance logic, supply logic, etc.) and
# instead build one generic "what is price doing at the zone it's
# interacting with" observation, reusable across every zone-based tier.
#
# Deliberately excludes TIER_3_STRUCTURE: audited its evaluate function
# (_tier3_structure_evaluate) and confirmed its trigger (a CHoCH) is atomic
# — by the time it's true the break already happened, there's no "sitting
# in a zone waiting to see what price does" phase for this observation to
# describe. Only Tier 1 (order block / FVG) and Tier 2 (fib pocket) — both
# genuine "price enters a zone, then does something" setups — apply.
#
# PURE OBSERVATION ONLY, matching the agreed Phase 2 scope exactly: no new
# gate, no new score, does not feed any tier's activated/fired decision.
# Classification uses only the zone's OWN already-computed boundary (the
# same order_block()/significant_fvg()/fib_pocket_bounds() low/high every
# tier already treats as meaningful) — deliberately not an invented numeric
# threshold (no "1.5 ATR", no "60% retracement"). That's a real constraint
# from the advisor conversation: measure first, calibrate/threshold later,
# once MIN has real resolved outcomes to research against.

def compute_zone_interaction_reset(state, leg_id):
    """PURE. Same trigger and shape as compute_signal_timeline_reset() —
    a new 1H leg means any prior zone-interaction record no longer
    describes a zone that's still relevant."""
    if state.get("zone_interaction_leg_id") != leg_id:
        return {"zone_interaction_leg_id": leg_id, "zone_interaction": {}}
    return {}


def _live_zone(facts):
    """Returns (zone_type, zone_low, zone_high) for whichever zone-based
    tier's zone is currently live, checked in the SAME priority order
    TIER_PRIORITY already uses (Tier 1 before Tier 2) so this observes
    whichever zone a tier would actually be evaluating against right now,
    not an arbitrary/different pick. Returns (None, None, None) if price
    isn't currently inside any recognized zone at all."""
    if facts.has_order_block() and facts.price_in_order_block():
        ob = facts.order_block()
        return "order_block", ob["low"], ob["high"]
    if facts.has_significant_fvg() and facts.price_in_fvg():
        gap = facts.significant_fvg()
        return "fvg", gap["low"], gap["high"]
    if facts.price_in_fib_pocket():
        bounds = facts.fib_pocket_bounds()
        if bounds is not None:
            return "fib_pocket", bounds[0], bounds[1]
    return None, None, None


def observe_zone_interaction(facts, interaction, now_utc):
    """
    PURE. `interaction` is read-only (the CURRENT state["zone_interaction"]
    dict, already reset for this leg if needed). Returns the updates dict
    to merge in via apply_state_updates(), same discipline as every other
    compute_*()/observe_*() in this file.

    Response categories, all derived purely from where the current close
    sits relative to the zone's own low/high (side = the direction a
    reaction is expected to favor, from facts.macro_bias — the SAME
    direction every zone-based tier already trades):
      stalled_in_zone     — close is still between zone_low and zone_high.
      held_and_rejected   — close is back out the side price entered from,
                             in the favorable direction (the zone did what
                             a pullback zone is supposed to do).
      broken_through       — close is out the FAR side, against the
                             favorable direction (the zone failed to hold).
      reclaimed_after_break — was broken_through on a PRIOR scan, and has
                             now closed back to held_and_rejected — a
                             swept-and-reclaimed pattern, tracked because
                             it's a materially different story than a clean
                             rejection even though both end up
                             "held_and_rejected"-shaped.
    ZONE FIX (found during testing, before this ever shipped): a zone must
    stay tracked using the bounds recorded at FIRST touch, not re-derived
    fresh from price_in_order_block()/price_in_fvg()/price_in_fib_pocket()
    every scan. Those facts mean "currently contained within" and go False
    the instant price exits — including by breaking through the far side,
    which is exactly the case this function most needs to keep watching.
    A first pass called _live_zone() unconditionally every scan and
    verified against a synthetic "price breaks through and keeps falling"
    scenario: it silently stopped updating the moment price closed outside
    the zone in EITHER direction, freezing at "stalled_in_zone" forever
    instead of ever reaching "broken_through". Fixed by only calling
    _live_zone() to establish a NEW zone when `interaction` doesn't already
    have one recorded for this leg; once recorded, its own persisted
    zone_low/zone_high are what every subsequent scan classifies against.
    """
    if interaction.get("zone_type") is not None:
        zone_type = interaction["zone_type"]
        zone_low, zone_high = interaction["zone_low"], interaction["zone_high"]
    else:
        zone_type, zone_low, zone_high = _live_zone(facts)
        if zone_type is None:
            return {}

    close = float(facts.last_candle_5m()["Close"])
    side = bias_to_side(facts.macro_bias)
    prior_response = interaction.get("response")
    # PERSISTENT flag, not just "was the LAST response broken_through" —
    # found via testing (scenario: break, one scan back inside the zone
    # first, THEN reclaim) that using prior_response alone loses the
    # break the moment an intermediate stalled_in_zone scan sits between
    # the break and the reclaim, since prior_response only remembers the
    # immediately-preceding scan.
    ever_broken_through = interaction.get("ever_broken_through", False) or prior_response == "broken_through"

    if zone_low <= close <= zone_high:
        response = "stalled_in_zone"
    elif (side == "BUY" and close > zone_high) or (side == "SELL" and close < zone_low):
        response = "reclaimed_after_break" if ever_broken_through else "held_and_rejected"
    else:
        response = "broken_through"

    updates = {}
    if interaction.get("zone_type") != zone_type or "first_touched_at" not in interaction:
        updates["zone_type"] = zone_type
        updates["zone_low"] = zone_low
        updates["zone_high"] = zone_high
        updates["first_touched_at"] = now_utc.isoformat()
        updates["bars_observed"] = 1
    else:
        updates["bars_observed"] = interaction.get("bars_observed", 0) + 1

    if response == "broken_through" and not ever_broken_through:
        updates["ever_broken_through"] = True

    if response != prior_response:
        updates["response"] = response
        updates["response_changed_at"] = now_utc.isoformat()
...
def _tier1_poi_evaluate(facts, ctx, state, now_utc):
    ...
    FIX, 2026-08-15 (per chat): the Order Block was previously the live
    bot's ONLY point of interest here — FVG reactions were shadow-only
    research (Shadow Pipeline Experiment 3 (POI), variant="fvg" rows).
    That research has now graduated: MarketFacts.significant_fvg() was
    already exposed as a live fact (see its own comment, dated
    2026-08-14) using the IDENTICAL calculation EXP3 validated —
    detect_significant_fvg(), zero new math — but this function was
    never actually updated to consume it, so the promotion was half-done
    until now. OB is tried FIRST; FVG is a genuine fallback POI, only
    considered when no OB reaction is live this scan. Confirmed
    independently (not just per the MarketFacts comment): detect_order_
    block() has zero references to FVG/gap logic anywhere in its body —
    the two have always been fully independent detectors, so this
    doesn't introduce any circularity between them.

    Mandatory:
      - facts.has_fresh_bos_aligned_with_bias()  (the leg backing the
        POI is still the live, bias-aligned leg — not a stale/superseded
        one)
      - EITHER an Order Block reaction (has_order_block() +
        price_in_order_block()) OR a quality-filtered FVG reaction
        (has_significant_fvg() + price_in_fvg()) — poi_type on the
        result/breakdown records which one, so Evidence/research can
        slice OB-driven vs FVG-driven Tier 1 fires separately rather
        than silently pooling two different POI types under one label.
      - facts.rejection_candle()            (real rejection, not just a
        touch, at whichever POI is live)

    Optional (score only, never gates):
      - facts.has_choch_15m()  (a flip-driven POI > a mere continuation POI)
    """
    label = "TIER_1_POI"
    ...
    ob_live = facts.has_order_block() and facts.price_in_order_block()
    fvg_live = (not ob_live) and facts.has_significant_fvg() and facts.price_in_fvg()

    if not (ob_live or fvg_live):
        if facts.has_order_block():
        ...
        if facts.has_significant_fvg():
            d = facts.fvg_distance_pips()
            return TierResult(
                tier_label=label,
                reason=(f"price not in FVG — {d} pips away" if d is not None
                         else "price not currently inside the FVG zone"),
                breakdown={"fvg_distance_pips": d},
            )
        return TierResult(tier_label=label, reason="no order block or quality FVG behind the current BOS impulse")
    ...
    if ob_live:
        ...
        poi_breakdown = {"order_block": True, "fvg": False}
    else:
        gap = facts.significant_fvg()
        poi_type = "fvg"
        sl_raw = gap["low"] if side == "BUY" else gap["high"]
        poi_breakdown = {
            "order_block": False, "fvg": True,
            "fvg_size_atr_ratio": round(gap["size_atr_ratio"], 2),
            "fvg_age_candles": gap["age_candles"],
        }
    ...
    # structural conditions above (aligned BOS + a live POI) are enough
    # to CLAIM/WATCH the leg; rejection_candle() is the separate trigger
    # that promotes WATCHING to FIRED. This also gives a higher-priority
    # tier a real multi-scan window to steal the leg before this one
    # actually fires.
    if not facts.rejection_candle():
        ...
        return TierResult(activated=True, fired=False, direction=side,
                           ...
                           reason=f"price inside {poi_type.replace('_', ' ')}, watching for rejection candle "
    ...
    leg_significant, leg_detail = macro_leg_is_significant(state)
    if not leg_significant:
        return TierResult(
            activated=True, fired=False, direction=side,
            entry=entry, sl_raw=sl_raw, tier_label=label,
            breakdown={
                "rejection_candle": True, "fresh_bos_aligned": True, "choch": choch,
                "leg_too_thin": True, **leg_detail, **poi_breakdown,
            },
            reason=(f"{'demand' if side == 'BUY' else 'supply'} POI reaction confirmed, but the 1H leg "
                    f"backing {direction.lower()} bias is only {leg_detail['leg_range_pips']} pips "
                    f"(needs {leg_detail['required_pips']}+ vs current 1H ATR) — too thin to trust as "
                    f"macro structure, held back"),
        )
    ...
    fire = True
    # classify_tier1_risk()'s own docstring still says "Tier 1 thesis: OB
    # reaction" — its actual logic (choch + rejection_strength only) is
    # POI-type agnostic, so it's reused unchanged for the FVG path too
    # rather than writing a near-duplicate classifier ("zero new math").
    risk_band, risk_reasons = classify_tier1_risk(choch, rejection_strength)
    poi_desc = (
        "Order block reaction ({} zone)".format("demand" if side == "BUY" else "supply")
        if ob_live else
        "FVG reaction ({} gap)".format("bullish" if side == "BUY" else "bearish")
    )
    base_reason = "{}{}, rejection strength {:.2f}".format(
        poi_desc, " + CHoCH" if choch else "", rejection_strength)
...
def _tier2_fib_evaluate(facts, ctx, state, now_utc):
    ...
    if not leg_significant:
        return TierResult(
            ...
            entry=entry, sl_raw=sl_raw, tier_label=label,
            breakdown={
                "in_fib_zone": True, "rejection_candle": True,
                "bos_aligned": bos_aligned, "liquidity_sweep": swept,
                "leg_too_thin": True, **leg_detail,
            },
            reason=(f"HTF fib pullback reaction confirmed, but the 1H leg backing {direction.lower()} "
                    f"bias is only {leg_detail['leg_range_pips']} pips "
                    f"(needs {leg_detail['required_pips']}+ vs current 1H ATR) — too thin to trust as "
                    f"macro structure, held back"),
        )
...
def _tier3_structure_evaluate(facts, ctx, state, now_utc):
    ...
    if not leg_significant:
        return TierResult(
            ...
            entry=entry, sl_raw=sl_raw, tier_label=label,
            breakdown={
                "choch": True, "bos_aligned": True, "liquidity_sweep": swept,
                "leg_too_thin": True, **leg_detail,
            },
            reason=(f"15M CHoCH/BOS confirmed, but the 1H leg backing {direction.lower()} bias is only "
                    f"{leg_detail['leg_range_pips']} pips (needs {leg_detail['required_pips']}+ vs "
                    f"current 1H ATR) — too thin to trust as macro structure, held back"),
        )
...
# =========================================================================
# =========================================================================
# LEG SIGNIFICANCE GATE (per chat, 2026-09-01) — a live fire-gate,
# separate from and in addition to Risk-at-Hand above. Risk-at-Hand is
# explicitly descriptive-only (see its module comment: "NEVER gates
# fired"); this one DOES gate fired, deliberately, because the gap it
# closes isn't "this setup is somewhat risky," it's "the 1H leg backing
# macro_bias_confirmed may be too small to trust as macro structure at
# all." Concrete case: a 75-pip bounce off a crash low held
# macro_bias_confirmed == BULLISH for 37+ hours inside an otherwise-
# bearish multi-day 1H downtrend, and a live BUY signal fired off it.
#
# Deliberately does NOT touch macro_bias_confirmed / compute_macro_bias
# — that function's "ONE place direction gets decided" contract is
# unchanged, and everything that reads macro_bias for narrative/phase/
# campaign purposes (Market Thesis, Market Phase, /understand, etc.)
# still sees whatever compute_macro_bias says, unfiltered. This ONLY
# blocks a tier from firing a live signal off a leg too thin to trust
# — narrowest blast radius that actually stops the trash-signal case.
#
# MIN_LEG_SIZE_ATR_MULT is a first guess (3.0), not tuned against
# history — see its own comment in scanner_common.py. Revisit once
# bias_minleg_log.json (the parallel shadow rule, unaffected by this
# gate) has real divergence data.
def macro_leg_is_significant(state, min_mult=MIN_LEG_SIZE_ATR_MULT):
    """
    PURE — reads state only, no dataframe needed (macro_atr_1h is
    persisted every scan by compute_macro_bias(), see its own comment).
    Returns (is_significant: bool, detail: dict) for logging/reasons.

    Fails OPEN (returns True) if macro_leg_origin/extreme/atr_1h aren't
    present yet (e.g. first few scans after a fresh deploy) — this gate
    should never be the reason a legitimately-sized setup silently never
    fires just because telemetry hasn't warmed up.
    """
    origin = state.get("macro_leg_origin")
    extreme = state.get("macro_leg_extreme")
    atr_1h = state.get("macro_atr_1h")

    if origin is None or extreme is None or atr_1h is None or atr_1h <= 0:
        return True, {"leg_range_pips": None, "required_pips": None, "gate_skipped": True}

    leg_range = abs(extreme - origin)
    required = min_mult * atr_1h
    is_significant = leg_range >= required
    detail = {
        "leg_range_pips": round(leg_range / PIP_SIZE, 1),
        "required_pips": round(required / PIP_SIZE, 1),
        "gate_skipped": False,
    }
    return is_significant, detail
