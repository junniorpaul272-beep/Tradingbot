...
def log_rejected_live_to_queue(facts, ctx, state, live_result, now_utc):
    ...
    leg_id = "{}|{:.5f}|{:.5f}|{}".format(side, facts.swing_high, facts.swing_low, tier_anchor)
    ...
    for update in resp["result"]:
        ...
        try:
            if cmd in ("/win", "win", "/w", "/loss", "loss", "/l"):
                ...
                try:
                    send_telegram(format_market_thesis(load_state()))
                except Exception as e:
                    send_telegram(f"📖 _Market Thesis error: {e}_")
...
def _maybe_send_daily_thesis_update(state, now_utc):
    ...
    thesis at THESIS_UPDATE_HOURS_LOCAL (9am/12pm/3pm local), every day,
    no weekend skip (per chat — "no need for forex weekend, we can flash
    it" i.e. don't bother special-casing it, just always send).
...
def _scan_once():
    ...
    # ── DAILY THESIS PUSH (wall-clock, not scan-count — see docstring) ────
    # Runs regardless of whether a signal fired this scan; that's the
    # whole point (per chat — this is the fix for "quiet mode risks users
    # thinking the bot is broken").
    try:
