...
def stitch_narrative(transition_narrative, mtf_5m, delta, timeline):
    """
    ...
    fields the thesis ALREADY computed into 2-4 short sentences using a
    ...
    `mtf_5m` / `delta` / `timeline` are each optional and independently
    None-safe — this degrades gracefully to just `transition_narrative`
    if all three are unavailable (e.g. this scan's 5M read failed).
...
def build_market_thesis(phase_result, ctx, facts, macro_bias, bias_stale, state, prev=None, timeline=None):
    """
