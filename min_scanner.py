...
def run_sr_tracking(facts, phase_result, macro_bias, current_atr_pips, now_utc,
    ...
    the flip+retest entry this whole detector exists to find. Because
    this runs entirely on MIN's independent pass, using MIN's own
    freshly-recomputed phase_result (see run_min_pass()), a
    RETEST_REACTION can never have been influenced by which live tier
    fired this scan — the same "can't cheat by seeing the outcome"
    property Market Intent has, for the same reason (see
...
# =============================================================================
# INTEGRITY COMMAND — /integrity (per chat, 2026-08-26). One-way read-only
# formatter, same discipline as format_world_state/format_market_thesis
# above: reads state.json (via the `state` dict scanner_live.py's
# _scan_once() already has finalized for this pass), runs scanner_integrity's
# three layers, and renders the result. Runs ZERO experiments, imports
# scanner_integrity ONLY (which itself never imports scanner_live.py, never
# writes anything scanner_live.py reads) — same one-way direction as every
# other command formatter in this file.
#
# Deliberately NOT wired to continuity (Layer 3) checkpoint recording yet —
# scanner_integrity.continuity_report() will simply report "insufficient
# history" until pipeline_checkpoints.jsonl exists, which is fine: Layers 1
# and 2 are useful on their own, and Layer 3 instrumentation is a separate,
# deliberate decision for scanner_live.py (see integration_notes.md).
# =============================================================================
...
def run_min_pass():
    ...
    # ── Market Phase — recomputed fresh here, same pattern as macro_bias
    # above ("Rebuild bias exactly like live does, against the read-only
    # snapshot"). Needed by the S/R tracker below (discover_sr_candidates()
    # keys birth off phase_result.phase/age_bars); everything else in
    # this pass already worked fine without it, so this is net-new, not
    # a duplicate of anything. prev_macro_bias intentionally omitted
    # (None) — BIAS_FLIP just won't be labeled as a transition cause on
    # MIN's copy, which only affects a narrative string, never S/R math.
    # prev_phase similarly omitted: MIN never persists its own phase (the
    # updates from this call are discarded — see _phase_updates_unused
    # below), so there's no "MIN's own last scan" to hand back in here.
    # This means MIN's copy doesn't get the EXHAUSTION exit-hysteresis
    # (PHASE_EXHAUSTION_EXIT_FRACTION) LIVE's persisted phase gets —
    # acceptable for now since nothing here gates on phase, only S/R
    # candidate birth-keying reads it, same "narrative/log only" caveat
    # as the rest of this block.
    try:
        extension = compute_measured_move_extension(state)
        phase_result, _phase_updates_unused = compute_market_phase(
            df_1h, macro_bias, bias_stale, facts, state, extension=extension)
    except Exception as e:
        print("[MIN PHASE ERROR] " + str(e))
        phase_result = None
