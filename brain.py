    ...
    if is_aging:  # aligned or no 5M read, but leg is mature
        ...
        if _reaccelerating(momentum):
            return {
                ...
                "primary_threat": (
                    "Leg maturity by break-count/EMA-distance and a currently "
                    "reaccelerating push are both true at once and haven't "
                    "been reconciled by anything upstream — treat 'aging' as a "
                    "caution about the broader leg's remaining runway, not a "
                    "sign this specific push lacks force."
                ),
                "structural_status": "not_invalidated",
                "confirmation_needed": "15-minute expansion cooling off, or a fresh continuation impulse — either would clarify which read matters more right now.",
                "alternative": "If the 15-minute expansion stalls without a fresh leg extreme, this reverts to a plain aging/maturing read.",
                "invalidation": None,
            }
        return {
...
def format_understanding_narrative(understanding):
    ...
    next_event = understanding.get("thesis_expected_next_event")
    if next_event:
