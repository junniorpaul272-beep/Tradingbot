...
def _compose_current_conditions(cc, mil_understanding):
    ...
    # Q3 — What's happening now? (volatility regime + any live counter-case)
    ...
    # Q4 — What matters next? (the actual level/condition that would
    # change this read — same failure_condition field narrate_deep_
    # thesis() already draws on, surfaced here too since the overview
    # previously never stated it at all).
    ...
    if fc_level:  # 0.0 is mil.py's own "not yet filled" placeholder
        ...
        parts.append(f"What would change this read: {level_phrase}.")
...
def _compose_recap(event, current_condition, mil_understanding, intent_hypothesis):
...
def narrate_overview(mil_understanding, current_condition, pending_events, elapsed_seconds, intent_hypothesis=None):
    ...
    try:
        ...
        if pending_events:
            ...
            event_lines = [_compose_recap(e, current_condition, mil_understanding, intent_hypothesis)
...
# ---------------------------------------------------------------------------
# Resolution follow-up — for a Forward Observation leg closing out.
# ADDED (2026-09-15, per chat — friend's "one owner of Telegram prose"
# rule, and the second producer it caught: min_scanner.py's
# _close_leg_obs() was building this push as a raw f-string, bypassing
# HFIS entirely — flat register, "Earlier: X; Y" concatenation, no
# relation to anything else the bot says). Same inputs that raw string
# used (headlines_summary from _summarize_pending_events(), direction,
# fate, bars_open) — no new detection, purely a narration-layer fix.
# ---------------------------------------------------------------------------
...
_RESOLUTION_GENERIC_OPENER = ["That leg has now resolved."]


def narrate_resolution(headlines_summary, direction, fate, bars_open):
    """
    Returns a composed follow-up string, or None on any internal error
    (caller falls back to the plain f-string it used before — see
    _close_leg_obs() in min_scanner.py). Never raises.

    `headlines_summary` — the already-built "Earlier: ..." recap text
    from _summarize_pending_events() (min_scanner.py); this function
    doesn't re-derive it, only wraps it in HFIS's own register instead
    of a flat "Earlier: {headlines}" line.
    """
    try:
        fate_key = (fate or "").upper()
        opener = random.choice(_RESOLUTION_OPENERS.get(fate_key, _RESOLUTION_GENERIC_OPENER))
        parts = [opener]
        if direction:
        ...
        print("[HFIS NARRATE_RESOLUTION ERROR] " + str(e))
        return None
...
def narrate(events, understanding, intent_hypothesis, mil_understanding=None, mechanisms=None):
    """
    ...
    try:
        ...
        # mil_understanding's supporting/conflicting_evidence — real
        # sentence fragments, but about the THESIS's overall health
        # (leg aging, EMA extension, etc.), not about whatever specific
        # event just fired. That's a coherent thing to say alongside a
        # structure/state event (both ARE about the thesis's condition)
        # but a non sequitur bolted onto an opportunity event (a new
        # zone coming into range has nothing to do with how extended the
        # leg is from the 100 EMA) — which is exactly why "Filed away
        # for now." was getting "still, price is 4.4 ATR away..."
        # tacked onto it with no connection to the zone just described.
