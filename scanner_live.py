...
# touches zero API. /understand and /briefing are dev/audit surfaces
# only (per chat) — see brain.py's format_market_understanding() /
# format_market_briefing() docstrings. build_market_briefing() is
# explicitly NOT wired into the 9am/12pm/3pm push here — its own
# docstring says that needs checking against real scans first via this
# dev/audit command, same promotion path /understand is still on
# relative to /world.
from brain import (
...
HELP_TEXT = (
    ...
    "`/understand` — Brain's interpretation of /world (dev/audit view — "
    "not yet folded into /world)\n"
    "`/integrity` — integrity/consistency diagnostics (read-only, never gates)\n"
    "`/briefing` — Brain's assembled Thesis+Logic+Intent briefing (dev/audit "
    "view — not yet the scheduled 9am/12pm/3pm push)\n\n"
    "`/help` — show this list"
)
    ...
    for update in resp["result"]:
        ...
        try:
            if cmd in ("/win", "win", "/w", "/loss", "loss", "/l"):
                ...
                # reasoning as /world above). Dev/audit surface only —
                # see brain.py's format_market_understanding() docstring.
            ...
            elif cmd in ("/briefing", "briefing"):
                # Deferred — same reasoning as /understand: Brain reads
                # this pass's own WorldState, not finalized until later
                # in _scan_once(). Dev/audit surface only — build_market_
                # briefing()/format_market_briefing() are explicitly NOT
                # the scheduled 9am/12pm/3pm push yet (per brain.py's own
                # docstrings), same promotion gate /understand is on
                # relative to /world. This command exists to sanity-check
                # the assembled object against real scans before anyone
                # schedules it.
                deferred_report_cmds.append("briefing")
...
def _answer_deferred_reports(deferred_report_cmds, state):
    ...
    /understand, /integrity, /briefing commands from check_result_commands().
    ...
    for _cmd in deferred_report_cmds:
        try:
            if _cmd == "thesis":
                ...
                send_telegram(format_market_understanding(_understanding))
            elif _cmd == "integrity":
                send_telegram(format_integrity_command(state))
            elif _cmd == "briefing":
                _ws_for_briefing = build_world_state(state)
                _briefing = build_market_briefing(_ws_for_briefing)
                _append_briefing_log(_briefing)
                send_telegram(format_market_briefing(_briefing))
        except Exception as e:
            try:
